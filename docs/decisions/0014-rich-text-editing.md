...
- Selected text supports links through the typography inspector. New links use
  ...
  updates the whole link and preserves every other attribute. Removing its link
  clears only `href`; the same anchor can receive a new URL after saving and
  reopening. Dynamic URLs stay
