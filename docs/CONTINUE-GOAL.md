...
### 2026-09-12 — current shared-inspector desktop archive and native welcome launch

Built a universal ad hoc archive from clean e948266. All 915 units passed. Extracted archive inventory (178 source files) and native/build/verifier hashes match source; strict signature passed before and after external packaged workflows and launch. Packaged React shared-gap geometry/scope/reset/Undo and Liquid WebKit shared-section persistence passed. External harness removed and absence verified. Generated cask syntax passed; installation not repeated. Receipt: desktop/verification/2026-09-12-shared-inspector.json. Archive: /private/tmp/retouch-desktop-shared-e948266/Retouch-0.1.0-mac.zip.

CUA native access recovered: Chrome window accessibility succeeded, then one controlled getApp call for the exact extracted Retouch.app opened its welcome window with Open project, URL and Open editor controls. No approval dialog appeared; PID 91169 confirmed the exact extracted executable and remains running. This verifies launch/accessibility only, not native editing/sampling. No security changes, notarization or publication. Native editor/sampler testing is now a useful next step while UI access works. Full goal remains active.


### 2026-09-12 — real WKWebView project startup, font edit, Undo and Stop

Using the already-running extracted e948266 archive through CUA, opened an isolated static HTML folder, confirmed auto-detected Edit HTML files mode, and started it through the native dialog. Packaged CLI indexed the fixture and native editor connected automatically at localhost:54543/rt. Selected h1 in Layers, typed 48px into Font size (CSS), observed native inspector at 48px and source rule font-size:48px. Native Undo returned inspector to 32px and restored original file bytes exactly. Stop reported project stopped; independent TCP check proved listener closed. Signature/inventory verification still passes. Receipt updated in desktop/verification/2026-09-12-shared-inspector.json.

Native sampler activation disabled its picker button, but CUA click on the fixture background failed -10005 noWindowsAvailable. Screenshot/AX observations remained available. Escape returned to editor with exact original source; no sample completion is claimed. Fixture retained at /private/var/folders/4t/xyt8km1s59l4th8pdwkwg2_m0000gn/T/retouch-native-edit-gp14xvbb; no project process remains. App remains running with stopped project. Full native parity and trusted distribution remain incomplete. Next useful work includes native responsive editing and improving stopped-project presentation (the old editor remains visible after Stop).


### 2026-09-12 — stopped native project presentation

499ed87 replaces the dead project editor with a light Project stopped welcome screen after its process exits. It invalidates pending connections to that project's discovered origin and retires its sampling document. An editor connected on a different origin is retained. Welcome markup is shared with initial launch. This addresses the stale interactive canvas observed in the prior real native Stop flow.

Swift typecheck passed; clean universal build and strict package verifier passed. Candidate archive /private/tmp/retouch-desktop-stop-state/Retouch-0.1.0-mac.zip, SHA256 31cad8672b8609238b2a2e1faa858b922992324f350d54257d29dcf2986a0534. Build log /private/tmp/retouch-desktop-stop-build.log. Controlled launch displayed initial welcome; native folder-picker testing then failed with ScreenCaptureKit -3812 and repeated Sky native pipe startup failures. Stop transition remains unverified in the UI. Prior app exited via Cmd+Q; candidate test PID 95262 terminated through exact executable match after CUA failed. No native project started in this run. Prior archive's verified native edit/Undo evidence remains distinct. No security changes or launch retries. Full goal stays active; resume native transition check when CUA recovers, otherwise continue independent product work.


### 2026-09-12 — cancel the actual pending editor request on project exit

Follow-up inspection found that stopped-project cleanup compared the editable URL field to project origins. If the user changed that text while a health request was in flight, the old request could escape cancellation and reopen the stopped editor. The native app now tracks pendingEditor separately, sets it when starting the request, clears it on replacement/completion, and uses it for project-exit cancellation. The existing request UUID rejects late callbacks. Connected external editors retain the previous separate-origin handling.

Swift typecheck and compiled non-GUI --self-test --launch-cli passed. The latter covers actual CLI working directory/exit status, HTML folder launch, dynamic port, health and stop, plus URL/quoting boundaries; it does not prove the asynchronous UI race or stopped welcome screen. Log /private/tmp/retouch-connection-self-test.log. No native window launch or package rebuild in this turn. Last CUA failure remains a verification limitation, not an impasse for independent work. Full goal stays active.


### 2026-09-12 — shared padding and gap label dragging

Shared padding (uniform and edges) and physical horizontal/vertical gaps now reuse numeric label dragging with unit-preserving values and 0–10000 bounds. Each drag previews all selected layers using temporary property previews respecting edit scope; physical gap axis maps through each layer's writing mode. Preview restoration occurs before the single source commit. Mixed/normal/nonnumeric values do not start a drag. Disconnection/selection replacement interrupts it. Shift/Option modifiers, Escape cancellation and existing interruption handling come from the shared numeric control.

Validation: 29 shared/inspector units passed. Added real pointer flows under RT_E2E_SHARED_SPACING_SCRUB in existing shared gap/padding browser tests. React Chromium shared gaps passed live preview for differing writing modes, no source write during drag, Escape restore, commit and exact single Undo. Liquid WebKit shared padding passed Shift +10 preview/cancel and Option +0.5 commit/Undo across all sides/layers, followed by existing responsive edits/resets/validation. Logs /private/tmp/retouch-shared-spacing-drag.log, /private/tmp/retouch-shared-padding-drag.log, /private/tmp/retouch-shared-spacing-drag-units.log. Non-pixel unit dragging and additional interruptions remain less directly covered. No desktop rebuild/native launch/push. Full goal remains active.


### 2026-09-12 — shared drag relative units, interruption and inactive range evidence

Extended shared-padding browser verification with RT_E2E_SHARED_SPACING_CONTEXT. A typed 1em draft dragged with Option becomes 1.5em, previews against each selected layer's font size, writes only on release and restores exact source with one Undo. A real browser window resize cancels an in-progress drag. With md selected while viewing 390px, dragging from 0 to 5 leaves phone padding at 0 during preview and after save; switching to 768px reveals 5px on all selected edges, and Undo restores the prior 12px source.

React Chromium and Liquid WebKit flows passed. Logs /private/tmp/retouch-shared-spacing-context-final.log and /private/tmp/retouch-shared-spacing-context-webkit-final.log. Test setup now waits for actual resize-event delivery before pointer release, and reacquires fields if a WebKit refresh detaches a label during scroll. Initial setup failures are not product-fix claims. No additional production changes were required. No native launch/package/push. Full goal remains active.


### 2026-09-12 — shared numeric label dragging with box-aware previews

Shared numeric size/constraint, opacity, typography and flex numeric fields now attach the numeric label drag controller and selection-wide temporary previews. Sizing converts the displayed outer size to each layer's box sizing, clamps drag minimum to the largest selected padding/border floor, and preserves existing source validation. Opacity displays percent but previews CSS fractions; text dimensions preview px; integer fields round displayed/preview amounts. Selection replacement/disconnection invalidates preview; scope-aware property previews and Escape/interruption restoration are reused.

Validation: 29 shared/inspector units passed. React Chromium shared sizing drag passed equal outer-width previews across content-box/border-box, Escape without writes, minimum clamping to 24px, commit and exact Undo. Expanded Liquid WebKit flow additionally passed shared opacity and font-size preview, source commit and both Undo steps, followed by existing responsive sizing/reset/preset checks. Logs /private/tmp/retouch-shared-size-drag.log, /private/tmp/retouch-shared-numeric-drag-webkit.log, /private/tmp/retouch-shared-number-drag-units.log. Individual other numeric fields are less directly covered. No native build/launch/push. Full goal remains active.


### 2026-09-12 — shared container layout controls

Added shared layout mode (flow/row/column/reversed/grid), flex wrapping, child alignment and distribution. The controls reuse existing modeClasses/arrangementClasses, preserve other scopes and important shorthand priorities, and provide per-control resets. Inline declarations controlling a selected property disable the field and are rechecked for all layers before constructing the batch source write. Visible labels are Layout, Wrap, Alignment and Distribution, with human-readable option names. Row/column follow each container's writing direction.

Validation: 30 shared/inspector units passed, including new scope/important/reset/unrelated-alignment cases. React Chromium and Liquid WebKit container changes passed computed results, 390px unaffected while md edits apply at 768px, reset, exact Undo and unchanged unselected sibling. Existing gap drag/geometry flow also passed. The shared-gap fixture had used Shift range selection and unintentionally selected intervening spans; changed to Meta additive selection and asserted exactly h1+p after every container edit. Screenshot /private/tmp/retouch-shared-containers-final.png inspected before final label casing/shortening. Logs /private/tmp/retouch-shared-containers-react-final.log, /private/tmp/retouch-shared-containers-liquid-final.log, /private/tmp/retouch-shared-containers-final-units.log. Other arbitrary CSS/layout shorthand cases remain broader parity work. No desktop build/native launch/push. Full goal remains active.


### 2026-09-12 — shared grid track counts and placement flow

When all selected containers are grids, the shared Layout section now provides paired column/row counts (1–24), row/column/dense placement flow and separate resets. Counts author equal fractional tracks through existing arrangementClasses; computed counts may include implicit content-created tracks. Inline grid/grid-template overrides disable the affected fields and are rechecked before batch writes. Flow normalization accepts implicit row for dense. New count fields handle Enter/Escape locally to avoid clearing the layer selection after invalid input.

Validation: 31 shared/inspector units passed with scope, important grid shorthand, reset and count validation cases. React Chromium and Liquid WebKit full shared-container/grid/gap flows passed: grid switch, 3 columns/4 rows, column-dense, invalid zero without writes, Escape retaining selection, reset/Undo, phone range unchanged and exact source restoration. Initial browser failure exposed missing Escape handling and was fixed. Screenshot /private/tmp/retouch-shared-grid-final.png visually inspected. Logs /private/tmp/retouch-shared-grid-react-final.log, /private/tmp/retouch-shared-grid-liquid-final.log, /private/tmp/retouch-shared-grid-final-units.log. Shared custom tracks and distinguishing child layout from each selected item's own layout controls remain useful next work. No native build/launch/push. Full goal remains active.


### 2026-09-12 — distinguish shared container layout from layout item controls

Moved shared Grow/Shrink, basis, item alignment and grid spans into a separate Layout item section. Layout keeps controls for selected containers' own children, gaps and padding. This separates the previously ambiguous child/item Alignment rows. Basis presets use concise labels with full accessible names and titles preserved. New item disclosure reuses persisted collapse-state storage and existing section presentation.

Validation: React Chromium shared-flex geometry/responsive/basis/reset/Undo flow passed; Liquid WebKit shared section keyboard/edit/Undo/reload persistence passed with the new item section added to collapsed-state assertions. 31 shared/inspector units passed. Screenshot /private/tmp/retouch-layout-item.png visually inspected. Logs /private/tmp/retouch-layout-item-flex.log, /private/tmp/retouch-layout-item-sections.log, /private/tmp/retouch-layout-item-units.log. No native build/launch/push. Full goal remains active.


### 2026-09-12 — shared custom grid track expressions

Shared grid containers now expose Custom grid tracks with independent column/row expressions and inline resets. Fields show authored selected-scope expressions when available, otherwise computed tracks or Mixed. Existing gridTemplateClasses validation/priority handling is reused; browser CSS support and inline grid overrides are checked for every selected layer before the batch write. Enter saves, Escape restores the field, and the disclosure remains open across edits.

Validation: 32 shared/inspector units passed, including scoped custom templates, important grid shorthand and malformed text. React Chromium and Liquid WebKit shared grid flows passed unequal 40/80/120px columns, repeat(4,30px) rows, authored-expression retention, invalid syntax without writes, Escape, separate reset and exact Undo. Existing container/grid/gap/responsive flows remained passing. Screenshot /private/tmp/retouch-shared-tracks.png visually inspected. Logs /private/tmp/retouch-shared-tracks-react.log, /private/tmp/retouch-shared-tracks-liquid.log, /private/tmp/retouch-shared-tracks-unit.log. No desktop build/native launch/push. Full goal remains active.


### 2026-09-12 — shared nine-point child alignment

Added the existing nine-point alignment UI beside shared gaps when every selected container is flex. Each point maps physical left/center/right and top/middle/bottom through each container's writing mode, direction, flex direction and wrap state via existing alignmentClasses/flexAlignment. Wrapped lines receive align-content too. Active point requires every selected container to match. Inline alignment shorthands/properties disable the point and are rechecked before source writes.

Validation: 33 shared/inspector units passed, including mixed writing modes, scope and important place-items. React Chromium and Liquid WebKit container flows passed bottom-right alignment across horizontal-tb and vertical-rl wrapped columns, distinct resulting justify values, common active-point state, top-left change/Undo, responsive scope isolation and exact full source Undo. Screenshot /private/tmp/retouch-shared-alignment.png visually inspected. Logs /private/tmp/retouch-shared-alignment-react.log, /private/tmp/retouch-shared-alignment-liquid.log, /private/tmp/retouch-shared-alignment-unit.log. Direct child-edge geometry across every writing mode remains broader validation work. No desktop build/native launch/push. Full goal remains active.
...
### 2026-09-12 — compact shared dimension sizing menus

Replaced separate width/height Auto and Hug preset rows with compact chevron menus beside each numeric dimension. The menu offers Fixed, Auto and Hug; Fixed writes each selected layer's own measured pixel size through the existing box-sizing conversion, preserving unequal layer sizes. It is an action menu, not an inferred CSS mode indicator. Existing CSS flex/min/max constraints continue to apply. The menu resets to its empty chevron after activation, including no-op edits. Shared constraints retain their existing presets.

Validation: 34 shared-selection/inspector tests passed. React Chromium and Liquid WebKit shared sizing flows passed Auto/Hug, per-layer Fixed after Hug, breakpoint isolation, resets and exact source Undo. WebKit also passed existing size/opacity/font-size drag previews, cancellation, clamping and Undo. Initial screenshot exposed a truncated numeric value beside the menu label; reducing the closed menu to a 20px chevron fixed it, verified in /private/tmp/retouch-shared-size-menu-final.png. Logs /private/tmp/retouch-shared-size-menu-react-final.log, /private/tmp/retouch-shared-size-menu-liquid-final.log, /private/tmp/retouch-shared-size-menu-unit.log. No native build, launch or push. Shared Fill sizing and complete Figma visual/behavioral parity remain incomplete.


### 2026-09-12 — shared Fill sizing and flex-aware dimension edits

Shared dimension menus now offer Fill using each live layer's parent display, flex direction and writing mode through existing sizeClasses. Fixed/Hug and numeric dimension writes now release main-axis flex growth/basis; reset clears the corresponding sizing behavior. Auto restores inherited flex behavior and explicitly authors automatic dimension sizing. Shared dimension drags preview fixed flex longhands along with the dimension, restoring them on cancellation. Inline sizing/flex/alignment overrides are guarded again before batch writes; scoped logical sizing remains refused by the existing change helper. Constraints and auto margins still affect CSS layout.

Validation: all 921 unit tests passed. React Chromium flex-row and Liquid WebKit flex-column flows passed Fill, parent-width/height changes, Fixed freezing current sizes, direct numeric edits, Hug, reset, phone isolation and exact Undo. React Chromium grid flow additionally passed cell-width and cell-height filling. Grid and WebKit flex-column tests verified live drag to 85px with source unchanged and Escape restoring 75px Fill. Existing React sizing/box-sizing/size-opacity-type drag regression passed. Logs /private/tmp/retouch-shared-fill-react.log, /private/tmp/retouch-shared-fill-grid.log, /private/tmp/retouch-shared-fill-drag-liquid.log, /private/tmp/retouch-shared-fill-sizing-regression.log, /private/tmp/retouch-shared-fill-all-units.log. No native build/launch/push. Mixed-parent and broader arbitrary-CSS Fill geometry remain further coverage; full goal remains incomplete.


### 2026-09-12 — mixed-parent Fill and normal-flow width overflow

Added RT_E2E_SHARED_FILL_MIXED coverage selecting a flex-row child with an important size and a padded content-box child in a vertical-rl grid. Both React Chromium and Liquid WebKit passed width/height Fill, distinct per-parent alignment, unequal Fixed sizes across a wider preview, breakpoint isolation and exact Undo. Logs /private/tmp/retouch-shared-fill-mixed-react.log and /private/tmp/retouch-shared-fill-mixed-liquid.log.

Normal-flow probing exposed width:100% plus content-box padding/borders overflowing the parent. Common sizeClasses now emits w-[-webkit-fill-available] plus [width:stretch] (and height counterparts), preserving margins/padding under supported stretch sizing. The different utility/property forms intentionally survive Liquid tailwind-merge; initially using several w-* fallback classes lost the WebKit fallback. Single-layer mode detection recognizes these fill utilities. 73 focused layout/selection/inspector tests passed. React Chromium full normal-flow integration passed, screenshot /private/tmp/retouch-shared-fill-flow.png inspected.

OPEN FAILING REGRESSION: RT_E2E_SHARED_FILL_MIXED=1 RT_E2E_SHARED_FILL_FLOW=1 with Liquid WebKit now passes width (210px border box within 220px parent and 5px side margins), but fails height at page-fonts.cjs's height-Fill assertion. A direct WebKit probe confirmed -webkit-fill-available yields 180px border-box height instead of 170px with 5px top/bottom margins in a 180px flow-root parent. CSS.supports(height,stretch) is false in this installed WebKit; Chromium supports the standard keyword and passes. Keep this assertion intact; do not count the flow WebKit test as green. Logs /private/tmp/retouch-shared-fill-flow-react-final.log and /private/tmp/retouch-shared-fill-flow-liquid-final.log. No native build/launch or push. Full goal remains active, and the height compatibility issue is unresolved.


### 2026-09-12 — compact shared Size limits

Moved paired min/max dimension fields and their presets into a collapsed Size limits disclosure beneath the primary width/height row. The disclosure keeps its open state across inspector rerenders, including source edits and preview changes. Existing fields, handlers and accessible names are retained. This frees vertical space for Layout without dropping constraint editing.

React Chromium and Liquid WebKit shared-constraint flows passed: keyboard Enter/Space, default collapsed visibility, open state through min/max edits, min/max precedence, responsive isolation, exact Undo, and closed state retained through a subsequent width edit. Liquid WebKit aspect-ratio/max-height regression passed. Screenshot /private/tmp/retouch-size-limits-closed.png inspected. Logs /private/tmp/retouch-size-limits-react.log, /private/tmp/retouch-size-limits-liquid.log, /private/tmp/retouch-size-limits-ratio.log. No native build/launch/push.

The prior normal-flow WebKit Fill-height regression remains open and its assertion remains intact. Additional direct browser probes found no correction from changing display to flow-root/inline-block/flex/grid, relative positioning, overflow:auto or max-height. With content-box padding/borders the legacy value produced 180px outer height in the 180px parent despite 5px vertical margins; border-box instead produced 156px. These are engine-compatibility findings, not proof that no workaround exists. No pixel snapshot or layout-changing workaround was introduced. Continue the full goal; this issue does not block independent editor work.


### 2026-09-12 — contextual shared child-layout controls

Shared Layout now offers Wrap only when every selected layer is flex, and alignment/distribution only when every selected layer is flex/grid. Gap fields appear only for applicable physical axes: both for flex/grid, and the column axis for multi-column text. Normal-flow selections retain their layout mode, clipping and padding without ineffective child controls. Writers recheck live applicability before saving.

React Chromium and Liquid WebKit context flows passed normal/flex/grid switching, control visibility, grouped exact Undo, and phone-preview isolation. Column-gap checks passed horizontal React Chromium and vertical-rl Liquid WebKit, including scoped writes and Undo. Existing Liquid WebKit shared-container/grid/custom-track/nine-point-alignment/gap regression passed. Screenshot /private/tmp/retouch-layout-context.png inspected. Logs /private/tmp/retouch-layout-context-react.log, /private/tmp/retouch-layout-context-liquid.log, /private/tmp/retouch-layout-columns-react.log, /private/tmp/retouch-layout-columns-liquid.log, /private/tmp/retouch-layout-context-regression.log. No native build/launch/push. The previously recorded normal-flow WebKit Fill-height regression remains unresolved; full goal remains active.


### 2026-09-12 — contextual layout-item controls through contents wrappers

Shared Grow/Shrink/basis and alignment/grid-placement controls now appear only for compatible in-flow selected items; an empty Layout item section is removed. Parent lookup walks through display:contents wrappers to the actual flex/grid parent, including shared sizing and live dimension previews. Item writes recheck the live parent layout and retain caught-error reporting. A removed section keeps its stored collapse preference when it returns.

React Chromium and Liquid WebKit flex flows through display:contents wrappers passed growth, shrink, basis, breakpoint changes and exact Undo. Collapse preference survived removal at phone size and return at tablet size. React Chromium grid Fill through wrappers passed both axes, Fixed/Hug/numeric edits, drag cancellation, scope and Undo. Liquid WebKit top-level section persistence/reload passed without an irrelevant Layout item section. Screenshot /private/tmp/retouch-item-context-sections.png inspected. Initial fixture range selection also selected wrapper divs; tests now explicitly Meta-select the two children. Logs /private/tmp/retouch-layout-item-contents-react-final.log, /private/tmp/retouch-layout-item-contents-liquid-final.log, /private/tmp/retouch-layout-item-contents-grid.log, /private/tmp/retouch-layout-item-context-sections.log. Syntax/diff checks passed. No native build/launch/push. Existing WebKit normal-flow Fill-height regression remains open; full parity remains incomplete.


### 2026-09-12 — consistent formatting-parent lookup across inspectors

Centralized layoutParent in inspector.js, skipping nested display:contents wrappers and stopping at the first real box. Single React/Liquid Layout, shared sizing/item controls, HTML flex/grid editing, grid-placement suggestions and grid-guide drawing now use that lookup. HTML item controls exclude absolute/fixed elements from flex/grid participation.

74 focused tests passed, including nested contents lookup. React Chromium single flex and Liquid WebKit single grid Fill/Fixed/numeric/responsive/exact-Undo flows passed through wrappers. WebKit grid-guide columns matched the actual parent's 0/75/150px tracks. HTML Chromium Fill/Hug through wrappers passed responsive scope and exact Undo. Shared React grid spans and grid alignment plus Liquid flex alignment regressions passed. Updated stale shared tests to expect absent inapplicable fields and invoke Auto via the sizing menu. Logs /private/tmp/retouch-single-contents-react.log, /private/tmp/retouch-single-contents-guides.log, /private/tmp/retouch-html-contents.log, /private/tmp/retouch-parent-layout-unit.log, /private/tmp/retouch-item-grid-spans-regression.log, /private/tmp/retouch-item-grid-alignment-regression.log, /private/tmp/retouch-item-alignment-regression-final.log. No native build/launch/push. Existing WebKit normal-flow Fill-height issue remains open; full goal remains incomplete.


### 2026-09-12 — current layout desktop candidate

Built clean 7f42a31244a8ab614a0a599c88aa0f7744398850 into /private/tmp/retouch-desktop-layout-7f42a31/Retouch-0.1.0-mac.zip, SHA-256 6c0616bb49d8dc7d40cd5deebe6944fb7c1313e5e19ab190346c70fd51169817. All 178 source hashes and native/build/verifier hashes match. Universal arm64/x86_64, extraction and strict ad hoc signature checks passed. All 923 unit tests passed. Generated local cask syntax passed; install was not repeated.

External harness using extracted packaged src/bin/shell passed React Chromium grid Fill through contents wrappers, Liquid WebKit contextual layout and HTML Chromium Fill/Hug. The context test initially asserted before the scheduled viewport refresh; updated its state check to poll the expected controls and reran successfully against the same archive. No runtime package change was needed. Packaged Liquid WebKit normal-flow Fill-height regression FAILED exactly as expected: [[250,180],[210,180]] instead of [[250,180],[210,170]]. Keep this failure open. Harness removed and final signature reverified. CUA inventory preflight failed with Sky Computer Use native pipe startup failed; no app was launched. Native launch/self-tests remain unverified. Receipt desktop/verification/2026-09-12-layout-context.json records all results and logs. No publication/push. Full parity, trusted distribution and the WebKit height issue remain unfinished.


### 2026-09-12 — shared absolute positioning from page flow

Shared Position now offers Use absolute positioning for eligible in-flow selections instead of a refusal. It captures all selected viewport bounds, temporarily removes the layers from flow together to resolve their containing blocks, restores preview styles, and writes one scoped batch. Initial top/left anchors preserve placement when separate auto-height containers collapse. Content-box dimensions and inherited important geometry priorities are preserved. Existing geometry/source guards remain; other page content may reflow.

RT_E2E_SHARED_ABSOLUTE passed React Chromium, Liquid WebKit and HTML Chromium: two separate relative containers, one padded/bordered content-box layer, exact original bounds after conversion, align left, exact-source Undo, phone isolation, tablet return, and Redo. Screenshot /private/tmp/retouch-shared-absolute.png inspected in the light inspector. All 924 unit tests passed, including responsive important end-anchor replacement. Logs /private/tmp/retouch-absolute-{react,liquid,html}-final.log and /private/tmp/retouch-absolute-full-units.log. No native rebuild/launch; latest desktop candidate predates this change. Existing WebKit normal-flow Fill-height regression remains open. Full Figma parity and trusted distribution remain incomplete.

The preceding user-requested push was verified at origin/feat/figma-design-studio = 25e7c688b2865ee84e9221ce02c975205fcff555. This positioning increment is local only.


### 2026-09-12 — compact shared alignment toolbar

Replaced the shared Position section's wrapping alignment/distribution text buttons with eight physical SVG actions in one 28px row above the alignment target. Preserved accessible labels, native tooltips, disabled distribution rules, and all existing geometry handlers. The toolbar has a single tab stop; Left/Right wrap through enabled actions, Home/End reach the ends, and source refresh restores focus to the activated action. Move/resize canvas actions remain separately available.

Reference: https://help.figma.com/hc/en-us/articles/360039956914-Adjust-alignment-rotation-position-and-dimensions. This improves the alignment row; it is not a claim of full inspector fidelity.

React Chromium and Liquid WebKit shared conversion flows passed keyboard navigation without source/geometry changes, disabled-action skipping, Enter alignment, scope, Undo and Redo. WebKit additionally verified restored focus and one tab stop after saving. HTML Chromium selection-layout.cjs passed all eight alignment/distribution actions, selection/layer/frame targets, exact gaps, content-box dimensions, responsive isolation, atomic Undo/Redo and protected source refusal. Ten focused geometry units passed; diff/syntax checks passed. Screenshot /private/tmp/retouch-align-toolbar-aligned-controls.png inspected; all eight icons fit on one row. Logs /private/tmp/retouch-align-toolbar-{react,liquid,html,focus}.log. No new native build or push. Full goal, trusted distribution and the known WebKit normal-flow Fill-height issue remain unfinished.


### 2026-09-12 — Shift-align selections to their containing frames

Shift-clicking any of the six shared alignment icons now moves selected siblings as a group to their containing frame. Selections crossing frames are partitioned by the actual absolute-position containing block and each group aligns independently. Group members receive identical offsets; dimensions, the other axis and relative spacing stay intact. Normal alignment targets are retained after the temporary modifier action. The existing scoped batch write and history paths keep the operation atomic and undoable. Tooltip and Position details expose the modifier.

Reference behavior: https://help.figma.com/hc/en-us/articles/360039956914-Adjust-alignment-rotation-position-and-dimensions, Shift alignment to parent frames. Existing transform/inline/source guards remain; this does not establish arbitrary transformed-frame parity.

All 925 unit tests passed. New group geometry tests cover all six edges/centers, two frames, identical sibling deltas, unchanged other axis and malformed targets. HTML Chromium browser suite verifies all six Shift actions for three siblings while preserving content-box dimensions and exact Undo, plus its full existing alignment/distribution regression. React Chromium and Liquid WebKit verify all six actions across two separate containing frames with exact viewport bounds and source Undo. Liquid WebKit additionally verifies tablet-only Shift alignment, unchanged phone flow and tablet restoration. Logs /private/tmp/retouch-align-frames-{html,react,liquid,scope}.log and /private/tmp/retouch-align-frames-units.log. No native build/launch/push. Full Figma parity, trusted native distribution and the known WebKit normal-flow Fill-height issue remain incomplete.


### 2026-09-12 — alignment shortcuts, Actions search, and accessible Hand toggle

Shared absolute selections now support Alt+A/H/D/W/V/S for left/horizontal center/right/top/vertical center/bottom alignment from both preview and shell. Physical key codes support macOS Option-generated characters; Shift forwards the containing-frame grouping modifier. Guards exclude composing/native field input, AltGraph/Ctrl/Meta, dialogs/menus, Interact mode, active drawing/panning and busy source/history operations. Repeated keydown is consumed without another write. Toolbar keyboard activation retains focus; canvas shortcuts no longer move focus into the inspector. Toolbar buttons expose aria-keyshortcuts and tooltip hints. Actions search includes alignment/distribution with live disabled state and distinct keywords.

The browser guard test exposed the pan surface covering the bottom tool dock. The dock now sits above the pan surface, allowing Hand to be toggled off by clicking it again. canvas-pan exposes its live active state for shortcut guards. Its broad regression test now opens the existing More properties disclosure before invoking SVG Draw.

All 926 units passed. React Chromium and Liquid WebKit workflows passed all six actual Alt-key actions from iframe focus, exact geometry/source Undo, retained canvas focus, field-input and Hand guards, disabled distribution search, modal input protection, and execution from Actions search. Existing Shift/frame, scope, toolbar keyboard and conversion Undo/Redo flows also passed. HTML Chromium canvas-pan.cjs passed Space-drag from shell/iframe, scale-independent pan, source/selection/page-scroll preservation, Escape recovery, typing/Interact guards, screen cancellation and Move/Draw exclusivity. Initial runs exposed the dock interception and overly broad Actions keywords; both fixed. An intermediate search assertion was scoped to the Actions dialog. Logs /private/tmp/retouch-align-keys-{react,liquid}-complete.log, /private/tmp/retouch-align-keys-pan-final.log and /private/tmp/retouch-align-keys-units.log. No native rebuild/launch or push. Single-layer alignment shortcut parity, full goal, trusted distribution and known WebKit normal-flow Fill-height behavior remain unfinished.


### 2026-09-12 — single-layer frame alignment

Single absolute HTML, React and Liquid layers now expose six compact frame-alignment icons, using the same toolbar and geometry helpers as shared selections. Alt alignment shortcuts and Actions search automatically cover these controls. One-layer geometry accepts an explicit frame target; distribution still requires a multi-layer selection. Alignment preserves the current anchor model, dimensions and content-box decorations. Class writes reuse scoped geometry generation, source uniqueness/inline guards and post-write bounds verification. HTML writes preserve its authored box model. The toolbar disappears where the active preview renders the layer in normal flow. Readable paired inset fields replace the narrow controls revealed in visual inspection.

All 927 units passed. RT_E2E_SINGLE_ALIGN passed React Chromium, Liquid WebKit and HTML Chromium: all six edges/centers, icon clicks and actual Alt keys, padded/bordered content-box 124x74 bounds, inherited important right anchor, exact-source Undo, tablet edits, phone flow isolation and toolbar visibility. Liquid screenshot /private/tmp/retouch-single-align-final.png inspected; each inset input has at least 60px rendered width. React initial run exposed an inactive-scope toolbar; corrected to check rendered absolute positioning. Liquid initially lacked the toolbar because a React revision attribute gated it; now uses the common classSelection capability. The inset assertion was scoped to Position to exclude unrelated advanced fields.

Shared HTML full selection/frame/layer alignment and distribution suite passed after toolbar extraction. Liquid shared Shift/group alignment, six Alt shortcuts, Actions search, focus/Hand/field guards and conversion Undo/Redo passed. Logs /private/tmp/retouch-single-align-react-final.log, /private/tmp/retouch-single-align-liquid-final.log, /private/tmp/retouch-single-align-html.log, /private/tmp/retouch-single-align-fields-final.log, /private/tmp/retouch-single-align-shared-html.log, /private/tmp/retouch-single-align-shared-keys.log and /private/tmp/retouch-single-align-units.log. No native build/launch/push. Full parity, trusted distribution and the known WebKit normal-flow Fill-height issue remain incomplete.


### 2026-09-12 — Liquid single-layer Move and Resize controls

Editable Liquid host classes now expose the same single-layer canvas Move and Resize controls as React, using classSelection capability alongside the existing React revision path. Host movement/resizing reuses the validated class geometry strategy, preserving content-box dimensions and per-property important anchors. It validates active scope, unique source rendering and inline geometry at gesture entry and commit. Existing React instance behavior is retained through its prior path. Shared/single alignment use the same single-member strategy adapter. A null no-op member now returns without saving rather than being projected into empty scope classes.

React Chromium and Liquid WebKit passed keyboard move (1px horizontal, Shift 10px vertical), unchanged source/page bounds until commit, pointer east-edge resize by 20px with snapping disabled, preserved content-box model, Escape cancellation, exact-source Undo and refusal when inline geometry changes during a gesture. Liquid additionally verifies a 0.01px no-op leaves all source bytes intact and a tablet movement leaves phone flow untouched before restoring tablet geometry. Existing six alignment/scope/Undo tests ran afterward. All 927 unit tests passed. Logs /private/tmp/retouch-single-canvas-react.log, /private/tmp/retouch-single-canvas-liquid.log, /private/tmp/retouch-single-canvas-liquid-final.log and /private/tmp/retouch-single-canvas-units.log. No native build/launch/push. Full parity, trusted distribution and the known WebKit normal-flow Fill-height regression remain incomplete.


### 2026-09-12 — HTML canvas box model and constrained single-layer resize

HTML single-layer Move/Resize now retains authored box sizing through the same preserveBox conversion as alignment. It checks active pixel scope and absolute positioning at entry, retains scope at commit, and refuses to save if layer/parent geometry changes during the gesture. The constrained browser fixture exposed a second legacy assumption: canvas-move's single-layer resize limits still treated content-box max sizes as border-box values. A preserveBox option now makes preview limits follow the writer: true for HTML and editable class hosts, default true for groups, prior behavior retained for the legacy component-instance path. Limit revalidation uses that same option.

The shared single-canvas fixture now includes max-width:120px/max-height:70px around a 100x50 content box with 24px decorations. HTML Chromium and WebKit plus React Chromium and Liquid WebKit passed keyboard movement, pointer resize from border-box width124 to144 without shrinking height74, preview/source separation until commit, content-box retention, Escape cancellation, exact Undo, tablet/phone isolation, and refusal on a mid-gesture inline position change. Existing six frame alignments also passed. Initial HTML runs showed incorrect120x70 bounds; corrected resize limit calculation produced144x74. All927 units passed on the final runtime. Logs /private/tmp/retouch-html-canvas-box-{chromium,webkit}-final.log, /private/tmp/retouch-canvas-box-limits-{react,liquid}.log and /private/tmp/retouch-canvas-box-limits-units.log. No native build/launch/push. Full parity, trusted distribution and known WebKit normal-flow Fill-height remain incomplete.


### 2026-09-12 — desktop candidate with current canvas controls

Built clean 2970d3c5e029b4a8776a39b56def9cd4ec37b376 into /private/tmp/retouch-desktop-canvas-2970d3c/Retouch-0.1.0-mac.zip, SHA256 2c31515d9c1b590633aac70e2b5b9ec7c80af60e108c6a0ae0aedb67f298ca58. All178 source hashes and native/build/verifier hashes match the checkout; universal arm64/x86_64 and strict ad hoc signatures passed after extraction and again after browser workflows. Matching source runtime has927 passing units. Generated cask syntax passed; installation was not repeated.

An external harness against extracted src/bin/shell passed React Chromium single alignment/move/constrained resize, Liquid WebKit shared conversion/Shift alignment/Alt shortcuts/Actions/input and Hand guards, and HTML WebKit single alignment/move/constrained resize, each with scope and Undo checks. Known packaged Liquid/WebKit Fill-height regression still fails at180px instead of170px with margins. The external harness was removed and verified absent. CUA inventory preflight failed with Sky Computer Use native pipe startup failed; no app launched, and native self-tests remain skipped/unverified. Receipt desktop/verification/2026-09-12-canvas-controls.json records exact artifact, results and log paths. No signing identity, notarization, publication or push. Full parity and trusted native delivery remain incomplete.


### 2026-09-12 — direct X/Y position fields

The single absolute-layer Position inspector now shows paired X and Y pixel fields beneath the alignment row. They measure from the containing frame and write through the existing scoped, anchor-preserving HTML/class geometry callbacks. Typing supports Enter/blur commit and Escape cancellation. Label dragging previews a translation of the selected layer, restores the original inline style before committing, and produces one source edit on release. Existing numeric drag interruptions cancel on scope/viewport changes and disconnected controls. Bounds remain limited to100000px and negligible changes are no-ops.

React Chromium, Liquid WebKit and HTML Chromium passed typed X/Y placement, unchanged dimensions, right-anchor preservation, exact source Undo, tablet edit/phone flow isolation, live 10px scrub without source writes, Escape restoration and single release commit with no residual translate style. Liquid additionally passed invalid100001 rejection and scope-switch-during-drag cancellation. Screenshot /private/tmp/retouch-position-xy.png inspected: paired fields fit below six alignment icons. All927 unit tests passed. Logs /private/tmp/retouch-position-xy-{react,liquid,html}.log, /private/tmp/retouch-position-xy-interrupt.log and /private/tmp/retouch-position-xy-units.log. No native build/launch/push; desktop candidate2970d3c predates these fields. Full Figma parity, trusted distribution and known WebKit normal-flow Fill-height remain incomplete.


### 2026-09-12 — shared X/Y position fields

Shared absolute selections now expose paired X/Y fields beneath the alignment toolbar. Siblings use containing-frame coordinates; selections across frames use page coordinates. Typed edits and label drags move every member by the same delta through the existing atomic, scoped geometry writer. Preview does not write source, release creates one edit, and Escape or a scope change restores the original styles.

All 927 unit tests passed. React Chromium, HTML Chromium and Liquid WebKit passed shared typed placement, label-drag preview/commit/cancel, exact Undo and phone isolation. Liquid WebKit additionally passed switching scope during a held drag. The three-sibling HTML suite passed frame-relative X/Y and its alignment/distribution regressions. Logs: /private/tmp/retouch-shared-xy-{react,html,liquid,scope,frame,units}.log. Screenshot /private/tmp/retouch-shared-xy-aligned-controls.png inspected. Desktop candidate 2970d3c predates both single and shared X/Y controls. Full Figma parity, trusted native distribution and the known WebKit normal-flow Fill-height regression remain incomplete.


### 2026-09-12 — shared class rotation

React and Liquid shared selections now expose Rotation (degrees) with scoped class writes, reset, mixed values and numeric label-drag preview. Scalar CSS rotation is normalized from degrees/radians/gradians/turns; inline and 3D rotation are guarded. Existing transform and axis classes remain authored. This edits each layer's own rotation, not a group transform; controls currently live in Appearance, and rotated canvas geometry editing remains incomplete.

All 928 units passed, including responsive/important/reset ownership and angle conversion. React Chromium and Liquid WebKit passed typed rotation, tablet/phone isolation, live label dragging without source writes, Escape cancellation, one commit on release, restored inline styles and exact Undo. Liquid additionally passed invalid angle rejection and scope changes during a held drag. Screenshot /private/tmp/retouch-shared-rotation-liquid.png inspected. Logs /private/tmp/retouch-shared-rotation-{react,liquid,interruption,units}.log. No native build or push for this increment; the known WebKit normal-flow Fill-height regression and full parity remain open.


### 2026-09-12 — single-layer rotation in Position

React and Liquid single layers now expose Rotation in Position, with inherited important priority, responsive class ownership, reset and live numeric label dragging. The field is mounted before absolute geometry measurement, so rotation remains editable when a rotated layer cannot use existing axis-aligned geometry controls. HTML rotation now appears in its Position section when available and normalizes angle units using the shared reader; inline and 3D rotation are guarded. HTML opacity/rotation fields now use the shared Enter/Escape draft behavior.

All 928 unit tests passed. React Chromium, Liquid WebKit and HTML Chromium passed inherited important -10 degree rotation overridden to30 at tablet size, phone isolation, reset, exact Undo, live preview without source changes, Escape cancellation and release commit without residual inline rotation. Liquid WebKit also passed with an absolute positioned layer, proving the field remains accessible when geometry measurement refuses transforms. The existing React single alignment/X-Y/move/resize regression passed. Screenshots inspected: /private/tmp/retouch-single-rotation-liquid.png. Logs /private/tmp/retouch-single-rotation-{react,liquid,html,absolute,position-regression,units}.log. No native build or push. Shared rotation still lives in Appearance; full rotated geometry editing, full Figma parity, trusted distribution and the known WebKit normal-flow Fill-height regression remain open.


### 2026-09-12 — shared Position rotation and neutral rotation geometry

The inspector organizer now moves shared Rotation and its reset into Position for React/Liquid and HTML, retaining accessible names and existing handlers. HTML shared rotation/opacity now have numeric label dragging with live previews, source writes only on release, Enter/Escape draft behavior and restored inline styles. HTML shared rotation uses the angle reader and guards non-scalar/inline rotation.

Geometry now accepts the neutral computed rotation 0deg. Body containing-block checks in geometry, frame alignment and canvas limits/guides distinguish a body with a neutral rotation from the untransformed viewport case. Other nonzero rotations and transforms remain outside current canvas geometry support.

All 928 units passed. Shared rotation in React Chromium and HTML Chromium passed Position placement, typed/scoped writes, live scrub, cancellation, exact Undo and scope interruption. Liquid WebKit passed the full single X/Y/alignment/move/resize suite with neutral rotation, plus a real browser probe proving a static body with rotate:0deg uses body-relative20/40 coordinates and body width. Logs /private/tmp/retouch-rotation-position-{react,html,units}.log, /private/tmp/retouch-zero-rotation-{canvas,body}.log. HTML screenshot inspected; redundant visible Shared rotation label shortened afterward. No native build or push. Full parity, nonzero rotated canvas geometry, trusted distribution and known WebKit normal-flow Fill-height remain incomplete.


### 2026-09-12 — nonzero rotated single-layer X/Y and alignment

Added optional rotation-aware layout geometry for a layer's own scalar CSS rotate. It recovers the unrotated layout rectangle mathematically from the visual bounds, resolved box dimensions, angle and transform origin without mutating page styles. Ancestor rotations, transform matrices, scale/translate/zoom and 3D origins remain guarded. Single absolute-layer X/Y and six frame alignment controls opt into this geometry; class save verification carries that measurement mode. Anchor presets and canvas move/resize retain their existing geometry restrictions. HTML controls mount before the strict geometry refusal so supported placement remains available.

All929 unit tests passed, including off-center and negative-angle rectangle recovery. React Chromium, Liquid WebKit and HTML Chromium passed X/Y placement and all six visual frame alignments on a30 degree rotated, padded content-box layer with a20px10px origin, preserving rotation,100x50 content dimensions, right-anchor ownership, tablet/phone scope and exact Undo. Liquid additionally passed live X-label scrub, untouched source during preview, Escape cancellation, single release commit and no residual translate. Screenshot /private/tmp/retouch-rotated-position-liquid.png inspected; afterward the existing generic red transform refusal was replaced with an accurate neutral note about remaining canvas/anchor limitations. Logs /private/tmp/retouch-rotated-position-{react,liquid,html,scrub,units}.log. No native build or push. Full parity, rotated canvas transforms/groups, trusted distribution and the known WebKit normal-flow Fill-height regression remain incomplete.


### 2026-09-12 — rotated single-layer canvas movement

Single-layer canvas Move now uses rotation-aware layout geometry in React/Liquid and HTML while the canvas overlay/snapping continues to track the visual bounding rectangle. Writes preserve the original layout size, rotation and anchors. Rotated layers expose Move beside their X/Y/alignment controls. Commit rechecks the layer's layout rectangle, parent dimensions and rotation, refusing stale gestures. Resize remains guarded for nonzero rotation.

Fixed a duplicated body-rotation expression in canvas resize limits that referenced local width before initialization for a static, unrotated body. A browser regression now mounts and cancels an actual viewport-relative resize overlay before verifying the neutral-rotation body coordinates.

All929 unit tests passed. React Chromium, Liquid WebKit and HTML Chromium passed rotated keyboard movement (+1,+10), pointer movement (+20,+15) with snapping disabled, untouched source/page bounds during preview, Escape, preserved30 degree angle/content-box dimensions, exact Undo and refusal when rotation changes to390 degrees mid-gesture. Prior rotated X/Y, scope and six alignment cases also passed. HTML WebKit passed viewport resize mount plus the neutral rotation single X/Y/alignment/move/resize suite. Logs /private/tmp/retouch-rotated-move-{react,liquid,html,units}.log and /private/tmp/retouch-viewport-resize-regression.log. Screenshot /private/tmp/retouch-rotated-position-liquid.png inspected. No native build or push. Full parity, rotated resizing/group geometry, trusted distribution and the known WebKit normal-flow Fill-height regression remain incomplete.


### 2026-09-12 — rotated multi-selection positioning

Shared selection geometry now measures each layer's own scalar rotation while retaining its unrotated layout size. Alignment and spacing operate on visual bounds; X/Y and canvas Move write layout-coordinate translations. Class batch save verification uses the same rotation-aware geometry. Every shared geometry write rechecks layout bounds, parent dimensions and rotation against the gesture measurement. Group Resize is disabled for rotated members and still validates strict geometry at invocation. Flow conversion measurement also uses the rotation-aware path; rotated flow conversion is not yet specifically browser verified.

All929 units passed. React Chromium, Liquid WebKit and HTML Chromium passed six shared alignments on30/-20 degree layers with different origins, shared X, group keyboard movement, exact visual gaps, preserved bounds/angles, exact Undo and refusal after a mid-gesture angle change. Liquid additionally passed all six Shift group-to-frame alignments and tablet edits preserving phone flow/unrotated styles. Existing three-layer HTML alignment/distribution/gap/reference/scope/atomic-refusal suite passed. Logs /private/tmp/retouch-shared-rotated-{react,liquid,html,frame-scope,units,regression}.log. Screenshot /private/tmp/retouch-shared-rotated-liquid.png inspected. No native build or push. Full parity, rotated resizing, transformed ancestor geometry, trusted distribution and known WebKit normal-flow Fill-height remain incomplete.


### 2026-09-12 — desktop candidate with rotation controls

Built clean432012328dea014a984cc0cb756105cb3c0cf347 into /private/tmp/retouch-desktop-rotation-4320123/Retouch-0.1.0-mac.zip, SHA256 67cb76044d99d987c39dd350b9c61f79b5d60b37efe11d3f6f7b4e424f9680b7. All178 packaged source hashes and native/build/verifier hashes match. Universal arm64/x86_64 and strict signatures passed after extraction, packaged workflows and isolated Homebrew installation. Matching source runtime has929 passing units.

Extracted-package workflows passed React Chromium rotated single X/Y/alignment/scrub/keyboard and pointer movement, Liquid WebKit shared rotated/frame alignment/movement/gaps/scope/Undo/refusal, and HTML WebKit viewport resize/neutral-body geometry/single canvas regressions. Known packaged WebKit Fill-height regression still fails at180px instead of170px with margins. Generated cask syntax passed; Homebrew installed and uninstalled this exact archive in an isolated app directory while retaining quarantine. Automatic dependency removal/cleanup were disabled; original cask inventory and Git remained intact. Temporary app directory, cask registration, tap, trust configuration and external test harness were verified removed.

CUA native preflight still failed with Sky Computer Use native pipe startup failed. No app was launched; native self-tests remain unverified. Receipt desktop/verification/2026-09-12-rotation-controls.json records the artifact, checks and logs. No DeveloperID signing, notarization, publication or push. Full parity and trusted native distribution remain incomplete.


### 2026-09-12 — preserve rotated layout boxes during flow conversion

New browser coverage exposed a real rotated flow-to-absolute bug: conversion replaced layout width/height with the rotated visual bounding box, enlarging both layers. Rotation-aware geometry now carries unrotated viewport layout coordinates. Batch conversion uses those coordinates across simultaneous containing-frame reflow and retains original layout dimensions, including automatic widths and percentage transform origins.

All929 units passed. React Chromium, Liquid WebKit and HTML Chromium now pass rotated flow conversion preserving visual bounds, CSS dimensions, angles and origins, followed by shared X/Y typing/scrubbing/cancellation, scope interruption, six Shift frame alignments, toolbar behavior, phone isolation and exact source Undo/Redo. Liquid additionally passed automatic width with a25%75% transform origin. Initial failures enlarged a168.56x131.96 visual box to211.94x198.55; corrected conversion preserves it. Rotated alignment assertions use0.04px tolerance for browser subpixel quantization (observed about0.01px); existing unrotated exact assertions and source-byte checks remain unchanged. Logs /private/tmp/retouch-rotated-flow-{react,liquid,html}-final.log, /private/tmp/retouch-rotated-flow-auto.log and /private/tmp/retouch-rotated-flow-units.log. The desktop4320123 candidate predates this fix; no rebuild or push. Full parity, rotated resizing, transformed ancestor geometry, trusted distribution and known WebKit normal-flow Fill-height remain incomplete.


### 2026-09-12 — oriented outlines for rotated layers

Single-element selection, co-selection, hover and editing outlines now follow supported scalar rotation and transform origin using the unrotated layout rectangle. Multi-element component groups retain their union outline; unsupported transforms retain the prior visual-bounds fallback. Geometry has a read-only layoutOnly path that returns before temporary absolute-position measurement, so painting a rotated normal-flow layer does not mutate its style on every animation frame.

All929 unit tests passed. React Chromium and Liquid WebKit passed outline angle/dimensions/origin and world-coordinate alignment at50/100/125% zoom, no style mutations across five paint frames on a temporary normal-flow layer, and the full rotated X/Y/alignment/movement/Undo suite. HTML Chromium passed independently oriented30/-20 degree multi-selection outlines plus group geometry regressions. First WebKit run left the inspector in the test's temporary flow state; explicitly restoring the panel after that test-only probe fixed the harness and the final run passed. Screenshot /private/tmp/retouch-rotated-outline-liquid.png inspected. Logs /private/tmp/retouch-rotated-outline-{react,shared,units}.log and /private/tmp/retouch-rotated-outline-liquid-final.log. No native rebuild or push. Full parity, rotated resizing, transformed ancestor geometry, trusted distribution and known WebKit normal-flow Fill-height remain incomplete.


### 2026-09-12 — oriented movement previews and light canvas hints

Canvas Move previews now draw each supported rotated member using its layout box, angle and scaled transform origin. A single rotated layer uses that outline instead of the axis-aligned placeholder; groups retain their union box and individually oriented members. Preview outlines and resize handles use the shared blue accent. The gesture hint uses light panel colors, sits above the bottom dock, and includes keyboard instructions. Duplicate startup instruction toasts were removed.

All929 units passed. React Chromium and Liquid WebKit passed preview world-coordinate alignment at50/100/125% zoom, cancellation/source preservation, and the full rotated X/Y/alignment/keyboard-pointer movement/Undo suite. HTML Chromium passed independently rotated group previews and shared geometry cases. Initial zoom tests attempted movement with the frame outside the usable view; event/error tracing identified the existing bring-into-view refusal. The harness now frames the canvas and settles layout before opening each preview; cancellation guards were unchanged. Final light-hint WebKit run passed and /private/tmp/retouch-rotated-move-preview-liquid.png was inspected. Logs /private/tmp/retouch-rotated-move-preview-{react,liquid}-visible.log, /private/tmp/retouch-rotated-move-preview-{shared,units,light}.log. No native rebuild or push. Full parity, rotated resizing, transformed ancestor geometry, trusted distribution and known WebKit normal-flow Fill-height remain incomplete.


### 2026-09-12 — direct canvas rotation

Single-layer Rotation fields now expose Rotate on canvas for HTML, React and Liquid. A canvas-clipped handle previews scalar rotation around the authored transform origin. Pointer release commits through the existing scoped field callback; arrows adjust 1 degree (Shift 15), Shift-drag snaps absolute angles to 15 degrees, Enter commits, and Escape cancels. Preview restoration precedes the source write, giving one exact undoable edit. Screen/scope/zoom/scroll changes, disconnected controls, selection/source identity changes and changed layout geometry cancel pending gestures. Unsupported transforms remain refused. Existing -360 to 360 field range remains; multi-layer canvas rotation and rotated resizing remain open.

Validation: 931 units passed. HTML Chromium, React Chromium and Liquid WebKit cover preview source isolation, pointer/keyboard commit, exact Undo, snapping, stale layout cancellation without residual inline rotation, screen cancellation, and 50/100/125 percent zoom. Existing single-field rotation/reset/scope/scrub checks also pass. Logs: /private/tmp/retouch-canvas-rotation-{html,react}-verified.log, /private/tmp/retouch-canvas-rotation-liquid-zoom.log, /private/tmp/retouch-canvas-rotation-units.log. Inspected /private/tmp/retouch-canvas-rotation-html.png; corrected initial handle overlap with toolbar by clipping its position to the canvas. One intermediate React run timed out on the first preview; diagnostic rerun passed without a confirmed root cause. Final harness waits two animation frames before opening the tool, and final React passed. Temporary diagnostic instrumentation removed.

Previous user-requested push verified GitHub at 1c77aa799709734a617da4b18222d3591e85a283. This increment is local only. No desktop rebuild or native launch; desktop candidate predates later source improvements. Full Figma parity, arbitrary-site guarantees, trusted macOS distribution, and known WebKit normal-flow Fill-height regression remain incomplete. Goal stays active.


### 2026-09-12 — rotate directly from selection corners

Single editable layers expose rotation hit regions just outside their four corners, with a rotation cursor and hover feedback. Their positions follow the layout box, authored pivot, scalar angle and canvas zoom; hit sizes stay constant in screen pixels. Controls remain mounted across paint frames, clip to the canvas, bind to the actual rotation field target, and hide during Hand mode, active tools, unavailable scopes, dialogs, busy states and multi-selection. A corner pointer-down transfers capture to the existing rotation tool so dragging continues outside the original hit region. The inspector entry remains available when corners are offscreen.

Chromium exposed a real deferred-inspector-refresh interruption: a focus/viewport refresh replaced the rotation input during the gesture. Same-selection inspector rendering now defers while the rotation surface exists, and the tool flushes deferred rendering after preview restoration. Source/selection identity and geometry cancellation guards remain. Diagnostic evidence in /private/tmp/retouch-rotation-corners-html-events.log showed current=false with preview restoration and no source save. Temporary instrumentation removed.

932 unit tests passed (/private/tmp/retouch-rotation-corners-units.log). React Chromium and Liquid WebKit passed compact absolute-layer corner drag/Undo/Escape/Hand checks plus existing rotation pointer/keyboard/snap/stale-layout/scope/zoom/field tests; HTML Chromium passed the same flow with a wide normal-flow heading. Final logs /private/tmp/retouch-rotation-corners-{react,html,liquid}-fixed.log. Explicit renderPanel during a held corner gesture retains the source field and commits correctly: /private/tmp/retouch-rotation-corners-refresh.log. Inspected /private/tmp/retouch-rotation-corners-liquid.png. Earlier failed Chromium logs are retained; final runs pass after the refresh fix. Local increment only; no push, native rebuild or launch. Full parity, rotated resize, multi-layer canvas rotation, transformed ancestor support, arbitrary-site guarantees, trusted desktop distribution and known WebKit Fill-height regression remain open.


### 2026-09-12 — single-layer rotated canvas resizing

Resize on canvas is now available for supported scalar-rotated absolute HTML, React and Liquid layers. Pointer deltas are mapped into local axes; the preview uses the layout box, rotation and transform origin. The size-dependent origin is resolved through a synchronous temporary dimension measurement with exact style restoration. Translation compensates for the origin change, preserving the opposite edge/corner for fixed and percentage origins. Content-box sizing, existing min/max limits, proportional and centered resize, keyboard editing, cancellation and source Undo are retained. Gesture observation additionally cancels if the selected layer’s angle or transform origin changes. Anchor presets remain unavailable for rotated layers; multi-layer rotated resizing, transformed ancestors and rotated edge snapping remain open.

933 unit tests passed, including mathematical fixed-anchor invariants across eight handles, three angles and fixed/percentage origins. React Chromium and Liquid WebKit passed rotated pointer resize, preview/source dimension agreement, fixed opposite edge, keyboard commit/Escape, Shift+Alt proportional resizing from center, screen cancellation and exact Undo alongside existing rotated position/alignment tests. Liquid uses a percentage origin and content-box max-size constraints; React additionally reruns rotated Move. HTML Chromium passed percentage-origin resizing and centered proportional modifiers; existing unrotated HTML canvas resizing/alignment also passes. Logs: /private/tmp/retouch-rotated-resize-{react,liquid}-verified.log, /private/tmp/retouch-rotated-resize-html-modifiers.log, /private/tmp/retouch-rotated-resize-unrotated-regression.log, /private/tmp/retouch-rotated-resize-units.log. Inspected /private/tmp/retouch-rotated-resize-html.png.

Initial test expectations assumed fractional pointer coordinates were preserved identically across engines; WebKit’s pointer quantization produced about 0.58px difference. Tests now verify actual committed content width agrees with the rendered resize preview within 0.04px, retaining the existing 0.6px world-geometry tolerance. A modifier harness attempt released Alt/Shift before Enter, legitimately reverting the preview; final test holds modifiers until commit. No assertions about exact source Undo were weakened. Local commit only; no push, native rebuild or launch. Full Figma parity, arbitrary-site guarantees, trusted desktop distribution and known WebKit normal-flow Fill-height remain incomplete. Goal stays active.


### 2026-09-12 — direct selection resize handles

Supported single absolute layers now expose eight resize handles directly on their selection outline; rotation remains just outside the corners. Handle positions follow the layout box, scalar rotation and authored transform origin. Handles remain 9 screen pixels at every canvas zoom, and both direct and active-tool resize cursors follow rotated local axes. Direct pointer-down transfers capture into the existing resize gesture and source-writing callbacks; inspector controls remain keyboard-accessible. Handles hide during active tools, Hand mode, busy states, dialogs, multi-selection and unavailable edit scopes. Rotation-field target binding keeps geometry attached to the actual edited node. Read-only inline rotation can retain resize handles when resizing is otherwise allowed. Flow-layer resizing and shared direct handles remain open.

934 units passed (/private/tmp/retouch-direct-resize-units.log). React Chromium, Liquid WebKit and HTML Chromium pass direct rotated edge resize with exact Undo, Escape at50/100/125 percent zoom, constant-size handles, rotated cursor, Hand hiding, plus existing proportional/centered/keyboard/scope geometry regressions. Liquid includes percentage origin and content-box max-size constraints. HTML also passes unrotated direct edge resize and existing frame alignment. Logs /private/tmp/retouch-direct-resize-{react,liquid}.log, /private/tmp/retouch-direct-resize-html-final.log, /private/tmp/retouch-direct-resize-unrotated.log. Inspected /private/tmp/retouch-direct-resize-selection-html.png showing idle selection handles with no active inspector tool. Local increment only; no push or desktop rebuild/native launch. Full parity, arbitrary-site guarantees, trusted distribution, shared rotated resize, rotated snapping and known WebKit normal-flow Fill-height remain incomplete.


### 2026-09-12 — desktop direct-manipulation candidate and cask receipt

Built the clean6107d56 source as /private/tmp/retouch-desktop-direct-6107d56/Retouch-0.1.0-mac.zip (SHA2569068bf6aef261ab9e247e07a284f79ebdd724cec9d7e56252492f2ddb8467fe7). All179 packaged source hashes and native/build/verifier hashes match. Universal arm64/x86_64 and strict ad hoc signature verification passed after extraction, packaged workflows and actual isolated Homebrew installation. Matching-source934 units passed. External-harness packaged React Chromium and Liquid WebKit direct/rotated resize workflows passed; packaged HTML Chromium direct corner rotation and refresh-during-gesture regression passed. Harness removed without modifying bundle resources.

Homebrew cask install and uninstall passed with quarantine preserved. Temporary app directory, cask registration, task tap, isolated trust configuration and harness are verified absent; original cask inventory unchanged and Git2.55.0 retained. Automatic cleanup/autoremove disabled throughout. The packaged Liquid WebKit normal-flow Fill-height regression was freshly reproduced: expected[[250,180],[210,170]], actual[[250,180],[210,180]]. Native CUA preflight again failed with Sky Computer Use native pipe startup failed; no app was launched and no native self-tests ran. No security settings changed. Developer ID signing, notarization, publication, upgrades and full parity remain incomplete. This is a development candidate, not a release. Detailed evidence: desktop/verification/2026-09-12-direct-manipulation.json. No push; goal stays active.


### 2026-09-12 — resize layers within normal flow

Normal-flow layers now expose right, bottom and bottom-right resize handles plus a Layout inspector entry. A dedicated flow gesture previews actual width/height changes and sibling reflow rather than converting positioning or writing insets/margins. It respects the existing box model and browser size constraints. Resizing a flex main axis releases growth/shrink/basis as one fixed-size edit; the class writer uses existing sizing utilities and HTML writes equivalent flex longhands. Preview properties restore before one source transaction. Escape, outside click, pointer capture loss, viewport/scope/selection/source changes and external inline-style changes cancel. Same-selection inspector refreshes defer through the active flow draft. Keyboard edits can continue across width/height handles and commit together. Inline sizing, inline/contents/hidden display and unsupported transform geometry remain unavailable; complete flow/transform/grid/RTL coverage is not claimed.

All934 units passed (/private/tmp/retouch-flow-resize-units-final.log). HTML Chromium, React Chromium and Liquid WebKit passed normal-flow live sibling reflow, preserved static positioning/margins/untouched width, scoped source commits, phone isolation, exact Undo and two-axis cancellation. Growing column flex-item tests passed in all three renderers, including keyboard commit and screen cancellation: /private/tmp/retouch-flow-resize-{html,react,liquid}-verified.log. HTML additionally verifies switching keyboard handles preserves both dimensions in one source edit: /private/tmp/retouch-flow-resize-keyboard.log. Existing rotated absolute direct-resize regression passed (/private/tmp/retouch-flow-resize-absolute-regression.log). Inspected /private/tmp/retouch-flow-resize-html.png.

Initial HTML check exposed a layout-section visibility gate tied to child insertion; text layers now retain a Layout section when flow resize is available. A second HTML flex test exposed the authoring API rejecting the flex shorthand despite a correct preview; fixed by writing flex-grow, flex-shrink and flex-basis together. Diagnostic instrumentation removed; failed logs retained. Latest desktop6107d56 archive predates flow resizing; no rebuild, native launch or push. Full parity, arbitrary-site guarantees, trusted distribution and known WebKit normal-flow Fill-height remain incomplete; goal stays active.


### 2026-09-12 — live flow dimensions and rotated grid verification

Flow resize updates width/height inspector readouts during the live preview, retaining each field’s box-model semantics (class fields show border-box dimensions, HTML CSS fields show CSS dimensions). The gesture hint shows actual outer dimensions. Cancellation restores original field strings without firing change events or overwriting a separately changed field value. Shift press/release recomputes proportional preview immediately without another pointer move.

All934 units passed (/private/tmp/retouch-flow-readouts-units.log). HTML Chromium, React Chromium and Liquid WebKit pass single-column grid flow resizing with30-degree rotation and25%75% transform origin, real sibling reflow, scoped edits, exact source Undo, two-axis cancellation and matching live dimension readouts. React/WebKit additionally cover stationary Shift toggling. Logs /private/tmp/retouch-flow-grid-rotated-{html,react}.log and /private/tmp/retouch-flow-grid-rotated-liquid-final.log. HTML growing-flex regression verifies readouts, Shift toggling, switching keyboard handles, and external inline-color mutation cancelling the gesture while preserving that external change and restoring original dimension fields (/private/tmp/retouch-flow-readouts-flex.log). Inspected /private/tmp/retouch-flow-resize-html.png.

WebKit’s rounded pointer coordinates produced a rotated two-axis drag outside the original0.7px approximate-delta expectation. Rotated desired-delta checks now allow1.5px (covering coordinate rounding projected onto local axes), while committed width/height must match the actual preview within0.04px; unrotated tolerances and exact source Undo checks are unchanged. No desktop rebuild, native launch or push. Latest6107d56 archive predates flow resize and these improvements. Full parity, arbitrary-site guarantees, trusted distribution and known WebKit normal-flow Fill-height remain incomplete; goal stays active.


### 2026-09-12 — flow resize directions for RTL, vertical and reversed layouts

Flow resize chooses the physical growth edges from the containing layout’s writing mode, text direction, flex direction/wrap and common end alignment. Right/bottom handles become left/top where appropriate, including RTL blocks, vertical-rl RTL, row-reverse flex and end-aligned grid items. Direct selection handles and active-gesture handles share the mapping. Width/height previews use the selected edge’s sign; source writes still preserve positioning and authored margins. A changed direction mapping cancels an active draft.

All936 units passed (/private/tmp/retouch-flow-directions-units.log), including horizontal/vertical/sideways mappings, reversed flex axes and end-aligned grid cases. HTML Chromium RTL and fixed-track end-aligned grid tests, React Chromium row-reverse flex, and Liquid WebKit vertical-rl RTL verify visible handle sets, pointer growth, retained opposite edge, preview/source dimension agreement, exact source Undo and keyboard Escape. Logs /private/tmp/retouch-flow-direction-{rtl,grid,reverse}.log and /private/tmp/retouch-flow-direction-vertical-final.log. HTML RTL additionally verifies an external parent direction change cancels without source edits or residual inline sizing (/private/tmp/retouch-flow-direction-change.log); LTR flex/readout/Shift/Undo regressions pass (/private/tmp/retouch-flow-direction-ltr-regression.log). Inspected /private/tmp/retouch-flow-direction-vertical-liquid.png.

The first vertical test incorrectly required the used margin shorthand to remain constant. WebKit resolves the overconstrained vertical box with a used top margin that changes as height grows. The final test verifies unchanged authored margin utility tokens and the fixed opposite edge instead; no source margin changes or source-Undo assertions were weakened. Exhaustive CSS alignment, orthogonal child writing modes and floating layouts remain outside current evidence. No desktop rebuild, native launch or push. Full parity, arbitrary-site guarantees, trusted distribution and known WebKit normal-flow Fill-height remain incomplete; goal stays active.


### 2026-09-12 — proportional flow resize bounds

Shift resizing now corrects both dimensions from browser-resolved minimum/maximum bounds, preserving the original border-box aspect ratio without replacing percentage or calc constraints. Incompatible bounds cancel the draft. HTML Chromium, React Chromium and Liquid WebKit verify minimum/maximum clamping, unchanged source during preview, matching committed dimensions and exact Undo. HTML flex/readout/Shift regression passes; all 937 unit tests pass. Logs: /private/tmp/retouch-push-{react,liquid,flow-regression}.log and /private/tmp/retouch-push-units-final.log. The initial unit assertion exposed floating-point associativity only; its width comparison now uses a 1e-10 tolerance. No desktop rebuild; the known WebKit Fill-height regression and broader parity work remain open.


### 2026-09-12 — centered layout resize handles

Grid items with center self/item alignment and flex items centered on the cross axis now expose handles on both sides of each centered axis. Pointer deltas account for both edges moving, so a 20px edge drag grows that dimension by 40px while retaining the layout center. Keyboard arrows still change the dimension by 1px. The parent writing mode maps centered inline/block axes to physical axes; explicit child self alignment overrides parent item alignment. A changed handle mapping cancels an active gesture.

Before-fix HTML Chromium evidence reproduced 20px total growth for a 20px edge drag (/private/tmp/retouch-flow-center-before.log). Final HTML grid and React flex Chromium tests and Liquid grid WebKit tests pass both growth edges, center retention, preview/source agreement, exact Undo, responsive scope, Shift corner proportions, Escape and external alignment interruption. Logs: /private/tmp/retouch-flow-center-{html,react}-verified.log and /private/tmp/retouch-flow-center-liquid-final.log. End-aligned grid regression passes (/private/tmp/retouch-flow-center-end-regression.log). All 938 units pass (/private/tmp/retouch-flow-center-units.log). Inspected /private/tmp/retouch-flow-center-liquid.png. One verification attempt assigned the WebKit-only browser directory to Chromium; it failed before launch and was rerun with Chromium's normal browser location.

This does not yet cover auto-margin centering, flex main-axis distribution, safe/unsafe alignment modifiers, auto-sized grid-track changes, or all rotated/orthogonal layouts. No desktop rebuild, native launch or push in this increment. Full Figma parity, arbitrary-site coverage, trusted desktop distribution and the known WebKit normal-flow Fill-height regression remain incomplete.


### 2026-09-12 — typography primary field layout

Compared the inspector against Figma's official typography reference (https://help.figma.com/hc/en-us/articles/360039956634-Explore-text-properties, image https://help.figma.com/hc/article_attachments/27469557155991). Primary controls now appear together in font-family, weight/size, line-height/letter-spacing and alignment order. Font family spans the row; spacing uses consistent SVG icons and explicit captions. Existing controls, accessible names, numeric scrubbing handlers, source transactions and auxiliary settings are retained. This improves that section's fidelity but is not a claim of complete visual parity.

HTML/React Chromium and Liquid WebKit verify size and alignment changes with exact source Undo (/private/tmp/retouch-typography-{html,react,liquid}.log). Inspected screenshots /private/tmp/retouch-typography-primary-{html,react}.png. HTML's broader font-catalog and line-height suite passes (/private/tmp/retouch-typography-line-height-final.log), including automatic/custom line height, responsive isolation, reset and source Undo. Its initial run timed out because Type settings was closed after changing scope; the test now opens the disclosure via its summary before accessing those actions. All 24 targeted inspector unit tests pass (/private/tmp/retouch-typography-inspector-units.log). No desktop rebuild, launch or push. Full parity and existing platform/geometry gaps remain open.


### 2026-09-12 — logical text alignment and keyboard controls
...
### 2026-09-12 — keyboard layout mode toolbars
...
### 2026-09-12 — consistent HTML layout mode row

HTML stack/adaptive-grid presets now use the same compact SVG icon vocabulary and segmented treatment as class-based layout modes. Their accessible names and keyboard behavior are retained. A Normal flow action writes display:block as one source transaction; the row order is normal, vertical, horizontal, adaptive grid. Active normal flow includes existing ordinary block/inline/flow-root/list-item layouts. Detailed CSS controls remain available.

HTML Chromium, React Chromium and Liquid WebKit pass the extended keyboard sequence normal flow to vertical flex to grid to normal flow, followed by three independent exact source Undo steps. Logs /private/tmp/retouch-layout-modes-{html,react,liquid}.log. Inspected /private/tmp/retouch-layout-modes-html.png for row order, selected/focused state and layout. All 18 inspector unit tests pass (/private/tmp/retouch-layout-modes-units.log). No desktop rebuild, native launch or push; full parity remains incomplete.


### 2026-09-12 — collapsible inspector sections

Top-level property section headings now offer keyboard-accessible collapse/expand buttons with aria-expanded and persistent in-session collapse state keyed by section identity. Existing headings retain their accessible names. Content stays in place with its handlers/drafts; collapsed controls leave layout and keyboard traversal. Section add actions remain visible and reopen their section before running. Shared groups retain their existing disclosure mechanism.

HTML Chromium, React Chromium and Liquid WebKit pass collapse persistence across selection, viewport/scope changes, expansion via Enter, a scoped font edit, exact Undo while collapsed, and adding/undoing a gradient from a collapsed Fill section. Logs /private/tmp/retouch-section-collapse-{html,react,liquid}-final.log. Initial HTML run selected an unavailable tablet scope before switching the viewport; the test now sets the tablet viewport first. Inspected /private/tmp/retouch-section-collapse-react.png for section headings and focus treatment. All 938 units pass (/private/tmp/retouch-section-collapse-units.log). No desktop rebuild, native launch or push. Session reload persistence and full parity remain outside this increment.


### 2026-09-12 — canvas editing with collapsed sections
...
### 2026-09-12 — searchable property navigation

Actions now includes Edit commands for width, height, rotation, opacity, corner radius, font family/size/weight, line height and absolute/relative letter spacing when the corresponding editable controls exist. Executing a property command shows the inspector, expands its containing property section and nested disclosures, scrolls to the control and focuses/selects its value without writing source. Disabled/read-only controls remain unavailable. Reveal uses the section preference setter so the expanded state remains coherent.

HTML Chromium, React Chromium and Liquid WebKit pass property jumps from a hidden inspector/collapsed Typography to font size and nested relative letter spacing, followed by real edits and exact Undo. Logs /private/tmp/retouch-property-jump-{html,react,liquid}.log. The full Actions suite passes Chromium/WebKit (/private/tmp/retouch-property-actions-{chromium,webkit}-final.log), covering action availability/search/keyboard, iframe shortcuts, layer/context-menu operations and source history. Initial runs exposed an outdated test expecting a source write when applying 100% opacity to an already-opaque layer. Tests for 0 and 100 now prepare 40% opacity, verify restoration to 100%, and separately undo both edits; production opacity behavior was not changed. All 25 inspector/interaction units pass (/private/tmp/retouch-property-jump-units.log). No desktop rebuild, native launch or push. Full parity remains incomplete.


### 2026-09-12 — property commands for multi-selection

Property Actions now resolve shared field labels in both class and HTML editors. Commands identify their scope explicitly (for example Edit shared font size), use the existing reveal/focus path for nested shared disclosures, and retain disabled/read-only availability checks. Merely focusing a mixed field does not resolve it or author a change.

HTML Chromium, React Chromium and Liquid WebKit pass shared-property navigation from a hidden inspector, preserve the blank Mixed font-size field until explicit input, apply a common size to two layers, and restore exact source with Undo. Shared relative letter spacing applies 10% against each layer's distinct restored font size and also undoes exactly. Logs /private/tmp/retouch-shared-property-jump-{html,react,liquid}.log. Single-layer property navigation remains passing (/private/tmp/retouch-shared-property-single-regression.log), as do 25 inspector/interaction units (/private/tmp/retouch-shared-property-units.log). No desktop rebuild, native launch or push. Full parity remains incomplete.


### 2026-09-12 — Prefer standard stretch after the legacy Fill fallback

Normal-flow Fill now emits `[height:-webkit-fill-available] h-[stretch]` (and the width equivalent), retaining scope and important priority. Tailwind emits arbitrary properties before dimension utilities; the former arrangement reversed the intended cascade, so the legacy value won even when standard stretch was supported. Both declarations survive Liquid class merging. Existing declarations migrate on the next Fill edit and reset together. This changes newly authored classes, not previously saved site files automatically.

The full mixed-parent Fill editor workflow now passes Liquid in Playwright WebKit 26.6 and React/Liquid in Chromium 145.0.7632.6: width/height Fill, responsive growth, fixed sizing, breakpoint isolation and exact Undo. Before this fix, the same newer WebKit Liquid test failed with 180px height instead of the margin-aware 170px. Logs: `/private/tmp/retouch-flow-fill-modern-liquid-fixed.log`, `/private/tmp/retouch-fill-cascade-{react,chromium}.log`. All 962 existing units pass (`/private/tmp/retouch-fill-cascade-units.log`); all 39 layout tests pass after adding the migration/class-merge regression (`/private/tmp/retouch-fill-cascade-layout.log`).

Added `retouch/test/e2e/flow-fill-compat.cjs` and its standalone HTML fixture to isolate engine behavior for content-box/border-box sizing, changing container dimensions, and pixel/percentage margins. All six cases pass Chromium and newer WebKit (`/private/tmp/retouch-fill-cascade-compat-{chromium,modern}.log`). Browser harnesses accept `RT_E2E_PLAYWRIGHT_ROOT` separately from the application fixture and report the actual engine version. Newer Playwright 1.63.0 lives in `/private/tmp/retouch-webkit-compat-1.63.0`, with browsers in `/private/tmp/retouch-webkit-compat-browsers-1.63.0`; baseline dependencies/cache are untouched.

Older Playwright WebKit 26.0 still fails the unchanged full editor expectation, expected `[[250,180],[210,170]]`, actual `[[250,180],[210,180]]` (`/private/tmp/retouch-fill-cascade-old-webkit.log`). Its unsupported standard stretch leaves the legacy margin-height defect. This remains unresolved; neither native Safari behavior nor the desktop package is claimed fixed. No desktop rebuild, native launch or push for this increment. Full Figma parity and arbitrary-site support remain incomplete.


### 2026-09-13 — Direct resizing for flipped and scaled absolute layers

Single absolute layers with supported individual 2D scale now retain canvas resize controls and eight direct edge/corner handles, including negative and non-unit scale combined with rotation. Pointer movement is inverted through rotation and signed scale into source dimensions. The source placement compensates for changes to fixed or percentage transform origins, keeping the opposite local anchor stationary (or the center for Option/Alt). Existing proportional resizing, box-model preservation, source size limits, breakpoint scope and exact history paths remain in use. Keyboard arrows adjust local source dimensions as stated in the gesture hint.

Resize previews absorb scale into their outline geometry so strokes and handles retain their screen size. Handles follow flipped local edges; diagonal cursors account for signed unequal scales. The commit checks scale alongside position/size/rotation, and observing an external scale change cancels the gesture. The initial visibility check now uses actual visual bounds rather than the untransformed layout box. Group resizing with transformed members, matrix/ancestor/3D transforms and canvas rotation of scaled layers remain unfinished.

HTML/React Chromium and Liquid WebKit pass direct pointer resize, preview-to-committed visual bounds, preserved opposite edge, keyboard preview/cancel/commit, centered proportional resize, 50/125/100% zoom handles, screen-switch cancellation, external-scale cancellation, content-box dimensions, flips/position edits and exact Undo. HTML covers a fixed off-center origin; React/Liquid cover percentage origins. Logs: `/private/tmp/retouch-scaled-resize-html-complete.log`, `/private/tmp/retouch-scaled-resize-react-final.log`, `/private/tmp/retouch-scaled-resize-liquid-complete.log`. The existing unscaled HTML resize flow passes `/private/tmp/retouch-scaled-resize-unscaled-regression.log`. All 965 units pass `/private/tmp/retouch-scaled-resize-all-units-final.log`, including opposite-anchor/center math across eight handles, five signed scales, four rotations and fixed/percentage origins.

The first HTML direct-resize attempt had its flipped edge clipped above the canvas; a diagnostic confirmed the handle was intentionally hidden. The scaled-resize fixture now has more top margin so the tested edge is visible. A later React assertion saw both transient flow and positioned controls immediately after returning to tablet; it now waits for the same single enabled control condition before proceeding, and the final run passes. Reviewed screenshots `/private/tmp/retouch-direct-resize-selection-html.png` and `/private/tmp/retouch-direct-resize-selection-liquid.png` for transformed outline/handle placement and light inspector presentation.

No desktop rebuild, native launch or push for this increment. Older WebKit flow Fill height remains unresolved. Full Figma parity and universal site support remain incomplete.


### 2026-09-13 — Canvas rotation after flips and non-unit scale

Single layers with supported individual 2D scale now expose direct rotation corners and the inspector canvas rotation tool. Rotation measures the recovered layout box with signed scale, keeps the authored transform pivot, and places the gesture handle relative to the scaled corner. Corner hit regions extend ten screen pixels outward from the transformed corners regardless of canvas zoom or signed/non-unit layer scale. Rotation validation now checks both scale axes, so an external scale change cancels the active draft. Matrix/ancestor/3D transform support and shared selection rotation remain incomplete.

The rotation field now displays the live gesture angle and restores its original value on cancellation. Visual review of the initial scaled rotation screenshot exposed its stale pre-gesture value; the final browser tests assert the field tracks the preview and restores on Escape. The canvas preview still writes no source until commit, and each gesture remains one history entry.

HTML/React Chromium and Liquid WebKit pass a horizontally flipped, non-unit-scaled layer with a percentage off-center pivot: direct corner drag commit/cancel, inspector rotation handle, keyboard preview/commit, 15-degree pointer snapping, inspector refresh during a draft, 50/125/100% zoom, external size/scale cancellation, screen-switch cancellation, scoped rotation/reset, numeric scrubbing, exact source Undo and retained layout dimensions/scale/origin. Logs: `/private/tmp/retouch-scaled-rotation-{html,react,liquid}-final.log`. Unscaled rotation and the prior scaled direct-resize workflow also pass (`/private/tmp/retouch-scaled-rotation-unscaled-regression.log`, `/private/tmp/retouch-scaled-rotation-resize-regression.log`). All 966 units pass `/private/tmp/retouch-scaled-rotation-units-final.log`; the new corner geometry test covers four scale-sign combinations, three zoom levels and four rotations. Reviewed `/private/tmp/retouch-rotation-corners-liquid.png` for pivot/handle placement and the live angle field.

No desktop rebuild, native launch or push for this increment. Older WebKit flow Fill height and full Figma/universal-site parity remain incomplete.


### 2026-09-13 — Measured pointer resizing for transformed flow layers

Flow resize now measures supported individual 2D scale as well as rotation. For transformed pointer gestures it samples the browser's handle response to bounded dimension changes and solves the resulting one- or two-axis movement, including size-dependent transform origins. This keeps the dragged edge near the pointer while the element remains in normal layout. Proportional resizing retains the existing browser-resolved bound correction; keyboard gestures continue to edit source dimensions. The correction loop is bounded to four iterations and falls back to the movable axis for a singular response. Preview readouts use untransformed dimensions, handles retain signed-scale positioning/cursors, and a changed rotation/scale cancels the draft. Empty inline styles created solely by preview restoration are removed when the original attribute was absent; concurrent external style changes remain preserved.

HTML/React Chromium and Liquid WebKit pass a padded content-box layer with scale -1.5/.75, rotation 30 degrees and a 25%/75% origin: bottom and corner pointer resize, handle-following within 0.6px, sibling flow movement, retained margins/position/transform, live readouts, preview/commit equality, Shift toggle, keyboard multi-axis edits, maximum-height clamping, breakpoint isolation, cancellation and exact source Undo. Logs: `/private/tmp/retouch-flow-scale-html-complete.log`, `/private/tmp/retouch-flow-scale-react-stable.log`, `/private/tmp/retouch-flow-scale-react-stable-repeat.log`, `/private/tmp/retouch-flow-scale-liquid-complete.log`. Existing constrained proportional and unscaled rotated HTML flow tests pass `/private/tmp/retouch-flow-scale-constraints-regression.log` and `/private/tmp/retouch-flow-scale-rotation-regression.log`. All 967 units pass `/private/tmp/retouch-flow-scale-units-final.log`.

Verification exposed a test setup issue: its temporary dimension probes left an empty inline style before React's first gesture (the runtime restoration trace confirmed originalStyle was already empty). The expected pointer path now comes from independent transform geometry without modifying the test element. Fractional pointer delivery retains the existing 1.5px rotated-size tolerance; the separate handle-following and preview/committed-size assertions remain tighter. An intermittent first React drag timed out before the size changed; the harness now waits for a stable, hittable direct handle via hover and confirms the gesture surface mounts before moving. Two final React runs pass. Reviewed `/private/tmp/retouch-flow-resize-liquid.png` for the transformed edge, flow readout and retained light inspector.

Remaining handle-selection limitation: an exact horizontal flip around a centered origin can make the default local east edge stationary as width changes. A standalone Chromium probe confirmed width 200 to 220 keeps visual left at 268 while right moves 468 to 488. Flow handle selection still needs to choose a moving edge for this case; this increment does not establish universal scaled-flow behavior. Matrix/ancestor/3D transforms, arbitrary nonlinear layout convergence, full Figma parity and universal site support remain incomplete. No desktop rebuild, native launch or push for this increment.


### 2026-09-13 — Moving flow handles for exact centered flips

Flow handle selection now accounts for signed scale, rotation, layout anchoring and the transform origin's size response. If a modeled edge is stationary while its opposite moves, the opposite handle is offered and the corner combinations follow it. Centered exact horizontal/vertical/double flips therefore resize from moving edges. Fixed pixel origins keep their appropriate original handles even when those pixels happen to equal the initial center. The pointer solver's initial size estimate now also inverts signed scale, preventing reversed handles from starting beyond the wrong minimum before measured correction.

CSS Typed OM exposes unresolved origin percentages. The origin response evaluator supports numeric units, sums/products, negation/inversion and min/max/clamp, checking both one-pixel directions at piecewise boundaries. It does not temporarily resize the page element. If Typed OM or an expression is unavailable, existing flow handles remain the fallback; this does not establish universal engine/expression coverage. Source dimensions and transformed movement still use the existing bounded measured correction and source adapters.

Verified HTML horizontal/vertical flips and fixed origins, React calc origins, and Liquid/WebKit double flips through pointer edge/corner resizing, source dimensions/readouts, sibling flow, Shift toggle, maximum-height clamping, keyboard multi-axis editing, cancellation, scope isolation and exact Undo. Browser cases additionally verify percent/fixed/calc/min/max/clamp origin rates and zero style mutations during five idle handle-update frames. Logs: `/private/tmp/retouch-flow-exact-html.log`, `/private/tmp/retouch-flow-exact-vertical.log`, `/private/tmp/retouch-flow-exact-fixed-final.log`, `/private/tmp/retouch-flow-exact-react-final.log`, `/private/tmp/retouch-flow-exact-liquid-final.log`. The prior rotated non-unit Liquid/WebKit case passes `/private/tmp/retouch-flow-exact-nonunit-regression.log`. All 968 units pass `/private/tmp/retouch-flow-exact-units-final.log`.

The first double-flip maximum test exposed the incorrectly signed initial estimate; the corrected final run passes. An initial fixed-origin run did not mount the first pointer gesture; the final run passes with the existing stable-hover check, added origin/no-mutation checks and failure diagnostics. Reviewed `/private/tmp/retouch-flow-resize-html.png` for the flipped layer's moving edge, retained flow and inspector size feedback. The exact-centered-flip limitation recorded in the previous entry is resolved for the verified cases; arbitrary nonlinear layout convergence, matrix/ancestor/3D geometry and older Typed-OM-incomplete engines remain outside this evidence.

No desktop rebuild, native launch or push for this increment. Older WebKit flow Fill height, full Figma parity and universal site support remain incomplete.


### 2026-09-13 — Package the scaled editing and flow sizing improvements

Built clean source `23b15e9` into `/private/tmp/retouch-desktop-sizing-23b15e9/Retouch-0.1.0-mac.zip`, SHA-256 `32505549b89affa4ff39a94effc48b328600033d28a30261451e8b55bfc92a1f`. All 182 packaged source hashes and native/build/verifier hashes match. Universal arm64/x86_64 architecture and strict signatures pass after extraction, Homebrew installation and packaged runtime testing. All 968 matching-source unit tests pass.

Eight packaged workflow suites pass: HTML/React scaled absolute resize and exact history; React scaled rotation and live angle feedback; HTML vertical-flip and Liquid/WebKit double-flip flow resize with origin math, maximum bounds and exact source history; Liquid Fill in newer Playwright WebKit 26.6; and compact screen controls in Chromium/WebKit. Each suite uses the extracted runtime and its production dependencies, with browser/test dependencies in an external harness. The harness was removed after every process completed, and strict signature verification still passes.
...
### 2026-09-13 — Scrub SVG selection fields

Selection X/Y, width/height and relative rotation now use the shared numeric scrubber, including four vertical speed bands and Option/Alt-drag inside inputs. Canvas-only previews transform outermost selected vectors, preserving selected descendants' authored transforms. Related bounds fields update during previews; release uses the existing atomic source transaction, Escape restores owned preview state, and external transform replacements cancel without being overwritten. Locked proportions apply during width/height scrubbing.

Validation: all 1035 unit tests pass. Full HTML/WebKit and Liquid/Chromium vector workflows pass. Expanded focused workflows on both engines check all five fields for ordinary and nested selections, actual screen matrices, child transform preservation, source unchanged during preview, Escape, exact Undo/Redo/Undo, locked ratio, four speeds, Alt-input scrubbing and external ownership. Logs: `/private/tmp/retouch-selection-scrub-{units,html,liquid}.log` and `/private/tmp/retouch-selection-scrub-{html,liquid}-focused.log`. The full React/Chromium vector workflow also passed with terminal exit 0, including the expanded selection scrubbing checks; log `/private/tmp/retouch-selection-scrub-react.log`. No desktop rebuild or native launch for this increment; full parity, arbitrary-site editing and trusted public Mac distribution remain incomplete.

### 2026-09-13 — Keep vector pose fields synchronized during rotation scrubbing

Single-vector rotation previews now refresh X/Y from the same transform used on canvas. Previously, center-preserving rotation moved the transformed origin while the inspector displayed its old coordinates until release. Related pose fields restore on cancellation without replacing the actively scrubbed value before commit.

All 1035 unit tests passed. Focused SVG scrubbing workflows passed HTML/WebKit, React/Chromium and Liquid/Chromium with terminal exit 0. New checks compare inspector X/Y with the browser's actual transformed SVG origin during rotation and verify Escape restores all three pose fields, the original transform and unchanged source. Existing five-field previews, speed bands, locked proportions, external ownership and exact history checks also passed. Logs `/private/tmp/retouch-pose-scrub-{html-final,react,liquid,units}.log`. No desktop rebuild, native launch or push; full goal remains incomplete.

### 2026-09-13 — Cancel stale SVG scrub previews when page geometry changes

Single-vector and selection numeric previews now validate local bounding geometry and the expected screen transform throughout the gesture. A radius/bounds change or ancestor coordinate-system change cancels without committing an outdated transform. Cancellation restores owned preview transforms and field values while preserving external geometry/ancestor changes. Existing external-transform ownership protection remains intact. Single-vector matrix setup explicitly converts SVGMatrix to DOMMatrix before multiplication, covering engines whose legacy SVGMatrix method rejects DOMMatrix arguments.

All 1035 units passed after the compatibility correction. Combined single/selection scrub workflows passed HTML/WebKit, React/Chromium and Liquid/Chromium with terminal exit 0. Added browser cases change circle radius and the SVG viewBox mid-drag, including a nested parent/child selection, and verify cancellation, untouched source, restored transforms and preserved external changes. Existing pose, dimensions, speed bands, ratio lock and exact history checks also passed. Logs `/private/tmp/retouch-scrub-geometry-{html,react,liquid,units}-final.log`. Initial runs exposed the SVGMatrix/DOMMatrix incompatibility and failed; those results were not counted as passes. No desktop rebuild, native launch or push; full Figma parity, arbitrary-site support and trusted public distribution remain incomplete.

### 2026-09-13 — Align and distribute SVG selections

The SVG selection Position section now includes the existing compact alignment toolbar: left/center/right, top/middle/bottom and horizontal/vertical equal spacing. Each operation translates outermost selected vectors through their individual parent coordinate systems and writes one atomic selection transaction; selected descendants retain authored transforms. Distribution requires three outermost members, and alignment requires two. Zero-width and zero-height SVG lines are supported through an explicit opt-in to the shared arrangement math; ordinary HTML/CSS validation stays unchanged. Operations currently target selection bounds, not containing SVG frames. Reference: https://help.figma.com/hc/en-us/articles/360039956914-Adjust-alignment-rotation-position-and-dimensions.

All 1036 unit tests passed. HTML/WebKit, React/Chromium and Liquid/Chromium workflows passed all eight operations on ordinary and nested selections, verifying actual aligned bounds/equal spacing, preserved sizes/child transforms and exact Undo/Redo/Undo, with terminal exit 0. The first HTML test incorrectly re-sorted overlapping distributed layers; the corrected check uses original order and passed without changing production distribution behavior. Logs `/private/tmp/retouch-svg-align-{html-final,react,liquid,units}.log`. Screenshot `/private/tmp/retouch-svg-align.png` was visually inspected. No desktop rebuild, native launch or push. Full parity, arbitrary-site support and trusted public Mac distribution remain incomplete.

### 2026-09-13 — Align SVG selections to their viewport

SVG selection alignment now offers Selection and SVG viewport targets. The viewport target aligns individual members to the rendered viewport bounds and distributes spacing across those bounds. Shift-align moves outermost selected members together as a group, preserving their internal arrangement; selected descendants retain authored transforms. Target choice persists per selection through source refresh and history. A common SVG viewport is required for viewport operations; its option is disabled when selected outermost members belong to different viewports. Selecting a parent with its children can use viewport alignment even when there is only one outermost member.

All 1037 unit tests passed. HTML/WebKit, React/Chromium and Liquid/Chromium expanded workflows passed with terminal exit 0, covering existing selection alignment, all six viewport alignments, both viewport distributions, all six Shift-group alignments, target retention, transformed parents, nested child preservation, unchanged sizes and exact Undo/Redo/Undo. Logs `/private/tmp/retouch-svg-viewport-align-{html,react,liquid,units}.log`. No desktop rebuild, native launch or push. Full Figma parity, arbitrary-site editing and trusted public Mac distribution remain incomplete.

### 2026-09-13 — Align individual SVG vectors

The single-vector Position section now exposes the shared six-button alignment toolbar and Parent/SVG viewport target choice. It uses measured screen bounds and the existing SVG transform transaction, including translated/rotated/scaled parents and zero-height or zero-width lines. Shift-align targets the SVG viewport. Target choice persists per vector through source refresh and history. The full vector browser workflow includes the new alignment regression helper.

All 1037 unit tests passed. HTML/WebKit, React/Chromium and Liquid/Chromium single-alignment workflows passed all six controls on a rectangle, transformed group and zero-height line, plus Shift-to-viewport, target persistence, preserved dimensions and exact Undo/Redo/Undo. Combined single/selection scrubbing also passed HTML/WebKit after the toolbar addition, including four speeds, ratio lock, cancellation, external ownership/geometry interruptions and exact history. Every process exited 0. Logs `/private/tmp/retouch-svg-single-align-{html,react,liquid,units,scrub}.log`. No desktop rebuild, native launch or push. Full Figma parity, arbitrary-site editing and trusted public Mac distribution remain incomplete.

### 2026-09-13 — Exact SVG selection spacing with live scrubbing

Selection Position now includes compact horizontal/vertical gap fields. Unequal gaps show Mixed; values and numeric expressions set exact document-pixel spacing. The first outermost edge stays fixed for Selection, or starts at the SVG viewport edge for that target. Selected parent groups move as units, retaining child transforms. Gap labels and Option/Alt-input scrubbing preview each member independently, update bounds/related gap displays, cancel on Escape, and commit one atomic history transaction. The shared scrub preview geometry check now tracks each outermost member's transform, including covered descendants. Zero gaps are supported for zero-size-axis SVG lines; spacing that reverses the original order remains refused.

All 1038 unit tests passed after the zero-gap adjustment. HTML/WebKit, React/Chromium and Liquid/Chromium exact-gap workflows passed zero spacing, expressions, viewport anchors, nested groups/lines, live scrub, cancellation, invalid-order refusal, preserved dimensions/child transforms and exact Undo/Redo/Undo; all processes exited 0. Logs `/private/tmp/retouch-svg-gap-{html,react,liquid}-final.log` and `/private/tmp/retouch-svg-gap-units-zero.log`. The earlier combined single/selection WebKit scrubbing regression also passed after the per-member preview refactor (`/private/tmp/retouch-svg-gap-scrub-regression.log`). HTML/Chromium gap workflow passed and `/private/tmp/retouch-svg-gap.png` was visually inspected. No desktop rebuild, native launch or push for this increment. Full parity, arbitrary-site support and trusted distribution remain incomplete.

### 2026-09-13 — Edit individual SVG selection gaps

Selections with three or more outermost members now offer an Individual gaps disclosure. Each horizontal/vertical pair has an exact numeric field with expressions and live scrubbing; hover text identifies the neighboring vectors. Editing one gap keeps the first layer fixed and shifts following members while retaining every other gap. Selected descendants retain authored transforms. The disclosure stays open through source refresh and history, and related bounds/shared-gap/individual-gap fields update during previews.

All 1039 unit tests passed. Individual-gap workflows passed HTML/WebKit, HTML/Chromium, React/Chromium and Liquid/Chromium, testing first/last gaps on ordinary and nested selections, expressions, actual gap preservation, dimensions/child transforms, live previews, Escape and exact Undo/Redo/Undo. Existing single/selection WebKit scrubbing regression passed, including interruption/ownership guards. All processes exited 0. Logs `/private/tmp/retouch-svg-individual-gap-{html,chromium,react,liquid,units,scrub}.log`. Screenshot `/private/tmp/retouch-svg-individual-gap.png` was visually inspected. No desktop rebuild, native launch or push; full Figma parity, arbitrary-site support and trusted public distribution remain incomplete.

### 2026-09-13 — Drag SVG gaps directly on canvas

SVG selections now have Horizontal gaps and Vertical gaps toggles. Pink measurement lines, ticks and numeric labels show each adjacent gap on the canvas. Dragging a label adjusts that gap through the same numeric preview/atomic source transaction used by its inspector field; vertical gaps follow vertical motion, and both axes scale movement by canvas zoom. Other gaps remain unchanged. The associated inspector disclosure opens for keyboard Escape/Enter handling; controls rebind after source refresh/history, clip to the visible canvas and hide for other selections or unavailable editing state. The shared numeric scrubber supports additional handles with axis/scale options and detachable listeners while retaining existing label/Alt-input behavior.

All 1039 unit tests passed. HTML/WebKit, React/Chromium and Liquid/Chromium canvas-gap workflows passed both axes at 50/100/200 percent zoom, plus nested and two-layer selections, untouched source during preview, Escape and exact Undo/Redo/Undo. Measurement lines and compact buttons were added after those initial runs; the expanded WebKit workflow passed again and `/private/tmp/retouch-svg-gap-canvas-final.png` was visually inspected. Single/selection WebKit scrubbing and Chromium CSS inspector scrubbing regressions passed, including modifiers, lower bounds, responsive writes, typing, preview cancellation and exact history. Every process exited 0. Logs `/private/tmp/retouch-svg-gap-canvas-{html-final,html-guides,react,liquid,scrub,css-regression,units-final}.log`. No desktop rebuild, native launch or push; full parity, arbitrary-site support and trusted public distribution remain incomplete.

### 2026-09-13 — Keyboard editing for canvas SVG gaps

Focused canvas gap labels now respond to axis-appropriate arrow keys in document pixels, independently of zoom. Repeated and overlapping held keys form one live preview and one undo step when released; Shift uses ten-pixel increments and Option/Alt uses tenths. Escape cancels, including with Control held, and suppresses repeats from cancelled held keys until release. Blur cancels. Enter commits an active gesture or opens/focuses the exact inspector field when idle. Canvas labels retain focus after source refresh; the overlay no longer hides focused controls on every paint.

All 1039 unit tests passed. HTML/WebKit, React/Chromium and Liquid/Chromium expanded keyboard workflows passed normal/repeated/overlapping keys, zoom, nested selections, focus restoration, Enter-to-edit/commit, fine increments, Escape latch, blur and exact Undo/Redo/Undo. WebKit passed again after enabling modifier-held Escape. Chromium canvas pointer, WebKit SVG field scrubbing, and Chromium CSS numeric scrubbing regressions passed, including responsive source edits, modifiers, cancellation and exact history. Every process exited 0. Logs `/private/tmp/retouch-svg-gap-keyboard-{html-final,react-final,liquid-final,escape-modifiers,pointer-regression,scrub-regression,css-regression,units-final}.log`. No desktop rebuild, native launch or push; full parity, arbitrary-site support and trusted public distribution remain incomplete.

### 2026-09-13 — Equal-gap canvas adjustment

SVG canvas gap editing now offers Only the dragged gap and All gaps equally modes, retained per selection. Equal mode starts from the clicked gap even when the shared inspector field is Mixed. Pointer and keyboard gestures preview every gap at the same value, commit one atomic transaction, and restore the original uneven layout on Escape; a no-movement click never flattens the layout. The lower drag limit considers every intervening layer size so a negative mixed starting gap cannot reverse another layer's order. Switching back rebinds labels to individual fields. Shared numeric handles can supply a measured starting value and minimum without changing ordinary field behavior.

All 1039 unit tests passed. HTML/WebKit, React/Chromium and Liquid/Chromium equal-gap workflows passed mixed starts, no-op clicks, 50/100/200 percent zoom, horizontal/vertical previews, keyboard edits, cancellation, drag limits, mode retention and exact Undo/Redo/Undo. An expanded WebKit run verified switching back to individual editing preserves other gaps. Existing single/selection SVG WebKit and CSS Chromium scrubbing regressions passed. All processes exited 0. Logs `/private/tmp/retouch-svg-gap-equal-{html-final,react-final,liquid-final,switching,units,scrub,css}.log`. Screenshot `/private/tmp/retouch-svg-gap-equal.png` was visually inspected. No desktop rebuild, native launch or push; full parity, arbitrary-site support and trusted distribution remain incomplete.
