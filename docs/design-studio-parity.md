...
| Components | Create/reuse, variants, exposed properties, overrides, nested instances, swap/reset/detach, shared libraries | React can extract a source subtree into an explicitly reusable same-file component, preserve call-site keys/module references, select and duplicate linked instances, and Undo/Redo through the UI. Chromium/WebKit checks verify unchanged rendering and exact source restoration. Parent-local captures, scoped JSX styles and context-dependent expressions are refused pending prop/style extraction. Existing React/Liquid inspection and detach remain; cross-file creation, variants, exposed props, libraries and live Shopify proof remain incomplete. |
