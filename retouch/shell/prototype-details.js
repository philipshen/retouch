(function(root){
 ...
 root.RetouchPrototypeDetails={create,dismiss,place,reset(){dismiss();for(const dialog of dialogs)dialog.remove();dialogs.clear();},setBusy(value){for(const dialog of dialogs)dialog.inert=value;},snapshot:()=>current?{index:current.row.dataset.index,scroll:current.dialog.scrollTop,actions:root.RetouchPrototypeActionPanel?.snapshot(current.dialog),position:positions.get(current.dialog)}:null,restore(state){if(state&&current?.row.dataset.index===state.index){if(state.actions)root.RetouchPrototypeActionPanel?.restore(current.dialog,state.actions);positions.set(current.dialog,state.position);place();current.dialog.scrollTop=state.scroll;}}};
})(window);
