(function(root){
 ...
 function create(el,frames,timing){
  ...
  function render(){if(!el.isConnected||!names.length)return;write('transition-property','none');const css=d.defaultView.getComputedStyle(proxy);for(const name of names)write(name,css.getPropertyValue(name));}
