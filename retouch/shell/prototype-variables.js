(function(root){
 ...
 let epoch=0,values={},revision,library=null,queue=Promise.resolve(),scheduled=false;
 ...
 function currentBindings(info,d){
  ...
  for(const property of properties){
   ...
   const inherited=info.classVariables?B.classInherited(scoped,'min-['+(width+.01)+'px]:',property,d):B.inherited(scoped,width+1,property);
 ...
 async function project(next,ticket){
  ...
  for(const d of [...documents]){
   for(const el of d.querySelectorAll('[data-rt-variables]')){
    ...
    for(const request of currentBindings(info,d))targets.push({el,request});
   }
  }
  ...
  for(const [el,entries]of styles)for(const [property,entry]of entries)if(!desired.get(el)?.has(property)){restore(el,property,entry);entries.delete(property);}
 ...
 function refresh(){if(scheduled||!Object.keys(values).length)return;scheduled=true;queueMicrotask(()=>{scheduled=false;enqueue(ticket=>project(values,ticket));});}
 root.RetouchPrototypeVariables={
  ...
  mount(d){documents.add(d);const observer=new MutationObserver(refresh);observer.observe(d.body,{childList:true,subtree:true,attributes:true,attributeFilter:['data-rt-variables','class']});d.defaultView.addEventListener('resize',refresh);refresh();let released=false;return ()=>{if(released)return;released=true;observer.disconnect();d.defaultView?.removeEventListener('resize',refresh);documents.delete(d);restoreDocument(d);};},
  reset(){epoch++;values={};library=null;revision=undefined;queue=Promise.resolve();scheduled=false;restoreDocument();}
 };
})(window);
