(function(root){
 ...
 function mount(info,el,save){
  ...
  for(const [key,label]of V.svgFields){
   ...
   if(['fill','stroke'].includes(key)){I.fieldDraft(input);input.dataset.paintProperty=key;root.RetouchBackgroundPaintUI.bindSource(input,info,'',key,el);input.retouchPaintPreview=()=>root.RetouchPaintPicker.propertyPreview({el,input,property:key});}
