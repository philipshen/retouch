(function(root){
 ...
 function property(token){
  ...
  if(base.startsWith('stroke-')){const suffix=base.slice(7);if(suffix.startsWith('[length:')||/^\d+(?:\.\d+)?$/.test(suffix)||/^\[(?:length:)?(?:\d+\.?\d*|\.\d+)(?:px|%)?\]$/.test(suffix))return 'stroke-width';return 'stroke';}
  return null;
 }
 ...
 function update(classes,key,next){
  ...
  return I.replace(classes,t=>property(t)===key,next===null?'':`[${key}:${next.trim().replace(/\s+/g,'_')}]`);
 }
 function mount(info,el,save){
  ...
  for(const [key,label]of V.svgFields){
   ...
   input.oninput=()=>input.setCustomValidity('');input.onchange=()=>{try{if(!CSS.supports(key,input.value))throw Error('Use a supported SVG paint value.');save(update(info.className,key,input.value));}catch(error){input.setCustomValidity(error.message);input.reportValidity();}};
