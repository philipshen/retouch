(function(root){
  ...
  function sizeClasses(classes,axis,mode,value,parent={}) {
    ...
    // Tailwind emits arbitrary properties before dimension utilities. Keep the
    // fallback first so supported stretch wins, while class merging retains both.
  ...
  function mount(info,el,save) {
    ...
    function paddingInput(label,value,change,edges=paddingEdges){
      const input=document.createElement('input');input.type='text';input.value=value??'';input.disabled=!spacingActive()||edges.some(edge=>inlinePadding(el,edge,css,true));input.placeholder=value===null?'Mixed':'0';input.dataset.variableProperty=edges.length===4?'padding':'padding-'+edges[0];I.field(sec,label,input);
