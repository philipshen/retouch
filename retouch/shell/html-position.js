(function(root){
 ...
 function axis(g,dimension,mode){
  ...
  else if(mode==='scale'){if(parent===0)throw Error('Scale needs a container with nonzero '+(horizontal?'width.':'height.'));values[a]=round(start/parent*100)+'%';values[d]=round(size/parent*100)+'%';}
 ...
 function mount(info,el,width,save,onMove){
  ...
  const measure=()=>{for(let parent=el.parentElement;parent;parent=parent.parentElement)if(parent.namespaceURI==='http://www.w3.org/2000/svg')throw Error('HTML anchors inside an SVG viewport are not available yet.');return I.geometry(el,css.position==='absolute'?{allowRotation:true,allowScale:true}:{});};
  ...
  if(mode==='absolute'){
   ...
   if(!g){const coordinates=sec.querySelector('[aria-label="X"]');if(coordinates&&onMove){for(const action of ['move','resize']){const control=I.canvasTool(action,(opener,initial)=>onMove(I.geometry(el,{allowRotation:true,allowScale:action==='move'}),action,opener,initial));if(action==='resize'&&css.scale&&css.scale!=='none'){control.disabled=true;control.title='Canvas resizing for scaled layers is not available yet.';}sec.append(control);}}I.note(sec,coordinates?'Anchor presets for rotated or scaled layers are not available yet.':reason,coordinates?'':'refused');return sec;}
   I.note(sec,'Anchored to '+g.parentLabel);
   if(onMove){const tools=document.createElement('div');tools.className='stack-presets';for(const action of ['move','resize']){const button=I.canvasTool(action,(opener,initial)=>onMove(g,action,opener,initial));if(action==='resize'&&css.scale&&css.scale!=='none'){button.disabled=true;button.title='Canvas resizing for scaled layers is not available yet.';}tools.append(button);}sec.append(tools);}
   ...
   for(const [dimension,label,choices]of [['x','Horizontal anchor',[['start','Left'],['center','Center'],['end','Right'],['stretch','Left + right'],['scale','Scale']]],['y','Vertical anchor',[['start','Top'],['center','Center'],['end','Bottom'],['stretch','Top + bottom'],['scale','Scale']]]]){
    const input=I.select(sec,label,[['','Custom / inherited'],...choices],infer(inherited,dimension),value=>{if(value)try{g=measure();inherited=effective();write(root.RetouchSelectionLayout.preserveBox({margin:'0','box-sizing':'border-box',...axis(g,'x',dimension==='x'?value:infer(inherited,'x')||I.nearestAnchor(g.x,g.width,g.parentWidth)),...axis(g,'y',dimension==='y'?value:infer(inherited,'y')||I.nearestAnchor(g.y,g.height,g.parentHeight))},g,el.ownerDocument.defaultView.getComputedStyle(el)));}catch(error){report(error);}});
