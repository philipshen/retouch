(function(root){
 ...
 function mount(info,el,width,save,onMove){
  ...
  if(mode==='absolute'){
   ...
   if(!g){const coordinates=sec.querySelector('[aria-label="X"]');if(coordinates&&onMove){for(const action of ['move','resize']){const control=I.canvasTool(action,(opener,initial)=>onMove(I.geometry(el,{allowRotation:true}),action,opener,initial));sec.append(control);}}I.note(sec,coordinates?'Anchor presets for rotated layers are not available yet.':reason,coordinates?'':'refused');return sec;}
