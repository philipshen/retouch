(function(root){
 ...
 function mount(info,el,width,save,onMove){
  ...
  if(mode==='absolute'){
   ...
   if(onMove&&!g.localCoordinates){const tools=document.createElement('div');tools.className='stack-presets';for(const action of ['move','resize']){const button=I.canvasTool(action,(opener,initial)=>onMove(g,action,opener,initial));tools.append(button);}sec.append(tools);}
