(function(root){
 ...
 function mount(info,el,width,save,onMove){
  ...
  const measure=()=>{for(let parent=el.parentElement;parent;parent=parent.parentElement)if(parent.namespaceURI==='http://www.w3.org/2000/svg')throw Error('HTML anchors inside an SVG viewport are not available yet.');return css.position==='absolute'?I.positionGeometry(el):I.geometry(el);};
  ...
  if(mode==='absolute'){
   ...
   if(!g){const coordinates=sec.querySelector('[aria-label="X"]');if(coordinates&&onMove&&!coordinates.dataset.localCoordinates){for(const action of ['move','resize']){const control=I.canvasTool(action,(opener,initial)=>onMove(I.geometry(el,{allowRotation:true,allowScale:true}),action,opener,initial));sec.append(control);}}I.note(sec,coordinates?'Anchor presets and canvas transforms for this coordinate system are not available yet.':reason,coordinates?'':'refused');return sec;}
   I.note(sec,'Anchored to '+g.parentLabel);
   if(onMove){const tools=document.createElement('div');tools.className='stack-presets';for(const action of (g.localCoordinates?['move']:['move','resize'])){const button=I.canvasTool(action,(opener,initial)=>onMove(g,action,opener,initial));tools.append(button);}sec.append(tools);}
