(function(root){
 ...
 function mount(info,target,{current,selected=current,resolveTarget=()=>target,load,save,onCanvas}){
  ...
  for(const [property,label]of [['fill','Combined fill'],['stroke','Combined stroke'],['stroke-width','Combined stroke width']]){
   const input=root.document.createElement('input'),result=resolveTarget()?.querySelector(':scope > [data-rt-boolean-result]');input.type='text';input.value=meta.paints?.[property]??(result?result.ownerDocument.defaultView.getComputedStyle(result).getPropertyValue(property):'');
   input.onchange=()=>{if(!current())return;const value=input.value.trim()||null,target=resolveTarget()?.querySelector(':scope > [data-rt-boolean-result]');try{
    ...
    if(property!=='stroke-width'){const reason=root.RetouchSVGPaint.attributeReason(target,property);if(reason)throw Error(reason);}
    else {const previous=target.getAttribute(property);try{for(const sample of ['1','2']){target.setAttribute(property,sample);if(parseFloat(target.ownerDocument.defaultView.getComputedStyle(target).getPropertyValue(property))!==Number(sample))throw Error('Page styles control the combined stroke width.');}}finally{if(previous===null)target.removeAttribute(property);else target.setAttribute(property,previous);}}
    save('setSVGBooleanPaint',{property,value});
   }catch(error){fail(error);}};I.fieldDraft(input);if(property!=='stroke-width'){input.dataset.paintProperty=property;input.retouchPaintPreview=()=>root.RetouchPaintPicker.propertyPreview({el:resolveTarget()?.querySelector(':scope > [data-rt-boolean-result]'),input,property});}I.field(section,label,input);input.closest('.inspector-field').querySelector(':scope > span').textContent=({fill:'Fill',stroke:'Stroke','stroke-width':'Weight'})[property];
  }
