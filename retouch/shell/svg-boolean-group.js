(function(root){
 'use strict';
 const openOriginals=new Set(),openStrokes=new Set(),regroupSelections=new Map();
 const labels={union:'Union',subtract:'Subtract',intersect:'Intersect',exclude:'Exclude overlap'};
 const controlIcons={
  move:'M10 2v16 M2 10h16 M7 5l3-3 3 3 M7 15l3 3 3-3 M5 7l-3 3 3 3 M15 7l3 3-3 3',
  resize:'M4 8V4h4 M12 16h4v-4 M4 4l12 12 M4 12v4h4 M12 4h4v4',
  rotate:'M4 7a7 7 0 1 1-1 6 M4 3v4h4',
  remove:'M5 10h10'
 };
 function iconControl(button,icon,label){
  button.classList.add('boolean-icon-control');button.title=label;
  const regions={union:'M3 3h9v5h5v9H8v-5H3z',subtract:'M3 3h9v5H8v4H3z',intersect:'M8 8h4v4H8z',exclude:'M3 3h9v9H3z M8 8h9v9H8z'};
  const drawing=regions[icon]?'<path opacity=".3" d="M3 3h9v9H3z M8 8h9v9H8z"/><path fill="currentColor" fill-rule="evenodd" stroke="none" d="'+regions[icon]+'"/>':'<path d="'+controlIcons[icon]+'"/>';
  button.innerHTML='<svg viewBox="0 0 20 20" aria-hidden="true">'+drawing+'</svg>';
 }
 ...
 function prepare(infos,elements,operation,onPrepared){
 ...
 function mount(info,target,{current,selected=current,resolveTarget=()=>target,load,save,onCanvas,onNested,onBack}){
  const I=root.RetouchInspector,fragment=root.document.createDocumentFragment(),section=I.section('Boolean operations'),fillSection=I.section('Fill'),strokeSection=I.section('Stroke'),originalSection=I.section('Original shapes'),meta=info.svgBooleanGroup,select=root.document.createElement('select');select.setAttribute('aria-label','Boolean operation');for(const [value,label]of Object.entries(labels))select.append(new Option(label,value));select.value=meta.operation;I.field(section,'Boolean operation',select);select.parentElement.querySelector('span').textContent='Operation';
  ...
  for(const [property,shortLabel]of Object.entries(fieldLabels)){const label='Combined '+({fill:'fill',stroke:'stroke','stroke-width':'stroke width','stroke-linecap':'stroke ends','stroke-linejoin':'stroke joins','stroke-dasharray':'dash pattern','stroke-dashoffset':'dash offset','stroke-miterlimit':'miter limit','vector-effect':'stroke scaling'})[property];
   ...
   }catch(error){fail(error);}};if(!options)I.fieldDraft(input);if(['fill','stroke'].includes(property)){input.dataset.paintProperty=property;input.retouchPaintPreview=()=>root.RetouchPaintPicker.propertyPreview({el:resolveTarget()?.querySelector(':scope > [data-rt-boolean-result]'),input,property});}I.field(property==='fill'?fillSection:['stroke','stroke-width'].includes(property)?strokeSection:strokeDetails,label,input);input.closest('.inspector-field').querySelector(':scope > span').textContent=shortLabel;
  }
  strokeSection.append(strokeDetails);
  fillSection.title='Combined paint is independent. Releasing the group restores the originals’ paints.';
  const details=root.document.createElement('details'),summary=root.document.createElement('summary');summary.textContent='Original shapes';originalSection.querySelector('h3').remove();originalSection.classList.add('boolean-originals');Object.assign(summary.style,{fontSize:'12px',fontWeight:'600',padding:'8px 0',cursor:'pointer'});details.append(summary);const key=info.file+'#'+info.id;details.open=openOriginals.has(key);originalSection.append(details);let loaded=false;const previousChoice=regroupSelections.get(key),chosen=new Set(previousChoice?.hash===info.hash?previousChoice.ids.filter(id=>meta.operandIds.includes(id)):[]),regroupButtons=[];const updateRegroup=()=>{for(const button of regroupButtons)button.disabled=chosen.size<2||chosen.size>meta.operandIds.length;};
  details.ontoggle=async()=>{if(details.open)openOriginals.add(key);else openOriginals.delete(key);if(!details.open||loaded)return;try{const infos=await ready();if(!selected())return;loaded=true;if(meta.operandIds.length>=2){const row=root.document.createElement('div');row.className='boolean-operation-controls';row.setAttribute('role','group');row.setAttribute('aria-label','Combine selected originals');for(const [operation,label]of Object.entries(labels)){const button=I.button(label,()=>{if(current())save('createSVGBooleanGroup',{operation,ids:[...chosen]});});button.setAttribute('aria-label',label+' selected originals');iconControl(button,operation,label+' selected originals');regroupButtons.push(button);row.append(button);}details.append(row);I.note(details,'Select originals below to combine them into a nested group. The first selected shape supplies its paint.');updateRegroup();}for(const operand of infos){const box=root.document.createElement('div'),name=root.document.createElement('strong');box.className='boolean-original';name.textContent=operand.layerName||resolveTarget()?.querySelector('[data-rt="'+operand.id+'"]')?.getAttribute('data-rt-name')||resolveTarget()?.querySelector('[data-rt="'+operand.id+'"]')?.getAttribute('aria-label')||operand.tag;Object.assign(name.style,{display:'block',fontSize:'12px',margin:'8px 0'});box.append(name);const remove=I.button('Remove',()=>{if(current())save('removeSVGBooleanOperand',{operandId:operand.id});});remove.setAttribute('aria-label','Remove original '+name.textContent);iconControl(remove,'remove','Remove original '+name.textContent);if(meta.operandIds.length===1)remove.title='Remove the final original and its empty boolean group.';const heading=root.document.createElement('div');heading.className='boolean-original-heading';Object.assign(heading.style,{display:'flex',alignItems:'center',justifyContent:'space-between',gap:'8px',margin:'8px 0'});name.replaceWith(heading);name.style.margin='0';heading.append(name,remove);if(meta.operandIds.length>=2){const check=root.document.createElement('input');check.type='checkbox';check.checked=chosen.has(operand.id);check.style.marginRight='6px';check.setAttribute('aria-label','Select original '+name.textContent+' for grouping');check.onchange=()=>{if(check.checked)chosen.add(operand.id);else chosen.delete(operand.id);regroupSelections.delete(key);regroupSelections.set(key,{hash:info.hash,ids:[...chosen]});if(regroupSelections.size>100)regroupSelections.delete(regroupSelections.keys().next().value);updateRegroup();};name.prepend(check);}if(operand.svgBooleanGroup&&onNested){const edit=I.button('Edit boolean group',()=>onNested(operand));edit.setAttribute('aria-label','Edit nested '+name.textContent+' boolean group');box.append(edit);}if(onCanvas){const tools=root.document.createElement('div');tools.className='boolean-transform-controls';tools.setAttribute('role','group');tools.setAttribute('aria-label','Transform original '+name.textContent);for(const action of ['move','resize','rotate']){const label=action[0].toUpperCase()+action.slice(1),button=I.button(label,()=>{if(current())onCanvas(info,operand,infos,action);});button.setAttribute('aria-label',label+' original '+name.textContent+' on canvas');iconControl(button,action,label+' original '+name.textContent+' on canvas');tools.append(button);}box.append(tools);}const rows=new Map();for(const field of operand.svgGeometry?.fields||[]){const input=root.document.createElement('input');input.type='text';input.value=field.value??'';input.disabled=field.editable===false;I.field(box,'Original '+operand.tag+' '+field.label,input);rows.set(field.name,input.closest('.inspector-field'));input.parentElement.querySelector('span').textContent=field.label;input.onchange=()=>{if(!current())return;const value=input.value.trim()||null,edit={operandId:operand.id,operandOp:{type:'setSVGGeometry',property:field.name,value}};try{const path=compute(resolveTarget(),infos,meta.operation,edit);save('setSVGBooleanOperand',{...edit,path});}catch(error){input.value=field.value??'';fail(error);}};I.fieldDraft(input);}for(const pair of [['x','y'],['cx','cy'],['width','height'],['rx','ry'],['x1','y1'],['x2','y2']]){if(!pair.every(key=>rows.has(key)))continue;const grid=root.document.createElement('div');grid.className='property-pair';rows.get(pair[0]).before(grid);for(const key of pair){const row=rows.get(key);row.querySelector('span').textContent=({width:'W',height:'H',rx:'Rx',ry:'Ry'})[key]||key.toUpperCase();grid.append(row);}}details.append(box);}}catch(error){fail(error);}};
  I.note(originalSection,meta.ancestorId?'Edits here also update the containing boolean groups.':'Double-click the combined shape to move an original. Geometry edits apply to every screen size; the combined outline uses the current SVG size.');fragment.append(section,fillSection,strokeSection,originalSection);return fragment;
 }
 async function selectionRemoval(infos,elements,load){
  if(!infos.length||infos.length>100||infos.length!==elements.length||new Set(infos.map(i=>i.id)).size!==infos.length||new Set(infos.map(i=>i.file)).size!==1||new Set(infos.map(i=>i.hash)).size!==1||elements.some(el=>!el?.isConnected||el.ownerDocument!==elements[0].ownerDocument))throw Error('Select boolean originals from one source document.');
  const d=elements[0].ownerDocument,hash=infos[0].hash,cache=new Map(infos.map(info=>[info.id,info])),affected=new Map();
  const fetchInfo=async id=>{if(!cache.has(id)){const [info]=await load([id]);if(!info||info.hash!==hash||info.file!==infos[0].file)throw Error('The boolean source changed. Re-select the originals.');cache.set(id,info);}return cache.get(id);};
  for(const info of infos){let child=info,id=info.svgBooleanGroup?info.svgBooleanGroup.ancestorId:info.svgBooleanOwner;const seen=new Set();if(!id&&!info.svgBooleanGroup){const el=elements[infos.indexOf(info)];if(el.namespaceURI!=='http://www.w3.org/2000/svg'||!['svg','g','rect','circle','ellipse','line','path','polyline','polygon'].includes(el.localName))throw Error('Select SVG layers or direct boolean originals.');}while(id){if(seen.has(id)||seen.size>=100)throw Error('The boolean ancestry could not be resolved.');seen.add(id);const parent=await fetchInfo(id);if(!parent.svgBooleanGroup?.operandIds.includes(child.id))throw Error('Select direct originals, not generated boolean layers.');affected.set(id,parent);child=parent;id=parent.svgBooleanGroup.ancestorId;}}
  const missing=[...new Set([...affected.values()].flatMap(info=>info.svgBooleanGroup.operandIds))].filter(id=>!cache.has(id));for(const info of await load(missing)){if(info.hash!==hash||info.file!==infos[0].file)throw Error('The boolean source changed.');cache.set(info.id,info);}
  const element=id=>{const matches=[...d.querySelectorAll('[data-rt]')].filter(el=>el.getAttribute('data-rt')===id);if(matches.length!==1)throw Error('A boolean layer no longer resolves uniquely.');return matches[0];};for(const info of cache.values())element(info.id);
  const depth=el=>{let n=0;for(let parent=el.parentElement;parent;parent=parent.parentElement)n++;return n;},order=(a,b)=>a===b?0:a.compareDocumentPosition(b)&4?-1:1;
  const ordered=[...affected.values()].sort((a,b)=>depth(element(b.id))-depth(element(a.id))||order(element(a.id),element(b.id))),deleted=[...elements];
  const removed=el=>deleted.some(root=>root===el||root.contains(el));for(const info of ordered)if(info.svgBooleanGroup.operandIds.every(id=>removed(element(id))))deleted.push(element(info.id));
  const cuts=[...new Set(deleted)].filter(el=>!deleted.some(other=>other!==el&&other.contains(el))).sort(order).map(el=>({el,parent:el.parentNode,next:el.nextSibling})),surviving=ordered.filter(info=>!removed(element(info.id))),changes=[];
  const watched=[...new Set(surviving.flatMap(info=>[element(info.id),...element(info.id).querySelectorAll('*')]).filter(el=>!removed(el)))].map(el=>({el,values:css(el,shape)}));
  const put=(el,name,value)=>{if(!changes.some(c=>c.el===el&&c.name===name))changes.push({el,name,value:el.getAttribute(name)});if(value===null)el.removeAttribute(name);else el.setAttribute(name,value);};
  ...
  const live=info=>{if(!info.svgBooleanGroup)return {...info,booleanOperandPreview:true};const el=element(info.id),meta=info.svgBooleanGroup,result=element(meta.resultId);return {...info,svgBooleanGroup:{...meta,result:{...meta.result,svgGeometry:{...meta.result.svgGeometry,fields:meta.result.svgGeometry.fields.map(field=>({...field,value:result.getAttribute(field.name)}))},svgTransform:{...meta.result.svgTransform,value:result.getAttribute('transform'),matrix:matrix(result)}}}};};
  try{
   for(const info of surviving){const meta=info.svgBooleanGroup,operandIds=meta.operandIds.filter(id=>!removed(element(id))),baseId=operandIds.includes(meta.baseId)?meta.baseId:operandIds[0];cache.set(info.id,{...info,svgBooleanGroup:{...meta,operandIds,baseId}});put(element(info.id),'data-rt-boolean-base',String(operandIds.indexOf(baseId)));}
   for(const {el}of cuts)el.remove();if(watched.some(({el,values})=>!same(values,css(el,shape))))throw Error('Deleting these originals would change CSS-controlled geometry or appearance.');
   const results=[];for(const original of surviving){const info=cache.get(original.id),meta=info.svgBooleanGroup,el=element(info.id),base=element(meta.baseId),result=element(meta.resultId),nested=base.querySelector(':scope > [data-rt-boolean-result]'),effective=nested?root.RetouchSVGAffine.multiply(matrix(base),matrix(nested)):matrix(base);put(result,'transform',root.RetouchSVGAffine.format(effective));const operands=[meta.baseId,...meta.operandIds.filter(id=>id!==meta.baseId)].map(id=>live(cache.get(id))),path=compute(el,operands,meta.operation);put(result,'d',path);results.push({id:info.id,path});}return results;
  }finally{for(const {el,name,value}of changes.reverse()){if(value===null)el.removeAttribute(name);else el.setAttribute(name,value);}for(const {el,parent,next}of cuts.reverse())parent.insertBefore(el,next?.parentNode===parent?next:null);}
 }
  ...
  const run=(type,extra)=>{changes.length=0;changed.clear();changed.add(info.id);let restoreStructure=null;return visibleAncestors(group,()=>{try{
   ...
   if(type==='removeSVGBooleanOperand'){
    ...
    if(removing.length===meta.operandIds.length){
     ...
     if(watched.some(({el,values})=>!same(values,css(el,shape))))throw Error('Removing the empty group would change CSS-controlled geometry or appearance.');
   ...
   if(type==='createSVGBooleanGroup'){
    if(!Array.isArray(extra.ids)||extra.ids.length<2||extra.ids.length>meta.operandIds.length||new Set(extra.ids).size!==extra.ids.length||extra.ids.some(id=>!meta.operandIds.includes(id)))throw Error('Select two or more originals in this group.');
    const container=group.querySelector(':scope > [data-rt-boolean-operands]'),priorCache=new Map(cache),infos=extra.ids.map(id=>({...cache.get(id),booleanOperandPreview:true})),elements=extra.ids.map(element);
    put(container,'display',null);restoreStructure=()=>{cache.clear();for(const [id,item]of priorCache)cache.set(id,item);};
    return prepare(infos,elements,extra.operation,(created,derived,path)=>{
     const groupId='retouch-regroup-preview',resultId=groupId+'-result';if(d.querySelector('[data-rt="'+groupId+'"], [data-rt="'+resultId+'"]'))throw Error('A temporary boolean identity is already in use.');created.setAttribute('data-rt',groupId);derived.setAttribute('data-rt',resultId);
     const base=infos[0].svgBooleanGroup?.result||infos[0],resultInfo={...base,id:resultId,tag:'path',booleanOperandPreview:true,svgGeometry:{...base.svgGeometry,fields:[{name:'d',value:path,editable:true}]},svgTransform:{...base.svgTransform,value:derived.getAttribute('transform'),matrix:matrix(derived)}};
     cache.set(groupId,{...infos[0],id:groupId,tag:'g',svgTransform:{matrix:matrix(created),value:null,editable:true},svgBooleanGroup:{operation:extra.operation,operandIds:extra.ids,baseId:extra.ids[0],resultId,result:resultInfo}});
     let inserted=false;const operandIds=meta.operandIds.flatMap(id=>{if(!extra.ids.includes(id))return [id];if(inserted)return [];inserted=true;return [groupId];});cache.set(info.id,{...info,svgBooleanGroup:{...meta,operandIds,baseId:extra.ids.includes(meta.baseId)?groupId:meta.baseId}});
     // Each enclosing calculation expects retained operands to be hidden;
     // compute() reveals only its own operands while measuring geometry.
     put(container,'display','none');return {path,results:updateParents([info,...chain]),render:[]};
    });
   }
   ...
   const results=updateParents(chain);
 ...
 root.RetouchSVGBooleanGroup={check:group=>{neutral(group,group.hasAttribute('transform'));const operands=group.querySelector(':scope > [data-rt-boolean-operands]');if(!operands)throw Error('The original shapes are missing.');neutral(operands);if(group.ownerDocument.defaultView.getComputedStyle(operands).display!=='none')throw Error('CSS exposes the nested original shapes.');},selectionRemoval,cascade,prepareCascade,nestedPreview,prepare,compute,operandAtPoint,revealOriginals,preview,release,mount};
})(window);
