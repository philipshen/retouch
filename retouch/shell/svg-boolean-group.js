(function(root){
 ...
 async function selectionRemoval(infos,elements,load){
  ...
  for(const info of infos){let child=info,id=info.svgBooleanGroup?info.svgBooleanGroup.ancestorId:info.svgBooleanOwner;const seen=new Set();if(!id&&!info.svgBooleanGroup)throw Error('Select boolean originals or retained groups.');while(id){if(seen.has(id)||seen.size>=100)throw Error('The boolean ancestry could not be resolved.');seen.add(id);const parent=await fetchInfo(id);if(!parent.svgBooleanGroup?.operandIds.includes(child.id))throw Error('Select direct originals, not generated boolean layers.');affected.set(id,parent);child=parent;id=parent.svgBooleanGroup.ancestorId;}}
