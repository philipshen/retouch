 ...
 async function prepareCascade(info,group,load){
  ...
  const run=(type,extra)=>{changes.length=0;changed.clear();changed.add(info.id);let restoreStructure=null;return visibleAncestors(group,()=>{try{
   ...
   if(type==='createSVGBooleanGroup'){
    if(!Array.isArray(extra.ids)||extra.ids.length<2||extra.ids.length>=meta.operandIds.length||new Set(extra.ids).size!==extra.ids.length||extra.ids.some(id=>!meta.operandIds.includes(id)))throw Error('Select two or more originals and leave another shape in this group.');
