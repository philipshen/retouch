(function(root){
 ...
 function render(){
  ...
  for(const item of state.items){let node=nodes.get(item.id);if(!node){node=document.createElement('button');node.type='button';node.className='canvas-guide';node.dataset.guideId=item.id;node.onpointerdown=event=>start(event,item.axis,item.id);node.onkeydown=event=>keyboard(event,item.id);layer.append(node);nodes.set(item.id,node);}
   const vertical=item.axis==='x',position=(vertical?g.preview.left-g.bounds.left:g.preview.top-g.bounds.top)+(item.value-(vertical?g.x:g.y))*g.scale;node.dataset.guideAxis=item.axis;node.dataset.guideValue=item.value;node.setAttribute('aria-label',(vertical?'Vertical':'Horizontal')+' guide '+item.value+' px');node.title='Drag to move; arrows adjust by 1 px, Shift by 10 px. Delete removes; Command/Ctrl+Z undoes.';Object.assign(node.style,vertical?{left:(position-4)+'px',top:'20px',width:'9px',height:'calc(100% - 20px)'}:{left:'20px',top:(position-4)+'px',width:'calc(100% - 20px)',height:'9px'});
  }
 }
