(function (root) {
  ...
  function appearance(info, el, save, colorAction, backgroundAction) {
    ...
    if(colorAction){
      for(const [property,label]of [['color','Text color'],['background-color','Background color'],['border-color','Border color'],...(el.namespaceURI==='http://www.w3.org/2000/svg'?[['fill','SVG fill'],['stroke','SVG stroke']]:[])]){
        ...
        input.value=root.RetouchBackgroundPaintUI.sourceColor(info,'',property)??computed;input.dataset.paintProperty=property;input.retouchPaintPreview=()=>root.RetouchPaintPicker.propertyPreview({el,input,property});
  ...
  const textWrapToken=t=>/^(?:text-(?:wrap|nowrap|balance|pretty)|\[text-wrap:.+\])$/.test(t);
