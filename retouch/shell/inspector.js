(function (root) {
  ...
  function sharedVerticalAlignment(parent,getElements,ready,onChange,hasOwn){
    const read=()=>getElements().map(el=>{const css=el.ownerDocument.defaultView.getComputedStyle(el),layout=textVerticalLayout(css);if(el.namespaceURI!=='http://www.w3.org/1999/xhtml'||!isTextLayer(el.localName)||!layout)return null;const value=css.getPropertyValue(layout.property),start=layout.reverse?'bottom':'top',end=layout.reverse?'top':'bottom';return {...layout,current:value==='center'?'center':['start','flex-start','normal','stretch'].includes(value)?start:['end','flex-end'].includes(value)?end:'custom'};}),layouts=read();if(layouts.some(value=>!value))return;
