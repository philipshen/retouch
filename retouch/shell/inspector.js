(function (root) {
  ...
  function sharedVariationTypography(parent,getElements,ready,onChange,onReset,canReset){
    ...
    const write=(tag,value)=>{if(!ready())return;if(value!==null&&ranges){const range=ranges.get(tag);if(!range||value<range.min||value>range.max){note(details,'Choose an axis and value supported by every inspected font.','refused');return;}}const current=read();if(current.some(value=>value===null))return;const next=current.map(axes=>{const map=new Map(axes);if(value===null)map.delete(tag);else map.set(tag,value);return V.serializeVariations([...map]);});if(next.every(value=>V.parseVariations(value)!==null))onChange(next);else note(details,'A selected layer would exceed 16 axis overrides. Remove an override before adding another.','refused');};
    if(axes.every(value=>value!==null)){
      ...
      for(const tag of tags){const values=axes.map(axes=>axes.find(axis=>axis[0]===tag)?.[1]),mixed=values.some(value=>value!==values[0])||values[0]===undefined,row=document.createElement('div');row.className='property-row';details.append(row);
        const input=number(row,'Shared '+(labels[tag]||tag)+' axis',mixed?NaN:values[0],-10000,10000,value=>write(tag,value));if(mixed)input.placeholder='Mixed';input.disabled=!ready();input.closest('.inspector-field').querySelector('span').textContent=labels[tag]||tag;inputs.set(tag,input);
    ...
    if(metadata&&axes.every(value=>value!==null)){
      ...
      if([...files.values()].every(item=>item.url)){
        ...
        const apply=found=>{ranges=new Map(sharedAxisRanges(found).map(axis=>[axis.tag,axis]));for(const tag of discovered)inputs.delete(tag);discovered.clear();result.replaceChildren();
          ...
          for(const axis of ranges.values()){if(inputs.has(axis.tag))continue;const input=number(result,'Shared '+(labels[axis.tag]||axis.tag)+' axis',NaN,axis.min,axis.max,value=>write(axis.tag,value));input.placeholder='Font default';input.title=axis.name+' · shared range '+axis.min+' to '+axis.max;input.disabled=!ready();inputs.set(axis.tag,input);discovered.add(axis.tag);}
