(function(root){
 ...
 function mount(info,onSave,{open=false,onToggle=()=>{}}={}){
  ...
  const error=I.note(box,info._pictureSourceError||'','refused');error.hidden=!info._pictureSourceError;error.setAttribute('role','alert');
  const run=async(button,change)=>{info._pictureSourceError='';error.hidden=true;button.disabled=true;try{await onSave(change);}catch(reason){info._pictureSourceError=reason.message;error.textContent=reason.message;error.hidden=false;}finally{button.disabled=false;}};
  ...
  const draft=info._pictureSourceDraft||(info._pictureSourceDraft={path:'',range:'upTo',min:'0',max:'600',custom:''});
  const path=input('Artwork image path','Image path',draft.path);path.placeholder='/images/phone.svg';
  const range=I.select(box,'Artwork screen range',[['upTo','Up to width'],['from','From width'],['between','Between widths'],['all','All widths'],['custom','Custom condition']],draft.range,update);range.parentElement.querySelector('span').textContent='Screen range';
  const min=input('Artwork minimum width (px)','Min width',draft.min,'number'),max=input('Artwork maximum width (px)','Max width',draft.max,'number'),custom=input('Artwork media condition','Condition',draft.custom);
  for(const [key,field]of Object.entries({path,range,min,max,custom})){const remember=()=>{draft[key]=field.value;};field.addEventListener('input',remember);field.addEventListener('change',remember);}
  ...
  const add=I.button('Add picture source',()=>{let media;try{media=root.RetouchResponsiveImageSource.condition(range.value,min.value,max.value,custom.value);}catch(reason){info._pictureSourceError=reason.message;error.textContent=reason.message;error.hidden=false;return;}return run(add,{action:'add',src:path.value.trim(),media});});box.append(add);return details;
 }
