(function(root){
 ...
 function mount(parent,{actions,index,library,fields,pick,change}){
  ...
  function list(container,items,path){
   items.forEach((action,n)=>{
    const current=[...path,n],id=index+'.'+current.map(x=>typeof x==='number'?x+1:x).join('.'),card=document.createElement('details'),summary=document.createElement('summary'),content=document.createElement('div');card.className='prototype-action';card.open=true;summary.textContent=names[action.action]+(action.action==='set-variable'?' · '+(library?.variables.find(v=>v.id===action.assignment.id)?.name||'Missing variable'):'');summary.draggable=true;summary.setAttribute('aria-label','Action details '+id);card.append(summary,content);container.append(card);
 ...
 root.RetouchPrototypeActionPanel={mount};
})(window);
