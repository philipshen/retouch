(function(root){
 ...
 function mount(parent,{actions,index,library,fields,pick,change}){
  const tree=structuredClone(actions),box=document.createElement('div');box.className='prototype-action-list';parent.append(box);let dragged=null;const scrolling=root.RetouchPrototypeActionView.dragScroll(box,()=>{dragged=null;box.querySelectorAll('.prototype-action-drop').forEach(el=>el.classList.remove('prototype-action-drop'));}),stop=()=>{dragged=null;scrolling.stop();box.querySelectorAll('.prototype-action-drop').forEach(el=>el.classList.remove('prototype-action-drop'));};const status=I.note(box,'');status.setAttribute('role','status');
  ...
  function list(container,items,path){
   items.forEach((action,n)=>{
    ...
    summary.addEventListener('dragstart',event=>{dragged=current;scrolling.start(event);event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain','Retouch action');});summary.addEventListener('dragend',stop);summary.addEventListener('dragenter',event=>{if(dragged){event.preventDefault();summary.classList.add('prototype-action-drop');}});summary.addEventListener('dragleave',()=>summary.classList.remove('prototype-action-drop'));summary.addEventListener('dragover',event=>{if(dragged)event.preventDefault();});summary.addEventListener('drop',event=>{if(!dragged||!scrolling.active){stop();return;}event.preventDefault();event.stopPropagation();const from=dragged;stop();move(from,current);});
   ...
   const id=index+(path.length?'.'+path.map(x=>typeof x==='number'?x+1:x).join('.'):''),add=I.button('Add action',()=>update(next=>listAt(next,path).push({action:'navigate',destination:'/'})));add.setAttribute('aria-label','Add action '+id);container.append(add);add.addEventListener('dragover',event=>{if(dragged)event.preventDefault();});add.addEventListener('drop',event=>{if(!dragged||!scrolling.active){stop();return;}event.preventDefault();const from=dragged;stop();move(from,[...path,items.length]);});
  }
  list(box,tree,[]);
 }
