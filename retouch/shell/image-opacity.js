(function(root){
 'use strict';
 function mount({popup,group,row,source,index,current,currentPaint,upload,replace,feedback}){
  ...
  const saveStatus=document.createElement('p');saveStatus.className='image-fill-status';saveStatus.setAttribute('role','status');const saveFeedback=feedback(saveStatus),saveRetry=I.button('Retry paint '+(index+1)+' edit',()=>{const patch=saveFeedback.state.failedPatch;if(patch)return apply(patch);});saveRetry.textContent='Retry edit';saveRetry.setAttribute('aria-label','Retry paint '+(index+1)+' edit');controls.append(saveStatus,saveRetry);
  function sync(){
   saveRetry.hidden=!saveFeedback.state.failedPatch;saveRetry.disabled=!model||saving;
  ...
  async function apply(patch){
   ...
   saving=true;saveFeedback.state.failedPatch=null;sync();if(Object.hasOwn(patch,'opacity'))for(const control of [input,inline])control.value=String(patch.opacity*100);saveFeedback('Applying paint…');
   try{const next={...model,...patch},blob=new Blob([C.markup(next)],{type:'image/svg+xml'}),url=await upload(new File([blob],'opacity-paint.svg',{type:'image/svg+xml'}));if(!currentPaint())throw Error('The paint changed before it could be applied.');if(keepFocus)root.RetouchPanelFocus?.queue(eye.isConnected?eye:document.querySelectorAll('.paint-stack-fields > .paint-order > .paint-order-row')[index]?.querySelector('.paint-visibility'),(patch.hidden?'Show':'Hide')+' image paint '+(index+1));await replace(url,true);model=next;saveFeedback('');}
   catch(error){saveFeedback(error.message);if(current()){if(currentPaint())saveFeedback.state.failedPatch={...patch};popup.retouchOpen();}}
