(function(root){
 ...
 function mount({target,frame,canvas,control,save,current,onEnd,onError,initial=null}){
  ...
  const directions=handlesFor(target),names={e:'right',w:'left',n:'top',s:'bottom',ne:'top right',nw:'top left',se:'bottom right',sw:'bottom left'};const handles=directions.map(handle=>{const b=doc.createElement('button');b.type='button';b.dataset.flowHandle=handle;b.className='canvas-selection-resize';b.setAttribute('aria-label','Resize flow '+names[handle]);surface.append(b);return b;});
