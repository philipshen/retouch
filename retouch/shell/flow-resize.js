(function(root){
 ...
 function available(el){const css=el.ownerDocument.defaultView.getComputedStyle(el);return !['absolute','fixed'].includes(css.position)&&!['inline','contents','none'].includes(css.display)&&!['width','height','inline-size','block-size'].some(p=>el.style.getPropertyPriority(p)==='important')&&!['flex','flex-grow','flex-shrink','flex-basis'].some(p=>el.style.getPropertyValue(p));}
