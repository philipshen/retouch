(function(root){
 ...
 function available(el){const css=el.ownerDocument.defaultView.getComputedStyle(el);return !['absolute','fixed'].includes(css.position)&&!['inline','contents','none'].includes(css.display)&&!['width','height'].some(p=>el.style.getPropertyPriority(p)==='important')&&!['inline-size','block-size','flex','flex-grow','flex-shrink','flex-basis'].some(p=>el.style.getPropertyValue(p));}
 ...
 function mount({target,frame,canvas,control,save,current,onEnd,onError,initial=null}){
  let g;const measure=()=>{try{return root.RetouchInspector.geometry(target,{allowRotation:true,allowScale:true,layoutOnly:true});}catch{const g=root.RetouchInspector.localPositionGeometry(target,{allowFlow:true});return {...g,rotation:g.rotation||0,scaleX:g.scaleX??1,scaleY:g.scaleY??1};}};try{if(!available(target))throw Error('Choose a layer whose flow size can be edited.');g=measure();}catch(error){onError(error.message);return null;}
  ...
  const points=(geometry,zoom=1)=>geometry.localCoordinates?root.RetouchCanvasRotate.localControls(target,zoom,geometry).positions:root.RetouchCanvasRotate.resizeHandles(geometry,zoom),originalPoints=points(g);let frameSpace=g.localCoordinates?root.RetouchSVGDraw.stableNativeSpace(target.offsetParent):null;
  ...
  const signature=()=>{const css=w.getComputedStyle(target);return [css.transform,css.translate,css.transformOrigin,css.transformBox,css.zoom].join('|');};let expectedTransform=signature();
  ...
  function valid(){if(!target.isConnected||!control.isConnected||frameSpace&&!frameSpace.current()||signature()!==expectedTransform)return false;const css=w.getComputedStyle(target),signed=(root.RetouchFlip.parse(css.scale||'none'));return signed?.length===2&&signed[0]===g.scaleX&&signed[1]===g.scaleY&&root.RetouchReactSelection.rotationDegrees(css.rotate)===g.rotation&&target.isConnected&&control.isConnected&&target.getAttribute('style')===expectedStyle&&handlesFor(target).join(',')===directions.join(',')&&current();}
  function finish(commit=false){if(ended)return;const next=commit&&valid()?sizes:null,ownStyle=target.getAttribute('style')===expectedStyle;ended=true;root.cancelAnimationFrame(raf);surface.remove();cleanups.forEach(fn=>fn());[...previews.values()].reverse().forEach(p=>p.restore());if(ownStyle){if(originalStyle===null)target.removeAttribute('style');else target.setAttribute('style',originalStyle);}for(const field of readouts)if(field.last!==null&&field.input.value===field.last)field.input.value=field.original;onEnd();if(next&&Object.entries(next).some(([axis,value])=>Math.abs(value-g[axis])>.01))save(next);}
  function paint(){const dock=doc.querySelector('.design-tool-dock')?.getBoundingClientRect();if(dock)hint.style.bottom=(root.innerHeight-dock.top+8)+'px';const next=measure();if(sizes){const css=w.getComputedStyle(target);for(const field of readouts){if(!field.input.isConnected)continue;field.last=field.box==='css'?css.getPropertyValue(field.axis):String(Math.round(next[field.axis]*100)/100);field.input.value=field.last;}}hint.textContent=Math.round(next.width*100)/100+' × '+Math.round(next.height*100)/100+' px · Resize in layout · Shift keeps proportions · Arrows: 1px · Enter applies · Escape cancels';const positions=points(next,scale);for(const b of handles){const p=positions.find(p=>p.handle===b.dataset.flowHandle);Object.assign(b.style,{left:f.left+p.x-left+'px',top:f.top+p.y-top+'px',rotate:next.rotation+'deg',cursor:p.cursor||root.RetouchCanvasRotate.resizeCursor(p.handle,next.rotation,next.scaleX,next.scaleY)});}}
  function applyPreview(requested){
   ...
   expectedStyle=target.getAttribute('style');expectedTransform=signature();const measured=measure();if(frameSpace)frameSpace=root.RetouchSVGDraw.stableNativeSpace(target.offsetParent);return measured;
  }
  function followPointer(requested,actual,base,proportional){
   const point=g=>points(g).find(p=>p.handle===state.handle),start=originalPoints.find(p=>p.handle===state.handle),goal={x:start.x+state.dx/scale,y:start.y+state.dy/scale},axes=proportional?['width']:Object.keys(requested),ratio=base.height/base.width;
  ...
  function update(e){
   ...
   try{
    const base=state.base||g,local=g.localCoordinates&&!state.keyboard?pointerCorrection([{x:(originalPoints[2].x-originalPoints[0].x)/g.width,y:(originalPoints[2].y-originalPoints[0].y)/g.width},{x:(originalPoints[6].x-originalPoints[0].x)/g.height,y:(originalPoints[6].y-originalPoints[0].y)/g.height}],{x:state.dx/scale,y:state.dy/scale}):null,delta=state.keyboard?{x:state.dx,y:state.dy}:local?{x:local[0]*g.scaleX,y:local[1]*g.scaleY}:root.RetouchCanvasMove.rotateVector(state.dx/scale,state.dy/scale,-g.rotation),r=root.RetouchCanvasMove.resize(base.width,base.height,state.handle,delta.x*(!state.keyboard&&center.x?2:1)/(state.keyboard?1:g.scaleX),delta.y*(!state.keyboard&&center.y?2:1)/(state.keyboard?1:g.scaleY),{shiftKey:e.shiftKey});let requested={...state.previous};
    ...
    if(!state.keyboard&&(g.localCoordinates||g.rotation||g.scaleX!==1||g.scaleY!==1))({requested,actual}=followPointer(requested,actual,base,e.shiftKey));
