(function(root){
 ...
 function combineShapes(operands,operation){
  ...
  for(const operand of operands){
   if(!operand||!geometry.serializeCompound(operand.document)||!['nonzero','evenodd'].includes(operand.fillRule??'nonzero')||operand.document.subpaths.some(part=>!part.closed))return fail('Choose closed shapes with supported fill rules.');
  ...
  try{
   scope=new paper.PaperScope();scope.setup(new scope.Size(1,1));
   const items=prepared.map(operand=>{const item=new scope.CompoundPath({pathData:geometry.serializeCompound(operand.document),fillRule:operand.fillRule,insert:false});item.transform(new scope.Matrix(...operand.matrix));return item;});
