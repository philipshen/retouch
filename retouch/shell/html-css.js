(function(){
 ...
 function mountSelection(infos,elements,width,save){
  ...
  for(const [property,label]of sharedFields){
   ...
   const values=infos.map((info,i)=>{const raw=['visibility','opacity'].includes(property)?computed[i].getPropertyValue(property):info.cssRules?.[width]?.[property]??computed[i].getPropertyValue(property);return property==='rotate'?String(RetouchReactSelection.rotationDegrees(raw)):raw;}),mixed=values.some(value=>value!==values[0]),numeric=['opacity','rotate'].includes(property);
   const spacing=/^(?:gap|padding(?:-(?:top|right|bottom|left))?)$/.test(property),rangeGuarded=['visibility','opacity'].includes(property)||coreTypography.includes(property)||spacing||/^(?:min-|max-)?(?:width|height)$/.test(property)||['display','flex-direction','flex-wrap','align-items','align-content','justify-content','justify-items','grid-auto-flow'].includes(property),spacingActive=()=>elements.every(el=>el.isConnected&&width<=el.ownerDocument.defaultView.innerWidth);
   const input=document.createElement(options[property]?'select':'input');
   ...
   input.oninput=()=>input.setCustomValidity('');input.onchange=()=>{if(['visibility','opacity'].includes(property)&&!appearanceReady(elements,width,property)||rangeGuarded&&!spacingActive()||!input.value.trim()||!input.checkValidity())return;const value=property==='opacity'?String(Number(input.value)/100):property==='rotate'?input.value+'deg':input.value.trim();if(!valid(property,value)||!CSS.supports(property,value)){input.setCustomValidity('Enter a supported CSS value.');input.reportValidity();return;}save(property,value,width);};
   ...
   if(rangeGuarded){input.disabled=!spacingActive()||['visibility','opacity'].includes(property)&&!appearanceReady(elements,width,property)||coreTypography.includes(property)&&!typographyReady(elements,width,property);if(input.disabled)input.title=['visibility','opacity'].includes(property)?(elements.some(el=>width>el.ownerDocument.defaultView.innerWidth)?'Preview the selected edit range before changing appearance.':'An important inline appearance rule controls a selected layer.'):typographyHint(elements,width);}
   if(spacing){
