(function(){
 ...
 const coreTypography=['font-family','font-size','font-weight','font-style','line-height','letter-spacing','text-decoration-line','text-decoration-style','text-decoration-thickness','text-underline-offset','text-decoration-skip-ink','text-decoration-color','text-indent','text-wrap','font-variant-caps','font-variant-position'];
 function typographyReady(elements,width,property,reset=false){return elements.every(el=>el.isConnected&&width<=el.ownerDocument.defaultView.innerWidth&&(reset||!(property==='text-wrap'?['text-wrap','text-wrap-mode','text-wrap-style']:[property]).some(key=>el.style.getPropertyPriority(key)==='important')));}
 ...
 function mountSelection(infos,elements,width,save){
  ...
  const sharedFields=[['visibility','Visibility'],['opacity','Opacity (%)'],['rotate','Rotation (°)'],['mix-blend-mode','Blend mode'],['isolation','Blend group'],...fields,['text-wrap','Wrap style'],['font-variant-caps','Capital forms'],['font-variant-position','Number position'],...[['text-decoration-style','Underline style'],['text-decoration-thickness','Underline thickness'],['text-underline-offset','Underline offset'],['text-decoration-skip-ink','Underline skip ink'],['text-decoration-color','Underline color']],...(hasGrid?[['justify-items','Align columns']]:[]),...(elements.every(el=>el.namespaceURI==='http://www.w3.org/2000/svg')?svgFields:[])];
