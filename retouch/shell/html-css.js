(function(){
 ...
 function mountSelection(infos,elements,width,save){
  ...
  for(const property of ['filter','backdrop-filter'])RetouchFilterStack.mountSharedBlur(groups.effects,infos,elements,width?'min-['+width+'px]:':'',property,values=>save(null,null,width,Object.fromEntries(infos.map((info,i)=>[info.id,values[i]===undefined?{}:values[i]]))));
