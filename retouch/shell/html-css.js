(function(){
 ...
 function appearanceReady(elements,width,property,reset=false){return elements.every(el=>el.isConnected&&width<=el.ownerDocument.defaultView.innerWidth&&(reset||el.style.getPropertyPriority(property)!=='important'));}
 ...
 function mount(info,el,width,save,position=null,textStyleAction=null){
  ...
  if(el.namespaceURI==='http://www.w3.org/1999/xhtml'){
   const clipReady=(reset=false)=>el.isConnected&&width<=el.ownerDocument.defaultView.innerWidth&&(reset||!['overflow','overflow-x','overflow-y'].some(property=>el.style.getPropertyPriority(property)==='important'));
   const clipping=document.createElement('input');clipping.type='checkbox';clipping.checked=['hidden','clip'].includes(css.overflowX)&&['hidden','clip'].includes(css.overflowY);clipping.indeterminate=!clipping.checked&&!(css.overflowX==='visible'&&css.overflowY==='visible');clipping.disabled=!clipReady();clipping.onchange=()=>{if(clipReady())save({overflow:clipping.checked?'clip':'visible','overflow-x':null,'overflow-y':null},null,width);};I.field(layout,'Clip content',clipping);
   if(clipping.disabled)clipping.title=width>el.ownerDocument.defaultView.innerWidth?'Preview the selected edit range before changing clipping.':'An important inline overflow rule controls this layer.';
   const resetClipping=I.button('Reset clipping',()=>{if(clipReady(true))save({overflow:null,'overflow-x':null,'overflow-y':null},null,width);});resetClipping.disabled=!clipReady(true)||!['overflow','overflow-x','overflow-y'].some(p=>Object.hasOwn(own,p));layout.append(resetClipping);
  ...
  const visible=document.createElement('input');visible.type='checkbox';visible.checked=css.visibility==='visible';visible.disabled=!appearanceReady([el],width,'visibility');visible.onchange=()=>{if(appearanceReady([el],width,'visibility'))save('visibility',visible.checked?'visible':'hidden',width);};I.field(appearance,'Visible layer',visible);if(visible.disabled)visible.title=width>el.ownerDocument.defaultView.innerWidth?'Preview the selected edit range before changing visibility.':'An important inline visibility rule controls this layer.';
  const resetVisibility=I.button('Reset visibility',()=>{if(appearanceReady([el],width,'visibility',true))save('visibility',null,width);});resetVisibility.disabled=!appearanceReady([el],width,'visibility',true)||!Object.hasOwn(own,'visibility');appearance.append(resetVisibility);I.note(appearance,'Hidden layers keep their layout space. Select them in Layers to show them again.');

  for(const [property,label,min,max,unit]of [['opacity','Opacity (%)',0,100,''],['rotate','Rotation (°)',-360,360,'deg']]){
   const raw=property==='opacity'?css.opacity:own[property]??css.getPropertyValue(property),input=document.createElement('input');input.type='number';input.min=min;input.max=max;input.step='any';
   ...
   input.onchange=()=>{if(property==='opacity'&&!appearanceReady([el],width,property))return;if(input.value!==''&&input.checkValidity()){const value=property==='opacity'?String(Number(input.value)/100):input.value+unit;if(valid(property,value)&&CSS.supports(property,value))save(property,value,width);}};
   ...
   const reset=I.button('Reset '+property,()=>{if(property!=='opacity'||appearanceReady([el],width,property,true))save(property,null,width);});reset.disabled=property==='opacity'&&!appearanceReady([el],width,property,true)||!Object.hasOwn(own,property);host.append(reset);if(property==='opacity'){input.disabled=!appearanceReady([el],width,property);if(input.disabled)input.title='Preview the selected edit range and resolve important inline opacity rules.';}
  ...
  for(const [property,label]of [['mix-blend-mode','Blend mode'],['isolation','Blend group']]){
   const current=css.getPropertyValue(property),input=document.createElement('select');
   ...
   input.value=current;input.disabled=!appearanceReady([el],width,property);input.onchange=()=>{if(appearanceReady([el],width,property))save(property,input.value,width);};I.field(appearance,label,input);if(input.disabled)input.title='Preview the selected edit range and resolve important inline '+label.toLowerCase()+' rules.';
   const reset=I.button('Reset '+label.toLowerCase(),()=>{if(appearanceReady([el],width,property,true))save(property,null,width);});reset.disabled=!appearanceReady([el],width,property,true)||!Object.hasOwn(own,property);appearance.append(reset);
  }
  ...
  for(const [property,label] of [...fields,...(isGrid?[['justify-items','Align columns']]:[])]){
   ...
   input.dataset.variableProperty=property;I.field(target,label+' (CSS)',input);if(spacing)I.fieldDraft(input);if(rangeGuarded){input.disabled=!spacingActive()||coreTypography.includes(property)&&!typographyReady([el],width,property);if(input.disabled)input.title=typographyHint([el],width);}
 ...
 function mountSelection(infos,elements,width,save){
  ...
  if(elements.every(el=>el.namespaceURI==='http://www.w3.org/1999/xhtml')){
   const clipState=css=>['hidden','clip'].includes(css.overflowX)&&['hidden','clip'].includes(css.overflowY)?true:css.overflowX==='visible'&&css.overflowY==='visible'?false:null,clipValues=computed.map(clipState),clipping=document.createElement('input');
   const clipReady=(reset=false)=>elements.every(el=>el.isConnected&&width<=el.ownerDocument.defaultView.innerWidth&&(reset||!RetouchLayout.inlineOverflow(el,true)));
   const writeClip=value=>{if(clipReady(value===null))save(null,null,width,Object.fromEntries(infos.map(info=>[info.id,{overflow:value===null?null:value?'clip':'visible','overflow-x':null,'overflow-y':null}])));};
   clipping.type='checkbox';clipping.checked=clipValues.every(value=>value===true);clipping.indeterminate=!clipping.checked&&!clipValues.every(value=>value===false);clipping.disabled=!clipReady();clipping.onchange=()=>writeClip(clipping.checked);clipping.title=clipping.disabled?'Preview the selected edit range and resolve important inline overflow rules.':'Hide content outside the selected layers without adding scrollbars.';I.field(groups.layout,'Shared Clip content',clipping);
   const resetClip=I.button('Reset shared clip content',()=>writeClip(null));resetClip.disabled=!clipReady(true)||infos.every(info=>!['overflow','overflow-x','overflow-y'].some(property=>Object.hasOwn(info.cssRules?.[width]||{},property)));const clipField=clipping.closest('.inspector-field'),clipRow=document.createElement('div');clipRow.className='property-row';clipField.before(clipRow);clipRow.append(clipField,resetClip);clipField.querySelector(':scope > span').textContent='Clip content';resetClip.setAttribute('aria-label','Reset shared clip content');resetClip.title='Reset clipping';resetClip.textContent='↺';resetClip.classList.add('property-reset');
  ...
  for(const [property,label]of sharedFields){
   ...
   const values=infos.map((info,i)=>{const raw=['visibility','opacity','mix-blend-mode','isolation'].includes(property)?computed[i].getPropertyValue(property):info.cssRules?.[width]?.[property]??computed[i].getPropertyValue(property);return property==='rotate'?String(RetouchReactSelection.rotationDegrees(raw)):raw;}),mixed=values.some(value=>value!==values[0]),numeric=['opacity','rotate'].includes(property);
   const spacing=/^(?:gap|padding(?:-(?:top|right|bottom|left))?)$/.test(property),rangeGuarded=['visibility','opacity','mix-blend-mode','isolation'].includes(property)||coreTypography.includes(property)||spacing||/^(?:min-|max-)?(?:width|height)$/.test(property)||['display','flex-direction','flex-wrap','align-items','align-content','justify-content','justify-items','grid-auto-flow'].includes(property),spacingActive=()=>elements.every(el=>el.isConnected&&width<=el.ownerDocument.defaultView.innerWidth);
   const input=document.createElement(options[property]?'select':'input');
   ...
   if(rangeGuarded){input.disabled=!spacingActive()||['visibility','opacity','mix-blend-mode','isolation'].includes(property)&&!appearanceReady(elements,width,property)||coreTypography.includes(property)&&!typographyReady(elements,width,property);if(input.disabled)input.title=['visibility','opacity','mix-blend-mode','isolation'].includes(property)?(elements.some(el=>width>el.ownerDocument.defaultView.innerWidth)?'Preview the selected edit range before changing appearance.':'An important inline appearance rule controls a selected layer.'):typographyHint(elements,width);}
   if(spacing){
   ...
   if(numeric){
    ...
    if(elements.some(el=>el.style.getPropertyPriority(property)==='important')||property==='rotate'&&values.some(value=>!Number.isFinite(Number(value)))){input.disabled=true;input.title='Edit the selected layer’s inline or 3D property in its source first.';}
   }
