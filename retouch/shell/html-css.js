(function(){
 ...
 function mount(info,el,width,save,position=null,textStyleAction=null){
  ...
  if(info.structure?.canInsert){
   ...
   const normal=I.button('Normal flow',()=>save('display','block',width));normal.setAttribute('aria-pressed',String(['block','inline','inline-block','flow-root','list-item'].includes(css.display)));stacks.append(normal);
   for(const [axis,label]of [['vertical','Vertical stack'],['horizontal','Horizontal stack']]){const changes=stackLayout(axis,css.writingMode),button=I.button(label,()=>save(changes,null,width));button.setAttribute('aria-pressed',String(['flex','inline-flex'].includes(css.display)&&css.flexDirection===changes['flex-direction']&&css.flexWrap==='nowrap'));stacks.append(button);}layout.append(stacks);
 ...
 function mountSelection(infos,elements,width,save){
  ...
  const sharedFields=[['visibility','Visibility'],['opacity','Opacity (%)'],['rotate','Rotation (°)'],['mix-blend-mode','Blend mode'],['isolation','Blend group'],...fields,...(elements.every(el=>el.namespaceURI==='http://www.w3.org/2000/svg')?svgFields:[])];
  ...
  if(computed.every(css=>['flex','inline-flex'].includes(css.display))){
   ...
   const advanced=details(groups.layout,'layout-options','Layout options');for(const property of ['align-items','align-content','justify-content'])advanced.append(rows.get(property));
   for(let y=0;y<3;y++)for(let x=0;x<3;x++){
    const label='Shared Align children '+['top','middle','bottom'][y]+' '+['left','center','right'][x],button=I.button('•',()=>{try{const changes=Object.fromEntries(infos.map((info,i)=>{const el=elements[i];if(!el?.isConnected)throw Error('Re-select the current containers.');if(width>el.ownerDocument.defaultView.innerWidth)throw Error('Preview the selected edit range before aligning children.');const css=el.ownerDocument.defaultView.getComputedStyle(el);if(!['flex','inline-flex'].includes(css.display))throw Error('Select flex containers to align their children.');return [info.id,flexAlignment(x,y,css)];}));save(null,null,width,changes);}catch(error){I.note(groups.layout,error.message,'refused');}});
    button.setAttribute('aria-label',label);button.disabled=width>elements[0].ownerDocument.defaultView.innerWidth;button.title=button.disabled?'Preview the selected edit range before aligning children.':label;button.setAttribute('aria-pressed',String(computed.every(css=>Object.entries(flexAlignment(x,y,css)).every(([property,value])=>css.getPropertyValue(property)===value))));picker.append(button);
   }
  }
