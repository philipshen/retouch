(function(){
 ...
 function visibilityReady(elements,width,reset=false){return elements.every(el=>el.isConnected&&width<=el.ownerDocument.defaultView.innerWidth&&(reset||el.style.getPropertyPriority('visibility')!=='important'));}
 ...
 function mount(info,el,width,save,position=null,textStyleAction=null){
  ...
  const visible=document.createElement('input');visible.type='checkbox';visible.checked=css.visibility==='visible';visible.disabled=!visibilityReady([el],width);visible.onchange=()=>{if(visibilityReady([el],width))save('visibility',visible.checked?'visible':'hidden',width);};I.field(appearance,'Visible layer',visible);if(visible.disabled)visible.title=width>el.ownerDocument.defaultView.innerWidth?'Preview the selected edit range before changing visibility.':'An important inline visibility rule controls this layer.';
  const resetVisibility=I.button('Reset visibility',()=>{if(visibilityReady([el],width,true))save('visibility',null,width);});resetVisibility.disabled=!visibilityReady([el],width,true)||!Object.hasOwn(own,'visibility');appearance.append(resetVisibility);I.note(appearance,'Hidden layers keep their layout space. Select them in Layers to show them again.');
 ...
 function mountSelection(infos,elements,width,save){
  ...
  for(const [property,label]of sharedFields){
   ...
   const values=infos.map((info,i)=>{const raw=property==='visibility'?computed[i].visibility:info.cssRules?.[width]?.[property]??computed[i].getPropertyValue(property);return property==='rotate'?String(RetouchReactSelection.rotationDegrees(raw)):raw;}),mixed=values.some(value=>value!==values[0]),numeric=['opacity','rotate'].includes(property);
   const spacing=/^(?:gap|padding(?:-(?:top|right|bottom|left))?)$/.test(property),rangeGuarded=property==='visibility'||coreTypography.includes(property)||spacing||/^(?:min-|max-)?(?:width|height)$/.test(property)||['display','flex-direction','flex-wrap','align-items','align-content','justify-content','justify-items','grid-auto-flow'].includes(property),spacingActive=()=>elements.every(el=>el.isConnected&&width<=el.ownerDocument.defaultView.innerWidth);
   const input=document.createElement(options[property]?'select':'input');
   ...
   input.oninput=()=>input.setCustomValidity('');input.onchange=()=>{if(property==='visibility'&&!visibilityReady(elements,width)||rangeGuarded&&!spacingActive()||!input.value.trim()||!input.checkValidity())return;const value=property==='opacity'?String(Number(input.value)/100):property==='rotate'?input.value+'deg':input.value.trim();if(!valid(property,value)||!CSS.supports(property,value)){input.setCustomValidity('Enter a supported CSS value.');input.reportValidity();return;}save(property,value,width);};
   ...
   if(rangeGuarded){input.disabled=!spacingActive()||property==='visibility'&&!visibilityReady(elements,width)||coreTypography.includes(property)&&!typographyReady(elements,width,property);if(input.disabled)input.title=property==='visibility'?(elements.some(el=>width>el.ownerDocument.defaultView.innerWidth)?'Preview the selected edit range before changing visibility.':'An important inline visibility rule controls a selected layer.'):typographyHint(elements,width);}
   if(spacing){
