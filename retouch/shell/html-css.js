(function(){
 ...
 function mountSelection(infos,elements,width,save){
  ...
  if(computed.every(css=>['flex','inline-flex'].includes(css.display))){
   const picker=document.createElement('div');picker.className='layout-alignment';picker.setAttribute('role','group');picker.setAttribute('aria-label','Shared child alignment');groups.layout.prepend(picker);
   for(let y=0;y<3;y++)for(let x=0;x<3;x++){
