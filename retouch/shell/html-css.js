(function(){
 ...
 function mount(info,el,width,save,position=null,textStyleAction=null){
  ...
  if(gradients===null)I.note(fills,'The existing background image cannot be represented by these gradient controls. Clear background images to start a new fill.');
   ...
   gradients.forEach((gradient,index)=>{
    ...
    gradient.stops.forEach((stop,stopIndex)=>{
     ...
     const position=document.createElement('input');position.type='number';position.min=0;position.max=100;position.step='any';position.value=stop.position;position.onchange=()=>{if(position.value!==''&&position.checkValidity())update({...gradient,stops:gradient.stops.map((s,i)=>i===stopIndex?{...s,position:Number(position.value)}:s).sort((a,b)=>a.position-b.position)});};I.field(row,'Position (%)',position).setAttribute('aria-label',prefix+' position (%)');I.fieldDraft(color);I.fieldDraft(position);row.insertBefore(position.parentElement,color.parentElement);
  ...
  for(const [property,label] of fields){
   ...
   if(['color','background-color','border-color'].includes(property)){input.dataset.paintProperty=property;input.retouchPaintPreview=()=>RetouchPaintPicker.propertyPreview({el,input,property});}
