(function(root){
 ...
 function affineControls(points){
  if(points.length!==4||points.some(p=>![p.x,p.y].every(Number.isFinite)))throw Error('Use four finite layer corners.');
  const u={x:points[1].x-points[0].x,y:points[1].y-points[0].y},v={x:points[3].x-points[0].x,y:points[3].y-points[0].y},a=Math.hypot(u.x,u.y),b=Math.hypot(v.x,v.y);if(a<1e-6||b<1e-6)throw Error('Choose a layer with nonzero edges.');
  const rotation=Math.atan2(u.y,u.x)*180/Math.PI,offset=(x,y)=>({x:x*u.x/a+y*v.x/b,y:x*u.y/a+y*v.y/b});
  const corners=points.map((p,i)=>{const d=offset(i===0||i===3?-10:10,i<2?-10:10);return {x:p.x+d.x,y:p.y+d.y};});
 ...
 function localControls(target,scale=1,g=root.RetouchInspector.localPositionGeometry(target,{allowFlow:true})){
 ...
 function cornerControls({frame,canvas,onStart}){
  ...
  return {update(target,input,resizeControl){active=null;container.hidden=true;if(!target?.isConnected||!input?.isConnected||input.closest('[inert]'))return;const canRotate=!input.matches(':disabled'),canResize=resizeControl?.retouchCanvasStart&&!resizeControl.matches(':disabled')&&!resizeControl.closest('[inert]');
   try{const f=frame.getBoundingClientRect(),c=canvas.getBoundingClientRect(),scale=f.width/frame.contentWindow.innerWidth;let g,local;try{g=geometry(target,true);}catch{local=localControls(target,scale);g=local.g;}const points=local?local.corners:corners(g,scale),left=Math.max(c.left,f.left),top=Math.max(c.top,f.top),right=Math.min(c.right,f.right),bottom=Math.min(c.bottom,f.bottom);if(g.width<=0||g.height<=0||right<=left||bottom<=top)return;
    Object.assign(container.style,{left:left+'px',top:top+'px',width:right-left+'px',height:bottom-top+'px'});
    buttons.forEach((button,i)=>{const x=f.left+points[i].x-left,y=f.top+points[i].y-top;button.hidden=!canRotate||x<8||y<8||x>right-left-8||y>bottom-top-8;button.style.left=x+'px';button.style.top=y+'px';});const positions=local?local.positions:resizeHandles(g,scale),flowDirections=resizeControl?.retouchFlowHandles?.();resizeButtons.forEach((button,i)=>{button.title=resizeControl?.dataset.flowResize==='true'?'Drag to resize in layout · Shift: keep proportions':'Drag to resize · Shift: keep proportions · Option / Alt: from center';const p=positions[i],x=f.left+p.x-left,y=f.top+p.y-top;button.hidden=!canResize||local?.g.localFlow||resizeControl.dataset.flowResize==='true'&&!flowDirections?.includes(p.handle)||x<5||y<5||x>right-left-5||y>bottom-top-5;Object.assign(button.style,{left:x+'px',top:y+'px',cursor:p.cursor||resizeCursor(p.handle,g.rotation,g.scaleX,g.scaleY),rotate:(local?local.rotation:g.rotation)+'deg'});});active={target,input,resizeControl};container.hidden=false;
   }catch{}
  }};
 }
 const api={difference,angle,value,mount,corners,resizeHandles,resizeCursor,affineControls,localControls,cornerControls};if(typeof module==='object'&&module.exports)module.exports=api;else root.RetouchCanvasRotate=api;
