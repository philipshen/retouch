(function(){
 ...
 let library=null,collectionId='',variableId='',newType='color',busy=false;
 ...
 async function run(action){if(busy)return;busy=true;body.disabled=true;status.textContent='Working…';try{await action();render();status.textContent='Saved in this project.';}catch(error){status.textContent=error.message;}finally{busy=false;body.disabled=false;}}
 async function load(){await run(async()=>{library=await window.RetouchVariableLibraryRequest();});}
 ...
 function render(){
  body.replaceChildren();body.append(I.button('Reload collections',load));if(!library)return;
