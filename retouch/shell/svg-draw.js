(function(root){
 ...
 function mount({target,frame,canvas,preset,onCommit,onEnd,onError,native=false}){
  ...
  function paint(modifiers){state.points=constrained(preset,state.a,state.b,modifiers);for(const [key,value]of Object.entries(geometry(preset,...state.points)))preview.setAttribute(key,String(value));const m=matrix(),f=frame.getBoundingClientRect(),r=surface.getBoundingClientRect(),scale=f.width/w.innerWidth;preview.setAttribute('transform',`matrix(${m.a*scale} ${m.b*scale} ${m.c*scale} ${m.d*scale} ${m.e*scale+f.left-r.left} ${m.f*scale+f.top-r.top})`);if(!preview.isConnected)drawing.append(preview);}
