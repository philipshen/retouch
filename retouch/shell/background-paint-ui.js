(function(root){
 ...
 function filterState(info,scope,el,property,computed=false){
  const F=root.RetouchFilterVisibility,key=F.properties[property],own=computed?null:sourceValue(info,scope,property),stored=computed?null:sourceValue(info,scope,key),css=el.ownerDocument.defaultView.getComputedStyle(el),rendered=own??css.getPropertyValue(property),observed=css.getPropertyValue(key).trim()||'none',owned=(el.getAttribute('class')||'').includes('['+key+':'+observed+']')||Object.values(info.cssRules||{}).some(values=>values[key]===observed),metadata=stored??(own===null&&owned?observed:'none');
  let model;try{model=F.read(rendered,metadata);}catch{model=null;
   if(own===null&&metadata!=='none'){const d=el.ownerDocument,probe=d.createElement('span');try{const saved=F.metadata(metadata),expected=F.write(property,saved)[property];probe.style.cssText='all:initial;position:fixed;visibility:hidden;pointer-events:none';probe.style.setProperty('color',css.color,'important');probe.style.setProperty(property,expected,'important');d.documentElement.append(probe);if(d.defaultView.getComputedStyle(probe).getPropertyValue(property)===rendered)model=saved;}catch{}finally{probe.remove();}}
  }return {model,rendered,metadata,source:own!==null,value:model?model.map(item=>item.raw).join(' ')||'none':rendered};
 }
 function filterChanges(property,model,info){const key=root.RetouchFilterVisibility.properties[property],stored=Object.values(info.cssRules||{}).some(values=>Object.hasOwn(values,key))||((info.className||'')+' '+(info.anchorInheritedClasses||'')).includes('['+key+':');return model.some(item=>item.hidden)||stored?root.RetouchFilterVisibility.write(property,model):{[property]:model.map(item=>item.raw).join(' ')||'none'};}
 function filterClasses(classes,scope,property,changes){return Object.hasOwn(changes,root.RetouchFilterVisibility.properties[property])?root.RetouchFilterVisibility.classes(classes,scope,property,changes):root.RetouchResponsive.replaceScope(classes,root.RetouchInspector.filterClasses(root.RetouchResponsive.project(classes,scope),property,changes[property]),scope);}
 ...
 root.RetouchBackgroundPaintUI={read,bind,mountSelection,rangeActive,sourceColor,sourceState,sourceEffect,sourceShadows,readShadows,shadowChanges,shadowClasses,shadowEye,filterState,filterChanges,filterClasses,visibilityIcon,bindSource};
})(window);
