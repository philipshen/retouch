(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory(require('./html-css-values.js'),require('./responsive.js'),require('./inspector.js'));else root.RetouchCollectionBindings=factory(root.RetouchHTMLCSSValues,root.RetouchResponsive,root.RetouchInspector);})(typeof globalThis!=='undefined'?globalThis:this,function(V,R,I){
 ...
 function mount(parent,input,width,write,options={}){
  ...
  const I=RetouchInspector,details=document.createElement('details'),summary=document.createElement('summary');summary.textContent='Collection bindings';details.append(summary);parent.append(details);details.retouchVariableParent=parent;details.open=expanded;
  ...
  function browse(choices,type,trigger){
   const pickerModes={...modes},context=draftKey(),focusContext=details.dataset.variablePickerFocus,current=()=>[...document.querySelectorAll('[data-variable-picker-context]')].some(node=>node.dataset.variablePickerContext===context);
   const cssValue=resolved=>type==='boolean'?(resolved.value?'visible':'hidden'):type==='number'?V.variableNumberValue(target,resolved.value,unit):String(resolved.value);
   ...
   async function bindingAction(type){if(applying)return;if(!dialog.open||!current()){dialog.close();return;}applying=true;search.disabled=true;modeControls.disabled=bindingActions.disabled=true;for(const row of results.querySelectorAll('button'))row.disabled=true;message.textContent='Updating binding…';try{await writeBinding(type,width,{property:target,...(type==='resetVariable'?{libraryRevision:library.revision}:{})});dialog.close();}catch(error){message.textContent=error.message;}finally{applying=false;search.disabled=false;modeControls.disabled=bindingActions.disabled=false;for(const row of results.querySelectorAll('button'))row.disabled=row.dataset.available!=='true';if(!current())dialog.close();}}
   ...
   function list(){
    ...
    for(const collection of library.collections){const matches=choices.filter(variable=>variable.collectionId===collection.id&&(collection.name+' / '+variable.name).toLocaleLowerCase().includes(query));total+=matches.length;if(!matches.length||count>=100)continue;
     ...
     for(const variable of matches.slice(0,100-count)){const button=I.button('Apply '+collection.name+' / '+variable.name,async()=>{
      ...
      }catch(error){message.textContent=error.message;}finally{applying=false;search.disabled=false;modeControls.disabled=bindingActions.disabled=false;for(const row of results.querySelectorAll('button'))row.disabled=row.dataset.available!=='true';if(!current())dialog.close();}
     });button.disabled=true;button.dataset.available='false';button.classList.add('variable-picker-option');button.setAttribute('aria-label','Apply '+collection.name+' / '+variable.name);button.setAttribute('aria-pressed',String(variable.id===selected));const icon=document.createElement('span');icon.className='variable-picker-type';icon.textContent={color:'◈',number:'#',boolean:'◐',string:'T'}[type];icon.setAttribute('aria-hidden','true');const name=document.createElement('span');name.textContent=variable.name;name.className='variable-picker-name';const reason=document.createElement('small');reason.className='variable-picker-reason';reason.hidden=true;name.append(reason);const value=document.createElement('span');value.className='variable-picker-value';value.textContent='…';button.setAttribute('aria-description','Resolving value');button.replaceChildren(icon,name,value);previewRows.set(variable.id,{button,icon,value,reason});group.append(button);count++;}results.append(group);
    ...
    if(count)previewTimer=setTimeout(async()=>{
     try{const response=await RetouchVariableModePreview({revision:library.revision,modes:{...pickerModes},variableIds:[...previewRows.keys()]});if(!dialog.open||serial!==previewSerial)return;
      for(const resolved of response.values){const row=previewRows.get(resolved.id);if(!row)continue;const value=type==='boolean'?(resolved.value?'Visible':'Hidden'):String(resolved.value)+(type==='number'?unit:'');row.value.textContent=value;row.value.title=value;
       const path=resolved.path.map(step=>{const variable=library.variables.find(item=>item.id===step.variableId),collection=library.collections.find(item=>item.id===step.collectionId),mode=collection?.modes.find(item=>item.id===step.modeId);return (collection?.name||'')+' / '+(variable?.name||'')+' · '+(mode?.name||'');}).join(' → ');const available=V.valid(target,cssValue(resolved));row.button.dataset.available=String(available);row.button.disabled=applying||!available;row.reason.hidden=available;row.reason.textContent=available?'':target==='opacity'?'Opacity requires a value from 0 to 1.':'Not valid for '+(new Map(V.fields).get(target)||target).toLowerCase()+'.';row.button.title=path+' = '+value+(available?'':' · '+row.reason.textContent);row.button.setAttribute('aria-description',row.button.title);
      ...
      for(const error of response.errors||[]){const row=previewRows.get(error.id);if(row){row.value.textContent='Unavailable';row.button.disabled=true;row.button.dataset.available='false';row.reason.hidden=false;row.reason.textContent=error.reason;row.button.title=error.reason;row.button.setAttribute('aria-description',error.reason);}}
     }catch(error){if(!dialog.open||serial!==previewSerial)return;for(const row of previewRows.values()){row.value.textContent='Unavailable';row.button.disabled=true;row.button.dataset.available='false';row.reason.hidden=false;row.reason.textContent=error.message;row.button.title=error.message;row.button.setAttribute('aria-description',error.message);}}
    },120);
   }
   search.addEventListener('input',list);dialog.addEventListener('keydown',event=>{if(event.key==='Escape'){event.preventDefault();event.stopPropagation();dialog.close();return;}if(event.target.tagName==='SELECT'||!['ArrowDown','ArrowUp','Home','End'].includes(event.key)||applying)return;const rows=[...results.querySelectorAll('button:not(:disabled)')];if(!rows.length)return;const index=rows.indexOf(document.activeElement);if(index<0&&!['ArrowDown','ArrowUp'].includes(event.key))return;event.preventDefault();const next=event.key==='Home'?0:event.key==='End'?rows.length-1:index<0?(event.key==='ArrowDown'?0:rows.length-1):(index+(event.key==='ArrowDown'?1:-1)+rows.length)%rows.length;rows[next].focus();});
   // A successful write rebuilds the source context before the panel is enabled.
   // Let the write finish before closing and returning focus to its new control.
   const observer=new MutationObserver(()=>{if(!applying&&!current())dialog.close();});observer.observe(document.body,{childList:true,subtree:true});
   dialog.addEventListener('close',()=>{clearTimeout(previewTimer);previewSerial++;observer.disconnect();dialog.remove();const liveDetails=[...document.querySelectorAll('[data-variable-picker-focus]')].find(node=>node.dataset.variablePickerFocus===focusContext),focus=trigger.isConnected?trigger:[...(liveDetails?.retouchVariableParent.querySelectorAll('.property-variable')||[])].find(button=>button.getAttribute('aria-label')===trigger.getAttribute('aria-label')&&button.dataset.variableUnit===trigger.dataset.variableUnit&&button.dataset.variableField===trigger.dataset.variableField)||liveDetails?.querySelector('.variable-picker-trigger');
    if(focus){globalThis.RetouchPanelFocus?.queue(focus,undefined,{reveal:true});if(globalThis.RetouchInspectorUI?.reveal)globalThis.RetouchInspectorUI.reveal(focus);else focus.focus();}},{once:true});list();dialog.showModal();search.focus();
  }
  function render(){
   // Focus survives source revisions; permission to apply still uses the full hash.
   details.dataset.variablePickerContext=draftKey();details.dataset.variablePickerFocus=JSON.stringify([width,target,selection.map(item=>[item.id,item.file])]);
   ...
   const valid=value!==null&&RetouchHTMLCSSValues.valid(target,type==='number'?V.variableNumberValue(target,resolved.value,unit):value),apply=I.button('Apply collection binding',()=>run(()=>writeBinding('applyVariable',width,{property:target,libraryRevision:library.revision,binding:{id:selected,modes,...(type==='number'?{unit}:{})}})));apply.disabled=!valid;controls.append(apply);
 ...
 function decorate(parent){
  ...
  for(const input of parent.querySelectorAll('[data-variable-property]')){
   ...
   const label=(new Map(V.fields).get(property)||property).toLowerCase(),button=I.button('Apply variable to '+label,()=>controller.open(property,button,input.dataset.variableUnit));button.classList.add('property-variable');button.dataset.variableUnit=input.dataset.variableUnit||'';button.dataset.variableField=input.getAttribute('aria-label')||'';button.textContent='◈';button.setAttribute('aria-label','Apply variable to '+label);button.setAttribute('aria-haspopup','dialog');button.title='Apply variable to '+label;button.disabled=input.disabled;const state=controller.bound(property);if(state){button.classList.add('is-bound');button.setAttribute('aria-description',state);button.title+=' · '+state;}row.append(button);
  }
 }
