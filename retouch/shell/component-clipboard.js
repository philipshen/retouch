(function(root){
 ...
 async function open({components,infos,save,opener}){
  ...
  const matches=components.map(component=>match(component,payload)),available=new Set(matches.flatMap(result=>result.accepted.map(property=>property.name))),chosen=new Set(available),previewRows=[];
  ...
  const title=document.createElement('h2');title.textContent='Paste properties';const note=document.createElement('p');note.setAttribute('aria-live','polite');
  ...
  for(const property of payload.properties){const label=document.createElement('label'),input=document.createElement('input');input.type='checkbox';input.checked=available.has(property.name);input.disabled=!input.checked;input.setAttribute('aria-label','Paste '+property.name);input.onchange=()=>{if(input.checked)chosen.add(property.name);else chosen.delete(property.name);update();};label.append(input,document.createTextNode(property.name));choices.append(label);}
  ...
  const hint=document.createElement('p');hint.textContent='Only checked properties are pasted. Copied values become instance overrides; expression bindings stay unchanged.';
  ...
  function update(){
   const targets=selectedTargets(),count=targets.reduce((sum,target)=>sum+target.properties.length,0);note.textContent=count?'Apply '+count+' property values across '+targets.length+' selected instance'+(targets.length===1?'':'s')+'.':available.size?'Choose at least one property to paste.':'No copied properties are compatible with this selection.';
   for(const input of choices.querySelectorAll('input'))input.checked=chosen.has(input.getAttribute('aria-label').slice(6));
   for(const row of previewRows){row.label.classList.toggle('paste-property-excluded',!chosen.has(row.name));row.value.classList.toggle('paste-property-excluded',!chosen.has(row.name));}
   apply.disabled=busy||!count;chooseAll.disabled=chosen.size===available.size;chooseNone.disabled=!chosen.size;
  }
  update();cancel.onclick=()=>dialog.close();dialog.addEventListener('cancel',e=>{if(busy)e.preventDefault();});dialog.addEventListener('close',()=>{dialog.remove();const target=opener?.isConnected?opener:[...document.querySelectorAll('#panelBody button')].find(button=>button.textContent==='Paste properties');target?.focus();});apply.onclick=async()=>{const targets=selectedTargets();if(busy||!targets.length)return;busy=true;choices.disabled=true;cancel.disabled=apply.disabled=true;status.textContent='Pasting properties…';try{await save(targets);dialog.close();}catch(error){status.textContent=error.message;}finally{busy=false;choices.disabled=false;cancel.disabled=false;update();}};dialog.showModal();(available.size?apply:cancel).focus();
 }
