(function(root){
  ...
  function mount({target,frame,canvas,points,pathData=null,propertiesPane=null,onCommit,onEnd,onError}){
    ...
    if(subpaths){
      ...
      splitContourButton=action('Split at point',()=>{if(drag||!verify()||selectedPoints.size!==1||activeHandle)return;const result=root.RetouchSVGPath.splitContour({subpaths},contour,active);if(!result){announce('Choose an interior point of an open contour, or any point of a closed contour.');return;}subpaths.splice(0,subpaths.length,...result.subpaths);selectContour(result.selected);announce('Contour split at the selected point. Done saves; Escape cancels.');});splitContourButton.title='Separate an open contour into two, or open a closed contour at this point. Existing segments keep their geometry.';
