(function(root){
  ...
  function mount({target,frame,canvas,points,pathData=null,propertiesPane=null,onCommit,onEnd,onError}){
    ...
    for(const button of [...toolbar.children]){const label=button.textContent;if(actionIcons[label])compact(button,label);if(['Duplicate contour','Delete contour','Reverse contour','Close contour','Join next contour','Split at point'].includes(label)){button.style.textAlign='left';optionsPanel.append(button);}}
