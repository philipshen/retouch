(function(root){
  ...
  function mount({target,frame,canvas,points,pathData=null,propertiesPane=null,onCommit,onEnd,onError}){
    ...
    if(subpaths){
      ...
      splitContourButton=action('Split at point',()=>{if(drag||!verify()||!selectedPoints.size||activeHandle)return;const result=root.RetouchSVGPath.splitContourAtPoints({subpaths},contour,[...selectedPoints]);if(!result){announce('Choose an interior point of an open contour, or any point of a closed contour.');return;}subpaths.splice(0,subpaths.length,...result.subpaths);selectContour(result.selected);announce('Contour split at the selected points. Done saves; Escape cancels.');});splitContourButton.title='Split at selected anchors. Open endpoints are ignored; existing segments keep their geometry.';
      convertContourButton=action('Convert contour to Bézier',()=>{
        if(drag||cancelPen||joinPicking||!verify())return;
        ...
        const result=root.RetouchSVGPath.contourToCubics({subpaths},contour);
        if(!result){announce('This conversion cannot meet the precision and point limits. The contour is unchanged.');return;}
        subpaths.splice(0,subpaths.length,...result.subpaths);selectContour(result.selected);announce('All contour arcs converted to editable Bézier curves. Done saves; Escape cancels.');
      });convertContourButton.title='Convert every arc in this contour to editable curves within 0.01 SVG units. Other contours stay unchanged.';
      const booleans=root.document.createElement('div');booleans.setAttribute('role','group');booleans.setAttribute('aria-label','Combine contours');booleans.style.cssText='border-top:1px solid var(--line, #e6e6e6);padding-top:8px;margin-top:4px;display:flex;flex-direction:column;gap:4px;';const booleanLabel=root.document.createElement('label');booleanLabel.textContent='Combine with';booleanLabel.style.cssText='display:flex;flex-direction:column;gap:4px;';booleanTarget=root.document.createElement('select');booleanTarget.setAttribute('aria-label','Boolean target contour');booleanTarget.style.cssText='width:100%;box-sizing:border-box;padding:5px;border:1px solid var(--line, #e6e6e6);border-radius:4px;background:var(--control, #f5f5f5);color:var(--ink, #1e1e1e);font:12px Inter,system-ui;';booleanLabel.append(booleanTarget);booleans.append(booleanLabel);optionsPanel.append(booleans);
      for(const [operation,label] of [['union','Union contours'],['subtract','Subtract contour'],['intersect','Intersect contours'],['exclude','Exclude overlap']]){const button=action(label,()=>{
        if(drag||cancelPen||joinPicking||!verify())return;const result=root.RetouchSVGBoolean.combine({subpaths},contour,Number(booleanTarget.value),operation);
        if(!result.ok){announce(result.reason);return;}subpaths.splice(0,subpaths.length,...result.subpaths);selectContour(result.selected);announce(label+' applied. Done saves; Escape cancels.');
      });button.style.textAlign='left';button.title='Combine filled areas of two closed contours. Arcs become Bézier curves within 0.01 SVG units. Done saves; Escape cancels.';booleans.append(button);booleanButtons.push(button);}
    ...
    for(const button of [...toolbar.children]){const label=button.textContent;if(actionIcons[label])compact(button,label);if(['Duplicate contour','Delete contour','Reverse contour','Close contour','Join next contour','Split at point','Convert contour to Bézier'].includes(label)){button.style.textAlign='left';optionsPanel.append(button);}}
    ...
    function refreshContours(){
      ...
      pickJoinButton.disabled=subpaths.filter(part=>!part.closed).length<2&&!subpaths.some(part=>!part.closed&&part.nodes.length>=3);
