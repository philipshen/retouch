(function(root){
 ...
 function mount(parent,{value,index,change}){
  ...
  async function commit(focus){if(['stiffness','damping','mass'].every(key=>draft[key]===value[key]))return;await change({...draft});if(focus)document.querySelector('.prototype-details [aria-label="'+focus+'"]')?.focus({preventScroll:true});}
