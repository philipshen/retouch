(function(){
 ...
 function mount(selection,width,save,rules,saveIndividual=null,inheritedRules=[]){
  const elements=Array.isArray(selection)?selection:[selection],element=elements[0],owns=Array.isArray(rules)?rules:[rules],inherited=elements.map((_,index)=>inheritedRules[index]||{});
  ...
  function render(){
   ...
   const inheritedLinks=inherited.map((rules,index)=>Object.hasOwn(owns[index],property)?null:/^var\((--[a-zA-Z_][a-zA-Z0-9_-]{0,127})\)$/.exec(rules[property]||'')?.[1]),inheritedCount=inheritedLinks.filter(Boolean).length;
   if(inheritedCount){I.note(controls,elements.length===1?'Inherited binding: '+inheritedLinks[0]:inheritedCount+' of '+elements.length+' layers inherit a variable binding.');controls.append(I.button('Override inherited binding here',()=>{const changes=inheritedLinks.map(name=>name?{[property]:'var('+name+')'}:{});if(saveIndividual)saveIndividual(changes);else save(property,changes[0][property],width);}));I.note(controls,'Creates a binding in this scope. Smaller screen scopes remain unchanged.');}
   ...
   const inheritedNames=[...new Set(inherited.flatMap((rules,index)=>Object.keys(rules).filter(key=>RetouchHTMLCSSValues.variableName(key)&&!Object.hasOwn(owns[index],key))))].sort();
   if(inheritedNames.length)I.select(definitions,'Inherited variable definitions',[['','Choose an inherited definition…'],...inheritedNames.map(key=>[key,key])],'',key=>{name.value=key;const current=inherited.map((rules,index)=>owns[index][key]??rules[key]);value.value=current.every(item=>item===current[0])?current[0]||'':'';value.placeholder=current.some(item=>item!==current[0])?'Mixed definitions':'#2563eb, 24px, 0.5, var(--brand)';});
