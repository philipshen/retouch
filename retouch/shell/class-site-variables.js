(function(root){
 ...
 function compose(current,property,value){
  ...
  const result=kept.join(' ');if(values().variableName(property)&&typeof value==='string'&&value.startsWith('var('))checkCycle(bindings(result));return result;
 }
 ...
 function checkCycle(definitions){const cycle=values().variableCycle(definitions);if(cycle)throw Error('Variable alias cycle: '+cycle.join(' → '));}
 ...
 function selectionClasses(infos,scope,changes,inherited=[]){
  ...
  return Object.fromEntries(infos.map((info,index)=>{
   ...
   if(Object.entries(changes[index]).some(([property,value])=>values().variableName(property)&&typeof value==='string'&&value.startsWith('var(')))checkCycle({...bindings(inherited[index]),...bindings(projected)});
