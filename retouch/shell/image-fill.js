(function(root){
 ...
 function additions(section,info,el,save,saveCSS,layers,upload,saveImage,browseImages){
  const group=document.createElement('fieldset'),status=document.createElement('p');group.className='paint-add-actions';status.setAttribute('role','status');section.append(group,status);group.disabled=layers.length>=8;
  ...
  const gradient=I.button('Add gradient paint',async()=>{group.disabled=true;try{await add();}catch(error){status.textContent=error.message;}finally{group.disabled=layers.length>=8;}});group.append(gradient);
  if(browseImages)group.append(I.button('Add image paint',event=>{event.currentTarget.focus({preventScroll:true});browseImages(src=>add(src));}));
  if(upload){const input=document.createElement('input');input.type='file';input.accept='image/*';input.hidden=true;input.setAttribute('aria-label','Upload new image paint');const button=I.button('Upload new image paint',()=>input.click());group.append(button,input);input.onchange=async()=>{const file=input.files[0];if(!file)return;group.disabled=true;status.textContent='Uploading image…';try{await add(await upload(file));status.textContent='';}catch(error){status.textContent=error.message;}finally{group.disabled=layers.length>=8;input.value='';}};}
 }
