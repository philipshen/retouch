(function(root){
 ...
 function reset(){return Object.fromEntries(['background-image',...P.properties,V.paintVisibilityProperty].map(property=>[property,null]));}
 ...
 function paintPopover(section,button,body,key,title){
  ...
  const focusBack=()=>{const target=popup.retouchReturnFocus;(target?.isConnected?target:button.retouchPopoverAnchor||button).focus({preventScroll:true});};
  const position=()=>{const box=(button.retouchPopoverAnchor||button).getBoundingClientRect(),width=popup.offsetWidth;popup.style.left='clamp(8px, '+(box.left-width-8)+'px, calc(100vw - '+width+'px - 8px))';popup.style.top='clamp(8px, '+box.top+'px, calc(100vh - '+popup.offsetHeight+'px - 8px))';};
  ...
  button.onclick=()=>{popup.retouchReturnFocus=button.retouchPopoverAnchor||button;if(popup.matches(':popover-open')){openedPaint=null;popup.hidePopover();}else{open();close.focus({preventScroll:true});}};
 ...
 function paintActions(row,more,popup,commands,current){
  ...
  row.addEventListener('contextmenu',event=>{if(editable(event.target)||!current()||more.matches(':disabled'))return;event.preventDefault();event.stopPropagation();open(row.contains(document.activeElement)?document.activeElement:row.querySelector('.paint-row-editor')||row.querySelector('.paint-row-color')||more);});
 ...
 function solidPaintRow(editor,color,solid,index){
  const row=editor.parentElement,P=root.RetouchPaletteValues,swatch=editor.querySelector('.paint-row-swatch');
  const write=next=>{if(!color.isConnected||color.matches(':disabled'))return;color.value=next;color.dispatchEvent(new Event('input',{bubbles:true}));color.dispatchEvent(new Event('change',{bubbles:true}));};
  const button=I.button('Choose solid paint '+(index+1)+' color',()=>{button.focus({preventScroll:true});root.RetouchPaintPicker.open(color,{anchor:button,restorePopover:false,onClose:()=>{if(button.isConnected)button.focus({preventScroll:true});},onApply:next=>{root.RetouchPanelFocus?.queue(button);write(next);}});});
  button.classList.add('paint-row-color');button.setAttribute('aria-label','Choose solid paint '+(index+1)+' color');button.title='Choose color';button.disabled=color.matches(':disabled');swatch.style.backgroundImage+=', conic-gradient(#fff 25%, #e5e5e5 0 50%, #fff 0 75%, #e5e5e5 0)';button.replaceChildren(swatch);editor.before(button);
  let value=solid,model=null;try{model=P.parse(P.fromComputed(solid));value=model.space==='display-p3'?P.p3(model.channels,1):P.srgb(model.channels,1);}catch{}
  const inline=document.createElement('input');inline.type='text';inline.className='paint-row-value';inline.setAttribute('aria-label','Solid paint '+(index+1)+' color value');inline.title='Hex, RGB, or Display P3. Omitted alpha keeps the current opacity.';inline.spellcheck=false;const initial=/^#[a-f\d]{8}$/i.test(value)?value.slice(1,-2).toUpperCase():value.replace(/ \/ 1\)$/,')');inline.value=initial;inline.disabled=!model||color.matches(':disabled');inline.oninput=()=>inline.setCustomValidity('');inline.onchange=()=>{try{const next=P.editColor(inline.value,model.alpha);inline.setCustomValidity('');if(next!==model.value)write(next);else inline.value=initial;}catch(error){inline.setCustomValidity(error.message);inline.reportValidity();}};I.fieldDraft(inline);editor.before(inline);editor.classList.add('solid-paint-settings');editor.title='Solid paint settings';editor.textContent='Paint settings';editor.classList.remove('paint-row-editor');editor.retouchPopoverAnchor=row.querySelector('.paint-row-more');row.retouchPaintCommands.append(editor);
  const opacity=I.number(row,'Solid paint '+(index+1)+' opacity (%)',model?model.alpha*100:NaN,0,100,percent=>{if(!model||percent===model.alpha*100)return;write(model.space==='display-p3'?P.p3(model.channels,percent/100):P.srgb(model.channels,percent/100));}),field=opacity.closest('label');field.classList.add('paint-row-opacity');field.querySelector('span').textContent='%';opacity.disabled=!model||color.matches(':disabled');opacity.placeholder='—';if(model)opacity.value=String(Number((model.alpha*100).toPrecision(12)));row.querySelector('.paint-row-more').before(field);
 }
 ...
 function computedPaint(el,value){
  const d=el.ownerDocument,probe=d.createElement('div'),css=d.defaultView.getComputedStyle(el);
  probe.style.cssText='all:initial!important;position:fixed!important;visibility:hidden!important;width:0!important;height:0!important;pointer-events:none!important;';
  probe.style.setProperty('color',css.color,'important');for(const property of css)if(property.startsWith('--'))probe.style.setProperty(property,css.getPropertyValue(property));
  probe.style.setProperty('background-image',value,'important');d.body.append(probe);try{return d.defaultView.getComputedStyle(probe).backgroundImage;}finally{probe.remove();}
 }
 function visibilityState(info,el,layers){
  const css=el.ownerDocument.defaultView.getComputedStyle(el),stored=css.getPropertyValue(V.paintVisibilityProperty).trim()||'none';
  const own=(el.getAttribute('class')||'').includes('['+V.paintVisibilityProperty+':'+stored+']')||Object.values(info.cssRules||{}).some(values=>values[V.paintVisibilityProperty]===stored)||[...el.ownerDocument.querySelectorAll('style[data-rt-css]')].some(style=>style.dataset.rtCss===el.getAttribute('data-rt-style')&&style.textContent.includes(V.paintVisibilityProperty+':'+stored+' '));
  if(!own||stored==='none')return 'none';
  const normalized=V.serializePaintVisibility(V.parsePaintVisibility(stored).map(entry=>({...entry,paint:computedPaint(el,entry.paint)})));
  P.visibility(layers,{'background-size':css.backgroundSize},normalized);return normalized;
 }
 ...
 function additions(section,info,el,save,saveCSS,layers,upload,saveImage,browseImages){
  ...
  const add=async(src=null,solid=false)=>{
   ...
   const css=el.ownerDocument.defaultView.getComputedStyle(el),framing=Object.fromEntries(P.properties.map(property=>[property,css.getPropertyValue(property)])),layer=src?paint(src):solid?'linear-gradient(0deg, #d9d9d9 0%, #d9d9d9 100%)':'linear-gradient(180deg, #d9d9d9 0%, #ffffff 100%)',stored=visibilityState(info,el,layers),changes=stored==='none'?P.prepend(layers,framing,layer):P.prependVisibility(layers,framing,stored,layer);
   if(saveImage)await saveImage(src,false,'add',{layers:stackReferences(info,el,layers),framing,value:src?null:layer,visibility:stored});
 ...
 function mountStack(info,el,save,saveCSS,layers,upload,saveImage,browseImages){
  ...
  const frameControls=(group,index,image)=>{
   ...
   const tile=I.number(controls,prefix+' tile scale (%)',100,1,1000,value=>write({'background-size':framing('tile',image.naturalWidth,image.naturalHeight,value)['background-size']}));tile.closest('label').querySelector('span').textContent='Scale (%)';tile.closest('label').hidden=mode!=='tile';tile.disabled=true;const loaded=()=>{const value=scale(size,image.naturalWidth,image.naturalHeight);tile.disabled=!image.naturalWidth;sizing.querySelector('[value=tile]').disabled=!image.naturalWidth;tile.value=value===null?'':String(value);tile.placeholder='Custom';};image.addEventListener('load',loaded);if(image.complete&&image.naturalWidth)loaded();
