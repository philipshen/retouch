(function(root){
 ...
 function mountStack(info,el,save,saveCSS,layers,upload,saveImage,browseImages){
  ...
  const gradientDetails=section.querySelector(':scope > details');if(gradientDetails){for(const group of [...gradientDetails.querySelectorAll(':scope > .gradient-controls')]){const index=Number(group.querySelector('legend').textContent.match(/\d+/)[0])-1;paintPopover(section,editors[index],group,paintKey(info,index),'Gradient '+(index+1));}gradientDetails.remove();}
  for(const [index,layer]of layers.entries()){
   ...
   group.append(status);paintPopover(section,editors[index],group,paintKey(info,index),'Image '+(index+1));
  }
  return section;
 }
