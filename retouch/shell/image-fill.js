(function(root){
 ...
 function paintPopover(section,button,body,key,title){
  ...
  const open=()=>{if(!section.isConnected||section.closest('[data-collapsed="true"]'))return;popup.showPopover();openedPaint=key;button.setAttribute('aria-expanded','true');const box=button.getBoundingClientRect(),width=popup.offsetWidth;popup.style.left='clamp(8px, '+(box.left-width-8)+'px, calc(100vw - '+width+'px - 8px))';popup.style.top='clamp(8px, '+box.top+'px, calc(100vh - '+popup.offsetHeight+'px - 8px))';};
