(function(root){
 ...
 function solidPaintRow(editor,color,solid,index){
  ...
  const inline=document.createElement('input');inline.type='text';inline.className='paint-row-value';inline.setAttribute('aria-label','Solid paint '+(index+1)+' color value');inline.title='Hex, RGB, or Display P3. Omitted alpha keeps the current opacity.';inline.spellcheck=false;const initial=/^#[a-f\d]{8}$/i.test(value)?value.slice(1,-2).toUpperCase():value.replace(/ \/ 1\)$/,')');inline.value=initial;inline.disabled=!model||color.matches(':disabled');inline.oninput=()=>inline.setCustomValidity('');inline.onchange=()=>{try{const next=P.editColor(inline.value,model.alpha);inline.setCustomValidity('');if(next!==model.value)write(next);else inline.value=initial;}catch(error){inline.setCustomValidity(error.message);inline.reportValidity();}};I.fieldDraft(inline);editor.before(inline);editor.classList.add('solid-paint-settings');editor.title='Solid paint settings';editor.innerHTML='<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="M7 3v6m0 6v6M17 3v12m0 4v2M4 12h6m4 5h6"/><circle cx="7" cy="12" r="3"/><circle cx="17" cy="17" r="3"/></svg>';
