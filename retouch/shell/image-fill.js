(function(root){
 ...
 function paintPopover(section,button,body,key,title){
  const popup=document.createElement('div'),header=document.createElement('header'),label=document.createElement('strong'),close=I.button('Close '+title,()=>{openedPaint=null;popup.hidePopover();button.focus({preventScroll:true});});popup.className='image-paint-popover';popup.setAttribute('popover','auto');popup.setAttribute('role','dialog');popup.setAttribute('aria-label',title);close.textContent='×';close.setAttribute('aria-label','Close '+title);label.textContent=title;header.append(label,close);popup.append(header,body);section.append(popup);button.setAttribute('aria-haspopup','dialog');button.setAttribute('aria-expanded','false');
  const open=()=>{if(!section.isConnected)return;popup.showPopover();openedPaint=key;button.setAttribute('aria-expanded','true');const box=button.getBoundingClientRect(),width=popup.offsetWidth;popup.style.left='clamp(8px, '+(box.left-width-8)+'px, calc(100vw - '+width+'px - 8px))';popup.style.top='clamp(8px, '+box.top+'px, calc(100vh - '+popup.offsetHeight+'px - 8px))';};
  button.onclick=()=>{if(popup.matches(':popover-open')){openedPaint=null;popup.hidePopover();}else{open();close.focus({preventScroll:true});}};
  popup.addEventListener('keydown',event=>{if(event.key==='Escape'){event.preventDefault();event.stopPropagation();openedPaint=null;popup.hidePopover();button.focus({preventScroll:true});}});
  popup.addEventListener('beforetoggle',event=>button.setAttribute('aria-expanded',String(event.newState==='open')));
 ...
 function additions(section,info,el,save,saveCSS,layers,upload,saveImage,browseImages){
  ...
  const add=async(src=null)=>{
   ...
   const css=el.ownerDocument.defaultView.getComputedStyle(el),framing=Object.fromEntries(P.properties.map(property=>[property,css.getPropertyValue(property)])),layer=src?paint(src):'linear-gradient(180deg, #d9d9d9 0%, #ffffff 100%)',changes=P.prepend(layers,framing,layer);
 ...
 function mountStack(info,el,save,saveCSS,layers,upload,saveImage,browseImages){
  ...
  const currentOrder=()=>section.isConnected&&el.isConnected&&JSON.stringify(V.imageLayers(el.ownerDocument.defaultView.getComputedStyle(el).backgroundImage))===JSON.stringify(layers);
  ...
  const editors=[];layers.forEach((layer,index)=>{const row=document.createElement('div'),label=document.createElement('span'),editor=I.button('Edit '+(source(layer)?'image':'gradient')+' paint '+(index+1),()=>{}),swatch=document.createElement('span');row.className='paint-order-row';label.textContent=source(layer)?'Image':V.parseGradients(layer)?.[0]?.type==='radial'?'Radial':V.parseGradients(layer)?.[0]?.type==='conic'?'Angular':'Linear';editor.setAttribute('aria-label','Edit '+(source(layer)?'image':'gradient')+' paint '+(index+1));editor.classList.add('paint-row-editor');swatch.className='paint-row-swatch';swatch.style.backgroundImage=layer;editor.replaceChildren(swatch,label);editors[index]=editor;row.append(editor);for(const [action,text,disabled,indices]of [['up','↑',index===0,()=>{const next=layers.map((_,i)=>i);[next[index-1],next[index]]=[next[index],next[index-1]];return next;}],['down','↓',index===layers.length-1,()=>{const next=layers.map((_,i)=>i);[next[index],next[index+1]]=[next[index+1],next[index]];return next;}],['duplicate','+',layers.length>=8,()=>layers.flatMap((_,i)=>i===index?[i,i]:[i])],['remove','−',layers.length<=1,()=>layers.map((_,i)=>i).filter(i=>i!==index)]]){const name=(action==='up'||action==='down'?'Move paint '+(index+1)+' '+action:action[0].toUpperCase()+action.slice(1)+' paint '+(index+1)),button=I.button(name,()=>move(indices()));button.textContent=text;button.setAttribute('aria-label',name);button.title=name;button.disabled=disabled;row.append(button);}order.append(row);});
  ...
  const gradientDetails=section.querySelector(':scope > details');if(gradientDetails){for(const group of [...gradientDetails.querySelectorAll(':scope > .gradient-controls')]){const index=Number(group.querySelector('legend').textContent.match(/\d+/)[0])-1;frameControls(group,index,null);paintPopover(section,editors[index],group,paintKey(info,index),'Gradient '+(index+1));}gradientDetails.remove();}
 ...
 function mount(info,el,save,saveCSS,own={},upload=null,saveImage=null,browseImages=null){
  ...
  additions(section,info,el,save,saveCSS,url?[css.backgroundImage]:[],upload,saveImage,browseImages);
