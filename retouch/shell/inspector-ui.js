...
const layoutIcons={flow:'M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z',row:'M3 5v10 M8 5v10 M13 5v10 M16 10h4 M18 8l2 2-2 2',column:'M5 3h10 M5 8h10 M5 13h10 M10 16v4 M8 18l2 2 2-2',grid:'M3 3h14v14H3z M10 3v14 M3 10h14'};
 let activeTypeTab='Basics',activeSharedTypeTab='Basics',typeTabId=0;
 ...
 const rangeHint='Preview this screen range to preserve each layer’s existing values.';
 let rangeCleanups=[];
 const sharedRangeActive=input=>!input.retouchPreviewDocument||root.RetouchBackgroundPaintUI.rangeActive(input,{ownerDocument:input.retouchPreviewDocument});
 const sharedValuesReadable=input=>{try{return input.retouchHasScopedValues?.()===true||sharedRangeActive(input);}catch{return false;}};
 function watchSharedRange(input,sync){const w=input.retouchPreviewDocument?.defaultView;if(!w)return;w.addEventListener('resize',sync);rangeCleanups.push(()=>w.removeEventListener('resize',sync));}
 function compactPaint(section,input){
  if(input.closest('.compact-paint-row'))return;
  const shared=typeof input.retouchSetPaintValues==='function';
  const paintGroups=(allowNone=false)=>{if(!sharedValuesReadable(input))throw Error(rangeHint);const values=shared?input.retouchSelectionColors():[input.value.trim()];return values.map(value=>{const colors=input.dataset.paintProperty==='border-color'?root.RetouchHTMLCSSValues.parseBorderColors(value):[value];if(!colors?.length)throw Error('Choose literal paint colors first.');return colors.map(color=>{const paint=root.RetouchPaintPicker.parsePaint(color);if(!paint&&allowNone&&svg&&color==='none')return {space:'srgb',channels:[0,0,0],alpha:1};if(!paint)throw Error('Choose literal paint colors first.');return paint;});});};
  const sendShared=values=>{Promise.resolve(input.retouchSetPaintValues(values)).catch(error=>{input.setCustomValidity(error.message);input.reportValidity();});};
  const svgMatch=input.getAttribute('aria-label')?.match(/^(SVG|Combined|Shared SVG) (fill|stroke)(?: with alpha)?$/),svg=svgMatch?.[2],stop=input.closest('.gradient-stop-row')&&input.getAttribute('aria-label')?.match(/^((?:Paint|Fill|Gradient) \d+ stop \d+) color$/)?.[1],shadow=input.closest('.shadow-controls')&&input.getAttribute('aria-label')?.match(/^(?:Shared )?(Shadow \d+) color$/)?.[1],paint=stop||shadow||svg||({'color':'Text','background-color':'Fill','border-color':'Stroke','text-decoration-color':'Decoration'})[input.dataset.paintProperty];if(!paint||/^url\(/i.test(input.value.trim()))return;
  const control=input.closest('.paint-field-control'),previous=input.closest('.inspector-field');if(!control||!previous)return;
  const field=document.createElement('div');field.className=previous.className+' compact-paint-row';field.append(...previous.childNodes);previous.replaceWith(field);input.classList.add('compact-paint-value');input.retouchMountVisibility?.(field);
  const type=svg&&!shared?section.querySelector('[aria-label="'+(paint==='fill'?'Fill type':'Stroke type')+'"]'):null;if(type){const row=type.closest('.property-row')||type.closest('.inspector-field');type.classList.add('compact-paint-type');if(!type.title)type.title='Paint type';control.append(type);row?.remove();control.classList.add('has-paint-type');}
  ...
  const opacity=document.createElement('input'),alpha=document.createElement('span'),unit=document.createElement('span');alpha.className='compact-paint-alpha';opacity.type='number';opacity.min=0;opacity.max=100;opacity.step='any';opacity.required=true;opacity.setAttribute('aria-label',(svg?svgMatch[1]+' ':shared?'Shared ':'')+paint+' opacity (%)');unit.textContent='%';unit.setAttribute('aria-hidden','true');alpha.append(opacity,unit);control.append(alpha);
  const colors=()=>{try{return paintGroups().flat();}catch{return null;}},sync=()=>{const values=colors()||(document.activeElement===input?editColors:null),value=values?.[0],mixed=values?.some(item=>item.space!==value.space||item.alpha!==value.alpha||item.channels.some((channel,i)=>channel!==value.channels[i])),mixedAlpha=values?.some(item=>item.alpha!==value.alpha);control.classList.toggle('has-compact-label',!!value);display.textContent=value?(mixed?'Mixed':value.space==='display-p3'?'Display P3':value.channels.every(n=>Math.abs(n*255-Math.round(n*255))<1.3e-4)?value.channels.map(n=>Math.round(n*255).toString(16).padStart(2,'0')).join('').toUpperCase():'RGB'):'';opacity.disabled=input.disabled||!value||!sharedValuesReadable(input);opacity.title=!sharedValuesReadable(input)?rangeHint:input.disabled?input.title:value?'Paint opacity':'Choose a literal color to edit its opacity.';opacity.placeholder=mixedAlpha?'Mixed':'';if(document.activeElement!==opacity)opacity.value=value&&!mixedAlpha?String(Number((value.alpha*100).toFixed(8))):'';};
  opacity.oninput=()=>opacity.setCustomValidity('');opacity.onchange=()=>{if(!opacity.checkValidity()||input.disabled||!sharedValuesReadable(input))return;const current=colors(),value=Number(opacity.value)/100;if(!current||current.every(item=>Math.abs(item.alpha-value)<1e-10))return;root.RetouchPanelFocus?.queue(opacity);if(shared){sendShared(paintGroups().map(group=>group.map(item=>item.space==='display-p3'?root.RetouchPaletteValues.p3(item.channels,value):root.RetouchPaletteValues.srgb(item.channels,value)).join(' ')));return;}input.value=current.map(item=>item.space==='display-p3'?root.RetouchPaletteValues.p3(item.channels,value):root.RetouchPaletteValues.srgb(item.channels,value)).join(' ');input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}));};
  // CSSOM rounds color(srgb) channels to six decimals. Keep the original CSS when an unchanged displayed hex value leaves focus.
  let editColors=colors(),editValue=input.value,formattedHex=null;input.addEventListener('focus',()=>{const current=colors();if(!current)return;editColors=current;editValue=input.value;formattedHex=null;const value=editColors[0];if(value?.space==='srgb'&&value.channels.every(n=>Math.abs(n*255-Math.round(n*255))<1.3e-4)&&editColors.every(item=>item.space===value.space&&item.alpha===value.alpha&&item.channels.every((channel,i)=>channel===value.channels[i]))){input.value=value.channels.map(n=>Math.round(n*255).toString(16).padStart(2,'0')).join('').toUpperCase();formattedHex=input.value;input.select();}});
  ...
  const prepareHex=event=>{const hex=input.value.trim();if(input.disabled||!/^(?:[a-f\d]{3}|[a-f\d]{4}|[a-f\d]{6}|[a-f\d]{8})$/i.test(hex))return;if(formattedHex&&hex===formattedHex){input.value=editValue;return;}if(!sharedValuesReadable(input)&&(hex.length===3||hex.length===6)){if(event?.type==='change'){event.stopImmediatePropagation();input.setCustomValidity(rangeHint);input.reportValidity();}return;}const parsed=root.RetouchPaletteValues.parse('#'+hex);if(shared&&event?.type==='change'){event.stopImmediatePropagation();try{const values=paintGroups(true).map(group=>group.map(item=>root.RetouchPaletteValues.srgb(parsed.channels,hex.length===4||hex.length===8?parsed.alpha:item.alpha)).join(' '));input.value=values.every(value=>value===values[0])?values[0]:'';sendShared(values);}catch(error){input.setCustomValidity(error.message);input.reportValidity();}return;}const values=hex.length===4||hex.length===8?[parsed]:editColors?.length?editColors.map(item=>({...parsed,alpha:item.alpha})):[parsed];input.value=values.map(item=>root.RetouchPaletteValues.srgb(item.channels,item.alpha)).join(' ');};input.addEventListener('change',prepareHex,true);input.addEventListener('blur',()=>{prepareHex();sync();});
  input.addEventListener('input',sync);input.addEventListener('change',sync);new MutationObserver(sync).observe(input,{attributes:true,attributeFilter:['disabled','title']});sync();root.RetouchNumericExpression.calculation(opacity,{unit:'%'});root.RetouchInspector.fieldDraft(opacity);watchSharedRange(input,sync);
 }
 ...
 function strokePatternControls(settings,key){
  ...
  for(const [input,key]of [[dash,'dash'],[gap,'gap']]){input.type='text';input.oninput=()=>input.setCustomValidity('');input.onchange=()=>{const next=P.dashPair(raw.value,{[key]:input.value.trim()});if(next===null||raw.dataset.sourceUnits==='true'&&next.includes('%')){input.setCustomValidity(raw.dataset.sourceUnits==='true'?'Enter a nonnegative dash length in SVG source units.':'Enter a nonnegative dash length in pixels or percent.');input.reportValidity();return;}write(next);};}
 ...
 function sharedStrokePattern(settings){
  ...
  const patterns=()=>raw.retouchDashValues().map(value=>P.pattern(value)),sync=()=>{const parsed=patterns(),types=parsed.map(value=>value?.type||'custom');style.value=types.every(value=>value===types[0])?types[0]:'mixed';for(const [input,index]of [[dash,0],[gap,1]]){const values=parsed.map(value=>value?.parts[index]||value?.parts[0]||'4');if(document.activeElement!==input)input.value=values.every(value=>value===values[0])?values[0]:'';input.placeholder=values.every(value=>value===values[0])?'':'Mixed';input.disabled=raw.disabled||!sharedValuesReadable(raw)||!parsed.every(value=>value?.type==='dashed');input.title=sharedValuesReadable(raw)?raw.title:rangeHint;input.closest('.inspector-field').hidden=style.value==='solid'||style.value==='custom';}style.disabled=raw.disabled;style.querySelector('[value=dashed]').disabled=!sharedValuesReadable(raw);row.hidden=style.value!=='custom';};
  ...
  style.onchange=()=>{if(style.value==='dashed'&&!sharedValuesReadable(raw))return;if(style.value==='custom'){row.hidden=false;dash.closest('.inspector-field').hidden=true;gap.closest('.inspector-field').hidden=true;raw.focus();return;}write(raw.retouchDashValues().map(value=>style.value==='solid'?'none':P.dashPair(value)));};
  for(const [input,key]of [[dash,'dash'],[gap,'gap']]){input.type='text';input.oninput=()=>input.setCustomValidity('');input.onchange=()=>{if(input.disabled||!sharedValuesReadable(raw))return;const values=raw.retouchDashValues().map(value=>P.dashPair(value,{[key]:input.value.trim()}));if(values.some(value=>value===null)){input.setCustomValidity('Enter a nonnegative dash length in pixels or percent.');input.reportValidity();return;}root.RetouchPanelFocus?.queue(input);write(values);};}
  row.before(group);group.append(row);sync();for(const input of [dash,gap])I.fieldDraft(input);watchSharedRange(raw,sync);
 }
 ...
 function typeSettingsTabs(settings,section,shared=false){
  const body=settings.querySelector('.stroke-settings-body');if(!shared)compactTypeSettings(body,section);
  ...
  const preview=section.querySelector(shared?'.shared-type-preview':'.type-preview'),groups=new Map();
  ...
  for(const child of children){const summary=child.querySelector(':scope > summary')?.textContent;groups.get(summary==='Variable font axes'||shared&&!!child.querySelector('[aria-label="Shared Optical sizing"]')?'Variable':child.dataset.typeCategory==='Details'||shared&&!!child.querySelector('[aria-label="Shared Capital forms"],[aria-label="Shared Number position"]')||['Number formatting','Ligatures','Capital forms','Number position'].includes(summary)?'Details':'Basics').panel.append(child);}
  const select=(name,focus=false)=>{if(shared)activeSharedTypeTab=name;else activeTypeTab=name;for(const [key,{tab,panel}]of groups){const chosen=key===name;tab.setAttribute('aria-selected',String(chosen));tab.tabIndex=chosen?0:-1;panel.hidden=!chosen;}body.scrollTop=0;preview?.retouchSample?.(defaultSample());if(focus)groups.get(name).tab.focus();};
 ...
 function sharedTypographyPrimary(section){
  ...
  strokePopover(settings,tools,'shared-type-settings','Type settings','Shared type settings');typeSettingsTabs(settings,section,true);
 }
 ...
 function organize(panel){
  if(panel.dataset.organized==='true')return;rangeCleanups.forEach(cleanup=>cleanup());rangeCleanups=[];panel.dataset.organized='true';
  ...
  const order=['Component','Shared component properties','Group','Position','Selection transform','Combine shapes','Boolean group','Boolean operations','Mask','Vector position','Vector size','Geometry',...([...panel.children].some(el=>title(el)==='Vector size')&&!panel.querySelector('.shared-inspector-group')?[]:['Layout']),'Scale','Appearance',...((head.dataset.textLayer==='true'||RetouchInspector.isTextLayer(head.dataset.layerTag||''))?['Typography']:[]),'Link','Fill','Image fill','Fill gradient','Stroke','Stroke gradient','Original shapes','Effects','Image framing','Image','Export'];
  ...
  for(const section of panel.querySelectorAll('.inspector-section')){
   ...
   for(const input of paintInputs){
    ...
    if(name==='Effects'||input.dataset.paintProperty||input.closest('.gradient-stop-row')){const control=document.createElement('span');control.className='paint-field-control';input.replaceWith(control);control.append(input);}
    ...
    input.addEventListener('input',paint);input.addEventListener('change',paint);input.addEventListener('keydown',event=>{if(event.key==='Escape')queueMicrotask(paint);});paint();if(['Fill','Stroke','Image fill','Effects'].includes(name)||input.dataset.paintProperty==='color')compactPaint(section,input);
   }
   ...
   if(name==='Stroke'&&section.querySelector('[aria-label="SVG stroke width"], [aria-label="Stroke weight"]')){
    ...
    strokePatternControls(settings,head.dataset.strokeContext);strokeIconControls(settings);pair(settings,['SVG dash offset','SVG miter limit']);section.append(settings);const weightInput=section.querySelector('[aria-label="SVG stroke width"], [aria-label="Stroke weight"]');let weight=weightInput?.closest('.property-row');if(!weight&&weightInput){weight=document.createElement('div');weight.className='property-row';const field=weightInput.closest('.inspector-field');field.before(weight);weight.append(field);}if(weight)strokePopover(settings,weight);
   }
   ...
   if(name==='Layout'){
    ...
    if(css){
     ...
     if(options.children.length>1)section.append(options);
     const gap=section.querySelector('[aria-label="Gap (CSS)"]')?.closest('.property-row');if(alignment?.parentElement===section&&gap?.parentElement===section){const row=document.createElement('div'),pair=document.createElement('div');row.className='layout-alignment-spacing';pair.className='property-pair';alignment.before(row);pair.append(gap);row.append(alignment,pair);}
    ...
    if(select){const row=select.closest('.inspector-field');row.classList.add('layout-modes');row.querySelector('span').textContent='';
     ...
     const group=document.createElement('div');group.className='layout-mode-segments';const adaptive=section.querySelector(':scope > button[aria-label="Adaptive grid"]');
     for(const [value,label,icon] of [['flow','Normal flow','flow'],[select.dataset.inlineAxis==='vertical'?'row':'column','Vertical stack','column'],[select.dataset.inlineAxis==='vertical'?'column':'row','Horizontal stack','row'],['grid','Grid layout','grid']]){if(value==='grid'&&adaptive){adaptive.innerHTML='<svg viewBox="0 0 20 20" aria-hidden="true"><path d="'+layoutIcons.grid+'"/></svg>';group.append(adaptive);continue;}const button=document.createElement('button');button.type='button';button.title=label;button.setAttribute('aria-label',label);const axis=icon==='flow'?'flow':icon==='column'?'vertical':icon==='row'?'horizontal':null,preset=axis&&select.retouchPreset;button.disabled=!!preset&&preset.blocked(axis);if(button.disabled)button.title='Preview this edit range and remove inline layout overrides to arrange children.';button.setAttribute('aria-pressed',String(preset?preset.pressed(axis):select.value.replace('-reverse','')===value));button.innerHTML='<svg viewBox="0 0 20 20" aria-hidden="true"><path d="'+layoutIcons[icon]+'"/></svg>';button.onclick=()=>{if(preset)preset.apply(axis);else{select.value=value;select.dispatchEvent(new Event('change',{bubbles:true}));}};group.append(button);}row.insertBefore(group,select);keyboardToolbar(group,'Layout mode buttons');
     ...
     row.before(group);row.classList.remove('layout-modes');row.querySelector(':scope > span').textContent='Layout';const advanced=disclosure('Layout options','react-layout-options');advanced.setAttribute('aria-label','Layout options');advanced.append(row);for(const label of ['Wrap children','Align children','Distribute children']){const field=section.querySelector('select[aria-label="'+label+'"]')?.closest('.inspector-field'),container=field?.closest('.property-row')||field;if(container?.parentElement===section)advanced.append(container);}section.append(advanced);
    }
   }
  ...
  for(const shared of [...panel.children].filter(el=>el.querySelector(':scope > .shared-inspector-group'))){
   ...
   for(const group of groups){
    ...
    if(svg&&['fill','stroke'].includes(key)){
     ...
     if(field){const row=field.closest('.property-row')||field,body=group.querySelector('.shared-inspector-group-body'),extra=disclosure(key==='fill'?'CSS background':'CSS border','shared-svg-'+key);extra.append(...body.childNodes);body.append(row);for(const input of extra.querySelectorAll('[data-svg-stroke]')){const field=input.closest('.inspector-field'),strokeRow=field.closest('.property-row')||field,reset=strokeRow.nextElementSibling;body.append(strokeRow);field.querySelector(':scope > span').textContent=({'stroke-width':'Width','stroke-linecap':'Caps','stroke-linejoin':'Joins','stroke-dasharray':'Dashes','stroke-dashoffset':'Dash offset','stroke-miterlimit':'Miter limit','vector-effect':'Scaling'})[input.dataset.svgStroke];if(input.dataset.svgStroke==='vector-effect')for(const option of input.options)option.textContent=({'none':'Scale stroke','non-scaling-stroke':'Fixed stroke'})[option.value]||option.textContent;if(reset?.matches('button')&&/^Reset shared /i.test(reset.textContent))body.append(reset);}const reset=[...extra.querySelectorAll('button')].find(button=>button.textContent==='Clear selected SVG '+key);if(reset)body.append(reset);if(extra.querySelector('input,select,button'))css.append(extra);}
    }
    group.classList.add('sec');shared.before(group);for(const input of group.querySelectorAll('input[data-paint-property]'))if(['color','background-color','border-color','fill','stroke','shadow-color','text-decoration-color'].includes(input.dataset.paintProperty))compactPaint(group,input);
   }
   ...
   if(!svg){
    ...
    if(size&&layout){const dimensions=size.querySelector('.shared-inspector-group-body'),body=layout.querySelector('.shared-inspector-group-body');const primary=[...dimensions.children].find(row=>row.querySelector('[aria-label="Shared Width (px)"], [aria-label="Shared Width"]'));if(primary){body.prepend(primary);const resizing=body.querySelector('.text-resize-controls');if(resizing)primary.after(resizing);}body.append(...dimensions.childNodes);size.remove();}
   }
  ...
  if(fillSection&&imageSection&&!fillSection.querySelector('[aria-label="SVG fill"], [aria-label="Combined fill"], [data-gradient-paint]')){
   ...
   const hiddenGradient=!!imageSection.retouchHasHiddenPaints,stackControls=!!imageSection.querySelector('.paint-order');
   if(stackControls||hiddenGradient)for(const group of advancedFills.querySelectorAll('.gradient-controls'))group.hidden=true;
   if(hiddenGradient)root.RetouchInspector.note(advancedFills,'Edit hidden paints from the Fill rows above.');
   for(const button of advancedFills.querySelectorAll('button')){
    const name=button.getAttribute('aria-label')||button.textContent;
    if((stackControls||hiddenGradient)&&['Add gradient','Add gradient fill'].includes(name))button.hidden=true;
  ...
  const sharedInput=[...panel.querySelectorAll('input')].find(input=>input.retouchPreviewDocument&&typeof input.retouchSetPaintValues==='function'),screenScope=panel.querySelector('.screen-scope'),rangeSource=sharedInput||(screenScope?.retouchPreviewDocument?screenScope:null);
  if(rangeSource&&screenScope){
   const notice=document.createElement('div'),message=document.createElement('span'),preview=document.createElement('button');notice.className='shared-range-notice';notice.setAttribute('role','status');notice.setAttribute('aria-label',sharedInput?'Shared edit range':'Edit range');message.textContent='Preview the edit range to see style changes.';preview.type='button';preview.className='control-button';preview.textContent='Preview edit range';preview.setAttribute('aria-label','Preview edit range');
 ...
 const creationTools=[];
 for(const [action,label,path]of [['insertFrame','Insert frame into selection','M5 2v20 M19 2v20 M2 5h20 M2 19h20'],['insertText','Text tool','M4 5V3h16v2 M12 3v18 M8 21h8']]){
  const button=document.createElement('button');button.type='button';button.className='creation-tool';button.setAttribute('aria-label',label);button.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="'+path+'"/></svg>';button.onclick=()=>{if(!button.disabled)root.RetouchActions?.run(action==='insertText'?'shape-text':'layer-'+action);};if(action==='insertText'){button.setAttribute('aria-keyshortcuts','T');button.setAttribute('aria-pressed','false');}dock.insertBefore(button,dock.querySelector('#quickActions'));creationTools.push({action,button,label});
 }
 ...
 const scaleTool=toolButton('Scale tool','M4 14v6h6 M4 20L20 4 M14 4h6v6');scaleTool.id='canvasScale';scaleTool.setAttribute('aria-keyshortcuts','K');scaleTool.setAttribute('aria-pressed','false');dock.insertBefore(scaleTool,dock.querySelector('#canvasHand'));
 const scaleActive=()=>!!root.RetouchShapeTools?.scaleArmed?.()||!!document.querySelector('.canvas-move-surface[data-canvas-mode=scale]');
 ...
 const refreshShapes=()=>{const scaling=scaleActive();scaleTool.setAttribute('aria-pressed',String(scaling));scaleTool.disabled=!scaling&&!shapeEnabled(shapeSource('scale'));scaleTool.title=scaleTool.disabled?'Select editable layers in Edit mode and finish the current gesture.':root.RetouchShapeTools?.scaleArmed?.()?'Scale tool · Select a layer to scale · Escape cancels':scaling?'Scale tool · K · Click to return to Move':'Scale tool · K';const source=preferredShape('rectangle');shapeTool.disabled=!shapeEnabled(source);shapeTool.title=shapeTool.disabled?'Select an editable container, SVG canvas or group in Edit mode.':root.RetouchShapeTools?.armed?.()?.startsWith('draw-')?'Select a container to draw · Escape cancels':'Shape tools · R rectangle, O ellipse, L line · V returns to Move';const pen=shapeSource('pen');penTool.disabled=!shapeEnabled(pen);penTool.title=penTool.disabled?'Select an editable container, SVG canvas or group in Edit mode.':root.RetouchShapeTools?.armed?.()==='pen'?'Select a container to draw a path · Escape cancels':'Pen tool · P · V returns to Move';if(shapeMenu&&(shapeTool.disabled||shapeMenuSource!==shapeIdentity()))closeShapes();};
 shapeTool.onclick=()=>{
 ...
 scaleTool.onclick=()=>{closeShapes();if(scaleActive())root.RetouchActions?.run('shape-move');else{const source=shapeSource('scale');if(shapeEnabled(source))source.run(scaleTool);}};
 ...
 const updateCreationTools=()=>{const editing=document.getElementById('modeBtn').classList.contains('mode-edit');for(const {action,button,label}of creationTools){if(action==='insertText'){const source=root.RetouchShapeTools?.get('text');button.disabled=!source?.available();button.setAttribute('aria-pressed',String(root.RetouchShapeTools?.armed()==='text'||!!document.querySelector('.svg-draw-surface[data-tool=text]')));button.title=button.disabled?'Finish the current edit and switch to Edit mode.':'Text · Click to place text or drag a text box (T)';continue;}const source=document.querySelector('#layersPanel button[data-design-action="'+action+'"]');button.disabled=!editing||!source||source.hidden||source.matches(':disabled')||!!source.closest('[inert]');button.title=!editing?'Switch to Edit mode to add a layer.':button.disabled?source?.title||'Select an editable container first.':label;}};
 const main=document.getElementById('main'),canvas=document.getElementById('frameWrap');main.append(dock);
 new MutationObserver(refreshShapes).observe(document.getElementById('canvasHand'),{attributes:true,attributeFilter:['aria-pressed']});window.addEventListener('retouch:shape-tools',refreshShapes);window.addEventListener('retouch:selection',refreshShapes);new MutationObserver(refreshShapes).observe(document.getElementById('panelBody'),{subtree:true,childList:true,attributes:true,attributeFilter:['disabled','hidden','inert']});new MutationObserver(refreshShapes).observe(document.getElementById('modeBtn'),{attributes:true,attributeFilter:['class']});refreshShapes();
 const updateDrawingTool=()=>{const surface=document.querySelector('.svg-draw-surface'),armed=root.RetouchShapeTools?.armed?.(),preset=surface?.dataset.shape||(armed?.startsWith('draw-')?armed.slice(5):null),text=surface?.dataset.tool==='text',shape=!!surface&&!text||!!preset&&!text,pen=armed==='pen'||!!document.querySelector('.svg-pen-surface');if(shape&&!text&&shapeIcons[preset])shapeTool.querySelector('path').setAttribute('d',shapeIcons[preset]);shapeTool.setAttribute('aria-pressed',String(shape));penTool.setAttribute('aria-pressed',String(pen));const scaling=scaleActive();scaleTool.setAttribute('aria-pressed',String(scaling));dock.classList.toggle('drawing-active',shape||pen||scaling||armed==='text'||text);updateCreationTools();refreshShapes();};new MutationObserver(updateDrawingTool).observe(document.body,{childList:true});window.addEventListener('retouch:shape-tools',updateDrawingTool);updateDrawingTool();
 window.addEventListener('retouch:shape-tools',updateCreationTools);new MutationObserver(updateCreationTools).observe(document.getElementById('layersPanel'),{subtree:true,childList:true,attributes:true,attributeFilter:['disabled','hidden','inert','title']});new MutationObserver(updateCreationTools).observe(document.getElementById('modeBtn'),{attributes:true,attributeFilter:['class']});updateCreationTools();
 ...
 const place=()=>{
  ...
  const toasts=document.getElementById('toasts');if(toasts)toasts.style.bottom=(innerHeight-dock.getBoundingClientRect().top+8)+'px';
