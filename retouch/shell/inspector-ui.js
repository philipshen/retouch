...
const layoutIcons={flow:'M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z',row:'M3 5v10 M8 5v10 M13 5v10 M16 10h4 M18 8l2 2-2 2',column:'M5 3h10 M5 8h10 M5 13h10 M10 16v4 M8 18l2 2 2-2',grid:'M3 3h14v14H3z M10 3v14 M3 10h14'};
 ...
 function organize(panel){
  ...
  if([...panel.children].some(el=>title(el)==='Vector position')){
  ...
  const order=['Component','Shared component properties','Position','Vector position','Vector size','Geometry',...([...panel.children].some(el=>title(el)==='Vector size')?[]:['Layout']),'Appearance',...(RetouchInspector.isTextLayer(head.dataset.layerTag||'')?['Typography']:[]),'Fill','Stroke','Effects','Image framing','Image','Export'];
  ...
  for(const section of panel.querySelectorAll('.inspector-section')){
   ...
   if(name==='Vector position'){section.querySelector(':scope > h3').textContent='Position';pair(section,['Vector X','Vector Y']);for(const input of section.querySelectorAll('input'))input.closest('.inspector-field').querySelector(':scope > span').textContent=input.getAttribute('aria-label').replace('Vector ','').replace('rotation','Rotation');}
 ...
 shapeTool.setAttribute('aria-keyshortcuts','R O L');penTool.setAttribute('aria-keyshortcuts','P');shapeTool.setAttribute('aria-haspopup','menu');shapeTool.setAttribute('aria-expanded','false');
 ...
 const refreshShapes=()=>{const source=preferredShape('rectangle');shapeTool.disabled=!shapeEnabled(source);shapeTool.title=shapeTool.disabled?'Select an editable container, SVG canvas or group in Edit mode.':'Shape tools · R rectangle, O ellipse, L line';const pen=shapeSource('pen');penTool.disabled=!shapeEnabled(pen);penTool.title=penTool.disabled?'Select an editable container, SVG canvas or group in Edit mode.':'Pen tool · P';if(shapeMenu&&(shapeTool.disabled||shapeMenuSource!==shapeIdentity()))closeShapes();};
 shapeTool.onclick=()=>{
