...
const layoutIcons={flow:'M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z',row:'M3 5v10 M8 5v10 M13 5v10 M16 10h4 M18 8l2 2-2 2',column:'M5 3h10 M5 8h10 M5 13h10 M10 16v4 M8 18l2 2 2-2',grid:'M3 3h14v14H3z M10 3v14 M3 10h14'};
 ...
 function compactPaint(section,input){
  ...
  const paintGroups=()=>{if(!sharedValuesReadable(input))throw Error(rangeHint);const values=shared?input.retouchSelectionColors():[input.value.trim()];return values.map(value=>{const colors=input.dataset.paintProperty==='border-color'?root.RetouchHTMLCSSValues.parseBorderColors(value):[value];if(!colors?.length)throw Error('Choose literal paint colors first.');return colors.map(color=>{const paint=root.RetouchPaintPicker.parsePaint(color);if(!paint)throw Error('Choose literal paint colors first.');return paint;});});};
