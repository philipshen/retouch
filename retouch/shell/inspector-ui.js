 ...
 function organize(panel){
  ...
  for(const section of panel.querySelectorAll('.inspector-section')){
   ...
   if(name==='Stroke'&&section.querySelector('[aria-label="SVG stroke width"]')){
    ...
    pair(settings,['SVG line ends','SVG line joins']);pair(settings,['SVG dash pattern','SVG dash offset']);pair(settings,['SVG miter limit','SVG stroke scaling']);section.append(settings);const weight=section.querySelector('[aria-label="SVG stroke width"]')?.closest('.property-row');if(weight)strokePopover(settings,weight);
   }
