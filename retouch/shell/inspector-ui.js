...
const layoutIcons={flow:'M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z',row:'M3 5v10 M8 5v10 M13 5v10 M16 10h4 M18 8l2 2-2 2',column:'M5 3h10 M5 8h10 M5 13h10 M10 16v4 M8 18l2 2 2-2',grid:'M3 3h14v14H3z M10 3v14 M3 10h14'};
 ...
 const updateCreationTools=()=>{const editing=document.getElementById('modeBtn').classList.contains('mode-edit');for(const {action,button,label}of creationTools){if(action==='insertText'){const source=root.RetouchShapeTools?.get('text');button.disabled=!source?.available();button.setAttribute('aria-pressed',String(root.RetouchShapeTools?.armed()==='text'));button.title=button.disabled?'Finish the current edit and switch to Edit mode.':'Text · Click the page to place text (T)';continue;}const source=document.querySelector('#layersPanel button[data-design-action="'+action+'"]');button.disabled=!editing||!source||source.hidden||source.matches(':disabled')||!!source.closest('[inert]');button.title=!editing?'Switch to Edit mode to add a layer.':button.disabled?source?.title||'Select an editable container first.':label;}};
 ...
 const updateDrawingTool=()=>{const surface=document.querySelector('.svg-draw-surface'),armed=root.RetouchShapeTools?.armed?.(),preset=surface?.dataset.shape||(armed?.startsWith('draw-')?armed.slice(5):null),shape=!!surface||!!preset,pen=armed==='pen'||!!document.querySelector('.svg-pen-surface');if(shapeIcons[preset])shapeTool.querySelector('path').setAttribute('d',shapeIcons[preset]);shapeTool.setAttribute('aria-pressed',String(shape));penTool.setAttribute('aria-pressed',String(pen));const scaling=scaleActive();scaleTool.setAttribute('aria-pressed',String(scaling));dock.classList.toggle('drawing-active',shape||pen||scaling||armed==='text');refreshShapes();};new MutationObserver(updateDrawingTool).observe(document.body,{childList:true});window.addEventListener('retouch:shape-tools',updateDrawingTool);updateDrawingTool();
