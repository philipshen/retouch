...
const layoutIcons={flow:'M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z',row:'M3 5v10 M8 5v10 M13 5v10 M16 10h4 M18 8l2 2-2 2',column:'M5 3h10 M5 8h10 M5 13h10 M10 16v4 M8 18l2 2 2-2',grid:'M3 3h14v14H3z M10 3v14 M3 10h14'};
 ...
 const refreshShapes=()=>{const scaling=scaleActive();scaleTool.setAttribute('aria-pressed',String(scaling));scaleTool.disabled=!scaling&&!shapeEnabled(shapeSource('scale'));scaleTool.title=scaleTool.disabled?'Select editable layers in Edit mode and finish the current gesture.':root.RetouchShapeTools?.scaleArmed?.()?'Scale tool · Select a layer to scale · Escape cancels':scaling?'Scale tool · K · Click to return to Move':'Scale tool · K';const source=preferredShape('rectangle');shapeTool.disabled=!shapeEnabled(source);shapeTool.title=shapeTool.disabled?'Select an editable container, SVG canvas or group in Edit mode.':'Shape tools · R rectangle, O ellipse, L line · V returns to Move';const pen=shapeSource('pen');penTool.disabled=!shapeEnabled(pen);penTool.title=penTool.disabled?'Select an editable container, SVG canvas or group in Edit mode.':'Pen tool · P · V returns to Move';if(shapeMenu&&(shapeTool.disabled||shapeMenuSource!==shapeIdentity()))closeShapes();};
 shapeTool.onclick=()=>{
 ...
 const updateDrawingTool=()=>{const surface=document.querySelector('.svg-draw-surface'),shape=!!surface,pen=!!document.querySelector('.svg-pen-surface');if(shapeIcons[surface?.dataset.shape])shapeTool.querySelector('path').setAttribute('d',shapeIcons[surface.dataset.shape]);shapeTool.setAttribute('aria-pressed',String(shape));penTool.setAttribute('aria-pressed',String(pen));const scaling=scaleActive();scaleTool.setAttribute('aria-pressed',String(scaling));dock.classList.toggle('drawing-active',shape||pen||scaling);refreshShapes();};new MutationObserver(updateDrawingTool).observe(document.body,{childList:true});updateDrawingTool();
