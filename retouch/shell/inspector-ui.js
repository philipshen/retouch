...
const layoutIcons={flow:'M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z',row:'M3 5v10 M8 5v10 M13 5v10 M16 10h4 M18 8l2 2-2 2',column:'M5 3h10 M5 8h10 M5 13h10 M10 16v4 M8 18l2 2 2-2',grid:'M3 3h14v14H3z M10 3v14 M3 10h14'};
 ...
 function compactPaint(section,input){
  ...
  const prepareHex=()=>{const hex=input.value.trim();if(input.disabled||!/^(?:[a-f\d]{3}|[a-f\d]{4}|[a-f\d]{6}|[a-f\d]{8})$/i.test(hex))return;if(formattedHex&&hex===formattedHex){input.value=editValue;return;}const parsed=root.RetouchPaletteValues.parse('#'+hex),values=hex.length===4||hex.length===8?[parsed]:editColors?.length?editColors.map(item=>({...parsed,alpha:item.alpha})):[parsed];input.value=values.map(item=>root.RetouchPaletteValues.srgb(item.channels,item.alpha)).join(' ');};input.addEventListener('change',prepareHex,true);input.addEventListener('blur',()=>{prepareHex();sync();});
