...
const layoutIcons={flow:'M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z',row:'M3 5v10 M8 5v10 M13 5v10 M16 10h4 M18 8l2 2-2 2',column:'M5 3h10 M5 8h10 M5 13h10 M10 16v4 M8 18l2 2 2-2',grid:'M3 3h14v14H3z M10 3v14 M3 10h14'};
 ...
 function compactPaint(section,input){
  ...
  const svgMatch=input.getAttribute('aria-label')?.match(/^(SVG|Combined|Shared SVG) (fill|stroke)(?: with alpha)?$/),svg=svgMatch?.[2],stop=input.closest('.gradient-stop-row')&&input.getAttribute('aria-label')?.match(/^((?:Paint|Fill|Gradient) \d+ stop \d+) color$/)?.[1],shadow=input.closest('.shadow-controls')&&input.getAttribute('aria-label')?.match(/^(Shadow \d+) color$/)?.[1],paint=stop||shadow||svg||({'color':'Text','background-color':'Fill','border-color':'Stroke'})[input.dataset.paintProperty];if(!paint||/^url\(/i.test(input.value.trim()))return;
 ...
 function organize(panel){
  ...
  const order=['Component','Shared component properties','Position','Selection transform','Combine shapes','Boolean group','Boolean operations','Mask','Vector position','Vector size','Geometry',...([...panel.children].some(el=>title(el)==='Vector size')&&!panel.querySelector('.shared-inspector-group')?[]:['Layout']),'Appearance',...((head.dataset.textLayer==='true'||RetouchInspector.isTextLayer(head.dataset.layerTag||''))?['Typography']:[]),'Fill','Image fill','Fill gradient','Stroke','Stroke gradient','Original shapes','Effects','Image framing','Image','Export'];
  ...
  for(const shared of [...panel.children].filter(el=>el.querySelector(':scope > .shared-inspector-group'))){
   ...
   for(const group of groups){
    ...
    group.classList.add('sec');shared.before(group);for(const input of group.querySelectorAll('input[data-paint-property]'))if(['color','background-color','border-color','fill','stroke'].includes(input.dataset.paintProperty))compactPaint(group,input);
   }
