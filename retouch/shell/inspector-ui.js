...
const layoutIcons={flow:'M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z',row:'M3 5v10 M8 5v10 M13 5v10 M16 10h4 M18 8l2 2-2 2',column:'M5 3h10 M5 8h10 M5 13h10 M10 16v4 M8 18l2 2 2-2',grid:'M3 3h14v14H3z M10 3v14 M3 10h14'};
 ...
 function strokePopover(settings,weight){
  ...
  const position=()=>{if(!settings.isConnected){cleanup();return;}const box=summary.getBoundingClientRect(),panel=summary.closest('#panel')?.getBoundingClientRect();body.style.left=Math.max(8,Math.min(innerWidth-body.offsetWidth-8,(panel?.left??box.left)-body.offsetWidth-8))+'px';body.style.top=Math.max(8,Math.min(innerHeight-body.offsetHeight-8,box.top))+'px';body.style.visibility='visible';};
 ...
 function organize(panel){
  ...
  for(const section of panel.querySelectorAll('.inspector-section')){
   ...
   if(name==='Stroke'&&section.querySelector('[aria-label="SVG stroke width"]')){
    ...
    strokePatternControls(settings,head.dataset.strokeContext);pair(settings,['SVG line ends','SVG line joins']);pair(settings,['SVG dash offset','SVG miter limit']);section.append(settings);const weight=section.querySelector('[aria-label="SVG stroke width"]')?.closest('.property-row');if(weight)strokePopover(settings,weight);
   }
