 ...
 function organize(panel){
  ...
  for(const section of panel.querySelectorAll('.inspector-section')){
   ...
   for(const input of paintInputs){
    ...
    input.addEventListener('input',paint);input.addEventListener('change',paint);input.addEventListener('keydown',event=>{if(event.key==='Escape')queueMicrotask(paint);});paint();if(['Fill','Stroke','Image fill'].includes(name)||input.dataset.paintProperty==='color')compactPaint(section,input);
   }
