...
const layoutIcons={flow:'M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z',row:'M3 5v10 M8 5v10 M13 5v10 M16 10h4 M18 8l2 2-2 2',column:'M5 3h10 M5 8h10 M5 13h10 M10 16v4 M8 18l2 2 2-2',grid:'M3 3h14v14H3z M10 3v14 M3 10h14'};
 ...
 const shapeIcons={rectangle:'M4 4h16v16H4z',circle:'M12 3a9 9 0 1 0 0 18a9 9 0 1 0 0-18',ellipse:'M3 12a9 6 0 1 0 18 0a9 6 0 1 0-18 0',line:'M4 20L20 4',pen:'M5 19l2-10 6-6 8 8-6 6-10 2 M5 19l7-7 M10 10h4v4h-4z'};
 ...
 shapeTool.onclick=()=>{
  ...
  for(const preset of ['rectangle','ellipse','circle','line']){const source=preferredShape(preset),button=document.createElement('button');button.type='button';button.setAttribute('role','menuitem');button.tabIndex=-1;button.disabled=!shapeEnabled(source);button.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="'+shapeIcons[preset]+'"/></svg>';const label=document.createElement('span');label.textContent=source?.textContent||'Add '+preset;button.append(label);button.onclick=()=>{const latest=preferredShape(preset);if(!shapeEnabled(latest))return;closeShapes();shapeTool.querySelector('path').setAttribute('d',shapeIcons[preset]);latest.click();};menu.append(button);}
