...
const layoutIcons={flow:'M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z',row:'M3 5v10 M8 5v10 M13 5v10 M16 10h4 M18 8l2 2-2 2',column:'M5 3h10 M5 8h10 M5 13h10 M10 16v4 M8 18l2 2 2-2',grid:'M3 3h14v14H3z M10 3v14 M3 10h14'};
 ...
 function compactPaint(section,input){
  ...
  input.addEventListener('input',sync);input.addEventListener('change',sync);new MutationObserver(sync).observe(input,{attributes:true,attributeFilter:['disabled','title']});sync();root.RetouchInspector.fieldDraft(opacity);
 }
 ...
 function organize(panel){
  ...
  for(const section of panel.querySelectorAll('.inspector-section')){
   ...
   if(name==='Stroke')for(const input of section.querySelectorAll('input'))if(/^(?:Border(?: (?:top|right|bottom|left))? width \(.*\)|SVG (?:stroke width|dash pattern|dash offset|miter limit)|Dash length|Dash gap)$/.test(input.getAttribute('aria-label')||''))root.RetouchInspector.fieldDraft(input);
