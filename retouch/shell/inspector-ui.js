...
const layoutIcons={flow:'M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z',row:'M3 5v10 M8 5v10 M13 5v10 M16 10h4 M18 8l2 2-2 2',column:'M5 3h10 M5 8h10 M5 13h10 M10 16v4 M8 18l2 2 2-2',grid:'M3 3h14v14H3z M10 3v14 M3 10h14'};
 ...
 function compactPaint(section,input){
  ...
  const paintGroups=()=>{if(shared&&!sharedRangeActive(input))throw Error(rangeHint);const values=shared?input.retouchSelectionColors():[input.value.trim()];return values.map(value=>{const colors=input.dataset.paintProperty==='border-color'?root.RetouchHTMLCSSValues.parseBorderColors(value):[value];if(!colors?.length)throw Error('Choose literal paint colors first.');return colors.map(color=>{const paint=root.RetouchPaintPicker.parsePaint(color);if(!paint)throw Error('Choose literal paint colors first.');return paint;});});};
  ...
  const colors=()=>{try{return paintGroups().flat();}catch{return null;}},sync=()=>{const values=colors()||(document.activeElement===input?editColors:null),value=values?.[0],mixed=values?.some(item=>item.space!==value.space||item.alpha!==value.alpha||item.channels.some((channel,i)=>channel!==value.channels[i])),mixedAlpha=values?.some(item=>item.alpha!==value.alpha);control.classList.toggle('has-compact-label',!!value);display.textContent=value?(mixed?'Mixed':value.space==='display-p3'?'Display P3':value.channels.every(n=>Math.abs(n*255-Math.round(n*255))<1.3e-4)?value.channels.map(n=>Math.round(n*255).toString(16).padStart(2,'0')).join('').toUpperCase():'RGB'):'';opacity.disabled=input.disabled||!value||shared&&!sharedRangeActive(input);opacity.title=shared&&!sharedRangeActive(input)?rangeHint:input.disabled?input.title:value?'Paint opacity':'Choose a literal color to edit its opacity.';opacity.placeholder=mixedAlpha?'Mixed':'';opacity.value=value&&!mixedAlpha?String(Number((value.alpha*100).toFixed(8))):'';};
  opacity.oninput=()=>opacity.setCustomValidity('');opacity.onchange=()=>{if(!opacity.checkValidity()||input.disabled||shared&&!sharedRangeActive(input))return;const current=colors(),value=Number(opacity.value)/100;if(!current||current.every(item=>Math.abs(item.alpha-value)<1e-10))return;root.RetouchPanelFocus?.queue(opacity);if(shared){sendShared(paintGroups().map(group=>group.map(item=>item.space==='display-p3'?root.RetouchPaletteValues.p3(item.channels,value):root.RetouchPaletteValues.srgb(item.channels,value)).join(' ')));return;}input.value=current.map(item=>item.space==='display-p3'?root.RetouchPaletteValues.p3(item.channels,value):root.RetouchPaletteValues.srgb(item.channels,value)).join(' ');input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}));};
 ...
 function sharedStrokePattern(settings){
  ...
  const patterns=()=>raw.retouchDashValues().map(value=>P.pattern(value)),sync=()=>{const parsed=patterns(),types=parsed.map(value=>value?.type||'custom');style.value=types.every(value=>value===types[0])?types[0]:'mixed';for(const [input,index]of [[dash,0],[gap,1]]){const values=parsed.map(value=>value?.parts[index]||value?.parts[0]||'4');input.value=values.every(value=>value===values[0])?values[0]:'';input.placeholder=input.value?'':'Mixed';input.disabled=raw.disabled||!sharedRangeActive(raw)||!parsed.every(value=>value?.type==='dashed');input.title=sharedRangeActive(raw)?raw.title:rangeHint;input.closest('.inspector-field').hidden=style.value==='solid'||style.value==='custom';}style.disabled=raw.disabled;style.querySelector('[value=dashed]').disabled=!sharedRangeActive(raw);row.hidden=style.value!=='custom';};
  ...
  style.onchange=()=>{if(style.value==='dashed'&&!sharedRangeActive(raw))return;if(style.value==='custom'){row.hidden=false;dash.closest('.inspector-field').hidden=true;gap.closest('.inspector-field').hidden=true;raw.focus();return;}write(raw.retouchDashValues().map(value=>style.value==='solid'?'none':P.dashPair(value)));};
  for(const [input,key]of [[dash,'dash'],[gap,'gap']]){input.type='text';input.oninput=()=>input.setCustomValidity('');input.onchange=()=>{if(input.disabled||!sharedRangeActive(raw))return;const values=raw.retouchDashValues().map(value=>P.dashPair(value,{[key]:input.value.trim()}));if(values.some(value=>value===null)){input.setCustomValidity('Enter a nonnegative dash length in pixels or percent.');input.reportValidity();return;}root.RetouchPanelFocus?.queue(input);write(values);};}
 ...
 function organize(panel){
  ...
  if(sharedInput&&screenScope){
   const notice=document.createElement('div'),message=document.createElement('span'),preview=document.createElement('button');notice.className='shared-range-notice';notice.setAttribute('role','status');notice.setAttribute('aria-label','Shared edit range');message.textContent='Preview the edit range to adjust per-layer values.';preview.type='button';preview.className='control-button';preview.textContent='Preview edit range';
