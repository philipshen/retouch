...
const layoutIcons={flow:'M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z',row:'M3 5v10 M8 5v10 M13 5v10 M16 10h4 M18 8l2 2-2 2',column:'M5 3h10 M5 8h10 M5 13h10 M10 16v4 M8 18l2 2 2-2',grid:'M3 3h14v14H3z M10 3v14 M3 10h14'};
 ...
 function compactPaint(section,input){
  const svg=input.getAttribute('aria-label')?.match(/^SVG (fill|stroke)$/)?.[1],paint=svg||({'background-color':'Fill','border-color':'Stroke'})[input.dataset.paintProperty];if(!paint||/^url\(/i.test(input.value.trim()))return;
  ...
  const opacity=document.createElement('input'),alpha=document.createElement('span'),unit=document.createElement('span');alpha.className='compact-paint-alpha';opacity.type='number';opacity.min=0;opacity.max=100;opacity.step='any';opacity.required=true;opacity.setAttribute('aria-label',(svg?'SVG ':'')+paint+' opacity (%)');unit.textContent='%';unit.setAttribute('aria-hidden','true');alpha.append(opacity,unit);control.append(alpha);
 ...
 function typeSettingsTabs(settings,section){
  ...
  const samples={'Number width':'111111 · 888888','Number style':'0123456789','Fractions':'1/2 1/3 3/4','Ordinals':'1st 2nd 3rd','Zero style':'0 O 00 OO','Common ligatures':'fi fl ffi ffl','Rare ligatures':'st ct','Historical ligatures':'st ct tz','Contextual alternates':'affinity office'};
  ...
  for(const child of children){const summary=child.querySelector(':scope > summary')?.textContent;groups.get(summary==='Variable font axes'?'Variable':['Number formatting','Ligatures'].includes(summary)?'Details':'Basics').panel.append(child);}
 ...
 function organize(panel){
  ...
  for(const section of panel.querySelectorAll('.inspector-section')){
   ...
   if(name==='Typography'){
    ...
    for(const child of [...section.children])if((child.classList.contains('control-button')&&!child.dataset.primaryFontReset&&(/^(Reset |Automatic line height)/.test(child.textContent)))||(child.tagName==='DETAILS'&&['Variable font axes','Number formatting','Ligatures'].includes(child.querySelector('summary')?.textContent)))options.append(child);
