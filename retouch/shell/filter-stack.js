(function(root){
 ...
 function mountSharedShadows(parent,infos,elements,scope,save){
  ...
  const rowUpdates=[],editable=current=>current.every((stack,i)=>stack&&elements[i].isConnected&&elements[i].style.getPropertyPriority('box-shadow')!=='important')&&(own().every(stack=>stack!==null)||B.rangeActive(input,elements[0]));
  const initial=stacks();if(initial.every(Boolean))for(let index=0;index<Math.max(0,...initial.map(stack=>stack.length));index++){
   ...
   const label='Shared Shadow '+(index+1),update=change=>{try{const current=stacks();if(!editable(current))return;const next=current.map(stack=>stack[index]?V().serializeShadows(change(stack,index)):undefined);if(next.every((value,i)=>value===undefined||value===V().serializeShadows(current[i])))return;Promise.resolve(save(next)).catch(error=>I.note(parent,error.message,'refused'));}catch(error){I.note(parent,error.message,'refused');}};
   ...
   rowUpdates.push(current=>{const items=current.filter(Boolean).map(stack=>stack[index]).filter(Boolean),disabled=!items.length||!editable(current);type.disabled=remove.disabled=disabled;const types=items.map(item=>item.inset?'inner':'drop');if(type.ownerDocument.activeElement!==type)type.value=types.every(value=>value===types[0])?types[0]||'':'';for(const [key,field]of controls){const values=items.map(item=>item[key]),mixed=values.some(value=>value!==values[0]);field.disabled=disabled;field.placeholder=mixed?'Mixed':'';if(field.ownerDocument.activeElement!==field)field.value=mixed||!values.length?'':String(values[0]);}});
  }
