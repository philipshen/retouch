...
b.onclick=()=>onAction(action);actions.append(b);actionButtons[action]=b;
    ...
    function selection(el,info,busy=false,multiple=[],readOnly=false) {
      ...
      const capabilities=JSON.stringify([!!info,info?.kind,info?.canDuplicateComponent,info?.canDeleteComponent,info?.componentMovement,info?.componentDuplicateReason,!!info?.svgMovement,!!info?.selectionBooleanDelete,s,busy,copied,compatible,selectedSet.size]);
