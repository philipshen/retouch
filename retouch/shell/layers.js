...
b.onclick=()=>onAction(action);actions.append(b);actionButtons[action]=b;
    ...
    function selection(el,info,busy=false,multiple=[],readOnly=false) {
      ...
      if(selectedSet.size>1){for(const button of Object.values(actionButtons))button.disabled=true;if(info?.selectionBooleanDelete){actionButtons.deleteElement.disabled=busy;reason.textContent='Delete updates the affected boolean groups together. Undo restores the selection.';return;}if(!info?.cssAuthoring){reason.textContent=selectedSet.size+' source layers selected. Shared styles apply together.';return;}actionButtons.duplicateElement.disabled=busy;actionButtons.deleteElement.disabled=busy;actionButtons.reparentElement.disabled=busy;actionButtons.frameSelection.disabled=busy||!s?.canFrame;reason.textContent=selectedSet.size+' layers selected. Frame, move, duplicate and delete apply to the selection.';return;}
