(function(){
 ...
 function open(){
  ...
  const enabled=row=>row?.element?.isConnected&&!row.element.matches(':disabled')&&!row.element.hidden;
  ...
  for(const id of ['layersPanel','panelBody','toolbar','screenPreset','screenUndo','screenRedo']){const target=document.getElementById(id);if(target)observer.observe(target,{subtree:true,childList:true,attributes:true,attributeFilter:['disabled','hidden','title']});}
