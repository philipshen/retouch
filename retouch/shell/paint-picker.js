(function(root){
 ...
 function open(input){
  ...
  const channels=document.createElement('div');channels.className='paint-channels';const channelNames=['Red','Green','Blue'],channelInputs=channelNames.map((label,index)=>{const field=document.createElement('input');field.type='number';field.min=0;field.required=true;I.field(channels,label,field);field.parentElement.querySelector('span').textContent=label[0];field.oninput=()=>{if(!parsed||field.value===''||!field.checkValidity())return;let nextHSV=null;if(parsed.space==='srgb'&&['hsl','hsb'].includes(channelModel)){const values=channelModel==='hsl'?hsl(parsed.channels):[h,s*100,v*100];values[index]=Number(field.value);parsed.channels=channelModel==='hsl'?hslRGB(...values):rgb(values[0]%360,values[1]/100,values[2]/100);if(channelModel==='hsb')nextHSV=[values[0]%360,values[1]/100,values[2]/100];}else parsed.channels[index]=Number(field.value)/(parsed.space==='srgb'?255:1);value.value=parsed.space==='srgb'?srgbValue(parsed.channels,parsed.alpha):root.RetouchPaletteValues.p3(parsed.channels,parsed.alpha);sync(false,field,nextHSV);};return field;});dialog.append(channels);
  const hex=document.createElement('input');hex.type='text';hex.spellcheck=false;hex.required=true;hex.placeholder='RRGGBB';I.field(dialog,'Hex',hex);hex.setAttribute('aria-label','Hex color');const hexRow=hex.parentElement;hexRow.hidden=true;hex.oninput=()=>{const text=hex.value.trim(),valid=/^#?(?:[a-f\d]{3}|[a-f\d]{4}|[a-f\d]{6}|[a-f\d]{8})$/i.test(text);hex.setCustomValidity(valid?'':'Enter three, four, six or eight hexadecimal digits.');if(!valid||parsed?.space!=='srgb')return;let digits=text.replace(/^#/,'');if(digits.length<=4)digits=[...digits].map(c=>c+c).join('');value.value=srgbValue([0,2,4].map(index=>parseInt(digits.slice(index,index+2),16)/255),digits.length===8?parseInt(digits.slice(6),16)/255:parsed.alpha);sync(false,hex);};
  ...
  function sync(preserveOpacity=false,preserveChannel=null,preserveHSV=null){
   ...
   parsed=read();const srgb=parsed?.space==='srgb';conversion.replaceChildren();profile.disabled=!parsed;profile.value=parsed?.space||'srgb';for(const option of model.options)option.disabled=!srgb&&['hex','hsl','hsb'].includes(option.value);if(!srgb&&!['rgb','css'].includes(channelModel))channelModel='rgb';plane.hidden=!srgb;hue.parentElement.hidden=!srgb;channels.hidden=!parsed;hexRow.hidden=true;opacity.disabled=!parsed;opacityNumber.disabled=!parsed;model.disabled=!parsed;model.value=channelModel;
   status.textContent=parsed?'':valid()?'Use the CSS value to edit this color.':'';
   if(parsed){opacity.value=String(parsed.alpha*100);if(preserveOpacity!==true)opacityNumber.value=String(Number((parsed.alpha*100).toPrecision(12)));paintOpacity();if(srgb){[h,s,v]=preserveHSV||hsv(parsed.channels);paint();}syncChannels(preserveChannel);}updateRecent();
  }
  function syncChannels(preserve=null){
   if(!parsed)return;const srgb=parsed.space==='srgb',asHex=srgb&&channelModel==='hex',asHSL=srgb&&channelModel==='hsl',asHSB=srgb&&channelModel==='hsb',polar=asHSL||asHSB;
   channels.hidden=asHex||channelModel==='css';hexRow.hidden=!asHex;
   ...
   const values=asHSL?hsl(parsed.channels):asHSB?[h,s*100,v*100]:parsed.channels.map(n=>srgb?Math.round(n*255):n),names=polar?['Hue (deg)','Saturation (%)',asHSL?'Lightness (%)':'Brightness (%)']:channelNames.map(name=>srgb?name+' (0–255)':'Display P3 '+name.toLowerCase());
   channelInputs.forEach((field,index)=>{field.max=polar?(index===0?360:100):srgb?255:1;field.step=srgb&&!polar?1:'any';field.setAttribute('aria-label',names[index]);field.parentElement.querySelector('span').textContent=polar?(asHSL?'HSL':'HSB')[index]:channelNames[index][0];if(field!==preserve)field.value=String(polar?Number(values[index].toFixed(2)):Number(values[index].toPrecision(12)));});
  }
  ...
  function fromHSV(){const alpha=parsed?.alpha??1;value.value=srgbValue(rgb(h,s,v),alpha);parsed=read();preview.style.backgroundImage='linear-gradient('+value.value+','+value.value+'),repeating-conic-gradient(#ddd 0% 25%,white 0% 50%)';value.setCustomValidity('');draftPreview?.update(value.value);paint();paintOpacity();syncChannels();updateRecent();}
  hue.oninput=()=>{h=Number(hue.value);fromHSV();};
  const setAlpha=(percent,preserve)=>{if(!parsed)return;const alpha=percent/100;value.value=parsed.space==='display-p3'?root.RetouchPaletteValues.p3(parsed.channels,alpha):srgbValue(parsed.channels,alpha);sync(preserve,null,parsed.space==='srgb'?[h,s,v]:null);};
  opacity.oninput=()=>setAlpha(Number(opacity.value),false);opacityNumber.oninput=()=>{if(opacityNumber.value!==''&&opacityNumber.checkValidity())setAlpha(Number(opacityNumber.value),true);};opacityNumber.onblur=()=>{if(opacityNumber.value!==''&&opacityNumber.checkValidity())sync(false,null,parsed?.space==='srgb'?[h,s,v]:null);};
  value.oninput=sync;
