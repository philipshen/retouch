...
handle.style.left=s*100+'%';handle.style.top=(1-v)*100+'%';plane.setAttribute('aria-valuenow',String(Math.round(v*100)));plane.setAttribute('aria-valuetext',Math.round(s*100)+'% saturation, '+Math.round(v*100)+'% brightness');hue.value=String(h);}
  function sync(preserveOpacity=false,preserveChannel=null,preserveHSV=null){
   ...
   if(parsed){opacity.value=String(parsed.alpha*100);if(preserveOpacity!==true)opacityNumber.value=String(Number((parsed.alpha*100).toPrecision(12)));paintOpacity();[h,s,v]=preserveHSV||hsv(parsed.channels);paint();syncChannels(preserveChannel);}updateRecent();
  }
  function syncChannels(preserve=null){
   ...
   channels.hidden=asHex||channelModel==='css';hexRow.hidden=!asHex;hex.title=srgb?'Hex channels in sRGB':'Hex channels in Display P3';hexRow.querySelector('span').textContent=srgb?'Hex':'P3 Hex';
  ...
  function fromHSV(){const alpha=parsed?.alpha??1;value.value=paintValue(rgb(h,s,v),alpha);parsed=read();conversion.replaceChildren();preview.style.backgroundImage='linear-gradient('+value.value+','+value.value+'),repeating-conic-gradient(#ddd 0% 25%,white 0% 50%)';value.setCustomValidity('');draftPreview?.update(value.value);paint();paintOpacity();syncChannels();updateRecent();}
  hue.oninput=()=>{h=Number(hue.value);fromHSV();};
  const setAlpha=(percent,preserve)=>{if(!parsed)return;const alpha=percent/100;value.value=parsed.space==='display-p3'?root.RetouchPaletteValues.p3(parsed.channels,alpha):srgbValue(parsed.channels,alpha);sync(preserve,null,[h,s,v]);};
  opacity.oninput=()=>setAlpha(Number(opacity.value),false);opacityNumber.oninput=()=>{if(opacityNumber.value!==''&&opacityNumber.checkValidity())setAlpha(Number(opacityNumber.value),true);};opacityNumber.onblur=()=>{if(opacityNumber.value!==''&&opacityNumber.checkValidity())sync(false,null,parsed?[h,s,v]:null);};
  value.oninput=sync;
