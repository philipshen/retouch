(function(root){
 ...
 function open(input){
  ...
  const channels=document.createElement('div');channels.className='paint-channels';const channelNames=['Red','Green','Blue'],channelInputs=channelNames.map((label,index)=>{const field=document.createElement('input');field.type='number';field.min=0;field.required=true;I.field(channels,label,field);field.parentElement.querySelector('span').textContent=label[0];field.oninput=()=>{if(!parsed||field.value===''||!field.checkValidity())return;parsed.channels[index]=Number(field.value)/(parsed.space==='srgb'?255:1);value.value=parsed.space==='srgb'?srgbValue(parsed.channels,parsed.alpha):root.RetouchPaletteValues.p3(parsed.channels,parsed.alpha);sync(false,field);};return field;});dialog.append(channels);
  ...
  function sync(preserveOpacity=false,preserveChannel=null){
   ...
   parsed=read();const srgb=parsed?.space==='srgb';plane.hidden=!srgb;hue.parentElement.hidden=!srgb;channels.hidden=!parsed;opacity.disabled=!parsed;opacityNumber.disabled=!parsed;
  ...
  function syncChannels(preserve=null){if(!parsed)return;const srgb=parsed.space==='srgb';channelInputs.forEach((field,index)=>{field.max=srgb?255:1;field.step=srgb?1:'any';field.setAttribute('aria-label',srgb?channelNames[index]+' (0–255)':'Display P3 '+channelNames[index].toLowerCase());if(field!==preserve)field.value=String(srgb?Math.round(parsed.channels[index]*255):parsed.channels[index]);});}
