(function(root){
 ...
 function shadowPreview({el,group,shadows,index}){
  const originalStyle=el.getAttribute('style'),originalValue=el.style.getPropertyValue('box-shadow'),priority=el.style.getPropertyPriority('box-shadow');let lastStyle=originalStyle,lastValue=null,restored=false;
  return {update(color){if(restored||!el.isConnected||!group.isConnected)return;el.style.setProperty('box-shadow',root.RetouchHTMLCSSValues.serializeShadows(shadows.map((shadow,i)=>i===index?{...shadow,color}:shadow)),'important');lastValue=el.style.getPropertyValue('box-shadow');lastStyle=el.getAttribute('style');},restore(){if(restored)return;restored=true;if(lastValue===null)return;if(el.getAttribute('style')===lastStyle){if(originalStyle===null)el.removeAttribute('style');else el.setAttribute('style',originalStyle);}else if(el.style.getPropertyValue('box-shadow')===lastValue&&el.style.getPropertyPriority('box-shadow')==='important'){if(originalValue)el.style.setProperty('box-shadow',originalValue,priority);else el.style.removeProperty('box-shadow');}}};
 }
 function propertyPreview({el,input,property}){
  const properties=property==='border-color'?['border-top-color','border-right-color','border-bottom-color','border-left-color']:[property],originalStyle=el.getAttribute('style'),original=properties.map(name=>({name,value:el.style.getPropertyValue(name),priority:el.style.getPropertyPriority(name)}));let lastStyle=null,last=null,restored=false;
  return {update(color){if(restored||!el.isConnected||!input.isConnected)return;el.style.setProperty(property,color,'important');last=properties.map(name=>el.style.getPropertyValue(name));lastStyle=el.getAttribute('style');},restore(){if(restored)return;restored=true;if(last===null)return;if(el.getAttribute('style')===lastStyle){if(originalStyle===null)el.removeAttribute('style');else el.setAttribute('style',originalStyle);}else original.forEach(({name,value,priority},index)=>{if(el.style.getPropertyValue(name)===last[index]&&el.style.getPropertyPriority(name)==='important'){if(value)el.style.setProperty(name,value,priority);else el.style.removeProperty(name);}});}};
 }
