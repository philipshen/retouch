(function(root){
 ...
 function open(input){
  ...
  let channelModel='rgb';const model=I.select(dialog,'Color model',[['hex','Hex'],['rgb','RGB'],['hsl','HSL'],['p3','Display P3']],'rgb',next=>{channelModel=next;syncChannels();});model.querySelector('[value=p3]').disabled=true;
  ...
  const hex=document.createElement('input');hex.type='text';hex.spellcheck=false;hex.required=true;hex.placeholder='RRGGBB';I.field(dialog,'Hex',hex);hex.setAttribute('aria-label','Hex color');const hexRow=hex.parentElement;hexRow.hidden=true;hex.oninput=()=>{const text=hex.value.trim(),valid=/^#?(?:[a-f\d]{3}|[a-f\d]{6})$/i.test(text);hex.setCustomValidity(valid?'':'Enter three or six hexadecimal digits.');if(!valid||parsed?.space!=='srgb')return;let digits=text.replace(/^#/,'');if(digits.length===3)digits=[...digits].map(c=>c+c).join('');value.value=srgbValue([0,2,4].map(index=>parseInt(digits.slice(index,index+2),16)/255),parsed.alpha);sync(false,hex);};
  ...
  function sync(preserveOpacity=false,preserveChannel=null){
   ...
   parsed=read();const srgb=parsed?.space==='srgb';plane.hidden=!srgb;hue.parentElement.hidden=!srgb;channels.hidden=!parsed;hexRow.hidden=true;opacity.disabled=!parsed;opacityNumber.disabled=!parsed;model.disabled=!srgb;model.value=srgb?channelModel:parsed?'p3':channelModel;
