(function(root){
 ...
 function open(input){
  ...
  const actions=document.createElement('div');actions.className='paint-picker-actions';actions.append(I.button('Cancel',()=>{draftPreview?.restore();dialog.close();}),I.button('Apply color',()=>{if(!valid()){value.reportValidity();return;}const next=value.value.trim();draftPreview?.restore();dialog.close();if(input.isConnected&&next!==original){root.RetouchPanelFocus?.queue(input);input.value=next;input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}));}}));dialog.append(actions);
