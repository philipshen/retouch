(function(root){
 ...
 function mount(infos,elements,scope,save,saveColor,resolveElement,saveBackground){
  ...
  const relativeWrite=(property,value,relative=true)=>{try{save(Object.fromEntries(infos.map((info,i)=>[info.id,change(info.className,scope,property,Array.isArray(value)?value[i]:value,liveElement(i).ownerDocument,relative,liveElement(i))])));}catch(error){I.note(groups.typography,error.message,'refused');}};
  ...
  for(const [property,field]of Object.entries(fields).filter(([,field])=>!field.constraint).flatMap(entry=>['width','height'].includes(entry[0])?[entry,...['min-','max-'].map(prefix=>[prefix+entry[0],fields[prefix+entry[0]]])]:[entry])){
   ...
   if(field.picker){
    ...
    const write=value=>{try{save(Object.fromEntries(infos.map((info,i)=>[info.id,change(info.className,scope,property,value,liveElement(i).ownerDocument,false,liveElement(i))])));}catch(error){I.note(sec,error.message,'refused');}};
    I.fontPicker(group,elements[0].ownerDocument,mixed?'':values[0],write,{mixed,label:'Shared Page font',disabled:elements.some(el=>el.style.getPropertyPriority('font-family')==='important')});
