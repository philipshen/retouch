(function(root){
 ...
 function change(classes,scope,property,value,document=null,relative=false){
  ...
  const dimension=['width','height'].includes(property),sizing=dimension||field.constraint,priorityMatch=token=>field.matches(token)||dimension&&/^size-/.test(token)||field.flexItem&&flexShorthand(token);
  ...
  if((dimension||field.flexItem)&&addition&&R.project(classes,scope).split(/\s+/).some(token=>I.base(token)!==null&&(dimension?/^size-/.test(I.base(token)):flexShorthand(I.base(token)))&&/^!|!$/.test(token)))addition='!'+addition.replace(/^!/,'');
 ...
 function mount(infos,elements,scope,save,saveColor){
  ...
  for(const [property,field]of Object.entries(fields).filter(([,field])=>!field.constraint).flatMap(entry=>['width','height'].includes(entry[0])?[entry,...['min-','max-'].map(prefix=>[prefix+entry[0],fields[prefix+entry[0]]])]:[entry])){
   ...
   if(field.constraint&&!mixed&&!display)input.placeholder=values[0]==='auto'?'Automatic':values[0]==='none'?'No limit':values[0];input.value=display;input.disabled=blocked;input.title=blocked?(field.flexItem?'Select in-flow items in a flex layout without an inline flex override.':'An inline style controls this property on a selected layer. Edit that source style first.'):'';
