(function(root){
 ...
 function change(classes,scope,property,value,document=null,relative=false,el=null){
  if(property==='visibility'&&value!==null&&el?.style.getPropertyPriority(property)==='important')throw Error('An important inline rule controls visibility.');
  ...
  if(addition&&R.project(classes,scope).split(/\s+/).some(token=>I.base(token)!==null&&(shorthandMatch(I.base(token))||(sizing||field.ratio||property==='visibility')&&field.matches(I.base(token)))&&/^!|!$/.test(token)))addition='!'+addition.replace(/^!/,'');
  if(addition&&document&&!['opacity','mix-blend-mode','isolation'].includes(property)&&I.catalog(document).some(name=>(classes||'').split(/\s+/).includes(name))&&!addition.startsWith('!'))addition='!'+addition;
  if(addition&&property==='visibility'&&el?.style.getPropertyValue(property))addition='!'+addition.replace(/^!|!$/g,'');
 ...
 function mount(infos,elements,scope,save,saveColor,resolveElement,saveBackground){
  ...
  for(const [property,field]of Object.entries(fields).filter(([,field])=>!field.constraint).flatMap(entry=>['width','height'].includes(entry[0])?[entry,...['min-','max-'].map(prefix=>[prefix+entry[0],fields[prefix+entry[0]]])]:[entry])){
   ...
   if(property==='visibility'&&!typeActive())input.title='Preview the selected edit range before changing visibility.';
