(function(root){
 ...
 function mount(infos,elements,scope,save,saveColor,resolveElement,saveBackground){
  ...
  if(saveColor)for(const [property,label]of [['color','Text color'],['background-color','Background color'],['border-color','Border color'],...(elements.every(el=>el.namespaceURI==='http://www.w3.org/2000/svg')?[['fill','SVG fill'],['stroke','SVG stroke']]:[])]){
   ...
   input.value=mixed?'':values[0];I.field(sec,'Shared '+label+' with alpha',input);root.RetouchPaintPicker.mountSelectionField(input,elements,property,saveBackground?changes=>saveBackground(Object.fromEntries(infos.map((info,i)=>[info.id,changes[i]]))):undefined);I.note(sec,mixed?'Mixed colors':values[0]);
