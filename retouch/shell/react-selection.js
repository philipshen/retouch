(function(root){
 ...
 function mount(infos,elements,scope,save,saveColor,resolveElement,saveBackground){
  ...
  for(const [property,field]of Object.entries(fields).filter(([,field])=>!field.constraint).flatMap(entry=>['width','height'].includes(entry[0])?[entry,...['min-','max-'].map(prefix=>[prefix+entry[0],fields[prefix+entry[0]]])]:[entry])){
   ...
   const writeSize=(mode,value)=>{try{save(Object.fromEntries(infos.map((info,i)=>{const el=liveElement(i),{css,context,blocked}=sizeContext(el,property);if(blocked&&mode!=='reset')throw Error('An important inline sizing rule or layout constraint controls a selected layer.');const pixels=mode==='fixed'?dimensionValue(css,property,value??dimensionSize(css,property)):0;return [info.id,changeSizeMode(info.className,scope,property,mode,pixels,context,el.ownerDocument)];})));}catch(error){I.note(sec,error.message,'refused');}};
