 ...
 function mount(infos,elements,scope,save,saveColor,resolveElement,saveBackground){
  ...
  for(const [property,field]of Object.entries(fields).filter(([,field])=>!field.constraint).flatMap(entry=>['width','height'].includes(entry[0])?[entry,...['min-','max-'].map(prefix=>[prefix+entry[0],fields[prefix+entry[0]]])]:[entry])){
   ...
   if(!field.options&&!field.text){
    ...
    const mixedDelta=dimension||property==='font-size',readSize=css=>dimension?dimensionSize(css,property):parseFloat(css.fontSize);
    ...
    I.numericLabelDrag(input,raw=>{if(mixedDelta&&input.value===''){const css=infos.map((_,i)=>liveElement(i).ownerDocument.defaultView.getComputedStyle(liveElement(i))),sizes=css.map(readSize),base=sizes[0];return {value:base,min:base+Math.max(...sizes.map((size,i)=>(dimension?decoration(css[i],property):minimum)-size)),max:base+(dimension?100000:field.max??100)-Math.max(...sizes),format};}return {value:Number(raw),min:minimum,max:field.max??100,format};});
    ...
    if(mixedDelta&&mixed){const label=input.parentElement.querySelector('span');label.title='Drag to change each selected size by the same amount. '+label.title;}
    input.retouchNumericPreview=()=>{
