(function(root){
 ...
 function rotationDegrees(value){
  ...
  const match=/^(?:z\s+)?(-?(?:\d*\.)?\d+)(deg|grad|rad|turn)$/.exec(value.trim());
  return match?Number(match[1])*({deg:1,grad:.9,rad:180/Math.PI,turn:360})[match[2]]:NaN;
 }
 fields.rotate={label:'Rotation (°)',min:-360,max:360,read:css=>String(rotationDegrees(css.getPropertyValue('rotate'))),matches:token=>/^-?rotate-(?![xyz]-)|^\[rotate:/.test(token),token:value=>'[rotate:'+value+'deg]'};
 ...
 Object.assign(fields,{
  ...
  'font-variant-position':{label:'Number position',matches:t=>inspector().fontPositionToken(t),options:['normal','super','sub'],optionLabels:{normal:'Normal',super:'Superscript',sub:'Subscript'},token:value=>'[font-variant-position:'+value+']'},
  'text-wrap':{label:'Wrap style',matches:t=>inspector().textWrapToken(t),options:['wrap','balance','pretty','nowrap'],optionLabels:{wrap:'Auto',balance:'Balance',pretty:'Pretty',nowrap:'No wrap'},token:value=>'[text-wrap:'+value+']'},
  'text-indent':{label:'Paragraph indent',text:true,indent:true,valid:value=>(root.RetouchHTMLCSSValues||require('./html-css-values.js')).valid('text-indent',value),matches:t=>inspector().textIndentToken(t),token:value=>'[text-indent:'+value.replace(/ /g,'_')+']'},
 ...
 function change(classes,scope,property,value,document=null,relative=false,el=null){
  if(['visibility','opacity','mix-blend-mode','isolation'].includes(property)&&value!==null&&el?.style.getPropertyPriority(property)==='important')throw Error('An important inline rule controls '+property+'.');
  ...
  if(addition&&R.project(classes,scope).split(/\s+/).some(token=>I.base(token)!==null&&(shorthandMatch(I.base(token))||(sizing||field.ratio||['visibility','opacity','mix-blend-mode','isolation'].includes(property))&&field.matches(I.base(token)))&&/^!|!$/.test(token)))addition='!'+addition.replace(/^!/,'');
  if(addition&&document&&I.catalog(document).some(name=>(classes||'').split(/\s+/).includes(name))&&!addition.startsWith('!'))addition='!'+addition;
  if(addition&&['visibility','opacity','mix-blend-mode','isolation'].includes(property)&&el?.style.getPropertyValue(property))addition='!'+addition.replace(/^!|!$/g,'');
 ...
 function changeTextTruncation(classes,scope,value,el=null){
  ...
  if(value!==null&&value!=='none'&&(!Number.isInteger(value)||value<1||value>1000))throw Error('Choose a whole number of lines between 1 and 1000.');
  if(value!==null&&el&&I.truncationProperties.some(key=>el.style.getPropertyPriority(key)==='important'))throw Error('An important inline rule controls text truncation.');
  return R.replaceScope(classes,I.replace(R.project(classes,scope),I.truncationToken,value===null?'':'!line-clamp-'+value),scope);
 }
 function changeTextResizing(classes,scope,changes,el=null){
  const I=inspector(),R=root.RetouchResponsive||require('./responsive.js');
  ...
  if(!changes||!dimension(changes.width)||!dimension(changes.height)||!['pre','pre-wrap'].includes(changes['white-space'])||!/^(?:wrap|nowrap)(?: (?:balance|pretty|stable))?$/.test(changes['text-wrap'])||Object.keys(changes).some(key=>![...I.textSizeLimits,'aspect-ratio','width','height','white-space','text-wrap','flex-grow','flex-shrink','flex-basis','align-self','justify-self'].includes(key))||changes['aspect-ratio']!==undefined&&changes['aspect-ratio']!=='auto'||I.textSizeLimits.some(key=>changes[key]!==undefined&&changes[key]!== (key.startsWith('min-')?'0px':'none'))||['flex-grow','flex-shrink'].some(key=>changes[key]!==undefined&&changes[key]!=='0')||changes['flex-basis']!==undefined&&changes['flex-basis']!=='auto'||['align-self','justify-self'].some(key=>changes[key]!==undefined&&changes[key]!=='flex-start'))throw Error('Choose supported text sizing values.');
 ...
 function changeRotation(classes,scope,value,el,inherited=''){
  ...
  const I=inspector(),R=root.RetouchResponsive||require('./responsive.js'),important=el.style.getPropertyValue('rotate')||(inherited||'').split(/\s+/).some(token=>/^!|!$/.test(token)&&fields.rotate.matches(I.base(token)||''));
  return important?R.replaceScope(next,I.replace(R.project(next,scope),fields.rotate.matches,'![rotate:'+value+'deg]'),scope):next;
 }
 ...
 function itemApplies(el,field){
  if(field.svg)return el.namespaceURI==='http://www.w3.org/2000/svg';
 ...
 function sizeContext(el,axis){
  ...
  const context={inlineDimensions:(root.RetouchLayout||require('./layout.js')).inlineDimensions(el,css),inlineFlex:['flex','flex-grow','flex-shrink','flex-basis'].some(property=>el.style.getPropertyValue(property)),display:parent?.display,direction:parent?.flexDirection,writingMode:parent?.writingMode},axes=(root.RetouchLayout||require('./layout.js')).layoutAxes(context),flex=/flex/.test(context.display||''),grid=/grid/.test(context.display||'');
  ...
  return {css,context,blocked:inline.some(property=>['min-inline-size','min-block-size','max-inline-size','max-block-size','place-self','justify-self','align-self'].includes(property)?el.style.getPropertyValue(property):el.style.getPropertyPriority(property)==='important')};
 }
 ...
 function changeGridTracks(classes,scope,axis,value,document=null,el=null){
  const R=root.RetouchResponsive||require('./responsive.js'),L=root.RetouchLayout||require('./layout.js');
  const inline=['grid','grid-template','grid-template-'+axis];if(value!==null&&el&&inline.some(key=>el.style.getPropertyPriority(key)==='important'))throw Error('An important inline rule controls these grid tracks.');
  return R.replaceScope(classes,L.gridTemplateClasses(R.project(classes,scope),axis,value,R.inherited(classes,scope,document),!!el&&inline.some(key=>el.style.getPropertyValue(key))),scope);
 }
 function changeStack(classes,scope,axis,context={},document=null,el=null){
  const R=root.RetouchResponsive||require('./responsive.js'),L=root.RetouchLayout||require('./layout.js');
  if(el&&(axis==='flow'?['display']:['display','flex-direction','flex-wrap','flex-flow']).some(property=>el.style.getPropertyPriority(property)==='important'))throw Error('An important inline rule controls this layout.');
 ...
 function changeAdaptiveGrid(classes,scope,size,document=null,el=null){
  if(el&&['display','grid','grid-template','grid-template-columns','grid-template-rows'].some(property=>el.style.getPropertyPriority(property)==='important'))throw Error('An important inline rule controls this grid.');
  const R=root.RetouchResponsive||require('./responsive.js'),L=root.RetouchLayout||require('./layout.js');
 ...
 function changeContainer(classes,scope,property,value,document=null,el=null){
  ...
  const gridInline=['columns','rows'].includes(property)?['grid','grid-template','grid-template-'+property]:property==='flow'?['grid','grid-auto-flow']:[];if(value!==null&&el&&gridInline.some(key=>el.style.getPropertyPriority(key)==='important'))throw Error('An important inline rule controls this grid.');
  const alignment=property==='align'?'align-items':property==='justify'?'justify-content':null;
  if(value!==null&&el&&['mode','wrap'].includes(property)&&(property==='mode'?['display','flex-direction','flex-flow']:['flex-wrap','flex-flow']).some(key=>el.style.getPropertyPriority(key)==='important'))throw Error('An important inline rule controls this layout.');
  ...
  const next=value===null?inspector().replace(active,token=>containerRules[property].test(token),''):['mode','wrap'].includes(property)?L.explicitLayoutClasses(active,property,value,inherited):L.arrangementClasses(active,property,value,inherited,!!el&&(!!alignment&&L.inlineAlignment(el,alignment)||gridInline.some(key=>el.style.getPropertyValue(key))));
 ...
 function changeGap(classes,scope,axis,value,document=null,writingMode='horizontal-tb',el=null){
  const R=root.RetouchResponsive||require('./responsive.js'),L=root.RetouchLayout||require('./layout.js');
  const property=L.layoutAxes({writingMode}).inline===axis?'column-gap':'row-gap',inline=['gap',property];
  if(value!==null&&el&&inline.some(p=>el.style.getPropertyPriority(p)==='important'))throw Error('An important inline gap controls a selected layer.');
  return R.replaceScope(classes,L.gapClasses(R.project(classes,scope),axis,value,writingMode,R.inherited(classes,scope,document),!!el&&inline.some(p=>el.style.getPropertyValue(p))),scope);
 }
 ...
 function mount(infos,elements,scope,save,saveColor,resolveElement,saveBackground){
  ...
  const typeActive=()=>!scope||root.document.querySelector('[aria-label="Edit range status"]')?.dataset.match!=='false';
  ...
  for(const [axis,label,path]of [['flow','Shared Normal flow','M3 3h5v5H3z M12 3h5v5h-5z M3 12h5v5H3z M12 12h5v5h-5z'],['vertical','Shared Vertical stack','M4 3h12v4H4z M4 12h12v4H4z'],['horizontal','Shared Horizontal stack','M3 4h4v12H3z M12 4h4v12h-4z']]){
   const active=()=>!scope||root.document.querySelector('[aria-label="Edit range status"]')?.dataset.match!=='false',blocked=el=>!active()||(axis==='flow'?['display']:['display','flex-direction','flex-wrap','flex-flow']).some(property=>el.style.getPropertyPriority(property)==='important');
   const button=I.button('',()=>{try{save(Object.fromEntries(infos.map((info,i)=>{const el=liveElement(i);if(blocked(el))throw Error('Preview this edit range and select containers without inline layout overrides.');return [info.id,changeStack(info.className,scope,axis,el.ownerDocument.defaultView.getComputedStyle(el),el.ownerDocument,el)];})));}catch(error){I.note(groups.layout,error.message,'refused');}});button.setAttribute('aria-label',label);button.title=label.replace('Shared ','');button.disabled=elements.some(blocked);button.innerHTML='<svg viewBox="0 0 20 20" aria-hidden="true"><path d="'+path+'"/></svg>';button.setAttribute('aria-pressed',String(computed.every(css=>axis==='flow'?['block','inline','inline-block','flow-root','list-item'].includes(css.display):Object.entries(root.RetouchHTMLCSSValues.stackLayout(axis,css.writingMode)).every(([property,value])=>css.getPropertyValue(property)===value))));presets.append(button);
  }
  {
   const R=root.RetouchResponsive,L=root.RetouchLayout,V=root.RetouchHTMLCSSValues,minimums=infos.map((info,i)=>L.adaptiveMinimum(R.project(info.className,scope),R.inherited(info.className,scope,elements[i].ownerDocument),elements[i])),active=()=>!scope||root.document.querySelector('[aria-label="Edit range status"]')?.dataset.match!=='false',blocked=el=>!active()||['display','grid','grid-template','grid-template-columns','grid-template-rows'].some(property=>el.style.getPropertyPriority(property)==='important');
   const write=value=>{try{save(Object.fromEntries(infos.map((info,i)=>{const el=liveElement(i);if(blocked(el))throw Error('Preview this edit range and select containers without important inline grid overrides.');return [info.id,changeAdaptiveGrid(info.className,scope,value??minimums[i]??240,el.ownerDocument,el)];})));}catch(error){I.note(groups.layout,error.message,'refused');}};
  ...
  if(computed.every(css=>['grid','inline-grid'].includes(css.display))){
   const active=()=>!scope||root.document.querySelector('[aria-label="Edit range status"]')?.dataset.match!=='false',L=root.RetouchLayout,write=(property,value,inline)=>{try{save(Object.fromEntries(infos.map((info,i)=>{const el=liveElement(i);if(!active()||!['grid','inline-grid'].includes(el.ownerDocument.defaultView.getComputedStyle(el).display)||value!==null&&inline.some(key=>el.style.getPropertyPriority(key)==='important'))throw Error('Preview the selected edit range and select grid containers without important inline grid overrides.');return [info.id,changeContainer(info.className,scope,property,value,el.ownerDocument,el)];})));}catch(error){I.note(groups.layout,error.message,'refused');}};
   for(const [property,label]of [['columns','Grid columns'],['rows','Grid rows']]){
    const cssProperty='grid-template-'+property,inline=['grid','grid-template',cssProperty],counts=computed.map(css=>L.gridTrackCount(css.getPropertyValue(cssProperty))),mixed=counts.some(count=>count!==counts[0]),input=I.number(groups.layout,'Shared '+label,mixed?NaN:counts[0]||NaN,1,24,value=>write(property,value,inline));input.step='1';const initial=input.value;input.onkeydown=event=>{if(event.isComposing||!['Enter','Escape'].includes(event.key))return;event.preventDefault();event.stopPropagation();if(event.key==='Escape'){input.value=initial;input.setCustomValidity('');}input.blur();};input.placeholder=mixed?'Mixed':'Auto';input.disabled=!active()||elements.some(el=>inline.some(key=>el.style.getPropertyPriority(key)==='important'));
    const reset=I.button('Reset shared '+label.toLowerCase(),()=>write(property,null,inline));reset.disabled=!active()||infos.every(info=>changeContainer(info.className,scope,property,null)===info.className);groups.layout.append(reset);
   }
   ...
   const input=I.select(groups.layout,'Shared Grid flow',choices,mixed?'':values[0],value=>write('flow',value,inline));input.disabled=!active()||elements.some(el=>inline.some(key=>el.style.getPropertyPriority(key)==='important'));if(mixed)input.options[0].disabled=true;
   const reset=I.button('Reset shared grid flow',()=>write('flow',null,inline));reset.disabled=!active()||infos.every(info=>changeContainer(info.className,scope,'flow',null)===info.className);groups.layout.append(reset);
   ...
   for(const axis of ['columns','rows']){
    const property='grid-template-'+axis,values=infos.map((info,i)=>L.ownGridTemplate(root.RetouchResponsive.project(info.className,scope),axis,['grid','grid-template',property].some(key=>elements[i].style.getPropertyValue(key)))??computed[i].getPropertyValue(property)),mixed=values.some(value=>value!==values[0]),initial=mixed?'':values[0],input=root.document.createElement('input'),inline=['grid','grid-template',property];input.type='text';input.value=initial;input.placeholder=mixed?'Mixed':'160px 1fr';input.disabled=!active()||elements.some(el=>inline.some(key=>el.style.getPropertyPriority(key)==='important'));
    const write=value=>{try{const changes=Object.fromEntries(infos.map((info,i)=>{const el=liveElement(i);if(!active()||value!==null&&inline.some(key=>el.style.getPropertyPriority(key)==='important')||!['grid','inline-grid'].includes(el.ownerDocument.defaultView.getComputedStyle(el).display))throw Error('Preview the selected edit range and select grid containers without important inline grid overrides.');if(value!==null&&!el.ownerDocument.defaultView.CSS.supports(property,value))throw Error('Enter supported grid track sizes.');return [info.id,changeGridTracks(info.className,scope,axis,value,el.ownerDocument,el)];}));save(changes);}catch(error){input.setCustomValidity(error.message);input.reportValidity();}};
    ...
    const label=axis==='columns'?'Column sizes':'Row sizes';I.field(custom,'Shared '+label,input);const reset=I.button('Reset shared '+label.toLowerCase(),()=>write(null));reset.disabled=!active()||infos.every(info=>changeGridTracks(info.className,scope,axis,null)===info.className);custom.append(reset);
   }
  ...
  }
  {
   ...
   const reset=I.button('Reset shared clip content',()=>write(null));reset.disabled=!active()||infos.every(info=>changeClip(info.className,scope,null)===info.className);const field=input.closest('.inspector-field'),row=root.document.createElement('div');row.className='property-row';field.before(row);row.append(field,reset);field.querySelector(':scope > span').textContent='Clip content';reset.setAttribute('aria-label','Reset shared clip content');reset.title='Reset clipping';reset.textContent='↺';reset.classList.add('property-reset');
  }
  ...
  const lengthDrag=(input,properties,normal=false)=>{
   const parse=raw=>{if(normal)return root.RetouchLayout.gapScrubValue(raw);const match=/^(\d+(?:\.\d+)?|\.\d+)(px|%|rem|em|vw|vh|ch)?$/.exec(raw.trim());return match?{value:Number(match[1]),min:0,max:10000,format:value=>String(value)+(match[2]||'')}:null;};
   ...
   input.retouchNumericPreview=()=>{
    ...
    const writingModes=targets.map(el=>el.ownerDocument.defaultView.getComputedStyle(el).writingMode);return {current:()=>targets.every((el,i)=>el.isConnected&&(!resolveElement||resolveElement(infos[i].id)===el)&&el.ownerDocument.defaultView.getComputedStyle(el).writingMode===writingModes[i])&&previews.every(preview=>preview.current()),update:value=>{const raw=parsed.format(value),css=/[a-z%]$/i.test(raw)?raw:raw+'px';previews.forEach(preview=>preview.update(css));},restore:()=>previews.forEach(preview=>preview.restore())};
   };
  };

  {
   ...
   for(const edge of ['all','top','bottom','left','right']){
    ...
    input.dataset.variableProperty=edge==='all'?'padding':'padding-'+edge;I.field(row,edge==='all'?'Shared Padding':'Shared Padding '+edge,input);input.parentElement.querySelector('span').textContent=edge==='all'?'Padding':edge[0].toUpperCase()+edge.slice(1);lengthDrag(input,()=>edge==='all'?['padding']:['padding-'+edge]);
  ...
  {
   const L=root.RetouchLayout,active=()=>!scope||root.document.querySelector('[aria-label="Edit range status"]')?.dataset.match!=='false',inlineGap=el=>['gap','row-gap','column-gap'].some(property=>el.style.getPropertyPriority(property)==='important'),blocked=!active()||elements.some(inlineGap),pair=root.document.createElement('div');pair.className='property-pair';const alignment=groups.layout.querySelector(':scope > .layout-alignment');if(alignment){const spacing=root.document.createElement('div');spacing.className='layout-alignment-spacing';presets.after(spacing);spacing.append(alignment,pair);}else presets.after(pair);
   ...
   for(const [axis,label,icon]of [['width','Horizontal gap','↔'],['height','Vertical gap','↕']]){
    ...
    const R=root.RetouchResponsive,values=computed.map((css,i)=>L.ownGap(R.project(infos[i].className,scope),axis,css.writingMode,R.inherited(infos[i].className,scope,elements[i].ownerDocument),!!elements[i].style.getPropertyValue(L.layoutAxes({writingMode:css.writingMode}).inline===axis?'column-gap':'row-gap'))??css.getPropertyValue(L.layoutAxes({writingMode:css.writingMode}).inline===axis?'column-gap':'row-gap').replace(/px$/,'')),mixed=values.some(value=>value!==values[0]),initial=mixed?'':values[0],input=root.document.createElement('input');input.type='text';input.value=initial;input.placeholder=mixed?'Mixed':'normal';input.disabled=blocked;input.title=!active()?'Switch to a screen inside the selected edit range.':blocked?'An important inline gap controls a selected layer.':label+'; px, %, rem, em, vw, vh, ch or normal.';
    const write=value=>{try{if(!active())throw Error('Switch to a screen inside the selected edit range.');save(Object.fromEntries(infos.map((info,i)=>{const el=liveElement(i);if(value!==null&&inlineGap(el)||!gapApplies(el.ownerDocument.defaultView.getComputedStyle(el),axis))throw Error('Select compatible containers without important inline gap overrides.');return [info.id,changeGap(info.className,scope,axis,value,el.ownerDocument,el.ownerDocument.defaultView.getComputedStyle(el).writingMode,el)];})));}catch(error){input.setCustomValidity(error.message);input.reportValidity();}};
  ...
  for(const property of ['filter','backdrop-filter'])for(const mount of ['mountSharedBlur','mountSharedFilters'])root.RetouchFilterStack[mount](groups.effects,infos,elements,scope,property,values=>save(Object.fromEntries(infos.map((info,i)=>[info.id,values[i]===undefined?info.className:root.RetouchBackgroundPaintUI.filterClasses(info.className,scope,property,values[i])]))));
  ...
  if(saveColor)for(const [property,label]of [['color','Text color'],['background-color','Background color'],['border-color','Border color'],...(elements.every(el=>el.namespaceURI==='http://www.w3.org/2000/svg')?[['fill','SVG fill'],['stroke','SVG stroke']]:[])]){
   ...
   input.value=mixed?'':values[0];input.dataset.variableProperty=property;I.field(sec,'Shared '+label+' with alpha',input);root.RetouchPaintPicker.mountSelectionField(input,elements,property,saveBackground?changes=>saveBackground(Object.fromEntries(infos.map((info,i)=>[info.id,changes[i]]))):undefined,values=>saveColor(property,undefined,Object.fromEntries(infos.map((info,i)=>[info.id,values[i]]))),i=>root.RetouchBackgroundPaintUI.sourceColor(infos[i],scope,property),i=>root.RetouchBackgroundPaintUI.sourceState(infos[i],scope));I.note(sec,mixed?'Mixed colors':values[0]);
   ...
   input.oninput=()=>input.setCustomValidity('');input.onchange=()=>{const value=input.value.trim();if(!root.RetouchHTMLCSSValues.valid(property==='border-color'?'border-color':'color',value,false)||!elements[0].ownerDocument.defaultView.CSS.supports(property==='border-color'?'border-color':'color',value)){input.setCustomValidity('Enter a supported literal CSS color.');input.reportValidity();return;}saveColor(property,value).catch(error=>{input.setCustomValidity(error.message);input.reportValidity();});};
  }
  ...
  const relativeWrite=(property,value,relative=true)=>{if(!typeActive())return;try{save(Object.fromEntries(infos.map((info,i)=>[info.id,change(info.className,scope,property,Array.isArray(value)?value[i]:value,liveElement(i).ownerDocument,relative,liveElement(i))])));}catch(error){I.note(groups.typography,error.message,'refused');}};
  for(const [property,label,min]of [['line-height','Shared Line height (%)',0],['letter-spacing','Shared Letter spacing (%)',-100]]){
   ...
   const input=I.relativeNumber(group,label,mixed?NaN:values[0],min,1000,value=>relativeWrite(property,value));if(mixed)input.placeholder='Mixed / automatic';input.title='Relative to each selected layer’s own font size.';group.disabled=elements.some(el=>el.style.getPropertyPriority(property)==='important');
  }
  ...
  I.sharedTextResizing(groups.layout,()=>infos.map((_,i)=>liveElement(i)),typeActive,changes=>save(Object.fromEntries(infos.map((info,i)=>[info.id,changeTextResizing(info.className,scope,changes[i],liveElement(i))]))));
  I.sharedTruncationTypography(groups.typography,()=>infos.map((_,i)=>liveElement(i)),typeActive,value=>save(Object.fromEntries(infos.map((info,i)=>[info.id,changeTextTruncation(info.className,scope,value,liveElement(i))]))),infos.some(info=>changeTextTruncation(info.className,scope,null)!==info.className));
  ...
  for(const [property,field]of Object.entries(fields).filter(([,field])=>!field.constraint).flatMap(entry=>['width','height'].includes(entry[0])?[entry,...['min-','max-'].map(prefix=>[prefix+entry[0],fields[prefix+entry[0]]])]:[entry])){
   ...
   if(field.ratio){
    ...
    const ready=(reset=false)=>typeActive()&&infos.every((_,i)=>{const el=liveElement(i);return el?.isConnected&&(reset||!['aspect-ratio','height','inline-size','block-size'].some(key=>el.style.getPropertyValue(key))&&!['inline','contents','none'].includes(el.ownerDocument.defaultView.getComputedStyle(el).display));});
    const blocked=!ready();input.disabled=blocked;if(!typeActive())input.title='Preview the selected edit range before changing aspect ratio.';
    const write=value=>{if(!ready(value===null))return;try{save(Object.fromEntries(infos.map((info,i)=>[info.id,changeRatio(info.className,scope,value,liveElement(i).ownerDocument)])));}catch(error){input.setCustomValidity(error.message);input.reportValidity();}};
    input.oninput=()=>input.setCustomValidity('');input.onchange=()=>write(input.value);input.onkeydown=event=>{if(event.key==='Escape'){event.preventDefault();event.stopPropagation();input.value=mixed?'':values[0];input.setCustomValidity('');input.blur();}else if(event.key==='Enter'){event.preventDefault();event.stopPropagation();input.blur();}};I.field(sec,'Shared Aspect ratio',input).closest('.inspector-field').querySelector(':scope > span').textContent='Aspect ratio';
    ...
    const reset=I.button('Reset shared aspect ratio',()=>write(null));try{reset.disabled=!ready(true)||infos.every(info=>changeRatio(info.className,scope,null)===(info.className||''));}catch{reset.disabled=true;}sec.append(reset);
   ...
   if(field.picker){
    ...
    const write=value=>{if(!typeActive())return;try{save(Object.fromEntries(infos.map((info,i)=>[info.id,change(info.className,scope,property,value,liveElement(i).ownerDocument,false,liveElement(i))])));}catch(error){I.note(sec,error.message,'refused');}};
    I.fontPicker(group,elements[0].ownerDocument,mixed?'':values[0],write,{mixed,label:'Shared Page font',disabled:!typeActive()||elements.some(el=>el.style.getPropertyPriority('font-family')==='important')});
    if(elements.some(el=>el.style.getPropertyPriority('font-family')==='important'))I.note(sec,'An important inline font family controls a selected layer. Edit that source style first.','refused');
   ...
   const dimension=['width','height'].includes(property),sizing=dimension||field.constraint,values=elements.map((el,index)=>{const css=el.ownerDocument.defaultView.getComputedStyle(el),owned=field.svg?root.RetouchSVGPaint.scopedValue(infos[index].className,scope,property):null;if(owned!==null)return owned;return field.read?field.read(css):sizing&&Number.isFinite(dimensionSize(css,property))?dimensionSize(css,property)+'px':css.getPropertyValue(property);}),mixed=values.some(v=>v!==values[0]),input=root.document.createElement(field.options?'select':'input'),blocked=['visibility','opacity','mix-blend-mode','isolation'].includes(property)&&!typeActive()||elements.some(el=>{const css=el.ownerDocument.defaultView.getComputedStyle(el);return property==='rotate'&&!Number.isFinite(rotationDegrees(css.getPropertyValue('rotate')))||dimension&&sizeContext(el,property).blocked||(['visibility','opacity','mix-blend-mode','isolation'].includes(property)||property==='rotate'||dimension||inlineTypography.includes(property)?typographyKeys(property).some(key=>el.style.getPropertyPriority(key)==='important'):el.style.getPropertyValue(property))||field.layoutItem&&((field.gridPlacement?['grid-area',property+'-start',property+'-end'].some(key=>el.style.getPropertyValue(key)):el.style.getPropertyValue('place-self'))||!itemApplies(el,field))||field.flexItem&&(el.style.getPropertyValue('flex')||!itemApplies(el,field))||sizing&&([...(dimension?[]:['inline-size','block-size']),'min-inline-size','min-block-size','max-inline-size','max-block-size'].some(key=>el.style.getPropertyValue(key))||['inline','contents'].includes(css.display)||dimension&&!Number.isFinite(dimensionSize(css,property)));});
   if(field.options){if(mixed){const o=root.document.createElement('option');o.value='';o.textContent='Mixed';o.disabled=true;input.append(o);}else if(!field.options.includes(values[0])){const o=root.document.createElement('option');o.value=values[0];o.textContent=values[0]||'Custom';o.disabled=true;input.append(o);}for(const value of field.options){const o=root.document.createElement('option');o.value=value;o.textContent=field.optionLabels?.[value]||value;input.append(o);}}
   else if(field.text){input.type='text';input.placeholder=mixed?'Mixed':field.svg?'SVG value':property==='text-decoration-color'?'CSS color or currentColor':field.indent?'24px, -12px, 10%':field.underline?'auto, 2px, 10%':'auto, 100px, 50%';}
   ...
   if(field.constraint&&!mixed&&!display)input.placeholder=values[0]==='auto'?'Automatic':values[0]==='none'?'No limit':values[0];input.value=display;input.disabled=blocked;input.title=blocked?(property==='rotate'?'Edit the selected layer’s important inline or 3D rotation in its source first.':field.layoutItem?'Select in-flow items in '+(field.gridItem?'a grid':'a flex or grid')+' layout without an inline alignment override.':field.flexItem?'Select in-flow items in a flex layout without an inline flex override.':'An inline style controls this property on a selected layer. Edit that source style first.'):'';
   if(['visibility','opacity','mix-blend-mode','isolation'].includes(property)&&!typeActive())input.title='Preview the selected edit range before changing '+property+'.';
   ...
   const write=value=>{if((['visibility','opacity','mix-blend-mode','isolation'].includes(property)||inlineTypography.includes(property))&&!typeActive()||['visibility','opacity','mix-blend-mode','isolation'].includes(property)&&infos.some((_,i)=>!liveElement(i)?.isConnected))return;try{if(value!==null&&property==='rotate'&&infos.some((_,i)=>{const el=liveElement(i);return el.style.getPropertyPriority('rotate')==='important'||!Number.isFinite(rotationDegrees(el.ownerDocument.defaultView.getComputedStyle(el).rotate));}))throw Error('Edit the selected layer’s important inline or 3D rotation in its source first.');if(!infos.every((_,i)=>itemApplies(liveElement(i),field)))throw Error('The selected layers no longer share a compatible parent layout.');if(dimension)return writeSize(value===null?'reset':value==='auto'?'auto':value==='fit-content'?'hug':'fixed',value);save(Object.fromEntries(infos.map((info,i)=>[info.id,property==='rotate'?changeRotation(info.className,scope,value,liveElement(i)):change(info.className,scope,property,sizing?dimensionValue(liveElement(i).ownerDocument.defaultView.getComputedStyle(liveElement(i)),property,value):Array.isArray(value)?value[i]:value,liveElement(i).ownerDocument,false,liveElement(i))])));}catch(error){I.note(sec,error.message,'refused');}};
   input.oninput=()=>input.setCustomValidity('');input.onchange=()=>{if(field.text&&!field.valid(input.value.trim())){input.setCustomValidity(field.svg?'Enter a supported SVG stroke value.':property==='text-decoration-color'?'Enter a supported CSS color or currentColor.':field.indent?'Enter a length or percentage; negative values create a hanging indent.':field.underline?'Enter a supported length, auto, or from-font for thickness.':'Enter auto, content, a positive length such as 100px, or a percentage such as 50%.');input.reportValidity();return;}if(input.value!==''&&input.checkValidity())write(field.options||field.text?input.value.trim():Number(input.value));};input.onkeydown=event=>{if(event.key==='Escape'){event.preventDefault();event.stopPropagation();input.value=display;input.setCustomValidity('');input.blur();}else if(event.key==='Enter'){event.preventDefault();event.stopPropagation();input.blur();}};input.dataset.variableProperty=property;I.field(sec,'Shared '+field.label,input);if(property==='text-decoration-color'){if(!typeActive()){input.disabled=true;input.title='Switch to a screen inside the selected edit range.';}root.RetouchPaintPicker.mountSelectionField(input,elements,property,undefined,values=>write(values));}if(property==='text-align')input.retouchTextAlignments=()=>elements.map(el=>{const css=el.ownerDocument.defaultView.getComputedStyle(el);return ({start:css.direction==='rtl'?'right':'left',end:css.direction==='rtl'?'left':'right'})[css.textAlign]||css.textAlign;});if(field.svg)input.dataset.svgStroke=property;if(property==='stroke-dasharray'){input.retouchPreviewDocument=elements[0].ownerDocument;input.retouchHasScopedValues=()=>infos.every(info=>root.RetouchSVGPaint.scopedValue(info.className,scope,property)!==null);input.retouchDashValues=()=>infos.map((info,i)=>root.RetouchSVGPaint.scopedValue(info.className,scope,property)??liveElement(i).ownerDocument.defaultView.getComputedStyle(liveElement(i)).strokeDasharray);input.retouchSetDashValues=values=>save(Object.fromEntries(infos.map((info,i)=>[info.id,change(info.className,scope,property,values[i],liveElement(i).ownerDocument)])));}
   let percentDisplay=false,mixedPercentages=null;
   if(['font-size','line-height','letter-spacing'].includes(property)){
    ...
    percentDisplay=relative&&percentages.every(value=>value!==null);
    if(percentDisplay&&percentages.some(value=>Math.abs(value-percentages[0])>=.0001)){mixedPercentages=percentages;input.placeholder='Mixed';input.retouchNumericInitialValue=()=>input.value===''?String(percentages[0])+'%':input.value;}
    const initial=mixedPercentages?'':percentDisplay?E.decimal(percentages[0])+'%':input.value;
   ...
   if(['text-decoration-thickness','text-underline-offset','text-indent'].includes(property))I.sharedLengthDrag(input,()=>infos.map((_,i)=>liveElement(i)),property,()=>typeActive()&&infos.every((_,i)=>liveElement(i).style.getPropertyPriority(property)!=='important'),values=>write(values));
   if(!field.options&&!field.text){
    ...
    if(mixedDelta)input.retouchNumericInitialValue=()=>input.value===''?readSize(liveElement(0).ownerDocument.defaultView.getComputedStyle(liveElement(0))):input.value;
    I.numericLabelDrag(input,raw=>{if(mixedDelta&&input.value===''){const css=infos.map((_,i)=>liveElement(i).ownerDocument.defaultView.getComputedStyle(liveElement(i))),sizes=css.map(readSize),base=sizes[0];if(sizes.some(size=>!Number.isFinite(size)))return null;return {value:base,min:base+Math.max(...sizes.map((size,i)=>(dimension?decoration(css[i],property):minimum)-size)),max:base+(dimension?100000:field.max??100)-Math.max(...sizes),format};}return {value:Number(raw),min:minimum,max:field.max??100,format};});
    if(mixedPercentages)input.parentElement.querySelector('span').title='Drag to change each selected spacing by the same number of percentage points. Escape cancels.';
    if(mixedDelta&&mixed){const label=input.parentElement.querySelector('span');label.title='Drag to change each selected '+(['line-height','letter-spacing'].includes(property)?'spacing':'size')+' by the same amount. '+label.title;}
    input.retouchNumericPreview=()=>{
     const targets=infos.map((_,i)=>liveElement(i)),relativeSizes=mixedPercentages&&input.value===''?mixedPercentages:null,mixedSizes=mixedDelta&&input.value===''?targets.map(el=>readSize(el.ownerDocument.defaultView.getComputedStyle(el))):null,previews=targets.map(el=>root.RetouchPaintPicker.propertyPreview({el,input,property,respectScope:true}));
     ...
     return {current:()=>targets.every((el,i)=>el.isConnected&&(!resolveElement||resolveElement(infos[i].id)===el))&&previews.every(preview=>preview.current())&&flexPreviews.every(({preview})=>preview.current()),update:value=>{
      ...
      const values=targets.map((el,i)=>percentDisplay?String((relativeSizes?relativeSizes[i]+amount-relativeSizes[0]:amount)/100)+(property==='letter-spacing'?'em':''):sizing?dimensionValue(el.ownerDocument.defaultView.getComputedStyle(el),property,mixedSizes?mixedSizes[i]+amount-mixedSizes[0]:amount)+'px':property==='opacity'?String(amount/100):property==='rotate'?amount+'deg':['font-size','line-height','letter-spacing'].includes(property)?(mixedSizes?mixedSizes[i]+amount-mixedSizes[0]:amount)+'px':String(amount));
      flexPreviews.forEach(({preview,value})=>preview.update(value));previews.forEach((preview,i)=>preview.update(values[i]));
     },...(relativeSizes?{commit:value=>{if(value===relativeSizes[0]){input.value='';return;}relativeWrite(property,relativeSizes.map(size=>size+value-relativeSizes[0]));}}:{}),...(mixedSizes?{commit:value=>{if(value===mixedSizes[0]){input.value='';return;}const next=mixedSizes.map(size=>size+value-mixedSizes[0]);if(dimension)writeSize('fixed',next);else write(next);}}:{}),restore:()=>{previews.forEach(preview=>preview.restore());flexPreviews.forEach(({preview})=>preview.restore());}};
    };
   }
