 ...
 function mount(infos,elements,scope,save,saveColor,resolveElement,saveBackground){
  ...
  for(const [property,field]of Object.entries(fields).filter(([,field])=>!field.constraint).flatMap(entry=>['width','height'].includes(entry[0])?[entry,...['min-','max-'].map(prefix=>[prefix+entry[0],fields[prefix+entry[0]]])]:[entry])){
   ...
   if(!field.options&&!field.text){
    ...
    if(dimension)input.retouchNumericInitialValue=()=>input.value===''?dimensionSize(liveElement(0).ownerDocument.defaultView.getComputedStyle(liveElement(0)),property):input.value;
    I.numericLabelDrag(input,raw=>{if(dimension&&input.value===''){const css=infos.map((_,i)=>liveElement(i).ownerDocument.defaultView.getComputedStyle(liveElement(i))),sizes=css.map(style=>dimensionSize(style,property)),base=sizes[0];return {value:base,min:base+Math.max(...sizes.map((size,i)=>decoration(css[i],property)-size)),max:base+100000-Math.max(...sizes),format};}return {value:Number(raw),min:minimum,max:field.max??100,format};});
    if(dimension&&mixed){const label=input.parentElement.querySelector('span');label.title='Drag to change each selected size by the same amount. '+label.title;}
    input.retouchNumericPreview=()=>{
     const targets=infos.map((_,i)=>liveElement(i)),mixedSizes=dimension&&input.value===''?targets.map(el=>dimensionSize(el.ownerDocument.defaultView.getComputedStyle(el),property)):null,previews=targets.map(el=>root.RetouchPaintPicker.propertyPreview({el,input,property,respectScope:true}));
