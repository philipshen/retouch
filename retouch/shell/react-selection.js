(function(root){
 ...
 function mount(infos,elements,scope,save,saveColor,resolveElement,saveBackground){
  ...
  const relativeWrite=(property,value,relative=true)=>{try{save(Object.fromEntries(infos.map((info,i)=>[info.id,change(info.className,scope,property,value,liveElement(i).ownerDocument,relative,liveElement(i))])));}catch(error){I.note(groups.typography,error.message,'refused');}};
  ...
  for(const [property,field]of Object.entries(fields).filter(([,field])=>!field.constraint).flatMap(entry=>['width','height'].includes(entry[0])?[entry,...['min-','max-'].map(prefix=>[prefix+entry[0],fields[prefix+entry[0]]])]:[entry])){
   ...
   if(!field.options&&!field.text){
    ...
    input.retouchNumericPreview=()=>{
     const targets=infos.map((_,i)=>liveElement(i)),mixedSizes=mixedDelta&&input.value===''?targets.map(el=>readSize(el.ownerDocument.defaultView.getComputedStyle(el))):null,previews=targets.map(el=>root.RetouchPaintPicker.propertyPreview({el,input,property,respectScope:true}));
