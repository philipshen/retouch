 ...
 function mount(parent,info,element,save){
  ...
  if(!gradients)inspector.note(details,'This background image cannot be edited as a gradient. Clear background images to start a gradient fill.');
   ...
   gradients.forEach((gradient,index)=>{const group=d.createElement('fieldset'),legend=d.createElement('legend'),label='Gradient '+(index+1);legend.textContent=label;group.className='gradient-controls';group.append(legend);const update=next=>write(gradients.map((item,i)=>i===index?next:item));
    ...
    gradient.stops.forEach((stop,stopIndex)=>{const prefix=label+' stop '+(stopIndex+1),row=d.createElement('fieldset'),title=d.createElement('legend');title.textContent='Stop '+(stopIndex+1);row.className='gradient-stop-row';row.append(title);const color=d.createElement('input');color.value=stop.color;color.retouchPaintPreview=()=>root.RetouchPaintPicker.gradientPreview({el:element,group,preview,gradient,gradients,index,stopIndex});inspector.field(row,prefix+' color',color);color.oninput=()=>color.setCustomValidity('');color.onchange=()=>{const value=color.value.trim();if(!V().valid('color',value)||!d.defaultView.CSS.supports('color',value)){color.setCustomValidity('Enter a supported color.');color.reportValidity();return;}update({...gradient,stops:gradient.stops.map((item,i)=>i===stopIndex?{...item,color:value}:item)});};
     const positionInput=inspector.number(row,prefix+' Position (%)',stop.position,0,100,position=>update({...gradient,stops:gradient.stops.map((item,i)=>i===stopIndex?{...item,position}:item).sort((a,b)=>a.position-b.position)}));inspector.fieldDraft(color);inspector.fieldDraft(positionInput);row.insertBefore(positionInput.parentElement,color.parentElement);const remove=inspector.button('Remove '+prefix.toLowerCase(),()=>update({...gradient,stops:gradient.stops.filter((_,i)=>i!==stopIndex)}));remove.setAttribute('aria-label','Remove '+prefix.toLowerCase());remove.title='Remove stop';remove.textContent='−';remove.disabled=gradient.stops.length<=2;row.append(remove);group.append(row);
    });
    group.append(inspector.button('Reverse '+label.toLowerCase()+' stops',()=>update({...gradient,stops:gradient.stops.map(stop=>({...stop,position:100-stop.position})).reverse()})));
    const duplicate=inspector.button('Duplicate '+label.toLowerCase(),()=>{const next=[...gradients];next.splice(index+1,0,gradient);write(next);});duplicate.disabled=gradients.length>=8;group.append(duplicate);
    const addStop=inspector.button('Add '+label.toLowerCase()+' stop',()=>update({...gradient,stops:[...gradient.stops,{color:'#808080',position:50}].sort((a,b)=>a.position-b.position)}));addStop.disabled=gradient.stops.length>=16;group.append(addStop,inspector.button('Remove '+label.toLowerCase(),()=>write(gradients.filter((_,i)=>i!==index))));if(index>0)group.append(inspector.button('Move '+label.toLowerCase()+' up',()=>{const next=[...gradients];[next[index-1],next[index]]=[next[index],next[index-1]];write(next);}));if(index<gradients.length-1)group.append(inspector.button('Move '+label.toLowerCase()+' down',()=>{const next=[...gradients];[next[index],next[index+1]]=[next[index+1],next[index]];write(next);}));details.append(group);
   });
