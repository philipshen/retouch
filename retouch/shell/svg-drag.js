(function(root){
 'use strict';
 function mount({document:d,frame,candidate,prepare,onStart,onError=()=>{},allowAlt=false,preventSelection=false}){
  ...
  listen(d,'pointerdown',e=>{ignore=null;cancel();active=null;if(e.isPrimary===false||e.button!==0||e.ctrlKey||e.metaKey||e.altKey&&!allowAlt)return;const selected=candidate(e.target);if(!selected)return;if(preventSelection)e.preventDefault();pending={selected,event:e,x:e.clientX,y:e.clientY,id:e.pointerId};});
