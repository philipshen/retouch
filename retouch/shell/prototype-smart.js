(function(root){
 ...
 function play({old,next,config}){
  ...
  const animations=[],motions=new Map(),z=[old.style.zIndex,next.style.zIndex],timing={duration:config.duration,easing:root.RetouchPrototypeValues.easingCss(config.easing),fill:'both'};let canceled=false,job=null,disposed=false;
  ...
  try{for(const forward of [true,false]){
   ...
   for(const row of rows){const rect=row.here.el.getBoundingClientRect(),target=row.there.rect;row.delta={x:target.left-rect.left,y:target.top-rect.top};offsets.set(row.here.el,row);}
