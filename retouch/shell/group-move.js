(function(root){
 ...
 function scalePlan(members,factor,offset={x:0,y:0}){
  ...
  const entries=members.map(item=>{const style=item.el.style,values=parse(item.el.ownerDocument.defaultView.getComputedStyle(item.el).scale||'none');if(!values||values.length!==2||values.some(value=>value===0||Math.abs(value*factor)>10000)||style.getPropertyPriority('scale')==='important'&&!item.el.ownerDocument[Symbol.for('retouch.group-scale.runtime')]?.owns(item.el,'scale'))throw Error('Choose editable two-dimensional child scales.');return {item,scale:values.map(value=>String(value*factor)).join(' '),value:style.getPropertyValue('scale'),priority:style.getPropertyPriority('scale'),hadStyle:item.el.hasAttribute('style')};});
  const expected=members.map(item=>({x:left+(item.rect.x-left)*factor+offset.x,y:top+(item.rect.y-top)*factor+offset.y,width:item.rect.width*factor,height:item.rect.height*factor}));let deltas;
 ...
 const api={scaleMovement,translation,measure,measureSelection,selectionBounds,parentBounds,memberDeltas,scalePlan,scalePreview,scaleClasses,classes,multiply,localDelta,parentMatrix,preview};if(typeof module==='object'&&module.exports)module.exports=api;else root.RetouchGroupMove=api;
