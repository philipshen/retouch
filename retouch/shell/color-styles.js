(function(root){
 ...
 function mount(parent,options={}){
  ...
  function render(){
   ...
   const hex=document.createElement('input');hex.type='text';hex.value=normalize(style?.properties.color||'#2563eb');hex.spellcheck=false;hex.placeholder='Hex, color(srgb …) or color(display-p3 …)';I.field(controls,'Color value with alpha',hex);
   ...
   function sync(updateOpacity=true){try{const value=normalize(hex.value.trim());hex.setCustomValidity('');const parsed=root.RetouchPaletteValues.parse(value);spacePicker.value=parsed.space;conversion.replaceChildren();picker.disabled=parsed.space!=='srgb';p3Group.hidden=parsed.space!=='display-p3';if(!p3Group.hidden)p3Inputs.forEach((input,index)=>{input.value=String(parsed.channels[index]);});picker.title=picker.disabled?'Display P3: edit the color value to retain its gamut.':'';if(!picker.disabled)picker.value='#'+parsed.channels.map(n=>Math.round(n*255).toString(16).padStart(2,'0')).join('');if(updateOpacity)opacity.value=String(Number((parsed.alpha*100).toPrecision(12)));swatch.style.backgroundColor=value;swatch.setAttribute('aria-label','Color preview '+value);}catch(error){hex.setCustomValidity(error.message);}}
   if(options.readColor)controls.append(I.button('Use selected layer color',()=>{try{hex.value=fromComputed(options.readColor(target));sync();hex.focus();status.textContent='Layer color copied into the editor. Save to update the palette.';}catch(error){status.textContent=error.message;}}));
   hex.oninput=sync;picker.oninput=()=>{try{const parsed=root.RetouchPaletteValues.parse(hex.value.trim()),channels=root.RetouchPaletteValues.parse(picker.value).channels;hex.value=root.RetouchPaletteValues.srgb(channels,parsed.alpha);sync();}catch{}};opacity.oninput=()=>{if(opacity.value!==''&&opacity.checkValidity()){try{const parsed=root.RetouchPaletteValues.parse(hex.value.trim());hex.value=parsed.space==='display-p3'?root.RetouchPaletteValues.p3(parsed.channels,Number(opacity.value)/100):root.RetouchPaletteValues.srgb(parsed.channels,Number(opacity.value)/100);sync(false);}catch{}}};sync();
