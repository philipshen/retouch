 ...
 function tree(roots,components){
  ...
  for(const component of components)for(const usage of component.usages||[]){
   ...
   for(const instance of group(matches,component.rootGroups)){
    ...
    const item={el:instance.element,componentId:usage.id,movement:usage.movement,label:(usage.layerName||component.name)+' · component',children,parent,componentRoots:instance.elements};
