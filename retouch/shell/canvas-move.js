(function(root){
 ...
 function mount({target,targets=[target],selectionId=target.getAttribute('data-rt'),frame,canvas,mode='move',spacing=null,opener=root.document.activeElement,onCommit,onEnd,onError}){
  ...
  function paint(modifiers){
   if(isSpacing){const amount=(axis==='x'?state.rawX:state.rawY)/scale,gap=Math.max(1/32-Math.min(...gapInfo.sorted.map(item=>item.size)),Math.min(100000,state.gapStart+amount)),deltas=amount===0?rects.map(()=>({x:0,y:0})):root.RetouchSelectionLayout.setSpacing(rects,axis,gap,spacing),next=rects.map((rect,i)=>({left:rect.left+deltas[i].x,top:rect.top+deltas[i].y,width:rect.width,height:rect.height})),box=union(next);state.delta={x:0,y:0,gap,deltas};Object.assign(preview.style,{left:(x+(box.left-r.left)*scale)+'px',top:(y+(box.top-r.top)*scale)+'px',width:box.width*scale+'px',height:box.height*scale+'px'});preview.querySelectorAll('.canvas-group-layer').forEach((ghost,i)=>Object.assign(ghost.style,{left:(next[i].left-box.left)*scale+'px',top:(next[i].top-box.top)*scale+'px'}));paintGaps(deltas);}
