(function(root){
 ...
 function mount({target,frame,canvas,mode='move',opener=root.document.activeElement,onCommit,onEnd,onError}){
  ...
  function paint(modifiers){
   ...
   else{const d=delta(state.rawX,state.rawY,!state.keyboard&&modifiers.shiftKey);state.delta={x:d.x/scale,y:d.y/scale};preview.style.transform=`translate(${d.x}px,${d.y}px)`;}
  }
