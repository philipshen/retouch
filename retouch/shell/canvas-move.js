(function(root){
 ...
 function rotatedBounds(resized,rotation,oldOrigin,newOrigin){
  if([...oldOrigin,...newOrigin].some(n=>!Number.isFinite(n)))throw Error('Use a resolved two-dimensional transform origin.');
  const movement=rotateVector(resized.x,resized.y,rotation),change={x:newOrigin[0]-oldOrigin[0],y:newOrigin[1]-oldOrigin[1]},turned=rotateVector(change.x,change.y,rotation);
 ...
 function mount({target,targets=[target],selectionId=target.getAttribute('data-rt'),frame,canvas,mode='move',spacing=null,preserveBox=targets.length>1,opener=root.document.activeElement,onCommit,onEnd,onError,initial=null}){
  ...
  if(mode==='move'||mode==='resize'&&targets.length===1)for(const item of snapshots)try{const g=mode==='move'?root.RetouchInspector.outlineGeometry(item.el):root.RetouchInspector.geometry(item.el,{allowRotation:true,layoutOnly:true});if(g.rotation||mode==='move'&&(g.scaleX!==1||g.scaleY!==1))item.shape=g;}catch{}
  ...
  if(rotated){preview.style.rotate=rotated.rotation+'deg';preview.style.transformOrigin=rotated.transformOrigin.split(/\s+/).map(value=>parseFloat(value)*scale+'px').join(' ');}
  ...
  function paint(modifiers){
   if(isSpacing){
    ...
    const base=state.base||{x:0,y:0,width:r.width,height:r.height},local=state.keyboard?{x:state.rawX/scale,y:state.rawY/scale}:rotateVector(state.rawX/scale,state.rawY/scale,-rotated.rotation),resized=resize(base.width,base.height,state.handle,local.x,local.y,{...bounds,...modifiers}),oldOrigin=state.base?resizeOrigin(target,base.width,base.height):rotated.transformOrigin.split(/\s+/).slice(0,2).map(parseFloat),nextOrigin=resizeOrigin(target,resized.width,resized.height);
    state.delta=rotatedBounds(resized,rotated.rotation,oldOrigin,nextOrigin);state.delta.x+=base.x;state.delta.y+=base.y;
    Object.assign(preview.style,{left:x+state.delta.x*scale+'px',top:y+state.delta.y*scale+'px',width:state.delta.width*scale+'px',height:state.delta.height*scale+'px',transformOrigin:nextOrigin.map(n=>n*scale+'px').join(' ')});paintGuides([]);
   }
