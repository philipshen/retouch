(function(root){
 ...
 function mount({target,targets=[target],selectionId=target.getAttribute('data-rt'),frame,canvas,mode='move',spacing=null,preserveBox=targets.length>1,opener=root.document.activeElement,onCommit,onEnd,onError,initial=null,measureBounds=el=>el.getBoundingClientRect(),current=()=>true,contentPreview=null}){
  ...
  if(mode==='move'||mode==='resize'&&targets.length===1)for(const item of snapshots)try{const g=mode==='move'?root.RetouchInspector.outlineGeometry(item.el):root.RetouchInspector.geometry(item.el,{allowRotation:true,allowScale:true,layoutOnly:true});if(g.rotation||g.scaleX!==1||g.scaleY!==1)item.shape={...g,sourceScale:item.el.ownerDocument.defaultView.getComputedStyle(item.el).scale};}catch{}
  const rotated=mode==='resize'&&targets.length===1?snapshots[0].shape:null;if(rotated)r={left:rotated.layoutLeft,top:rotated.layoutTop,width:rotated.width,height:rotated.height};
  ...
  (isSpacing?gapHandles[0]:mode==='resize'?preview.querySelector('[data-resize-handle=se]'):surface).focus({preventScroll:true});return cancel;
 }
