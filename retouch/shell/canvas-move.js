(function(root){
 ...
 function rotatedBounds(resized,rotation,oldOrigin,newOrigin,scale=[1,1]){
  if([...oldOrigin,...newOrigin].some(n=>!Number.isFinite(n)))throw Error('Use a resolved two-dimensional transform origin.');
  const movement=rotateVector(resized.x*scale[0],resized.y*scale[1],rotation),change={x:newOrigin[0]-oldOrigin[0],y:newOrigin[1]-oldOrigin[1]},turned=rotateVector(change.x*scale[0],change.y*scale[1],rotation);
  return {...resized,x:movement.x+turned.x-change.x,y:movement.y+turned.y-change.y};
 }
 function resizeOrigin(el,width,height){
  const w=el.ownerDocument.defaultView,css=w.getComputedStyle(el),original=el.getAttribute('style'),number=p=>parseFloat(css.getPropertyValue(p))||0;
  const extra=axis=>css.boxSizing==='content-box'?(axis==='width'?['left','right']:['top','bottom']).reduce((sum,edge)=>sum+number('padding-'+edge)+number('border-'+edge+'-width'),0):0,extraX=extra('width'),extraY=extra('height');
  try{el.style.setProperty('width',Math.max(0,width-extraX)+'px','important');el.style.setProperty('height',Math.max(0,height-extraY)+'px','important');const next=w.getComputedStyle(el),origin=next.transformOrigin.split(/\s+/).map(parseFloat);if(origin.length<2||origin[2]||Math.abs(parseFloat(next.width)+extraX-width)>.1||Math.abs(parseFloat(next.height)+extraY-height)>.1)throw Error('The layer’s sizing rules prevent this canvas resize.');return origin.slice(0,2);}finally{if(original===null){el.setAttribute('style','');el.removeAttribute('style');}else el.setAttribute('style',original);}
 }
 ...
 function mount({target,targets=[target],selectionId=target.getAttribute('data-rt'),frame,canvas,mode='move',spacing=null,preserveBox=targets.length>1,opener=root.document.activeElement,onCommit,onEnd,onError,initial=null,measureBounds=el=>el.getBoundingClientRect(),current=()=>true,contentPreview=null,outlinePoints=null}){
  const isScale=mode==='scale',isResize=mode==='resize'||isScale;
  ...
  let snapshots,r;try{snapshots=targets.map(el=>({el,rect:measureBounds(el),parent:el.offsetParent,parentWidth:el.offsetParent?.clientWidth,parentHeight:el.offsetParent?.clientHeight}));r=union(snapshots.map(item=>item.rect));}catch(error){onError(error.message);return null;}
  if(mode==='move'||isResize&&!isScale&&targets.length===1)for(const item of snapshots)try{const g=mode==='move'?root.RetouchInspector.outlineGeometry(item.el):root.RetouchInspector.geometry(item.el,{allowRotation:true,allowScale:true,layoutOnly:true});if(g.rotation||g.scaleX!==1||g.scaleY!==1)item.shape={...g,sourceScale:item.el.ownerDocument.defaultView.getComputedStyle(item.el).scale};}catch{}
  const rotated=isResize&&!isScale&&targets.length===1?snapshots[0].shape:null;if(rotated)r={left:rotated.layoutLeft,top:rotated.layoutTop,width:rotated.width,height:rotated.height};
  ...
  Object.assign(preview.style,{position:'absolute',left:x+'px',top:y+'px',width:r.width*scale+'px',height:r.height*scale+'px',border:'0',outline:'2px solid var(--accent)',outlineOffset:'-2px',boxSizing:'border-box',background:'rgba(13,153,255,.08)',cursor:'move'});surface.append(preview);if(isSpacing){preview.style.pointerEvents='none';preview.style.background='transparent';}
  if(targets.length===1&&snapshots[0].shape&&!rotated){preview.style.outline='none';preview.style.background='transparent';}
  if(targets.length>1||snapshots[0].shape&&!rotated)for(const {rect,shape}of snapshots){const ghost=root.document.createElement('div');ghost.className='canvas-group-layer';ghost.setAttribute('aria-hidden','true');Object.assign(ghost.style,{position:'absolute',pointerEvents:'none',left:((shape?.layoutLeft??rect.left)-r.left)*scale+'px',top:((shape?.layoutTop??rect.top)-r.top)*scale+'px',width:(shape?.width??rect.width)*scale+'px',height:(shape?.height??rect.height)*scale+'px',outline:(targets.length===1?'2':'1')+'px solid var(--accent)',outlineOffset:'-1px',background:'rgba(13,153,255,.08)'});if(shape){ghost.style.rotate=shape.rotation+'deg';ghost.style.transformOrigin=shape.transformOrigin.split(/\s+/).map(value=>parseFloat(value)*scale+'px').join(' ');}preview.append(ghost);}

  if(outlinePoints&&mode==='move'&&targets.length===1){surface.dataset.localMove=selectionId;
   const svg=root.document.createElementNS('http://www.w3.org/2000/svg','svg'),polygon=root.document.createElementNS(svg.namespaceURI,'polygon');svg.setAttribute('aria-hidden','true');Object.assign(svg.style,{position:'absolute',inset:'0',width:'100%',height:'100%',overflow:'visible',pointerEvents:'none'});polygon.setAttribute('points',outlinePoints.map(p=>[(p.x-r.left)*scale,(p.y-r.top)*scale].join(',')).join(' '));polygon.setAttribute('fill','rgba(13,153,255,.08)');polygon.setAttribute('stroke','var(--accent)');polygon.setAttribute('stroke-width','2');svg.append(polygon);preview.append(svg);preview.style.outline='none';preview.style.background='transparent';
  }
  function paintTransformed(g){const shape=root.RetouchInspector.scaledOutline(g);Object.assign(preview.style,{left:f.left+shape.layoutLeft*scale-left+'px',top:f.top+shape.layoutTop*scale-top+'px',width:shape.width*scale+'px',height:shape.height*scale+'px',rotate:shape.rotation+'deg',transformOrigin:shape.transformOrigin.split(/\s+/).map(value=>parseFloat(value)*scale+'px').join(' ')});}
  if(rotated)paintTransformed(rotated);
  ...
  {const hint=root.document.createElement('div');hint.textContent=isScale?'Drag handles to scale proportionally · Alt: scale from center · Arrow keys: 1px · Enter applies · Escape cancels':isSpacing?spacingHint+' · Arrow keys: 1px · Shift: 10px · Enter applies · Escape cancels':mode==='move'?'Drag to move · Arrow keys: 1px · Shift: 10px / axis lock · Option / Alt: no snapping · Enter applies · Escape cancels':rotated?'Drag handles to resize · Arrows follow local axes · Shift keeps proportions · Option / Alt resizes from center · Escape cancels':'Drag handles to align · Shift keeps proportions · Option / Alt resizes from center · ⌘ / Ctrl disables snapping · Escape cancels';Object.assign(hint.style,{position:'absolute',bottom:'76px',left:'8px',right:'8px',padding:'6px 8px',background:'var(--panel)',color:'var(--ink)',border:'1px solid var(--line)',fontSize:'11px',borderRadius:'6px',boxShadow:'0 2px 8px #00000014',pointerEvents:'none'});surface.append(hint);}
  ...
  function changed(result){return isSpacing?result.deltas?.some(d=>Math.abs(d.x)+Math.abs(d.y)>=1/32):Math.abs(result.x)+Math.abs(result.y)+(isResize?Math.abs(result.width-r.width)+Math.abs(result.height-r.height):0)>1e-6;}
  if(isResize)for(const handle of ['nw','n','ne','e','se','s','sw','w']){const button=root.document.createElement('button');button.type='button';button.dataset.resizeHandle=handle;const name={n:'top',s:'bottom',e:'right',w:'left',ne:'top right',nw:'top left',se:'bottom right',sw:'bottom left'}[handle];button.setAttribute('aria-label',(isScale?'Scale ':'Resize ')+name);button.title=(isScale?'Scale ':'Resize ')+name+'. Arrow keys adjust; Enter applies; Escape cancels.';Object.assign(button.style,{position:'absolute',padding:'0',width:'10px',height:'10px',minWidth:'0',border:'1px solid var(--accent)',borderRadius:'1px',background:'white',left:handle.includes('w')?'0%':handle.includes('e')?'100%':'50%',top:handle.includes('n')?'0%':handle.includes('s')?'100%':'50%',transform:'translate(-50%,-50%)',cursor:root.RetouchCanvasRotate?.resizeCursor(handle,rotated?.rotation||0)||handle+'-resize'});if(rotated){if(rotated.scaleX<0)button.style.left=handle.includes('w')?'100%':handle.includes('e')?'0%':'50%';if(rotated.scaleY<0)button.style.top=handle.includes('n')?'100%':handle.includes('s')?'0%':'50%';button.style.cursor=root.RetouchCanvasRotate.resizeCursor(handle,rotated.rotation,rotated.scaleX,rotated.scaleY);}preview.append(button);}
  const visible=union(snapshots.map(item=>item.rect));if(f.left+(visible.left+visible.width)*scale<=left||f.top+(visible.top+visible.height)*scale<=top||f.left+visible.left*scale>=right||f.top+visible.top*scale>=bottom){onError('Bring the layer into view before moving it.');return null;}
  ...
  function cancel(restoreFocus=false){if(ended)return;ended=true;surface.remove();cleanups.forEach(fn=>fn());contentPreview?.restore();onEnd();if(restoreFocus===true)(opener?.isConnected?opener:root.document.querySelector('[data-canvas-tool='+mode+']'))?.focus({preventScroll:true});}
  function gestureCurrent(){try{return current()&&(!contentPreview||contentPreview.current());}catch{return false;}}
  function paint(modifiers){
   if(!gestureCurrent())throw Error('The selection changed during the gesture.');
   if(isSpacing){
  ...
  listen(surface,'pointerdown',e=>{if(e.button!==0||state)return;e.preventDefault();e.stopImmediatePropagation();const handle=isSpacing?e.target.dataset.spacingIndex:e.target.dataset.resizeHandle;if(isSpacing?handle===undefined:isResize?!handle:e.target!==preview){cancel();return;}state={id:e.pointerId,handle,gapStart:isSpacing?gapInfo.values[Number(handle)]:null,x:e.clientX,y:e.clientY,rawX:0,rawY:0,delta:{x:0,y:0}};surface.setPointerCapture(e.pointerId);});
  ...
  listen(surface,'focusin',e=>{if(state?.keyboard&&isResize&&e.target.dataset.resizeHandle&&e.target.dataset.resizeHandle!==state.handle){state={keyboard:true,handle:e.target.dataset.resizeHandle,id:null,rawX:0,rawY:0,base:state.delta,delta:state.delta};}});
  listen(surface,'pointermove',move);
  function finishPointer(e){if(!state||e.pointerId!==state.id)return;e.preventDefault();move(e);if(ended)return;const result=state.delta,distance=isSpacing?Math.abs(axis==='x'?state.rawX:state.rawY):Math.hypot(state.rawX,state.rawY);cancel();if(distance>=4&&changed(result)&&targets.every(el=>el.isConnected))onCommit(result);}
  listen(surface,'pointerup',finishPointer);
  const frameEvent=e=>({pointerId:e.pointerId,clientX:f.left+e.clientX*scale,clientY:f.top+e.clientY*scale,shiftKey:e.shiftKey,altKey:e.altKey,metaKey:e.metaKey,ctrlKey:e.ctrlKey,preventDefault:()=>e.preventDefault()});
  if(initial?.framePointer){listen(w,'pointermove',e=>move(frameEvent(e)),true);listen(w,'pointerup',e=>finishPointer(frameEvent(e)),true);listen(w,'pointercancel',cancel,true);listen(w,'lostpointercapture',cancel,true);}
  ...
  listen(root,'keydown',e=>{
   ...
   const handle=isSpacing?e.target.dataset.spacingIndex:e.target.dataset.resizeHandle;if(isSpacing?(handle===undefined||direction[axis==='x'?0:1]===0):isResize&&!handle)return;e.preventDefault();e.stopImmediatePropagation();
  ...
  let tick;const observe=()=>{if(ended)return;if(!gestureCurrent()||snapshots.some(({el,rect,parent,parentWidth,parentHeight,sizeLimits,shape},index)=>{let current;try{current=measureBounds(el);}catch{return true;}if(shape){const css=w.getComputedStyle(el);if(root.RetouchReactSelection.rotationDegrees(css.rotate)!==shape.rotation||css.transformOrigin!==(shape.sourceTransformOrigin??shape.transformOrigin)||shape.sourceScale!==undefined&&css.scale!==shape.sourceScale)return true;}if(sizeLimits)try{const next=limits(el,preserveBox);if(Object.keys(sizeLimits).some(key=>next[key]!==sizeLimits[key]))return true;}catch{return true;}return !el.isConnected||el.offsetParent!==parent||parent?.clientWidth!==parentWidth||parent?.clientHeight!==parentHeight||!isScale&&['left','top','width','height'].some(key=>Math.abs(current[key]-rect[key]-(contentPreview?(key==='left'?(isSpacing?state?.delta?.deltas?.[index]?.x:state?.delta?.x)||0:key==='top'?(isSpacing?state?.delta?.deltas?.[index]?.y:state?.delta?.y)||0:0):0))>.5);})){cancel();return;}tick=root.requestAnimationFrame(observe);};tick=root.requestAnimationFrame(observe);cleanups.push(()=>root.cancelAnimationFrame(tick));
