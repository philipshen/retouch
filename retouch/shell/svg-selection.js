(function(root){
 ...
 function capture(infos,elements){
  ...
  if(elements.some(el=>elements.some(parent=>parent!==el&&parent.contains(el))))throw Error('Select a group or its contents separately to transform them together.');
  const members=infos.map((info,i)=>{const el=elements[i],reason=root.RetouchSVGResize.reason(el,info,true);if(reason)throw Error(reason);if(el.getAttribute('transform')!==info.svgTransform.value)throw Error('The vector changed. Re-select it.');const data=root.RetouchSVGResize.measure(el),parent=A().multiply(array(data.m),inverse(info.svgTransform.matrix));return {info,el,parent,rect:el.getBoundingClientRect()};});
  const left=Math.min(...members.map(m=>m.rect.left)),top=Math.min(...members.map(m=>m.rect.top)),right=Math.max(...members.map(m=>m.rect.right)),bottom=Math.max(...members.map(m=>m.rect.bottom));return {members,box:{left,top,width:right-left,height:bottom-top},w:elements[0].ownerDocument.defaultView};
 }
 function mount(infos,elements,{save,current}){
  ...
  function apply(kind,value){if(!current())throw Error('Re-select these vectors before transforming.');const {members,box,w}=capture(infos,elements),cx=box.left+box.width/2,cy=box.top+box.height/2;let g;
   ...
   const matrices=Object.fromEntries(members.map(m=>[m.info.id,transform(m.parent,g,m.info.svgTransform.matrix)]));if(Object.values(matrices).some(m=>!A().valid(m)))throw Error('Keep every transformed vector within the supported range.');if(members.some(m=>!A().equivalent(matrices[m.info.id],m.info.svgTransform.matrix)))save(matrices);
  }
 ...
 const api={inverse,transform,capture,mount};if(typeof module==='object'&&module.exports)module.exports=api;else root.RetouchSVGSelection=api;
