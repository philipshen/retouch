(function(root){
 ...
 function gapControls({frame,canvas}){
  const host=root.document.createElement('div');Object.assign(host.style,{position:'fixed',overflow:'hidden',pointerEvents:'none',zIndex:32});host.hidden=true;root.document.body.append(host);let key=null,axis=null,buttons=[],scale=1;
  function clear(){for(const item of buttons){item.unbind?.();item.line.remove();item.button.remove();}buttons=[];}
  return {active:(infos,value)=>key===selectionKey(infos)&&axis===value,toggle(infos,value){const next=selectionKey(infos);axis=key===next&&axis===value?null:value;key=next;clear();if(!axis)host.hidden=true;},update(infos,elements){
   if(!infos||key!==selectionKey(infos)||!axis||!elements?.every(Boolean)){host.hidden=true;return;}
   try{const outer=elements.filter(el=>!elements.some(parent=>parent!==el&&parent.contains(el))),rects=outer.map(el=>el.getBoundingClientRect()),{sorted,values}=root.RetouchSelectionLayout.gaps(rects,axis),f=frame.getBoundingClientRect(),c=canvas.getBoundingClientRect(),left=Math.max(f.left,c.left),top=Math.max(f.top,c.top),right=Math.min(f.right,c.right),bottom=Math.min(f.bottom,c.bottom);scale=f.width/frame.contentWindow.innerWidth;
    if(buttons.length!==values.length)clear();Object.assign(host.style,{left:left+'px',top:top+'px',width:Math.max(0,right-left)+'px',height:Math.max(0,bottom-top)+'px'});
    values.forEach((value,index)=>{const label=(axis==='x'?'Horizontal':'Vertical')+' gap '+(index+1),input=root.document.querySelector('[data-svg-gap-axis="'+axis+'"][data-svg-gap-index="'+index+'"]');if(!input?.retouchNumericHandle)return;
     let item=buttons[index];if(!item){const button=root.document.createElement('button');button.type='button';button.dataset.svgCanvasGap='';button.setAttribute('aria-label','Adjust '+label.toLowerCase()+' on canvas');button.title='Drag or use arrow keys to adjust this gap · Shift: coarse · Option: fine · Escape cancels';Object.assign(button.style,{position:'absolute',pointerEvents:'auto',touchAction:'none',padding:'2px 5px',border:'0',borderRadius:'3px',background:'#a21caf',color:'white',fontSize:'11px',lineHeight:'16px',whiteSpace:'nowrap',transform:'translate(-50%,-50%)'});const line=root.document.createElement('div');line.setAttribute('aria-hidden','true');Object.assign(line.style,{position:'absolute',background:'#a21caf',opacity:'.65',pointerEvents:'none'});for(let i=0;i<2;i++){const tick=root.document.createElement('i');Object.assign(tick.style,{position:'absolute',background:'#a21caf'});line.append(tick);}host.append(line,button);item=buttons[index]={button,line};}
     if(item.input!==input){item.unbind?.();item.input=input;item.unbind=input.retouchNumericHandle(item.button,{axis,canvas:true,scale:()=>scale,onCommit:()=>{item.restoreFocus=true;}});item.button.onclick=event=>{if(event.detail===0){for(let parent=input.parentElement;parent;parent=parent.parentElement)if(parent.tagName==='DETAILS')parent.open=true;input.scrollIntoView({block:'nearest'});input.focus({preventScroll:true});input.select();}};}
     const a=rects[sorted[index].i],b=rects[sorted[index+1].i],x=axis==='x'?(a.left+a.width+b.left)/2:(a.left+a.width/2+b.left+b.width/2)/2,y=axis==='y'?(a.top+a.height+b.top)/2:(a.top+a.height/2+b.top+b.height/2)/2,px=f.left+x*scale-left,py=f.top+y*scale-top;const text=String(Math.round(value*100)/100);if(item.button.textContent!==text)item.button.textContent=text;item.button.style.left=px+'px';item.button.style.top=py+'px';item.button.style.cursor=axis==='x'?'ew-resize':'ns-resize';item.button.hidden=px<20||py<12||px>right-left-20||py>bottom-top-12;const from=(axis==='x'?a.left+a.width:a.top+a.height)*scale+(axis==='x'?f.left-left:f.top-top),to=(axis==='x'?b.left:b.top)*scale+(axis==='x'?f.left-left:f.top-top);Object.assign(item.line.style,{left:(axis==='x'?Math.min(from,to):px)+'px',top:(axis==='y'?Math.min(from,to):py)+'px',width:axis==='x'?Math.abs(to-from)+'px':'1px',height:axis==='y'?Math.abs(to-from)+'px':'1px'});item.line.hidden=item.button.hidden;[...item.line.children].forEach((tick,i)=>Object.assign(tick.style,{left:axis==='x'?i*100+'%':'-3px',top:axis==='y'?i*100+'%':'-3px',width:axis==='x'?'1px':'7px',height:axis==='y'?'1px':'7px'}));
    });host.hidden=false;for(const item of buttons)if(item.restoreFocus){item.restoreFocus=false;if(root.document.activeElement===root.document.body||root.document.activeElement===item.button)item.button.focus({preventScroll:true});}
 ...
 function mount(infos,elements,{save,current,onGaps,gapsActive}){
  ...
  if(onGaps&&count>1){const tools=root.document.createElement('div');tools.className='stack-presets';for(const axis of ['x','y']){const button=I.button('Adjust '+(axis==='x'?'horizontal':'vertical')+' gaps on canvas',()=>{onGaps(axis);for(const b of tools.children)b.setAttribute('aria-pressed',String(gapsActive(b.dataset.gapAxis)));});button.setAttribute('aria-label','Adjust '+(axis==='x'?'horizontal':'vertical')+' gaps on canvas');button.textContent=axis==='x'?'Horizontal gaps':'Vertical gaps';button.dataset.gapAxis=axis;button.setAttribute('aria-pressed',String(gapsActive(axis)));tools.append(button);}section.append(tools);}
  ...
  for(const [axis,label]of [['x','Horizontal gap (px)'],['y','Vertical gap (px)']]){
   ...
   root.RetouchNumericExpression.field(input);I.field(spacing,label,input);input.parentElement.querySelector('span').textContent=axis==='x'?'H gap':'V gap';inputs['gap-'+axis]=input;if(count===2){input.dataset.svgGapAxis=axis;input.dataset.svgGapIndex='0';}scrub(input,'gap-'+axis,calculate);
  }
  if(count>2){
   ...
   for(const axis of ['x','y']){const outer=initial.members.filter(m=>!m.covered),{sorted,values}=root.RetouchSelectionLayout.gaps(outer.map(m=>m.rect),axis);values.forEach((gap,index)=>{
    ...
    root.RetouchNumericExpression.field(input);I.field(individual,(axis==='x'?'Horizontal':'Vertical')+' gap '+(index+1),input);inputs[fieldKey]=input;input.dataset.svgGapAxis=axis;input.dataset.svgGapIndex=String(index);individualInputs.push({key:fieldKey,input,axis,index});scrub(input,fieldKey,calculate);
   });}
  }
