(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.RetouchHTMLCSSValues=api;})(typeof window==='object'?window:globalThis,function(){
 ...
 function flexAlignment(x,y,{writingMode='horizontal-tb',direction='ltr',flexDirection='row'}={}){
  ...
  const mainSign=(row?inlineSign:blockSign)*(flexDirection.endsWith('-reverse')?-1:1),crossSign=row?blockSign:inlineSign;
  const value=(position,sign)=>position===1?'center':(position===0)===(sign===1)?'flex-start':'flex-end';
  return {'justify-content':value(mainHorizontal?x:y,mainSign),'align-items':value(mainHorizontal?y:x,crossSign)};
 }
