(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.RetouchHTMLCSSValues=api;})(typeof window==='object'?window:globalThis,function(){
 ...
 const ligatureGroups=[['Common ligatures','common-ligatures'],['Rare ligatures','discretionary-ligatures'],['Historical ligatures','historical-ligatures'],['Contextual alternates','contextual']].map(([label,on])=>[label,[['','Font default'],[on,'On'],['no-'+on,'Off']]]);
 function ligatureValid(value){
  if(value==='normal'||value==='none')return true;if(typeof value!=='string'||!value.trim()||value.length>200)return false;const seen=new Set();
  return value.trim().split(/\s+/).every(token=>{const index=ligatureGroups.findIndex(([,choices])=>choices.some(([v])=>v===token));if(index<0||seen.has(index))return false;seen.add(index);return true;});
 }
 function ligatureChange(current,label,value){
  const group=ligatureGroups.find(([name])=>name===label);if(!group||!group[1].some(([v])=>v===value)||!ligatureValid(current))return null;
  const parts=current==='normal'?[]:current==='none'?ligatureGroups.map(([,choices])=>choices[2][0]):current.trim().split(/\s+/),kept=parts.filter(token=>!group[1].some(([v])=>v===token));if(value)kept.push(value);return kept.join(' ')||'normal';
 }
 const options={'justify-self':['auto','normal','start','end','center','stretch','flex-start','flex-end','self-start','self-end','left','right','baseline','first baseline','last baseline'],'align-self':['auto','normal','start','end','center','stretch','flex-start','flex-end','self-start','self-end','baseline','first baseline','last baseline'],'text-box':['normal','trim-both cap alphabetic'],'white-space':['normal','pre','nowrap','pre-wrap','pre-line','break-spaces'],'text-wrap':['wrap','nowrap','balance','pretty','stable',...['wrap','nowrap'].flatMap(mode=>['balance','pretty','stable'].map(style=>mode+' '+style))],'text-decoration-style':['solid','double','dotted','dashed','wavy'],'text-decoration-skip-ink':['auto','none','all'],'font-variant-position':['normal','sub','super'],'font-variant-caps':['normal','small-caps','all-small-caps','petite-caps','all-petite-caps','unicase','titling-caps'],'font-optical-sizing':['auto','none'],visibility:['visible','hidden'],overflow:['visible','hidden','clip','auto','scroll'],'overflow-x':['visible','hidden','clip','auto','scroll'],'overflow-y':['visible','hidden','clip','auto','scroll'],'mix-blend-mode':['normal','multiply','screen','overlay','darken','lighten','color-dodge','color-burn','hard-light','soft-light','difference','exclusion','hue','saturation','color','luminosity','plus-lighter'],isolation:['auto','isolate'],'font-style':['normal','italic','oblique'],'text-decoration-line':['none','underline','line-through','overline','underline line-through'],'text-transform':['none','uppercase','lowercase','capitalize'],'object-fit':['cover','contain','fill','none','scale-down'],display:['block','inline-block','flex','inline-flex','grid','inline-grid','none'],'flex-direction':['row','column','row-reverse','column-reverse'],'flex-wrap':['nowrap','wrap','wrap-reverse'],'align-content':['flex-start','flex-end','center','stretch','space-between','space-around','space-evenly'],'align-items':['flex-start','flex-end','start','center','end','stretch','baseline'],'justify-content':['flex-start','flex-end','start','center','end','space-between','space-around','space-evenly'],'text-align':['start','end','left','center','right','justify'],'border-style':['none','solid','dashed','dotted','double']};
 ...
 Object.assign(options,{'justify-items':['normal','start','center','end','stretch','baseline'],position:['static','relative','absolute','fixed','sticky'],'box-sizing':['border-box','content-box'],'stroke-linecap':['butt','round','square'],'stroke-linejoin':['miter','round','bevel'],'vector-effect':['none','non-scaling-stroke']});
 const sides=['top','right','bottom','left'];
 for(const side of sides)options['border-'+side+'-style']=options['border-style'];
 ...
 function alignmentProperties(css={}){return ['grid','inline-grid'].includes(css.display)?['justify-items','align-items']:['justify-content','align-items','align-content'];}
 // Grid items align inside their grid areas on the container's logical axes.
 function childAlignment(x,y,css={}){
  if(!['grid','inline-grid'].includes(css.display))return flexAlignment(x,y,css);
  const mode=css.writingMode||'horizontal-tb',vertical=/^(vertical|sideways)/.test(mode),inlineSign=(css.direction==='rtl'?-1:1)*(mode==='sideways-lr'?-1:1),blockSign=vertical&&mode.endsWith('-rl')?-1:1;
  const value=(position,sign)=>position===1?'center':(position===0)===(sign===1)?'start':'end';
  return {'justify-items':value(vertical?y:x,inlineSign),'align-items':value(vertical?x:y,blockSign)};
 }
 ...
 function parseBorderColors(value){if(typeof value!=='string'||!value.trim()||value.length>1024)return null;const parts=shadowTokens(value.trim());return parts&&parts.length>=1&&parts.length<=4&&parts.every(part=>valid('color',part,false))?parts:null;}
 // Source-backed hidden paints retain their original framing and paint identity.
 // The hex CSS identifier cannot introduce declarations or executable URL syntax.
 const paintVisibilityProperty='--rt-hidden-paints';
 function parsePaintVisibility(value){
  if(value==='none')return [];
  if(typeof value!=='string'||value.length>40000||!/^rtpv1-(?:[a-f0-9]{2})+$/.test(value))throw Error('The stored paint visibility is invalid.');
  let data;try{data=JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(Uint8Array.from(value.slice(6).match(/../g),pair=>parseInt(pair,16))));}catch{throw Error('The stored paint visibility is invalid.');}
  if(!data||data.version!==1||Object.keys(data).sort().join(',')!=='paints,version'||!Array.isArray(data.paints)||data.paints.length>8)throw Error('The stored paint visibility is invalid.');
  const slots=new Set();for(const entry of data.paints){
   if(!entry||Object.keys(entry).sort().join(',')!=='index,paint,size'||!Number.isInteger(entry.index)||entry.index<0||entry.index>7||slots.has(entry.index)||typeof entry.paint!=='string'||parseGradients(entry.paint)?.length!==1||typeof entry.size!=='string'||splitLayers(entry.size)?.length!==1||!valid('background-size',entry.size))throw Error('The stored paint visibility is invalid.');
   slots.add(entry.index);
  }
  return data.paints;
 }
 function serializePaintVisibility(paints){
 ...
 function variableNumberValue(property,value,unit){return property==='letter-spacing'&&unit==='%'?String(value/100)+'em':String(value)+unit;}
 function valid(property,value,allowVariable=true){
  ...
  if(property==='border-color')return value===null||parseBorderColors(value)!==null;
  ...
  if(value===null)return ['flex-grow','flex-shrink','grid-template-columns','grid-template-rows','grid-column','grid-row','opacity','rotate','scale','translate','object-position'].includes(property)||lengths.has(property)||colors.has(property)||Object.hasOwn(options,property);
  ...
  return parts.every(token=>{
   ...
   if(numeric){
    if(numeric[1]&&!['letter-spacing','text-indent','text-underline-offset'].includes(property)&&!property.startsWith('margin')&&!sides.includes(property))return false;
 ...
 function serializeGradients(gradients){return gradients.length?gradients.map(g=>g.type==='image'?g.value:`${g.repeat?'repeating-':''}${g.type}-gradient(${g.type==='linear'?g.angle+'deg':g.type==='conic'?'from '+g.angle+'deg at '+g.x+'% '+g.y+'%':g.shape+(g.size?' '+g.size:'')+' at '+g.x+'% '+g.y+'%'}${g.colorSpace?' in '+g.colorSpace+(g.hue?' '+g.hue+' hue':''):''}, ${g.stops.map(s=>s.color+' '+s.position+'%').join(', ')})`).join(', '):'none';}
 ...
 function overlaps(a,b){const logicalSize=/^(min-|max-)?(?:inline|block)-size$/,physicalSize=/^(min-|max-)?(?:width|height)$/;if(logicalSize.test(a)&&physicalSize.test(b)||logicalSize.test(b)&&physicalSize.test(a)){const family=p=>/^(min-|max-)/.exec(p)?.[0]||'';if(family(a)===family(b))return true;}if(a==='line-clamp'&&['display','overflow','overflow-x','overflow-y','-webkit-line-clamp','-webkit-box-orient'].includes(b)||b==='line-clamp'&&['display','overflow','overflow-x','overflow-y','-webkit-line-clamp','-webkit-box-orient'].includes(a))return true;if(a==='-webkit-backdrop-filter')a='backdrop-filter';if(b==='-webkit-backdrop-filter')b='backdrop-filter';return a==='all'||b==='all'||/^inset-(?:inline|block)(?:-(?:start|end))?$/.test(a)&&sides.includes(b)||/^inset-(?:inline|block)(?:-(?:start|end))?$/.test(b)&&sides.includes(a)||a==='background'&&b.startsWith('background-')||b==='background'&&a.startsWith('background-')||a==='flex'&&['flex-grow','flex-shrink','flex-basis'].includes(b)||a==='grid'&&b.startsWith('grid-')||a==='grid-template'&&b.startsWith('grid-template-')||a==='grid-area'&&['grid-row','grid-column'].includes(b)||affected(a).some(p=>affected(b).includes(p))||a==='border'&&b.startsWith('border-')&&!b.endsWith('radius')||b==='border'&&a.startsWith('border-')&&!a.endsWith('radius')||['font','font-variant'].includes(a)&&['font-variant-numeric','font-variant-ligatures','font-variant-caps','font-variant-position'].includes(b)||['font','font-variant'].includes(b)&&['font-variant-numeric','font-variant-ligatures','font-variant-caps','font-variant-position'].includes(a)||a==='font'&&['font-family','font-weight','font-style','font-size','line-height','font-variation-settings','font-optical-sizing'].includes(b)||a==='text-decoration'&&b==='text-decoration-line';}
 return {variableNumberValue,hiddenBackgroundProperty,parseHiddenBackground,serializeHiddenBackground,paintVisibilityProperty,parsePaintVisibility,serializePaintVisibility,imageURL,imageLayers,splitLayers,ligatureGroups,ligatureValid,ligatureChange,parseBorderColors,variableCycle,variableName,variableValue,parseVariations,serializeVariations,numericGroups,numericValid,numericChange,options,fields,svgFields,families,adaptiveColumns,parseAdaptiveColumns,stackLayout,flexAlignment,childAlignment,alignmentProperties,valid,overlaps,parseShadows,serializeShadows,parseFilters,withBlur,parseGradients,serializeGradients,gradientColorSpaces};
});
