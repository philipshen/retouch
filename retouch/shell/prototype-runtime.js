(function(root){
 'use strict';
 const V=root.RetouchPrototypeValues,frame=document.getElementById('app');let running=false,release=null,trail=[],pendingScroll=null;
 function current(){try{const w=frame.contentWindow;if(w.location.origin!==location.origin)return null;return {url:w.location.pathname+w.location.search+w.location.hash,x:w.scrollX,y:w.scrollY};}catch{return null;}}
 function navigate(destination,scroll){const url=new URL(destination,location.href);pendingScroll={...scroll,url:url.href};frame.src=url.pathname+url.search+url.hash;}
 function perform(item){const before=current();if(!before)return;
  if(item.action==='navigate'){if(!V.route(item.destination))return;trail.push(before);if(trail.length>100)trail.shift();navigate(item.destination,item.preserveScroll?before:{x:0,y:0});}
  else if(item.action==='back'){const previous=trail.pop();if(previous)navigate(previous.url,previous);}
  else if(item.action==='scroll'){const d=frame.contentDocument,targets=[...d.querySelectorAll('[id]')].filter(el=>el.id===item.destination);if(targets.length!==1){root.RetouchPresentationHost.error('The scroll destination is missing or duplicated.');return;}targets[0].scrollIntoView({behavior:'instant',block:'start'});}
 }
 function mount(){release?.();release=null;if(!running)return;
  ...
  function handle(event,trigger){if(!running||event.isComposing||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey||event.type==='click'&&event.button!==0)return;const match=find(event,trigger);if(!match)return;if(trigger!=='click'&&match.el.contains(event.relatedTarget))return;event.preventDefault();event.stopImmediatePropagation();perform(match.item);}
  ...
  release=()=>{cancelScroll();observer.disconnect();listeners.forEach(fn=>fn());style.remove();for(const [el,before]of changed)restore(el,before);};
  if(pendingScroll){const scroll=pendingScroll;pendingScroll=null;if(d.URL===scroll.url){
 ...
 root.RetouchPrototypeRuntime={start(){running=true;trail=[];pendingScroll=null;mount();},stop(){running=false;release?.();release=null;trail=[];pendingScroll=null;},restart(){trail=[];pendingScroll=null;}};
})(window);
