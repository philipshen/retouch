(function(){
  ...
  function openAddScreen(){
    ...
    const presetLabel=document.createElement('label'),preset=document.createElement('select');presetLabel.textContent='Start from';preset.setAttribute('aria-label','Start from');presetLabel.append(preset);preset.add(new Option('Custom size','custom'));
    const choices=new Map(),seen=new Set();let suggestedName='';
    for(const option of document.querySelectorAll('#screenPreset option')){
      const match=option.value.match(/^(?:saved:)?(\d+)x(\d+)$/);if(!match)continue;
      const w=Number(match[1]),h=Number(match[2]);if(!valid(w)||!valid(h))continue;
      const title=option.textContent.replace(new RegExp(' · '+w+' × '+h+'$'),'');
      for(const [label,x,y]of [[title,w,h],[title+(w<h?' landscape':' portrait'),h,w]]){
        const key=x+'x'+y;if(seen.has(key))continue;seen.add(key);choices.set(key,{label,width:x,height:y});preset.add(new Option(label+' · '+x+' × '+y,key));
      }
    }
    preset.onchange=()=>{const choice=choices.get(preset.value);if(!choice)return;width.input.value=choice.width;height.input.value=choice.height;name.input.setCustomValidity('');width.input.setCustomValidity('');if(!name.input.value.trim()||name.input.value===suggestedName){let label=choice.label.slice(0,70),index=2;while(sizes.some(size=>size[0].toLowerCase()===label.toLowerCase())){const suffix=' '+index++;label=choice.label.slice(0,70-suffix.length)+suffix;}suggestedName=label;name.input.value=label;}updateExisting();};
    const existing=document.createElement('button');existing.type='button';existing.className='screen-add-existing';existing.setAttribute('aria-label','Show existing screen');
    function matchingScreen(){return sizes.find(size=>size[1]===Number(width.input.value)&&size[2]===Number(height.input.value));}
    function updateExisting(){const match=matchingScreen();existing.hidden=!match;existing.textContent=match?'Show '+match[0]+' instead':'';}
    existing.onclick=()=>{const match=matchingScreen();if(!match||!window.RetouchComparisons.canShowSize({width:match[1],height:match[2]}))return;dialog.close('shown');window.RetouchComparisons.showSize({width:match[1],height:match[2]});};
    const footer=document.createElement('footer'),cancel=document.createElement('button'),submit=document.createElement('button');cancel.type='button';cancel.textContent='Cancel';cancel.onclick=()=>dialog.close();submit.type='submit';submit.textContent='Add screen';submit.className='screen-add-submit';footer.append(cancel,submit);form.append(title,hint,presetLabel,name.wrap,dimensions,existing,footer);dialog.append(form);updateExisting();
    for(const input of [name.input,width.input,height.input])input.oninput=()=>{name.input.setCustomValidity('');width.input.setCustomValidity('');if(input!==name.input)preset.value='custom';updateExisting();};
    ...
    dialog.addEventListener('close',()=>{dialog.remove();addScreenDialog=null;if(!['created','shown'].includes(dialog.returnValue)&&addScreenButton?.isConnected)addScreenButton.focus();},{once:true});document.body.append(dialog);dialog.showModal();name.input.focus();
  }
