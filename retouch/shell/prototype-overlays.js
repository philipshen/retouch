(function(root){
 'use strict';
 const V=root.RetouchPrototypeValues,M=root.RetouchPrototypeMotion,stage=document.getElementById('siteStage'),base=document.getElementById('app'),closeButton=document.getElementById('closePrototypeOverlay'),stack=[];let latest=Promise.resolve(true);
 function begin(entry){entry.completion=root.RetouchPrototypeCompletion.create();latest=entry.completion.finished;}
 ...
 function cancel(entry){entry.completion?.finish(false);entry.serial++;entry.controller?.abort();entry.motion?.cancel();entry.motion=null;if(entry.pending){removeFrame(entry,entry.pending);entry.pending=null;}}
 ...
 function close(restoreFocus=true,transition=null,force=false){
  ...
  const wasLoading=entry.phase==='loading';cancel(entry);begin(entry);entry.phase='closing';entry.card.inert=true;entry.status.hidden=true;
  ...
  entry.motion.finished.then(ok=>{if(entry.serial!==serial||stack.at(-1)!==entry)return;if(ok){entry.completion.finish(true);finalize(entry,restoreFocus);}else entry.completion.finish(false);});return true;
 }
 ...
 function swap(destination,opener,transition={type:'instant'}){
  ...
  cancel(entry);begin(entry);for(const frame of [...entry.frames])if(frame!==entry.frame)removeFrame(entry,frame);const old=entry.frame;entry.phase='swapping';entry.card.inert=true;entry.card.setAttribute('aria-busy','true');entry.status.hidden=false;
 ...
 root.RetouchPrototypeOverlays={open,swap,close,clear,get finished(){return latest;},get topFrame(){return stack.at(-1)?.frame||null;},get count(){return stack.length;},get phase(){return stack.at(-1)?.phase||'idle';},get animations(){return stack.at(-1)?.motion?.animations||[];}};
})(window);
