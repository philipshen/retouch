...
function hookFrame(d, w) {
  ...
  d.addEventListener('keydown', (e) => {
    ...
    if(alignmentShortcut(e)||opacityShortcut(e)||visibilityShortcut(e)||canvasZoomShortcut(e)||lockShortcut(e)||layerNavigationShortcut(e)||canvasLayerShortcut(e))return;
    if (editing) {
...
function moveHTMLLayer(info,target,width,g,action='move',opener){
  ...
  try{if(!Number.isInteger(width)||width>target.ownerDocument.defaultView.innerWidth)throw Error('Choose a screen where this scope is active.');if(target.ownerDocument.defaultView.getComputedStyle(target).position!=='absolute')throw Error('Choose an absolute layer in the current screen.');g=RetouchInspector.geometry(target,{allowRotation:action==='move'});}catch(error){toast(error.message,'err');return;}
...
window.addEventListener('keydown', (e) => {
  ...
  if(alignmentShortcut(e)||opacityShortcut(e)||visibilityShortcut(e)||canvasZoomShortcut(e)||lockShortcut(e)||((e.metaKey||e.ctrlKey)&&['[',']','{','}'].includes(e.key)&&canvasLayerShortcut(e)))return;
