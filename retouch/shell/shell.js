...
function transformHTMLSelection(elements,commit,opener,action='move',spacing=null){
  ...
  stopDrawing=RetouchCanvasMove.mount({target:elements[0],targets:elements,selectionId:activeId(),frame:iframe,canvas:canvasSurface,mode:action,spacing,opener,
    onCommit:async(delta,options)=>{if(sel?.info.hash!==hash||styleScope!==scope||sel.multiple?.map(info=>info.id).sort().join(',')!==key)return;try{if(![delta.x,delta.y,...(action==='resize'?[delta.width,delta.height]:spacing?[delta.gap]:[])].every(n=>Number.isFinite(n)&&Math.abs(n)<=100000))throw Error('Keep selection bounds within 100,000 pixels.');await commit(delta);if(options?.keyboard)document.querySelector('[data-canvas-tool='+action+']')?.focus({preventScroll:true});}catch(error){toast(error.message,'err');}},
    onEnd:()=>{stopDrawing=null;},onError:message=>toast(message,'err')});
  if(stopDrawing)toast(spacing?'Drag a gap label to set equal spacing. Arrow keys: 1px; Shift: 10px. Enter applies; Escape cancels.':action==='resize'?'Drag a selection handle. Shift keeps proportions; Option/Alt centers. Enter applies keyboard changes; Escape cancels.':'Drag the selection outline or use arrow keys (Shift: 10px). Enter applies keyboard changes; Escape cancels.','ok');
}
