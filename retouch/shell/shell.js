...
function renderPanelContents(textEditing=false) {
  ...
  if(info.svgStrokeOwner||(sel.multiple||[]).some(item=>item.svgStrokeOwner)){
   ...
   if(sel.multiple?.length>1){
    ...
    if(infos.every(item=>item.svgStrokeSource))mountStrokeSelectionControls(section,infos);
    else RetouchInspector.note(section,'Select aligned vectors to edit their stroke settings together.');
   }
  ...
  if(!sel.multiple?.length&&info.linkedStyleAuthoring!==false&&(info.cssAuthoring||info.classEffectStyles)&&!info.effectStyleLinkReason){
...
function componentSection(id) {
  ...
  api('GET', componentUrl(id,sel?.info?.context)).then(component => {
    ...
    componentClipboardActions(sec,[component],[{id,hash:component.usageHash}],panelSelectionKey());
...
function mountLayerStyleClipboard(){
 const infos=sel.multiple||[sel.info];if(!infos.every(info=>info.kind==='host'&&(info.cssAuthoring||info.classSelection&&!info.classNameDynamic&&(!info.contextSelection||info.classSourceLiteral))&&!info.svgBooleanOwner))return;
 ...
 const paste=RetouchInspector.button('Paste styles',async()=>{try{
  if(infos[0].cssAuthoring&&!Number.isInteger(width)||!selectionEditRangeActive())throw Error('Choose an active screen scope before pasting styles.');
  await RetouchStyleClipboard.open({infos,elements,opener:paste,scopeLabel:scope?(Number.isInteger(width)?'Applies at '+width+' px and larger.':'Applies in '+(document.querySelector('[aria-label="Style screen scope"] option:checked')?.textContent||scope)+'.'):'Applies at all sizes.',save:targets=>pasteLayerStyles(infos,key,scope,width,targets)});
...
async function pasteLayerStyles(infos,key,scope,width,targets){
 ...
 if(!selectionEditRangeActive())throw Error('Choose a screen where the paste range is active.');
 for(const target of targets){const info=infos.find(info=>info.id===target.id),el=matchingEls(target.id)[0];if(!info||!el)throw Error('Re-select the target layers.');const current=RetouchStyleClipboard.target(info,el);if(target.properties.some(property=>!current.props.find(p=>p.name===property.name)?.editor.editable))throw Error('An important inline rule now controls a selected property. Review the paste again.');}
 busyPanel(true);try{
  const first=infos.find(info=>info.id===targets[0].id),multi=targets.length>1,changesById=Object.fromEntries(targets.map(target=>[target.id,first.cssAuthoring?RetouchStyleClipboard.cssChanges(target.properties,doc().URL):RetouchStyleClipboard.sourceChanges(target.properties,doc().URL)]));
  const before=Object.fromEntries(infos.map(info=>[info.id,info.className])),targetInfos=targets.map(target=>infos.find(info=>info.id===target.id));
  const result=await api('POST','/rt/__api/op',first.cssAuthoring?{type:multi?'setCSSSelection':'setCSS',id:first.id,fileHash:first.hash,width,...(multi?{ids:targets.map(target=>target.id),changesById}:{changes:changesById[first.id]})}:{type:'pasteClassStylesSelection',id:first.id,fileHash:first.hash,ids:targets.map(target=>target.id),changesById,scope,context:first.context,...selectionSourceContexts(targetInfos)});if(!result?.ok)throw Error(result?.reason||result?.error||'Could not paste styles.');
  const selectionIds=infos.map(info=>info.id),changedIds=targets.map(target=>target.id);if(result.undoId)editorHistory.record({type:'pasteLayerStyles',id:first.id,selectionIds,changedIds,classesBefore:before,classesAfter:Object.fromEntries((result.selection||[result.element]).filter(Boolean).map(info=>[info.id,info.className])),undoId:result.undoId});
  await refreshPastedLayerStyles(selectionIds,changedIds,before);toast('Styles pasted','ok');
 }finally{busyPanel(false);}
}
async function refreshPastedLayerStyles(selectionIds,changedIds,before=null){
 before??=Object.fromEntries(changedIds.map(id=>[id,matchingEls(id)[0]?.getAttribute('class')||'']));
 const resolved=await Promise.all(changedIds.map(id=>api('GET',resolveUrl(id))));if(resolved.some(result=>!result?.ok||!(result.element.cssAuthoring||result.element.classSelection)))throw Error('The pasted styles could not be resolved.');
 const infos=resolved.map(result=>result.element);await refreshGroupMove(infos,before);await restoreLayerSelection(selectionIds);await layers.refresh();if(sel)renderPanel();
}
function componentClipboardActions(section,components,infos,key){
 const actions=document.createElement('div');actions.className='component-actions';
 if(components.length===1){const copy=RetouchInspector.button('Copy properties',async()=>{try{const result=await RetouchComponentClipboard.copy(components[0]);toast('Copied '+result.count+' properties'+(result.system?'':' within this editor'),'ok');}catch(error){toast(error.message,'err');}});copy.dataset.componentClipboard='copy';try{RetouchComponentClipboard.snapshot(components[0]);}catch{copy.disabled=true;}actions.append(copy);}
 const paste=RetouchInspector.button('Paste properties',async()=>{try{await RetouchComponentClipboard.open({components,infos,opener:paste,save:targets=>pasteComponentProperties(infos,key,targets)});}catch(error){toast(error.message,'err');}});paste.dataset.componentClipboard='paste';actions.append(paste);section.append(actions);
}
async function pasteComponentProperties(infos,key,targets){
 if(key!==panelSelectionKey()||panelTasks||undoBusy||sourceRequests)throw Error('Re-select the target instances before pasting.');
 busyPanel(true);try{
  const result=await api('POST','/rt/__api/op',{type:'pasteComponentProps',id:targets[0].id,fileHash:infos[0].hash,targets});if(!result?.ok)throw Error(result?.reason||result?.error||'Could not paste properties.');
  const selectionIds=infos.map(info=>info.id),refreshIds=result.componentPaste.refreshIds;if(result.undoId)editorHistory.record({type:'pasteComponentProps',id:targets[0].id,selectionIds,refreshIds,undoId:result.undoId});
  if(result.componentPaste.changed)await refreshChangedComponentSelection(selectionIds,refreshIds);toast(result.componentPaste.changed?'Properties pasted':'Properties already match','ok');
 }finally{busyPanel(false);}
}
function componentSelectionSection(infos){
 ...
 Promise.all(ids.map(id=>api('GET',componentUrl(id)))).then(components=>{
  ...
  componentClipboardActions(section,components,infos,key);
...
async function refreshChangedComponentSelection(ids,refreshIds){
 for(const id of refreshIds){
  const result=await api('GET',resolveUrl(id));if(!result?.ok)throw Error('The component selection could not be refreshed.');
  ...
  if(comparisons?.failures.length)throw Error('Properties saved. Retry the failed comparison previews.');
 }
...
async function resetComponentPropertiesSelection(infos,key,revisions){
 ...
 try{
  ...
  await refreshChangedComponentSelection(ids,refreshIds);toast('Properties reset','ok');
 }catch(error){toast(error.message,'err');}finally{busyPanel(false);}
}
...
const imageThumbnailHeights=new Map();
function rememberImageThumbnail(image){
  image.style.minHeight='';if(!image.isConnected||!image.naturalWidth)return;
  const height=getComputedStyle(image).height;if(!(parseFloat(height)>0))return;
  imageThumbnailHeights.set(image.src,height);if(imageThumbnailHeights.size>128)imageThumbnailHeights.delete(imageThumbnailHeights.keys().next().value);
}
function setImageThumbnailSource(image,url){
  image.style.minHeight=imageThumbnailHeights.get(url)||'';image.onload=()=>rememberImageThumbnail(image);image.onerror=()=>{image.style.minHeight='';};image.src=url;
}
async function refreshPictureSources(imageId,scope,candidate,{sourceIdMap=[],removedSourceIds=[],createdSourceIds=[],direction='redo',authorStyles=false,revalidateStyles=false}={}){
  const scrollPanel=document.getElementById('panel'),panelScroll=scrollPanel.scrollTop,anchor=panelBody.querySelector('.picture-sources')?.getBoundingClientRect().top;
  ...
  info._responsiveCandidate=candidate;info._pictureSourcesOpen=true;sel={hostId:info.id,instanceId:null,scope:'host',info};await layers.refresh();renderPanel();
  // Candidate thumbnails can gain intrinsic height after the panel is rebuilt.
  // Restore the artwork position after that layout change, without waiting
  // indefinitely for an unavailable image.
  let thumbnailTimer;try{await Promise.race([Promise.all([...panelBody.querySelectorAll('img.imgthumb')].map(image=>image.decode().then(()=>rememberImageThumbnail(image)).catch(()=>{}))),new Promise(resolve=>{thumbnailTimer=setTimeout(resolve,1500);})]);}finally{clearTimeout(thumbnailTimer);}
  if(sel?.info!==info)return;
  const nextAnchor=panelBody.querySelector('.picture-sources')?.getBoundingClientRect().top;scrollPanel.scrollTop=panelScroll;if(Number.isFinite(anchor)&&Number.isFinite(nextAnchor))scrollPanel.scrollTop+=panelBody.querySelector('.picture-sources').getBoundingClientRect().top-anchor;
}
...
function responsiveImageControls(sec,info) {
  ...
  const target=matchingEls(info.id)[0],preview=document.createElement('img');preview.className='imgthumb';preview.alt='Selected image candidate';sec.append(preview);
  const choices=document.createElement('select');for(const candidate of descriptor.candidates)choices.add(new Option(candidate.label,candidate.key));
  ...
  async function apply(src,key=choices.value,hash=info.hash){
    ...
    busyPanel(true);try{
      const result=await api('POST','/rt/__api/op',{type:'setResponsiveImage',id:info.id,candidate:key,src,fileHash:hash,context:info.context});
      if(!result?.ok)throw Error(result?.reason||result?.error||'Image candidate could not be saved.');
      if(result.undoId)editorHistory.record({type:'setResponsiveImage',id:info.id,candidate:key,undoId:result.undoId,context:info.context});
      Object.assign(info,result.element||{hash:result.hash});await refreshResponsiveImage(info);if(sel?.info===info)renderPanel();toast('Saved','ok');
    }finally{busyPanel(false);}
  }
  async function saveSource(changes,sourceIndex,key){
    if(sel?.info!==info)throw Error('Re-select the image before changing its source settings.');
    busyPanel(true);try{
      const result=await api('POST','/rt/__api/op',{type:'setResponsiveImageSource',id:info.id,sourceIndex,changes,fileHash:info.hash,context:info.context});
      if(!result?.ok)throw Error(result?.reason||result?.error||'Source settings could not be saved.');
      if(result.undoId)editorHistory.record({type:'setResponsiveImageSource',id:info.id,candidate:key,undoId:result.undoId,context:info.context});
...
function imageSection(info) {
  ...
  if(thumbnail)setImageThumbnailSource(img,thumbnail);else img.hidden=true;
...
function selectionColorOptions(width){
  ...
  async function write(type,property,styleId,libraryRevision){
    ...
    try{
      ...
      if(result.undoId)editorHistory.record({type:info.contextSelection?'collectionSelection':react?'setClassesSelection':'setCSSSelection',managedCSS:!!info.cssRendering,id:info.id,selectionIds:ids,undoId:result.undoId});
      sel.info=result.element;sel.multiple=result.selection;if(info.contextSelection){await reloadFrame();await restoreLayerSelection(ids);}else if(react)await refreshWrittenElement(result.element,el=>classSelectionMatches(result.selection,el.ownerDocument),{classSource:true});else if(info.cssRendering){await RetouchRenderSync.syncCSS({frame:iframe,entries:result.selection.map(item=>({id:item.id,rules:item.cssRules,texts:item.cssRuleTexts,rendering:item.cssRendering}))});await window.RetouchComparisons?.syncCSS(result.selection);}else await reloadFrame();renderPanel();toast('Selected colors updated','ok');
    }finally{busyPanel(false);}
  }
...
function mountSelectionEffectStyles(){
  ...
  if(!element||!selection.every(info=>info.linkedStyleAuthoring!==false&&(info.cssAuthoring||info.classEffectStyles)))return;
...
async function setHTMLCSSSelection(property,value,width,changesById,resetScope=false){
  ...
  try{
    ...
    sel.info=result.element;sel.multiple=result.selection;await RetouchRenderSync.syncCSS({frame:iframe,entries:result.selection.map(item=>({id:item.id,rules:item.cssRules,texts:item.cssRuleTexts,rendering:item.cssRendering}))});await window.RetouchComparisons?.syncCSS(result.selection);renderPanel();toast('Selected layers updated','ok');
...
function strokeGradientTarget(info,paint){return [...(matchingEls(info.id)[0]?.children[1]?.children||[])].filter(el=>el.localName==='path')[paint==='fill'?0:1];}
function strokeGradients(info){return Object.entries(info.svgStrokeSource.model.gradients||{}).map(([paint,gradient])=>RetouchSVGStrokeGradient.describe(gradient,info.svgStrokeSource.definitionId+'-'+paint,paint));}
function mountStrokeSourcePaint(section,info,property){
 const I=RetouchInspector,model=info.svgStrokeSource.model;
 if(model.gradients?.[property])return;
 const type=I.select(section,property==='fill'?'Fill type':'Stroke type',[['solid','Solid'],['linearGradient','Linear'],['radialGradient','Radial']],'solid',type=>{if(type!=='solid')void setSVGGradient(info,property,undefined,undefined,'create',{type,color:model[property]==='none'?'#000000':model[property]});});type.disabled=!strokeSourceEditable(info);
 const input=document.createElement('input');input.type='text';input.spellcheck=false;input.dataset.paintProperty=property;
 const parsed=model[property]==='none'?null:RetouchPaintPicker.parsePaint(model[property]);
 const original=parsed?RetouchPaletteValues[parsed.space==='display-p3'?'p3':'srgb'](parsed.channels,parsed.alpha*model[property+'Opacity']):model[property];
 input.value=original;input.disabled=!strokeSourceEditable(info);input.retouchPreviewDocument=matchingEls(info.id)[0]?.ownerDocument;
 const current=()=>strokeSourceEditable(info)&&!panelTasks&&!sourceRequests&&!undoBusy&&!editing;
 input.oninput=()=>input.setCustomValidity('');input.onchange=async()=>{
  const value=input.value.trim();if(!current()||value===original){input.value=original;return;}
  try{RetouchSVGStrokeAlignment.setPaint({...model,document:RetouchSVGPath.parseCompound(model.path)},property,value);}catch(error){input.setCustomValidity(error.message);input.reportValidity();return;}
  if(!await writeSVGStrokeSource('setSVGStrokeSourceStyle',{property,value}))input.value=original;
 };
 input.retouchPaintPreview=()=>{
  if(!current())throw Error('Select one unlocked vector rendered once on this page.');
  const preview=RetouchSVGStrokeFidelity.previewProperty(matchingEls(info.id)[0],model,info.svgStrokeSource.definitionId,property);
  return {update:value=>{if(!current())throw Error('The selected stroke changed.');preview.update(value);},restore:()=>preview.restore()};
 };
 I.field(section,'SVG '+property,input);I.fieldDraft(input);
}
function strokeSelectionEditable(infos){
 return sel?.multiple===infos&&infos.length>1&&new Set(infos.map(info=>info.file+'#'+info.hash)).size===1&&infos.every(info=>{
...
function mountStrokeSelectionControls(section,infos){
 ...
 for(const [property,label,choices]of [['position','Shared stroke alignment',[['inside','Inside'],['center','Center'],['outside','Outside']]],['linecap','Shared SVG line ends',[['butt','No caps'],['round','Round'],['square','Square']]],['linejoin','Shared SVG line joins',[['miter','Miter'],['round','Round'],['bevel','Bevel']]]]){
  ...
  input.title='Applies to every selected stroke on every screen.';input.disabled=!strokeSelectionEditable(infos);if(property==='position'){input.closest('.inspector-field').querySelector('span').textContent='Align';for(const option of input.options)option.disabled=option.value==='__mixed'||!infos.every(info=>info.svgStrokeSource.positions.includes(option.value));}
 }
 ...
 if(!strokeSelectionEditable(infos))I.note(section,'Select unlocked aligned vectors rendered once in one source file.');
}
async function writeSVGStrokeSelection(infos,property,value){
 ...
 try{
  ...
  layerLocks.remap(result.sourceIdMap);await refreshSVGBooleanSelection(result.parentId,result.selectionIds);toast('Stroke settings updated','ok');return true;
 }catch(error){toast(error.message,'err');return false;}finally{busyPanel(false);}
}
function mountStrokeSourceControls(section,info){
 info.svgGradients=strokeGradients(info);
 ...
 if(info.svgGradients.length)mountSVGGradients(info,matchingEls(info.id)[0]);
...
function mountSVGGradients(info,ordinaryTarget){
 for(const gradient of info.svgGradients){
  const target=info.svgStrokeSource?strokeGradientTarget(info,gradient.paint):ordinaryTarget;
  ...
  RetouchInspector.note(section,info.svgStrokeSource?'Gradient for this shape':(gradient.type==='linearGradient'?'Linear':'Radial')+' · #'+gradient.id);
  const reason=gradient.reason||(info.svgStrokeSource&&!strokeSourceEditable(info)?'Select one unlocked vector.':null); if(reason){RetouchInspector.note(section,reason,'refused');panelBody.append(section);continue;}
  ...
  if(info.svgStrokeSource){
   const model=info.svgStrokeSource.model,property=gradient.paint+'Opacity',current=()=>strokeSourceEditable(info)&&!panelTasks&&!sourceRequests&&!undoBusy&&!editing;
   const alpha=RetouchInspector.number(section,(gradient.paint==='fill'?'Fill':'Stroke')+' gradient opacity (%)',model[property]*100,0,100,value=>setSVGGradient(info,gradient.paint,undefined,undefined,'opacity',value/100));RetouchInspector.fieldDraft(alpha);
   alpha.retouchNumericPreview=()=>{if(!current())throw Error('Select one unlocked gradient.');const preview=RetouchSVGStrokeFidelity.previewProperty(matchingEls(info.id)[0],model,info.svgStrokeSource.definitionId,property);return {current:()=>current()&&preview.current(),update:value=>preview.update(value/100),restore:()=>preview.restore()};};alpha.retouchNumericHandle(alpha,{canvas:true,keyboardOnly:true,keys:['ArrowDown','ArrowUp']});
  }
  ...
  const unique=RetouchInspector.button('Make unique',()=>setSVGGradient(info,gradient.paint,undefined,undefined,'detach'));unique.setAttribute('aria-label','Make '+gradient.paint+' gradient unique');if(!info.svgStrokeSource)section.append(unique);
  ...
  bindStops?.();
  RetouchInspector.note(section,info.svgStrokeSource?'Applies to this shape on every screen.':'Shared gradient · Edits affect all referencing layers and screen sizes. Page styles can override stop colors.');section.lastElementChild.classList.add('gradient-scope');panelBody.append(section);
 }
}
async function setSVGGradient(info,paint,changes,stop,action,value,refreshOptions={}){
 if(info.svgStrokeSource){if(sel?.info!==info)return false;return writeSVGStrokeSource('setSVGStrokeSourceGradient',{paint,...(changes===undefined?{}:{changes}),...(stop===undefined?{}:{stop}),...(action===undefined?{}:{action,value})});}
...
async function refreshGroupMove(infos,before){
 if(infos.every(info=>info.cssAuthoring)){await RetouchRenderSync.syncCSS({frame:iframe,entries:infos.map(info=>({id:info.id,rules:info.cssRules,texts:info.cssRuleTexts,rendering:info.cssRendering}))});await window.RetouchComparisons?.syncCSS(infos);}
...
window.RetouchPrototypeHost={
 async variableInfo(el){const id=el.getAttribute('data-rt');if(!id)throw Error('A variable-bound layer has no source identity.');const result=await api('GET',resolveUrl(id,renderContext(el)));if(!result?.ok)throw Error(result?.reason||result?.error||'Could not resolve a variable-bound layer.');return result.element;},
 ...
 async pickScroll(info,index,creation,actionPath){
  await this.prepare();if(mode!=='edit')setEditorMode('edit');
  const target=await RetouchPrototypePicker.pick(iframe);if(target)return this.connectScroll(info,index,target,creation,actionPath);
 },
 async connectScroll(info,index,target,creation,actionPath){
  await this.prepare();
...
window.RetouchVariableLibraryRequest=async operation=>{
 ...
 busyPanel(true);try{const result=await api('POST','/rt/__api/variables',operation);if(!result?.ok)throw Error(result?.reason||result?.error||'Could not save variable collections.');if(result.undoId)editorHistory.record({type:'sourceHistory',undoId:result.undoId});if(result.updated){const info=sel?.info,ids=sel?.multiple?.map(item=>item.id),fresh=info?await api('GET',resolveUrl(info.id,info.context)):null;if(fresh?.ok&&sel?.info.id===info.id)sel.info=fresh.element;if(['vue','svelte'].includes(result.renderRevisions?.renderer))await refreshSourceHistory(result.renderRevisions);else if(fresh?.ok&&fresh.element.classVariables)await refreshTextStyleElement(fresh.element);else await reloadFrame();if(ids?.length>1)await restoreLayerSelection(ids);else if(sel)renderPanel();}return result;}finally{busyPanel(false);}
};
...
async function setHTMLCSS(property,value,width,resetScope=false){
  ...
  try{
    ...
    sel.info=result.element;await RetouchRenderSync.syncCSS({frame:iframe,id:info.id,rules:result.element.cssRules,texts:result.element.cssRuleTexts,rendering:result.element.cssRendering});await window.RetouchComparisons?.syncCSS(result.element);renderPanel();toast('Saved','ok');
...
async function refreshSourceHistory(manifest) {
  ...
  let stable=0;
  for(let attempt=0;attempt<(['vue','svelte'].includes(manifest.renderer)?160:40);attempt++){
    ...
    try{
      ...
      if(!stable&&manifest.renderer==='svelte'&&attempt%20===0)RetouchRenderSync.requestSourceSync(iframe);
      if(stable>=3){if(['vue','svelte'].includes(manifest.renderer))await window.RetouchComparisons?.syncHistory(manifest);return;}
    }catch{stable=0;}
    await new Promise(resolve=>setTimeout(resolve,50));
  }
  if(['vue','svelte'].includes(manifest.renderer))throw Error('The updated templates and styles have not reached the preview yet.');
...
async function restoreHistory(direction,op) {
  ...
  try {
    ...
    if(op.type==='pasteLayerStyles'){await refreshPastedLayerStyles(op.selectionIds,op.changedIds,direction==='undo'?op.classesAfter:op.classesBefore);toast(direction==='undo'?'Undone':'Redone','ok');return result;}
...
function setEditorMode(next) {
  window.RetouchPrototypeConnections?.cancel();
