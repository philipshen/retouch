...
function propTable(props,instanceId,fileHash) {
  ...
  for(const prop of props){
    ...
    if(instanceId&&prop.editor?.editable&&prop.editor.choices&&!(prop.editor.type==='boolean'&&typeof prop.editor.value==='boolean'&&prop.editor.choices.length===2&&!prop.editor.allowUnset&&!prop.editor.unset)){
      ...
      const meta=prop.editor,input=document.createElement(meta.type==='string'?'textarea':'input');let emptyButton=null;input.setAttribute('aria-label','Component property '+prop.name);if(meta.type!=='string')input.type=meta.type==='boolean'?'checkbox':'number';
      ...
      input.addEventListener('keydown',event=>{if(event.isComposing)return;if(event.key==='Enter'&&(meta.type!=='string'||event.metaKey||event.ctrlKey)){event.preventDefault();event.stopPropagation();window.RetouchPanelFocus.queue(input);input.blur();restorePanelFocus();}if(event.key==='Escape'){event.preventDefault();event.stopPropagation();if(meta.type==='boolean')input.checked=meta.value;else input.value=meta.unset?'':String(meta.value);if(emptyButton)emptyButton.hidden=false;if(meta.type==='string')sizeComponentText(input);input.blur();}});value.append(input);
