...
async function writeSVGBooleanGroup(type,extra){
 ...
 const checkLocks=()=>{const affected=type==='createSVGBooleanGroup'?extra.ids:type==='setSVGBooleanOperand'?[extra.operandId]:ids;for(const id of new Set([primary.id,...(affected||[])])){const matches=matchingEls(id);if(matches.length!==1)throw Error('Re-select the boolean shapes rendered once on this page.');if(layerLocks.locked(matches[0]))throw Error('Unlock the selected original or its containing layer before editing it.');}};
 try{checkLocks();let request={type,id:primary.id,fileHash:primary.hash,...extra};if(primary.svgBooleanGroup&&(primary.svgBooleanGroup.ancestorId||type==='createSVGBooleanGroup')){const group=matchingEls(primary.id)[0],prepared=await RetouchSVGBooleanGroup.prepareCascade(primary,group,async ids=>{const responses=await Promise.all(ids.map(id=>api('GET',resolveUrl(id))));if(responses.some(result=>!result.ok||result.element.hash!==primary.hash))throw Error('The boolean source changed. Re-select the group.');return responses.map(result=>result.element);});const derived=prepared.run(type,extra);request={type:'setSVGBooleanNested',id:primary.id,fileHash:primary.hash,targetId:primary.id,edit:{type,...extra,...(type==='createSVGBooleanGroup'?{path:derived.path}:{})},results:derived.results};}checkLocks();const result=await api('POST','/rt/__api/op',request);if(!result?.ok)throw Error(result?.reason||result?.error||'Could not update the boolean group.');if(result.unchanged)return true;
