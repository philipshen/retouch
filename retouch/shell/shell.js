...
function moveHTMLLayer(info,target,width,g,action='move',opener,initial=null){
  ...
  try{if(!Number.isInteger(width)||width>target.ownerDocument.defaultView.innerWidth)throw Error('Choose a screen where this scope is active.');if(target.ownerDocument.defaultView.getComputedStyle(target).position!=='absolute')throw Error('Choose an absolute layer in the current screen.');g=RetouchInspector.geometry(target,{allowRotation:true,allowScale:action==='move'});}catch(error){toast(error.message,'err');return;}
