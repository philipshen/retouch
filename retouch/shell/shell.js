...
function renderPanelContents() {
  ...
  if(info.svgGeometry){
    ...
    if(parametric){
      ...
      const writeArrow=points=>setSVGGeometry(pointField.name,pointField.name==='d'?RetouchSVGParametric.arrowPath(points):points);
...
async function convertSVGToPath(info,toArrow=false){
  const conversion=toArrow?info.svgConversion?.arrow:info.svgConversion;if(!conversion)return;const targetTag='path',desiredPath=toArrow?RetouchSVGParametric.arrowPath(conversion.points):conversion.path,operation=toArrow?'convertSVGToArrow':'convertSVGToPath';
  ...
  if(toArrow){probe.setAttribute('d',desiredPath);probe.setAttribute('fill','none');probe.setAttribute('data-rt-shape','arrow');}else probe.setAttribute('d',conversion.path);probe.style.setProperty('visibility','hidden','important');
  try{
