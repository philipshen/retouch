...
for(const kind of ['html','react','liquid'])test(kind+' selected gradient edits preserve independent paints and refuse partial member changes',t=>{
 const adapter=require('../src/adapters/'+kind+'.cjs'),gradient=(type,color)=>({type,fields:{},stops:[{offset:'0',color,opacity:'.7'},{offset:'1',color:'blue',opacity:'0'}]});
 let r=resolve(kind);r=resolve(kind,r.source.replace('<circle','<rect x="20" y="20" width="60" height="60" fill="red" stroke="blue"/><circle'));
 for(const [type,color]of [['linearGradient','red'],['radialGradient','lime']]){const v=view(r,kind);r.element=r.elements.find(element=>v.tag(element)==='rect'&&S.creation({...r,element},kind));const change=create(r,kind,{model:{...model,fillOpacity:.4,gradients:{fill:gradient(type,color),stroke:gradient('linearGradient','purple')}}});assert.ok(change.ok,change.reason);r=resolve(kind,change.edits[0].after,change.selectionIds[0]);}
 const groups=r.elements.filter(element=>S.context({...r,element},kind)),ids=groups.map(e=>e.id),before=groups.map(element=>S.context({...r,element},kind));r.element=groups[0];
 const root=fs.mkdtempSync(path.join(os.tmpdir(),'rt-gradient-selection-')),file=path.join(root,r.relPath);t.after(()=>fs.rmSync(root,{recursive:true,force:true}));r.file=file;fs.writeFileSync(file,r.source);
 const edits=[{value:{paint:'fill',action:'reverse'}},{value:{paint:'fill',action:'setType',value:'linearGradient'}},{value:{paint:'fill',stop:0,changes:{'stop-color':'#12345680','stop-opacity':'.35'}}},{value:{paint:'fill',changes:{spreadMethod:'reflect'}}},{values:Object.fromEntries(ids.map((id,i)=>[id,{paint:'fill',action:'insertStop',stop:1,value:{offset:'.5',color:i?'orange':'cyan',opacity:'.5'}}]))}];
 for(const extra of edits){
  const op={type:'setSVGStrokeSelection',fileHash:r.hash,ids,property:'gradient',...extra},change=adapter.planOp(r,op);assert.ok(change.ok,change.reason);assert.equal(S.validatePlan(r,op,kind,change),change);assert.equal(change.edits.length,1);
  const after=change.edits[0].after,next=resolve(kind,after,change.selectionIds[0]);change.selectionIds.forEach((id,i)=>{const c=S.context({...next,element:next.elements.find(e=>e.id===id)},kind);assert.deepEqual({...c.model,gradients:before[i].model.gradients},before[i].model);assert.deepEqual(c.model.gradients.stroke,before[i].model.gradients.stroke);assert.equal(c.source,before[i].source);assert.equal(c.id,before[i].id);const edit=extra.value||extra.values[ids[i]],g=c.model.gradients.fill;if(edit.action==='reverse')assert.deepEqual(g.stops.map(s=>s.color),before[i].model.gradients.fill.stops.map(s=>s.color).reverse());if(edit.action==='setType')assert.equal(g.type,'linearGradient');if(edit.action==='insertStop'){assert.equal(g.stops.length,3);assert.equal(g.stops[1].color,i?'orange':'cyan');}if(edit.changes?.['stop-color'])assert.equal(g.stops[0].color,'#12345680');if(edit.changes?.spreadMethod)assert.equal(g.fields.spreadMethod,'reflect');});
  const history=new(require('../src/history.cjs').SourceHistory)(),applied=require('../src/transactions.cjs').applyPlan(root,change);assert.ok(applied.ok,applied.reason);const entry=history.record(applied.edits);assert.ok(history.apply(root,'undo',entry,{}).ok);assert.equal(fs.readFileSync(file,'utf8'),r.source);assert.ok(history.apply(root,'redo',entry,{}).ok);assert.equal(fs.readFileSync(file,'utf8'),after);assert.ok(history.apply(root,'undo',entry,{}).ok);
 }
 const good={paint:'fill',action:'reverse'};
 for(const bad of [null,[],42,{...good,paint:'other'},{...good,type:'restoreSVGStrokeSource'},{...good,fileHash:r.hash},{paint:'fill',stop:4,changes:{'stop-color':'red'}},{paint:'fill',changes:{x2:'80%'}},{paint:'fill',action:'removeStop',stop:0}]){const op={type:'setSVGStrokeSelection',fileHash:r.hash,ids,property:'gradient',values:{[ids[0]]:good,[ids[1]]:bad}},refused=adapter.planOp(r,op);assert.equal(refused.refused,true,JSON.stringify(bad));assert.equal(refused.edits,undefined);assert.equal(fs.readFileSync(file,'utf8'),r.source);}
});
