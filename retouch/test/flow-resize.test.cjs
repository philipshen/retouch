...
test('flow resize accepts ordinary inline dimensions but refuses important sizing conflicts',()=>{
 ...
 for(const property of ['inline-size','block-size','flex','flex-grow','flex-shrink','flex-basis']){assert.equal(available(el({[property]:'100px'})),true);assert.equal(available(el({[property]:'100px'},{[property]:'important'})),false);}
});
