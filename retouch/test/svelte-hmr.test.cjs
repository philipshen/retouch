...
function fixture() {
  ...
  vm.runInContext(runtime().replace("import {writable} from 'svelte/store';", '').replace('export function sourceState', 'function sourceState').replace('export function literalProp(', 'function literalProp(').replace('export function literalProps(', 'function literalProps(').replace("import {getContext,setContext,untrack} from 'svelte';", '').replace('export function componentState', 'function componentState').replace('export const legacyProp', 'const legacyProp').replaceAll('import.meta.hot', 'hot'), context);
