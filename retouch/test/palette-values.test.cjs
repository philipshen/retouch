...
test('color-space conversion preserves in-gamut colors and requires explicit clipping',()=>{
 ...
 assert.deepEqual(values.convert('color(display-p3 1 0 0 / 0.5)','srgb'),{value:null,clipped:true});assert.deepEqual(values.convert('color(display-p3 1 0 0 / 0.5)','srgb',true),{value:'#ff000080',clipped:true});assert.throws(()=>values.convert('#fff','unknown'));
});
