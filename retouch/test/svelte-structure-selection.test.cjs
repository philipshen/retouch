...
const source = '<main><section><div aria-label="A"><span>A</span></div><div aria-label="B">B</div></section><section><div aria-label="C">C</div><div aria-label="D" bind:this={unique}>D</div></section></main><style>section{padding:4px}</style>';
function resolve(text = source, name = 'A') {
  const elements = adapter.collect(text, 'App.svelte').elements;
  return { source: text, relPath: 'App.svelte', file: '/app/App.svelte', hash: adapter.contentHash(text), elements,
...
test('Svelte duplicates roots across parents with independent responsive styles and a combined identity map', () => {
...
test('Svelte batch delete cleans subtree styles and duplicate/delete each restore in one history step', t => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'retouch-svelte-batch-')), file = path.join(root, 'App.svelte');
  ...
  for (const type of ['duplicateSelection', 'deleteSelection']) {
    ...
    const after = fs.readFileSync(file, 'utf8'); require('svelte/compiler').compile(after,{filename:'App.svelte'});
    if (type === 'deleteSelection') {
      assert.equal(result.removedSourceIds.length, 3); assert.doesNotMatch(after, /data-rt-svelte-css|data-rt-style/);
...
test('Svelte batch actions normalize ancestor/descendant selections and refuse the whole edit if a later root is invalid', t => {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'retouch-svelte-batch-refuse-')), file = path.join(root, 'App.svelte');
...
test('Svelte batch ordering preserves sparse selection order in every direction with exact history', t => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'retouch-svelte-batch-order-')), file = path.join(root, 'App.svelte');
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const original = '<main>' + ['A', 'B', 'C', 'D', 'E'].map(name => '<div aria-label="' + name + '"><span>' + name + '</span></div>').join('\n ') + '</main>';
...
test('Svelte batch ordering treats adjacent roots as a block and refuses cross-parent or ineffective moves', () => {
  const text = '<main>' + ['A', 'B', 'C', 'D'].map(name => '<div aria-label="' + name + '">' + name + '</div>').join('') + '</main>';
...
test('Svelte batch reparent preserves source order, responsive styles and exact history across containers', t => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'retouch-svelte-batch-parent-')), file = path.join(root, 'App.svelte');
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  for (const position of ['inside', 'before', 'after']) {
    ...
    assert.deepEqual(target.node.fragment.nodes.filter(node => node.type === 'RegularElement').map(node => node.attributes.find(prop => prop.name === 'aria-label').value[0].data), position === 'before' ? ['A', 'C', 'Anchor'] : ['Anchor', 'A', 'C']);
...
test('Svelte batch reparent expands empty destinations and refuses cycles or a later incompatible root atomically', t => {
  ...
  const invalid = text.replace('<section><div aria-label="C">', '{#each items as item}<section><div aria-label="C">').replace('D</div></section>', 'D</div></section>{/each}');
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'retouch-svelte-parent-refuse-')), file = path.join(root, 'App.svelte');
