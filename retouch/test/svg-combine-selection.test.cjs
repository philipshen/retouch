...
function resolve(kind,markup){if(kind==='react')markup=markup.replace('<!--keep-->','{/*keep*/}');const source=kind==='react'?'export default function Artwork(){return ('+markup+');}':markup,adapter=require('../src/adapters/'+kind+'.cjs'),elements=adapter.collect(source,kind==='react'?'art.jsx':'art.html').elements;return {source,elements,element:elements.find(e=>(kind==='react'?ids.jsxElementName(e.node):e.tag)==='rect'),file:'/tmp/art.'+(kind==='react'?'jsx':'html'),relPath:kind==='react'?'art.jsx':'art.html',hash:adapter.contentHash(source)};}
...
for(const kind of ['html','react'])test(kind+' adapter exposes atomic replacement through its ordinary operation planner',()=>{
...
for(const kind of ['html','react'])test(kind+' replacement writes one history entry and restores exact source through Undo/Redo',t=>{
