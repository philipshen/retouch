...
(async()=>{const browser=await require(path.join(fixture,'node_modules/playwright'))[engine].launch();try{
 ...
 for(const css of cases){const result=await page.evaluate(async css=>{
  ...
  await load(old,.5);const midpoint=rects(old);await load(old,0);await load(next,1);const initial=rects(old),final=rects(next),baseline=[...next.contentDocument.querySelectorAll('[data-rt]')].map(el=>el.getAttribute('style'));
  const motion=RetouchPrototypeSmart.play({old,next,config:{type:'smart-animate',duration:1000,easing:'linear'}}),samples=[];
  for(const time of [0,500,1000]){motion.animations.forEach(a=>{a.pause();a.currentTime=time;});await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));samples.push([rects(old),rects(next)]);}
  motion.cancel();await motion.finished;return {matches:motion.matches,samples,expected:[initial,midpoint,final],restored:baseline.every((s,i)=>s===next.contentDocument.querySelectorAll('[data-rt]')[i].getAttribute('style')),hosts:next.contentDocument.querySelectorAll('.prototype-style-motion').length};
 },css);assert.equal(result.matches,2,css);for(let i=0;i<3;i++)for(const frame of result.samples[i])for(let layer=0;layer<2;layer++)for(let axis=0;axis<4;axis++)assert.ok(Math.abs(frame[layer][axis]-result.expected[i][layer][axis])<.1,`${css} sample ${i} layer ${layer} axis ${axis}: ${frame[layer][axis]} expected ${result.expected[i][layer][axis]}`);assert.equal(result.restored,true);assert.equal(result.hosts,0);}
