...
module.exports=async({page,app,read,wait,settled,kind})=>{
 ...
 const reselect=async()=>{await page.getByRole('treeitem',{name:'p · Other text',exact:true}).click();await page.getByRole('treeitem',{name:'h1 · Headline',exact:true}).click({modifiers:shared?['Shift']:[]});await settled();};
 await targets.first().evaluate(el=>el.style.setProperty('grid-template-columns','30px 50px','important'));await reselect();assert.equal(await button.getAttribute('aria-pressed'),'false');assert.equal(await minimum.count(),0);assert.equal(read(),changed);
 await targets.first().evaluate((el,style)=>el.setAttribute('style',style),styles[0]);await reselect();assert.equal(await button.getAttribute('aria-pressed'),'true');assert.equal(await minimum.inputValue(),'180');
