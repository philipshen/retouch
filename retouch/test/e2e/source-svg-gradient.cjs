...
const markup='<svg width="400" height="240" viewBox="0 0 400 240"><defs><linearGradient id="paint"><stop offset="0" stop-color="#ff0000"/><stop offset="100%" stop-color="#0000ff"/></linearGradient><radialGradient id="radial"><stop offset="0" stop-color="white"/><stop offset="1" stop-color="black"/></radialGradient></defs><rect aria-label="Gradient box" x="10" y="10" width="180" height="100" fill="url(#paint)"/><circle aria-label="Shared gradient" cx="260" cy="60" r="50" fill="url(#paint)"/><rect aria-label="Radial box" x="10" y="140" width="180" height="80" fill="url(#radial)"/><rect aria-label="Stroke gradient" x="210" y="140" width="160" height="80" fill="none" stroke="url(#paint)" stroke-width="8"/><rect aria-label="Solid box" style="opacity:0.8;stroke-width:3px" class="layout-marker" x="330" y="110" width="50" height="25" fill="#22aa44" stroke="#4466aa" stroke-width="3"/><rect aria-label="Inline fill box" x="330" y="225" width="50" height="10" fill="#22aa44" stroke="#4466aa" style="fill:#112233;opacity:0.8"/></svg>';
exports.markup=kind=>{let source=markup;if(process.env.RT_E2E_GRADIENT_TRANSFORM)source=source.replaceAll('Gradient id=', 'Gradient gradientTransform="rotate(20 .5 .5)" id=').replace('<rect aria-label="Gradient box"','<rect transform="rotate(12 100 60)" aria-label="Gradient box"');return kind==='react'?source.replace('style="opacity:0.8;stroke-width:3px"','style={{opacity:0.8,strokeWidth:3}}').replace('style="fill:#112233;opacity:0.8"','style={{fill:"#112233",opacity:0.8}}').replaceAll('class=','className=').replaceAll('stop-color','stopColor').replaceAll('stroke-width','strokeWidth'):source;};
exports.run=async function({page,app,file,wait,settled,kind,errors}){
 if(process.env.RT_E2E_SVG_DYNAMIC_CREATE){await require('./svg-gradient-dynamic-create.cjs').run({page,app,file,wait,settled});assert.deepEqual(errors,[]);return;}
