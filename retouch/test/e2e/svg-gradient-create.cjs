...
exports.run=async({page,app,file,wait,settled})=>{
 ...
 await page.getByRole('treeitem',{name:'rect · Inline fill box',exact:true}).click();await settled();{
 ...
 }assert.equal(await page.getByLabel('Stroke type',{exact:true}).isEnabled(),true);assert.equal(read(),original);
