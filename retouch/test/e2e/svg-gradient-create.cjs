...
exports.run=async({page,app,file,wait,settled})=>{
 ...
 await page.getByRole('treeitem',{name:'rect · Inline fill box',exact:true}).click();await settled();if(file.endsWith('.jsx')){
  const inline=app.locator('[aria-label="Inline fill box"]'),beforeStyle=await inline.evaluate(el=>({fill:getComputedStyle(el).fill,opacity:getComputedStyle(el).opacity})),beforeFill=await inline.evaluate(el=>getComputedStyle(el).fill);
  await inline.evaluate(el=>{const style=el.ownerDocument.createElement('style');style.id='inline-gradient-underlying-css';style.textContent='[aria-label="Inline fill box"]{fill:rgb(1,2,3)}';el.ownerDocument.head.append(style);});
  await wait(async()=>await page.getByLabel('Fill type',{exact:true}).isDisabled());assert.equal(read(),original);assert.equal(await inline.evaluate(el=>getComputedStyle(el).fill),beforeFill);
  await inline.evaluate(el=>el.ownerDocument.getElementById('inline-gradient-underlying-css').remove());await wait(async()=>await page.getByLabel('Fill type',{exact:true}).isEnabled());
  for(const type of ['linearGradient','radialGradient']){
   await page.getByLabel('Fill type',{exact:true}).selectOption(type);await settled();await wait(async()=>/^url\(#rt-gradient-/.test(await inline.getAttribute('fill')));const converted=read(),id=(await inline.getAttribute('fill')).slice(5,-1);
   assert.equal(await inline.evaluate(el=>el.style.fill),'');assert.equal(await app.locator('#'+id+' > stop').first().evaluate(el=>getComputedStyle(el).stopColor),beforeFill);
   const stop=page.getByLabel('Stop 1 color',{exact:true});await stop.fill('#ff0000');await stop.press('Tab');await settled();await wait(async()=>await app.locator('#'+id+' > stop').first().getAttribute('stop-color')==='#ff0000');
   await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===converted);
   await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===original);assert.deepEqual(await inline.evaluate(el=>({fill:getComputedStyle(el).fill,opacity:getComputedStyle(el).opacity})),beforeStyle);
   await page.getByRole('button',{name:'Redo',exact:true}).click();await settled();await wait(()=>read()===converted);
   await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===original);
  }
 }else{assert.equal(await page.getByLabel('Fill type',{exact:true}).isDisabled(),true);assert.match(await page.getByLabel('Fill type',{exact:true}).getAttribute('title'),/Inline styles/);}assert.equal(await page.getByLabel('Stroke type',{exact:true}).isEnabled(),true);assert.equal(read(),original);
