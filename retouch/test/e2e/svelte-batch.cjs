...
exports.run = async ({ page, app, phone:live, file, original:source, state }) => {
  ...
  const wait = async predicate => { for (let i = 0; i < 160; i++) { if (await predicate()) return; await new Promise(resolve => setTimeout(resolve, 50)); } throw Error('Svelte batch assertion did not settle'); };
  ...
  console.log('SVELTE BATCH DUPLICATE/DELETE, INDEPENDENT STYLES, SELECTION AND EXACT HISTORY PASS');
};
