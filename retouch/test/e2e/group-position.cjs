...
exports.run=async({page,app,read,wait,settled,kind})=>{
 ...
 for(const multiple of [false,true]){
  ...
  for(const axis of ['x','y']){
   const index=axis==='x'?0:1,input=page.getByLabel('Group '+axis.toUpperCase()+' (px)',{exact:true}),before=await measure(),parent=await app.locator('main').evaluate((el,index)=>{const r=el.getBoundingClientRect();return index?r.y:r.x;},index),start=Math.min(...before.slice(0,multiple?3:2).map(box=>box[index]))-parent,value=Number(await input.inputValue())+23;
