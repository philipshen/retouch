...
(async()=>{
 ...
 try{
  await page.goto(`http://localhost:${server.address().port}/rt`);await page.frameLocator('#app').locator('div').waitFor();assert.equal(await page.evaluate(()=>window.__RT_RENDERING.selectionStyling),true);await page.locator('.layer-tools > summary').click();assert.equal(await page.getByRole('button',{name:'Select visible layers',exact:true}).isVisible(),true);await page.locator('.layer-tools > summary').click();
