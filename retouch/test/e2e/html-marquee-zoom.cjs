...
(async()=>{
 ...
 try{
  ...
  for(const gray of [false,true]){
   ...
   await page.mouse.move(f.x+85,f.y+85,{steps:5});await wait(async()=>await marquee.count()===1);await page.mouse.up();await wait(async()=>await page.getByRole('treeitem',{name:'div · Box',exact:true}).getAttribute('aria-selected')==='true');
  }
