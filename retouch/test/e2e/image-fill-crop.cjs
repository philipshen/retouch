...
exports.run=async({page,app,file,wait,settled,kind})=>{
 ...
 const open=async()=>{const button=page.getByRole('button',{name:'Edit image paint 1',exact:true});if(await button.getAttribute('aria-expanded')!=='true')await button.click();await page.getByRole('button',{name:'Crop image paint 1',exact:true}).click();await page.getByRole('button',{name:'Apply crop',exact:true}).waitFor();await wait(()=>page.getByRole('button',{name:'Apply crop',exact:true}).isEnabled());};
