...
module.exports=async({page,app,kind,read,wait,settled})=>{
 ...
 const step=async(redo,expected,control)=>{
  if(control)await control.press(redo?'Control+Shift+z':'Control+z');
  else{await target.evaluate(el=>el.focus());await page.keyboard.press(redo?'Control+Shift+z':'Control+z');}
  assert.equal(await target.innerHTML(),expected);assert.equal(await selection(),text.slice(1,4));assert.equal(read(),initial);
 };
 ...
 for(let n=states.length-2;n>=0;n--)await step(false,states[n],n===states.length-2?weight:undefined);
 for(let n=1;n<states.length;n++)await step(true,states[n],n===1?button('Underline selected text'):undefined);
 ...
 await target.evaluate(el=>el.focus());await page.keyboard.press('Control+Shift+z');assert.equal(await target.innerHTML(),branch);assert.equal(read(),initial);
 ...
 await page.keyboard.press('Control+z');assert.equal(await target.innerHTML(),branch);
