...
(async()=>{
  ...
  await page.getByRole('button',{name:'Use project image /assets/favicon-512x512.png',exact:true}).click();
  ...
  await image.click();
  await page.getByLabel('Upload image source',{exact:true}).setInputFiles({name:'parity-upload.png',mimeType:'image/png',buffer:fs.readFileSync(path.join(root,'assets/favicon-512x512.png'))});
