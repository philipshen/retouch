...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>{
   ...
   source.body.innerHTML='<main data-rt="main"><h1 data-rt="heading"><span data-rt="paragraph" data-retouch-paragraph style="display:block">Hello <strong data-rt="strong">beautiful <a data-rt="link" href="/">world</a></strong></span><span data-rt="second" data-retouch-paragraph style="display:block">Again</span></h1><div data-rt="list"><ol data-rt="ol"><li data-rt="li">First<ul data-rt="ul"><li data-rt="nested">Nested</li></ul></li></ol></div><div data-rt="paragraphs"><p data-rt="native-p1">Ordinary <strong data-rt="native-strong">paragraph</strong></p><p data-rt="native-p2">Next paragraph</p></div><div data-rt="layout" style="display:grid"><p data-rt="p1">One</p><p data-rt="p2">Two</p></div><p data-rt="component"><span data-rt="host" data-rt-i="instance">Component</span></p><p data-rt="image"><img data-rt="img" alt="Image"></p><p data-rt="block"><span data-rt="blockspan" style="display:block">Block</span></p><h2 data-rt="flex" style="display:flex"><span data-rt="flexspan">Flex</span></h2><p data-rt="position"><span data-rt="absolute" style="position:absolute">Positioned</span></p></main>';
  ...
  assert.equal(result.owner,'heading');assert.equal(await page.evaluate(()=>layers.textOwner(get('native-strong')).getAttribute('data-rt')),'paragraphs');assert.equal(result.component,'host');
  const ids=result.nodes.map(n=>n[0]);for(const id of ['paragraph','strong','link','second','ol','li','ul','nested','native-p1','native-p2','native-strong'])assert.ok(!ids.includes(id),id+' is a text run');
  ...
  assert.equal(await page.getByRole('treeitem',{name:'div · Ordinary paragraph Next paragraph',exact:true}).getAttribute('data-layer-kind'),'text');
  await page.evaluate(async()=>{locked.add(get('native-p1'));await layers.refresh();});await page.getByRole('treeitem',{name:'p · Ordinary paragraph',exact:true}).waitFor();assert.equal(await page.evaluate(()=>layers.textOwner(get('native-strong')).getAttribute('data-rt')),'native-p1');
  await page.evaluate(async()=>{locked.clear();await layers.refresh();});assert.equal(await page.getByRole('treeitem',{name:'p · Ordinary paragraph',exact:true}).count(),0);
