 ...
 try{
  ...
  for(const value of ['row','column','row-reverse','column-reverse','grid','flow']){
   ...
   for(const [label,next,prop,expected] of checks){
    const baseline=read(),control=['Columns','Rows'].includes(label)?page.getByLabel(label,{exact:true}):page.getByRole('combobox',{name:label,exact:true});
