...
(async()=>{
 ...
 const original='"use client";export default function Page(){return <main><section id="layout" className="!block sm:!grid sm:![flex-flow:column_wrap] sm:!place-items-end sm:!place-content-end sm:![grid-template-columns:40px_40px] md:[display:grid] md:grid-cols-2 w-[400px] h-[300px] gap-[10px]"><div className="w-[40px] h-[40px]">One</div><div className="w-[40px] h-[40px]">Two</div></section></main>}';fs.writeFileSync(file,original);const read=()=>fs.readFileSync(file,'utf8');
 ...
 try{
  ...
  for(const value of ['row','column','row-reverse','column-reverse','grid','flow']){
   ...
   const checks=value==='row'?[['Wrap children','nowrap','flexWrap','nowrap'],['Align children','center','alignItems','center'],['Distribute children','between','justifyContent','space-between']]:value==='grid'?[['Columns','3','gridTemplateColumns',null]]:[];
   for(const [label,next,prop,expected] of checks){
    const baseline=read(),control=page.getByLabel(label,{exact:true});
    if(label==='Columns'){await control.fill(next);await control.press('Tab');}else await control.selectOption(next);
