...
(async()=>{
 ...
 try{
  ...
  const verify=async(factor,shift=[0,0],pixels=[0,0])=>{const before=await measure(baseline),left=Math.min(...before.slice(0,-1).map(r=>r[0])),top=Math.min(...before.slice(0,-1).map(r=>r[1])),width=Math.max(...before.slice(0,-1).map(r=>r[0]+r[2]))-left,height=Math.max(...before.slice(0,-1).map(r=>r[1]+r[3]))-top,expected=before.map((r,i)=>i===before.length-1?r:[left+(r[0]-left)*factor+shift[0]*width+pixels[0],top+(r[1]-top)*factor+shift[1]*height+pixels[1],r[2]*factor,r[3]*factor]);for(let i=0;i<100;i++){if((await measure(page)).every((r,n)=>r.every((v,j)=>Math.abs(v-expected[n][j])<.1)))return;await page.waitForTimeout(20);}console.error({errors,scaleErrors:await page.evaluate(()=>scaleErrors)});assert.deepEqual(await measure(page),expected);};
