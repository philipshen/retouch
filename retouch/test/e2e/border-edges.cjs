...
module.exports=async({page,app,kind,read,wait,settled})=>{
 ...
 for(const [side,value,expected]of [['top',7,[7,3,3,3]],['right',0,[7,0,3,3]],['bottom',5,[7,0,5,3]]]){await field(side).fill('('+value+' * 2) / 2'+(kind==='html'?'px':''));await field(side).press('Enter');await settled();await record();await check(expected);}
 ...
 const all=page.getByLabel('Border width ('+(kind==='html'?'CSS':'px')+')',{exact:true});await all.fill(kind==='html'?'8px':'8');await all.press('Escape');await settled();assert.equal(read(),states.at(-1));assert.equal(await all.inputValue(),'');await all.fill(kind==='html'?'(6 / 3)px':'6 / 3');await all.press('Enter');await settled();await record();await check([2,2,2,2]);
