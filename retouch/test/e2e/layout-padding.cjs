...
(async()=>{
 ...
 try{
  ...
  const verifyScrub=async(input,indices)=>{const before=read(),initial=await snapshot(),value=await input.inputValue(),unit=/[a-z%]+$/.exec(value)?.[0]||'',step=unit==='rem'?16:unit==='%'?7.68:1,expected=initial.map((v,i)=>indices.includes(i)?v+step:v),style=await parent.getAttribute('style');const start=async()=>{const label=input.locator('..').locator('span').first();await label.evaluate(el=>el.scrollIntoView({block:'nearest'}));const box=await label.boundingBox();await page.mouse.move(box.x+box.width/2,box.y+box.height/2);await page.mouse.down();await page.mouse.move(box.x+box.width/2+1,box.y+box.height/2);};const matches=async()=>(await snapshot()).every((v,i)=>Math.abs(v-expected[i])<.02);
   await start();await wait(matches);assert.equal(read(),before);await page.keyboard.press('Escape');await page.mouse.up();assert.equal(await parent.getAttribute('style'),style);assert.equal(await input.inputValue(),value);assert.equal(read(),before);
   await start();await wait(matches);await page.mouse.up();await settled();await wait(()=>read()!==before);const after=read();await wait(matches);await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();assert.equal(read(),before);await page.getByRole('button',{name:'Redo',exact:true}).click();await settled();assert.equal(read(),after);await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();assert.equal(read(),before);
  };
  ...
  await shared.fill(sharedValue);await shared.press('Enter');await settled();await wait(async()=>JSON.stringify(await snapshot())==='[24,24,24,24]');const uniform=read();assert.equal(await shared.inputValue(),sharedValue);await verifyScrub(shared,[0,1,2,3]);
  await page.getByLabel('Screen size',{exact:true}).selectOption('390x844');await settled();await wait(async()=>JSON.stringify(await snapshot())==='[10,10,10,10]');await wait(async()=>await shared.isDisabled());assert.equal(await page.getByRole('button',{name:'Reset padding',exact:true}).isDisabled(),true);assert.equal(read(),uniform);
  ...
  const resetAll=page.getByRole('button',{name:'Reset padding',exact:true});await wait(async()=>await resetAll.isEnabled());assert.equal(await resetAll.isEnabled(),true);await resetAll.click();await settled();await wait(async()=>JSON.stringify(await snapshot())===JSON.stringify(initial));const resetAllSource=read();assert.equal(await resetAll.isEnabled(),false);assert.ok(!resetAllSource.includes('md:[padding-left:20px]'));assert.ok(resetAllSource.includes(basePadding+' '+inheritedClasses));
