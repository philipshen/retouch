 ...
 try{
  ...
  if(process.env.RT_E2E_OPTICAL_SIZING){
    ...
    if(process.env.RT_E2E_OPTICAL_FONT){
      ...
      await change(async()=>{await input.fill('8');await input.press('Tab');},'none');const small=await extent();await change(async()=>{await input.fill('144');await input.press('Tab');},'none');const large=await extent();assert.ok(Math.abs(small-large)>.1,JSON.stringify({small,large}));
      await change(()=>page.getByLabel('Optical sizing',{exact:true}).selectOption('auto'),'auto');assert.ok(Math.abs((await extent())-large)<.05);console.log(engine+' '+kind+': PASS real opsz metadata, visible axis changes and explicit-axis precedence '+JSON.stringify({small,large}));
    }
