...
(async()=>{
 ...
 try{
  ...
  await page.getByRole('button',{name:'Compare screens',exact:true}).click();await page.getByRole('button',{name:'Screen controls',exact:true}).click();await preview('Phone').locator('a').waitFor();await preview('Tablet').locator('article').waitFor();if(process.env.RT_E2E_COMPARE_WIDE){const edge=page.getByRole('separator',{name:'Resize comparison panel',exact:true});await edge.press('Shift+ArrowLeft');await edge.press('Shift+ArrowLeft');await wait(async()=>await page.locator('#screenComparisons').evaluate(el=>el.offsetWidth)===370);}
  if(process.env.RT_E2E_COMPARE_PANEL_CHROME){
   const rail=page.locator('#screenComparisons'),help=page.getByRole('button',{name:'Comparison help',exact:true}),hint=page.locator('#comparisonNavigationHint'),focus=page.getByRole('button',{name:'Screen controls',exact:true});
   const firstTop=()=>page.locator('.compare-viewport').first().evaluate(el=>el.getBoundingClientRect().top-document.getElementById('screenComparisons').getBoundingClientRect().top);
   for(const name of ['Phone','Tablet','Desktop'])await preview(name).locator('body').evaluate(()=>{window.comparisonChromeDocument=document;const input=document.createElement('input');input.id='chromeDraft';input.value='Unsaved preview';document.body.append(input);});
   assert.equal(await rail.evaluate(el=>getComputedStyle(el).backgroundColor),'rgb(255, 255, 255)');assert.equal(await hint.isVisible(),false);assert.equal(await help.getAttribute('aria-expanded'),'false');
   ...
   const add=page.locator('.comparison-pin');assert.equal(await add.isVisible(),true);assert.equal(await add.getAttribute('aria-label'),'Show current size');
   ...
   await page.getByRole('button',{name:'Hide Custom 800 × 900 preview',exact:true}).click();
   await page.getByRole('button',{name:'Show current size',exact:true}).click();
   assert.equal(await addedPreview.isVisible(),true);assert.equal(await addedPreview.evaluate(el=>el===document.activeElement),true);assert.equal(await page.locator('.compare-card').count(),4);
   ...
   assert.equal(await page.evaluate(()=>[801,802,803,804].every(width=>RetouchComparisons.showSize({width,height:900}))),true);
   assert.equal(await page.locator('.compare-card').count(),8);
  ...
  await wait(async()=>await card('Tablet').locator('.compare-selection').count()===1);assert.equal(await card('Tablet').locator('.compare-overlay').evaluate(async el=>{let changes=0;const observer=new MutationObserver(records=>{changes+=records.length;});observer.observe(el,{childList:true,subtree:true});await new Promise(resolve=>setTimeout(resolve,450));observer.disconnect();return changes;}),0,'unchanged comparison outlines do not rebuild on every refresh');
  assert.equal(await card('Tablet').evaluate(async card=>{let changes=0;const observer=new MutationObserver(records=>{changes+=records.length;});for(const node of card.querySelectorAll('.compare-scope-message,p.hint,button[aria-label^="Show selection in"]'))observer.observe(node,{childList:true,subtree:true,attributes:true,attributeFilter:['disabled','title','data-scope-applies']});await new Promise(resolve=>setTimeout(resolve,450));observer.disconnect();return changes;}),0,'unchanged comparison statuses and reveal controls remain stable');
  ...
  const beforeRemoval=await page.locator('.compare-card').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('aria-label')));
  await preview('Desktop').locator('body').evaluate(()=>{window.restoreSiblingMarker='preserved';});
  await page.getByRole('button',{name:'Remove Tablet comparison',exact:true}).click();await wait(async()=>await card('Tablet').count()===0);
  await page.getByRole('button',{name:'Remove Checkout narrow comparison',exact:true}).click();await wait(async()=>await card('Checkout narrow').count()===0);
  await page.getByRole('button',{name:'Undo remove: Checkout narrow',exact:true}).click();await preview('Checkout narrow').locator('body').waitFor();assert.equal(await page.getByLabel('Checkout narrow comparison width',{exact:true}).inputValue(),'820');
  await page.getByRole('button',{name:'Undo remove: Tablet',exact:true}).click();await preview('Tablet').locator('body').waitFor();
  ...
  for(const [value,message] of [[{version:2,screens:[]},/version 1/],[{version:1,screens:[{name:'Tiny',width:239,height:600}]},/240 to 7680/],[{version:1,screens:[{name:' ',width:400,height:600}]},/Each screen/]]){await load(value);await wait(async()=>message.test(await page.getByLabel('Screen set status',{exact:true}).textContent()));}
  await page.getByLabel('Screen set file',{exact:true}).setInputFiles({name:'invalid.json',mimeType:'application/json',buffer:Buffer.from('{broken')});await wait(async()=>/valid screen-set JSON/.test(await page.getByLabel('Screen set status',{exact:true}).textContent()));
  await page.getByLabel('Screen set file',{exact:true}).setInputFiles({name:'large.json',mimeType:'application/json',buffer:Buffer.alloc(65537,32)});await wait(async()=>/64 KB/.test(await page.getByLabel('Screen set status',{exact:true}).textContent()));
  assert.equal(await preview('Desktop').locator('body').evaluate(()=>window.restoreSiblingMarker),'preserved');
  const mainSizeBefore=await page.getByLabel('Screen width',{exact:true}).inputValue();
  await load({version:1,screens:[{name:'Wide review',width:1100,height:700},{name:'Compact review',width:360,height:740}]});await preview('Wide review').locator('body').waitFor();await preview('Compact review').locator('body').waitFor();assert.equal(await page.locator('.compare-card').count(),2);assert.equal(await preview('Compact review').locator('body').evaluate(()=>innerWidth),360);
  await page.getByRole('button',{name:'Undo load screen set',exact:true}).click();await preview('Checkout narrow').locator('body').waitFor();assert.deepEqual(await page.locator('.compare-card').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('aria-label'))),beforeRemoval);
  assert.equal(await page.getByLabel('Screen width',{exact:true}).inputValue(),mainSizeBefore);assert.equal(fs.readFileSync(file,'utf8'),original);
  await page.getByLabel('Screen set file',{exact:true}).setInputFiles(savedPath);await wait(async()=>/Loaded 4/.test(await page.getByLabel('Screen set status',{exact:true}).textContent()));await preview('Checkout narrow').locator('body').waitFor();assert.equal(await page.getByLabel('Checkout narrow comparison width',{exact:true}).inputValue(),'820');
  await page.evaluate(()=>{window.originalScreenSetText=File.prototype.text;File.prototype.text=function(){const file=this;return new Promise(resolve=>{window.releaseScreenSetRead=async()=>resolve(await window.originalScreenSetText.call(file));});};});
  await load({version:1,screens:[{name:'Late read',width:500,height:700}]});await wait(()=>page.evaluate(()=>typeof window.releaseScreenSetRead==='function'));
  await page.getByRole('button',{name:'Compare screens',exact:true}).click();await wait(async()=>await page.locator('#screenComparisons iframe').count()===0);await page.getByRole('button',{name:'Compare screens',exact:true}).click();await preview('Checkout narrow').locator('body').waitFor();
  await page.evaluate(async()=>{await window.releaseScreenSetRead();File.prototype.text=window.originalScreenSetText;delete window.originalScreenSetText;delete window.releaseScreenSetRead;await new Promise(requestAnimationFrame);});
  assert.deepEqual(await page.locator('.compare-card').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('aria-label'))),beforeRemoval);
  if(process.env.RT_E2E_SCREEN_SET_SCREENSHOT){await page.locator('#screenComparisons').evaluate(node=>{node.scrollTop=0;});await page.screenshot({path:process.env.RT_E2E_SCREEN_SET_SCREENSHOT});}
  console.log(engine+': PASS saved screen-set download, file round trip, duplicate rejection without reload, replacement, undo load and unchanged main/source');
  console.log(engine+': PASS comparison removal undo restores names, dimensions and order without reloading surviving previews');
 ...
 }catch(error){console.error('COMPARISON DIAGNOSTIC',JSON.stringify({errors,cards:await page.locator('.compare-card').evaluateAll(cards=>cards.map(card=>({name:card.getAttribute('aria-label'),inert:card.inert,buttons:[...card.querySelectorAll('button')].filter(button=>button.getAttribute('aria-label')?.startsWith('Remove ')).map(button=>({name:button.getAttribute('aria-label'),disabled:button.disabled}))})))}));throw error;}finally{await browser.close();server.retouchIndex.close();server.closeAllConnections();await new Promise(r=>server.close(r));fs.rmSync(root,{recursive:true,force:true});}
