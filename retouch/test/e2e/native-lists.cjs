...
module.exports=async({page,app,kind,read,wait,settled})=>{
 ...
 if(process.env.RT_E2E_LIST_INSET){await require('./list-inset.cjs')({page,target,read,wait,settled,states,kind});return;}
 if(process.env.RT_E2E_LIST_SELECTION){await require('./list-selection.cjs')({page,target,read,wait,settled,states,kind});return;}
