...
module.exports=async({page,target,read,wait,settled,states,kind})=>{
 ...
 await save();await open();await choose('F');
 ...
 await save();await open();await choose('G');assert.equal(await button('Decrease list indentation').isEnabled(),true);await button('Decrease list indentation').click();assert.equal((await positions()).G.depth,4);await button('Undo').click();
