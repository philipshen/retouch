...
exports.run=async({page,app,file,wait,settled})=>{
 ...
 const drag=async(name,dx,dy)=>{const handle=page.getByRole('button',{name:'Gradient '+name+' handle',exact:true}),b=await handle.boundingBox();assert.ok(b);await page.mouse.move(b.x+b.width/2,b.y+b.height/2);await page.mouse.down();await page.mouse.move(b.x+b.width/2+dx,b.y+b.height/2+dy,{steps:4});await page.mouse.up();};
