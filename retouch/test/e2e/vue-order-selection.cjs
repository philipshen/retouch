...
exports.run = async ({ page, app, live, file, source }) => {
  ...
  const wait = async predicate => { for (let i = 0; i < 160; i++) { if (await predicate()) return; await new Promise(resolve => setTimeout(resolve, 50)); } throw Error('Vue selection ordering did not settle'); };
  ...
  console.log('VUE MULTI-LAYER ORDERING, RELATIVE ORDER, STYLES/LOCKS AND EXACT HISTORY PASS');
};
