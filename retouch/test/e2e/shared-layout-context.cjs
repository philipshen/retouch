...
exports.run=async({page,app,read,wait,settled,kind})=>{
 ...
 assert.equal(await display.isVisible(),false);assert.equal(await page.getByLabel('Shared Padding',{exact:true}).isVisible(),true);assert.equal(await options.evaluate(el=>el.open),false);
 ...
 for(const mode of ['flex','grid']){
  await options.locator(':scope > summary').click();await display.selectOption(mode);await wait(()=>read()!==original);await settled();await wait(async()=>await app.locator('h1,p').evaluateAll((nodes,mode)=>nodes.slice(0,2).every(el=>getComputedStyle(el).display===mode),mode));
  ...
  assert.equal(await page.getByLabel('Shared Direction',{exact:true}).isVisible(),false);assert.equal(await page.getByLabel('Shared Wrap',{exact:true}).isVisible(),false);
  if(mode==='flex'){await options.locator(':scope > summary').click();const beforeDirection=read();await page.getByLabel('Shared Direction',{exact:true}).selectOption('column');await wait(()=>read()!==beforeDirection);await settled();assert.deepEqual(await app.locator('h1,p').evaluateAll(nodes=>nodes.slice(0,2).map(el=>getComputedStyle(el).flexDirection)),['column','column']);await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===beforeDirection);await settled();await options.locator(':scope > summary').click();}
