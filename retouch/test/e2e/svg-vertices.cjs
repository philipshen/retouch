...
(async()=>{
  ...
  try{
    ...
    const start=async()=>{await page.getByRole('button',{name:'Edit vector points',exact:true}).click();try{await page.getByRole('group',{name:'Edit vector points',exact:true}).waitFor();}catch(error){console.log('POINT EDITOR DIAGNOSTIC',JSON.stringify({status:await page.locator('#status').textContent(),field:await page.getByLabel('Shape Points',{exact:true}).inputValue(),rendered:await app.locator('polygon,polyline').evaluateAll(els=>els.map(el=>({tag:el.tagName,points:el.getAttribute('points'),revision:el.getAttribute('data-rt-revision')}))),errors,source:read()}));throw error;}};
