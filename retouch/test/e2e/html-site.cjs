...
await page.getByLabel('Image path',{exact:true}).fill('/second.svg');await page.getByRole('button',{name:'Apply image path',exact:true}).click();
  ...
  async function marqueeGesture(region,{append=false,cancel=false,reverse=false}={}){
   ...
   const tag=await app.locator('body').evaluate((_,p)=>document.elementFromPoint(p.x,p.y)?.tagName,start);assert.ok(['HTML','BODY'].includes(tag),'marquee starts on empty canvas');
   if(append)await page.keyboard.down('Shift');await page.mouse.move(a.x,a.y);await page.mouse.down();await page.mouse.move(b.x,b.y,{steps:12});await wait(async()=>await page.locator('.selection-marquee').count()===1,'marquee rectangle visible');assert.equal(read(),original,'marquee preview does not write');if(process.env.RT_E2E_MARQUEE_SCREENSHOT)await page.screenshot({path:process.env.RT_E2E_MARQUEE_SCREENSHOT});
