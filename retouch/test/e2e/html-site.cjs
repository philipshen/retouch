...
(async()=>{
 ...
 try{
  ...
  await settled();
  assert.equal(await page.getByLabel('Font size (CSS)',{exact:true}).locator('xpath=ancestor::*[contains(@class,"property-pair")]').count(),1);
  await page.getByRole('button',{name:'Align text center',exact:true}).click();await settled();await wait(async()=>await app.locator('h1').evaluate(el=>getComputedStyle(el).textAlign)==='center','HTML text alignment');
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===original,'HTML alignment exact undo');
  await page.locator('#panel').evaluate(panel=>{panel.scrollTop+=panel.querySelector('[data-section=typography]').getBoundingClientRect().top-panel.getBoundingClientRect().top-48;});await page.locator('#panel').screenshot({path:'/private/tmp/retouch-html-type-'+engine+'.png'});
  await page.getByText('Individual corners',{exact:true}).click();
  const topLeft=page.getByLabel('Top left corner (CSS)',{exact:true});assert.equal(await topLeft.locator('xpath=ancestor::*[contains(@class,"corner-fields")]').count(),1);
  await topLeft.fill('12px');await topLeft.press('Tab');await settled();await wait(async()=>await app.locator('h1').evaluate(el=>getComputedStyle(el).borderTopLeftRadius)==='12px','individual corner');
  assert.equal(await app.locator('h1').evaluate(el=>getComputedStyle(el).borderTopRightRadius),'0px');
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===original,'corner exact undo');
  const padding=page.getByLabel('Padding (CSS)',{exact:true}),paddingTop=page.getByLabel('Padding top (CSS)',{exact:true});
  assert.equal(await paddingTop.isVisible(),false,'Individual padding starts collapsed');
  await padding.fill('8px');await padding.press('Tab');await settled();
  await wait(async()=>await app.locator('h1').evaluate(el=>getComputedStyle(el).padding)==='8px','uniform padding');
  await page.getByText('Individual padding',{exact:true}).click();await paddingTop.fill('20px');await paddingTop.press('Tab');await settled();
  await wait(async()=>await app.locator('h1').evaluate(el=>getComputedStyle(el).paddingTop)==='20px','individual top padding');
  assert.equal(await app.locator('h1').evaluate(el=>getComputedStyle(el).paddingRight),'8px','Other sides retain uniform padding');
  assert.equal(await paddingTop.isVisible(),true,'Individual padding stays open after editing');
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();
  await wait(async()=>await app.locator('h1').evaluate(el=>getComputedStyle(el).padding)==='8px','individual padding undo');
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===original,'all padding exact undo');
  await page.getByText('Individual padding',{exact:true}).click();assert.equal(await paddingTop.isVisible(),false);
  const minimum=page.getByLabel('Minimum width (CSS)',{exact:true});assert.equal(await minimum.isVisible(),false);
  await page.getByText('Size limits',{exact:true}).click();await minimum.fill('48px');await minimum.press('Tab');await settled();
  await wait(async()=>await app.locator('h1').evaluate(el=>getComputedStyle(el).minWidth)==='48px','minimum width');
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===original,'size limit exact undo');
  await page.getByText('Size limits',{exact:true}).click();
  await page.getByText('Outer spacing',{exact:true}).click();const margin=page.getByLabel('Margin left (CSS)',{exact:true});await margin.fill('12px');await margin.press('Tab');await settled();
  await wait(async()=>await app.locator('h1').evaluate(el=>getComputedStyle(el).marginLeft)==='12px','margin left');
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===original,'margin exact undo');await page.getByText('Outer spacing',{exact:true}).click();
  const direction=page.getByLabel('Direction (CSS)',{exact:true}),display=page.getByLabel('Display (CSS)',{exact:true});assert.equal(await direction.isVisible(),false);
  await display.selectOption('flex');await settled();await wait(async()=>await app.locator('h1').evaluate(el=>getComputedStyle(el).display)==='flex','contextual flex controls');assert.equal(await direction.isVisible(),true);assert.equal(await page.getByLabel('Gap (CSS)',{exact:true}).isVisible(),true);
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===original,'layout mode exact undo');assert.equal(await direction.isVisible(),false);
  await display.selectOption('grid');await settled();await wait(async()=>await app.locator('h1').evaluate(el=>getComputedStyle(el).display)==='grid','contextual grid controls');assert.equal(await direction.isVisible(),false);assert.equal(await page.getByLabel('Align items (CSS)',{exact:true}).isVisible(),true);
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===original,'grid mode exact undo');
  const size=async value=>{await page.getByLabel('Screen size',{exact:true}).focus();await page.getByLabel('Screen size',{exact:true}).selectOption(value);await wait(()=>page.locator('#app').evaluate((el,w)=>el.contentWindow.innerWidth===w,Number(value.split('x')[0])),'screen width');await page.waitForFunction(()=>!panelTasks&&!sourceRequests&&!undoBusy);};
  ...
  if(process.env.RT_E2E_PADDING_SCREENSHOT){await padding.scrollIntoViewIfNeeded();await page.locator('#panel').screenshot({path:process.env.RT_E2E_PADDING_SCREENSHOT});}
  ...
  if(!await typeOpener.evaluate(el=>el.parentElement.open))await typeOpener.click();
...
await page.getByLabel('Image path',{exact:true}).fill('/second.svg');await page.getByRole('button',{name:'Apply image path',exact:true}).click();
  ...
  await page.getByRole('button',{name:'Add gradient',exact:true}).click();await wait(async()=>(await effect('background-image')).startsWith('linear-gradient(90deg'),'add gradient');await settled();const beforeLastFillRemoval=read();await page.getByRole('button',{name:'Remove fill 1',exact:true}).press('Enter');await wait(()=>read()!==beforeLastFillRemoval);await settled();await wait(async()=>await page.getByRole('button',{name:'Add paint',exact:true}).evaluate(el=>el===document.activeElement));await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===beforeLastFillRemoval);await settled();const pickerCanvas=()=>app.locator('h1').evaluate(el=>({image:getComputedStyle(el).backgroundImage,style:el.getAttribute('style')})),pickerCanvasBefore=await pickerCanvas();const beforePicker=read(),pickerButton=page.getByRole('button',{name:'Edit Fill 1 stop 1 color',exact:true});await pickerButton.click();const picker=page.getByRole('dialog',{name:'Edit Fill 1 stop 1 color',exact:true});await picker.getByLabel('Color value',{exact:true}).fill('hsl(210deg 50% 40% / 50%)');await page.evaluate(()=>renderPanel());assert.ok(await picker.isVisible());assert.equal(await picker.getByLabel('Color value',{exact:true}).inputValue(),'hsl(210deg 50% 40% / 50%)');await picker.getByRole('slider',{name:'Saturation and brightness',exact:true}).press('ArrowRight');assert.notEqual((await pickerCanvas()).image,pickerCanvasBefore.image);assert.ok((await pickerCanvas()).style.includes('background-image:'));assert.equal(read(),beforePicker);await page.keyboard.press('Escape');await picker.waitFor({state:'detached'});assert.equal(read(),beforePicker);assert.deepEqual(await pickerCanvas(),pickerCanvasBefore);await pickerButton.click();await picker.getByLabel('Color value',{exact:true}).fill('not-a-color');assert.equal(await picker.getByRole('button',{name:'Apply color',exact:true}).isDisabled(),true);await picker.getByLabel('Color value',{exact:true}).press('Enter');assert.ok(await picker.isVisible());assert.equal(read(),beforePicker);await picker.getByLabel('Color value',{exact:true}).fill('color(display-p3 1 0.2 0.3 / 0.4)');assert.ok(await picker.getByLabel('Display P3 red',{exact:true}).isVisible());await picker.getByRole('slider',{name:'Opacity (%)',exact:true}).press('Home');assert.ok((await picker.getByLabel('Color value',{exact:true}).inputValue()).includes('display-p3 1 0.2 0.3 / 0'));await picker.getByLabel('Color value',{exact:true}).fill('#33669980');await picker.getByLabel('Opacity value (%)',{exact:true}).fill('50');assert.equal(await picker.getByLabel('Color value',{exact:true}).inputValue(),'rgb(51 102 153 / 0.5)');await picker.getByLabel('Red (0–255)',{exact:true}).fill('90');assert.equal(await picker.getByLabel('Color value',{exact:true}).inputValue(),'rgb(90 102 153 / 0.5)');await picker.getByLabel('Color model',{exact:true}).selectOption('hsl');assert.equal(await picker.getByLabel('Color value',{exact:true}).inputValue(),'rgb(90 102 153 / 0.5)');await picker.getByLabel('Hue (deg)',{exact:true}).fill('120');if(process.env.RT_E2E_PICKER_SCREENSHOT)await page.screenshot({path:process.env.RT_E2E_PICKER_SCREENSHOT});await picker.getByLabel('Color value',{exact:true}).press('Enter');await wait(()=>read()!==beforePicker);await settled();const savedPickerColor=await page.getByLabel('Fill 1 stop 1 color',{exact:true}).evaluate(el=>RetouchPaletteValues.parse(RetouchColorStyles.fromComputed(el.value)));assert.equal(savedPickerColor.space,'srgb');assert.deepEqual(savedPickerColor.channels.map(value=>Math.round(value*255)),[90,153,90]);assert.equal(savedPickerColor.alpha,.5);await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===beforePicker);await settled();assert.deepEqual(await pickerCanvas(),pickerCanvasBefore);const stopGeometry=await page.getByLabel('Fill 1 stop 1 color',{exact:true}).evaluate(el=>{const row=el.closest('.gradient-stop-row'),color=el.getBoundingClientRect(),position=row.querySelector('input[type=number]').getBoundingClientRect(),remove=row.querySelector(':scope > button').getBoundingClientRect();return {tabOrder:row.querySelector('input').type==='number',height:row.getBoundingClientRect().height,ordered:position.right<=color.left+1&&color.right<=remove.left+1,aligned:Math.abs(position.top-color.top)<2};});assert.equal(stopGeometry.tabOrder,true);assert.ok(stopGeometry.height<=36);assert.equal(stopGeometry.ordered,true);assert.equal(stopGeometry.aligned,true);
  ...
  const draftColor=page.getByLabel('Fill 1 stop 2 color',{exact:true}),draftBefore=read(),initialColor=await draftColor.inputValue();const swatch=draftColor.locator('xpath=..').locator('.gradient-stop-swatch');assert.equal(await swatch.isVisible(),true);const initialSwatch=await swatch.evaluate(el=>el.style.backgroundImage);await draftColor.fill('not-a-color');await draftColor.press('Enter');assert.ok(await draftColor.evaluate(el=>el.validationMessage));assert.equal(read(),draftBefore);await draftColor.press('Escape');assert.equal(await draftColor.inputValue(),initialColor);assert.equal(await draftColor.evaluate(el=>el.validationMessage),'');await draftColor.fill('#123456');assert.ok((await swatch.evaluate(el=>el.style.backgroundImage)).includes('rgb(18, 52, 86)'));assert.equal(read(),draftBefore);await draftColor.press('Escape');await settled();assert.equal(await swatch.evaluate(el=>el.style.backgroundImage),initialSwatch);assert.equal(read(),draftBefore);const draftPosition=page.getByLabel('Fill 1 stop 2 position (%)',{exact:true}),initialPosition=await draftPosition.inputValue();await draftPosition.fill('17');await draftPosition.press('Escape');await settled();assert.equal(await draftPosition.inputValue(),initialPosition);assert.equal(read(),draftBefore);  await page.getByLabel('Fill 1 stop 2 color',{exact:true}).fill('#00ff00');await page.getByLabel('Fill 1 stop 2 color',{exact:true}).press('Enter');await wait(async()=>(await effect('background-image')).includes('rgb(0, 255, 0) 50%'),'gradient stop color');await settled();
  ...
  const beforeCenterMove=read();await page.getByRole('button',{name:'Fill 1 center',exact:true}).press('ArrowDown');await wait(async()=>(await effect('background-image')).includes('25% 51%'),'keyboard gradient center');await settled();await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===beforeCenterMove,'center move exact undo');await settled();
  ...
  assert.equal(await page.getByLabel('Fill 1 Repeat',{exact:true}).isVisible(),false);await page.getByLabel('Fill 1 options',{exact:true}).click();  await page.getByLabel('Fill 1 Repeat',{exact:true}).check();await wait(async()=>(await effect('background-image')).startsWith('repeating-conic-gradient('),'repeating fill');await settled();
  ...
  const duplicateBefore=read(),stack=()=>page.evaluate(()=>RetouchHTMLCSSValues.parseGradients(getComputedStyle(document.querySelector('#app').contentDocument.querySelector('h1')).backgroundImage)),stackBefore=await stack();await page.getByRole('button',{name:'Duplicate fill 1',exact:true}).click();await wait(()=>read()!==duplicateBefore);await settled();assert.deepEqual(await stack(),[stackBefore[0],stackBefore[0],...stackBefore.slice(1)]);await wait(async()=>await page.getByLabel('Fill 2 type',{exact:true}).evaluate(el=>el===document.activeElement));const beforeFillRemoval=read();await page.getByRole('button',{name:'Remove fill 1',exact:true}).press('Enter');await wait(()=>read()!==beforeFillRemoval);await settled();await wait(async()=>await page.getByLabel('Fill 1 type',{exact:true}).evaluate(el=>el===document.activeElement));await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===beforeFillRemoval);await settled();const actionsGeometry=await page.getByRole('button',{name:'Move fill 1 down',exact:true}).evaluate(el=>{const bar=el.closest('.gradient-actions'),legend=bar.closest('legend').getBoundingClientRect(),rect=bar.getBoundingClientRect();return {count:bar.querySelectorAll('button').length,height:rect.height,fits:rect.right<=legend.right+1&&rect.left>=legend.left,icons:[...bar.querySelectorAll('button')].every(button=>button.querySelector('svg')&&button.getAttribute('aria-label')&&button.title)};});assert.equal(actionsGeometry.count,3);assert.ok(actionsGeometry.height<=28);assert.ok(actionsGeometry.fits);assert.ok(actionsGeometry.icons);const keyboardBefore=read(),actionBar=page.getByRole('toolbar',{name:'Fill 1 actions',exact:true}),moveDown=page.getByRole('button',{name:'Move fill 1 down',exact:true});assert.equal(await actionBar.locator('button[tabindex="0"]').count(),1);await moveDown.focus();await page.keyboard.press('ArrowRight');assert.equal(await page.evaluate(()=>document.activeElement.getAttribute('aria-label')),'Duplicate fill 1');await page.keyboard.press('End');assert.equal(await page.evaluate(()=>document.activeElement.getAttribute('aria-label')),'Remove fill 1');await page.keyboard.press('ArrowRight');assert.equal(await page.evaluate(()=>document.activeElement.getAttribute('aria-label')),'Move fill 1 down');await page.keyboard.press('ArrowLeft');assert.equal(await page.evaluate(()=>document.activeElement.getAttribute('aria-label')),'Remove fill 1');await page.keyboard.press('Home');assert.equal(await page.evaluate(()=>document.activeElement.getAttribute('aria-label')),'Move fill 1 down');assert.equal(read(),keyboardBefore);if(process.env.RT_E2E_ACTIONS_SCREENSHOT)await page.screenshot({path:process.env.RT_E2E_ACTIONS_SCREENSHOT});const beforeReverse=read(),reverseStack=await stack();await page.getByRole('button',{name:'Reverse fill 1 stops',exact:true}).press('Enter');await wait(()=>read()!==beforeReverse);await settled();const reversedStack=await stack();assert.deepEqual(reversedStack,[{...reverseStack[0],stops:reverseStack[0].stops.map(stop=>({...stop,position:100-stop.position})).reverse()},...reverseStack.slice(1)]);await wait(async()=>await page.getByRole('button',{name:'Reverse fill 1 stops',exact:true}).evaluate(el=>el===document.activeElement));await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===beforeReverse);await settled();const duplicatedSource=read();await page.getByLabel('Fill 2 stop 1 color',{exact:true}).fill('#ffee00');await page.getByLabel('Fill 2 stop 1 color',{exact:true}).press('Enter');await wait(()=>read()!==duplicatedSource);await settled();const changed=await stack();assert.deepEqual(changed[0],stackBefore[0]);assert.notDeepEqual(changed[1],stackBefore[0]);assert.deepEqual(changed.slice(2),stackBefore.slice(1));const beforeGradientMove=read();await page.getByRole('button',{name:'Move fill 1 down',exact:true}).press('Enter');await wait(()=>read()!==beforeGradientMove);await settled();assert.deepEqual(await stack(),[changed[1],changed[0],...changed.slice(2)]);await wait(async()=>await page.getByLabel('Fill 2 type',{exact:true}).evaluate(el=>el===document.activeElement));await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();assert.equal(read(),beforeGradientMove);await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();assert.equal(read(),duplicatedSource);await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();assert.equal(read(),duplicateBefore);await page.getByRole('button',{name:'Redo',exact:true}).click();await settled();assert.equal(read(),duplicatedSource);await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();assert.equal(read(),duplicateBefore);
  ...
  const canvasRegion=async tags=>app.locator('body').evaluate((_,tags)=>{const boxes=tags.map(tag=>document.querySelector(tag).getBoundingClientRect());return {start:{x:Math.min(...boxes.map(b=>b.left))-24,y:Math.min(...boxes.map(b=>b.top))-24},end:{x:Math.max(...boxes.map(b=>b.right))+24,y:Math.max(...boxes.map(b=>b.bottom))+24},width:innerWidth,height:innerHeight};},tags);
  async function marqueeGesture(region,{append=false,cancel=false,reverse=false,gray=false}={}){
   ...
   if(gray){assert.ok(['frameWrap','canvasExtent','siteStage'].includes(await page.evaluate(p=>document.elementFromPoint(p.x,p.y)?.id,a)),'marquee starts on gray canvas');}else{const tag=await app.locator('body').evaluate((_,p)=>document.elementFromPoint(p.x,p.y)?.tagName,start);assert.ok(['HTML','BODY'].includes(tag),'marquee starts on empty canvas');assert.equal(await page.evaluate(p=>document.elementFromPoint(p.x,p.y)?.id,a),'app','empty canvas must be clear of selection handles');}
