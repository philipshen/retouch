...
await page.getByLabel('Image path',{exact:true}).fill('/second.svg');await page.getByRole('button',{name:'Apply image path',exact:true}).click();
  ...
  await page.getByRole('treeitem',{name:'h1 · Hello HTML',exact:true}).click();await settled();
  await size('390x844');await page.getByLabel('Style screen scope').selectOption('');
  await page.getByRole('button',{name:'Add gradient',exact:true}).click();await wait(async()=>(await effect('background-image')).startsWith('linear-gradient(90deg'),'add gradient');await settled();const beforeLastFillRemoval=read();await page.getByRole('button',{name:'Remove fill 1',exact:true}).press('Enter');await wait(()=>read()!==beforeLastFillRemoval);await settled();await wait(async()=>await page.getByRole('button',{name:'Add gradient',exact:true}).evaluate(el=>el===document.activeElement));await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===beforeLastFillRemoval);await settled();const pickerCanvas=()=>app.locator('h1').evaluate(el=>({image:getComputedStyle(el).backgroundImage,style:el.getAttribute('style')})),pickerCanvasBefore=await pickerCanvas();const beforePicker=read(),pickerButton=page.getByRole('button',{name:'Edit Fill 1 stop 1 color',exact:true});await pickerButton.click();const picker=page.getByRole('dialog',{name:'Edit Fill 1 stop 1 color',exact:true});await picker.getByLabel('Color value',{exact:true}).fill('#33669980');await picker.getByRole('slider',{name:'Saturation and brightness',exact:true}).press('ArrowRight');assert.notEqual((await pickerCanvas()).image,pickerCanvasBefore.image);assert.ok((await pickerCanvas()).style.includes('background-image:'));assert.equal(read(),beforePicker);await page.keyboard.press('Escape');await picker.waitFor({state:'detached'});assert.equal(read(),beforePicker);assert.deepEqual(await pickerCanvas(),pickerCanvasBefore);await pickerButton.click();await picker.getByLabel('Color value',{exact:true}).fill('not-a-color');await picker.getByRole('button',{name:'Apply color',exact:true}).click();assert.ok(await picker.isVisible());assert.equal(read(),beforePicker);await picker.getByLabel('Color value',{exact:true}).fill('color(display-p3 1 0.2 0.3 / 0.4)');assert.ok(await picker.getByLabel('Display P3 red',{exact:true}).isVisible());await picker.getByRole('slider',{name:'Opacity (%)',exact:true}).press('Home');assert.ok((await picker.getByLabel('Color value',{exact:true}).inputValue()).includes('display-p3 1 0.2 0.3 / 0'));await picker.getByLabel('Color value',{exact:true}).fill('#33669980');await picker.getByLabel('Opacity value (%)',{exact:true}).fill('50');assert.equal(await picker.getByLabel('Color value',{exact:true}).inputValue(),'rgb(51 102 153 / 0.5)');await picker.getByLabel('Red (0–255)',{exact:true}).fill('90');assert.equal(await picker.getByLabel('Color value',{exact:true}).inputValue(),'rgb(90 102 153 / 0.5)');if(process.env.RT_E2E_PICKER_SCREENSHOT)await page.screenshot({path:process.env.RT_E2E_PICKER_SCREENSHOT});await picker.getByRole('button',{name:'Apply color',exact:true}).click();await wait(()=>read()!==beforePicker);await settled();assert.equal(await page.getByLabel('Fill 1 stop 1 color',{exact:true}).evaluate(el=>RetouchColorStyles.fromComputed(el.value)),'#5a669980');await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===beforePicker);await settled();assert.deepEqual(await pickerCanvas(),pickerCanvasBefore);const stopGeometry=await page.getByLabel('Fill 1 stop 1 color',{exact:true}).evaluate(el=>{const row=el.closest('.gradient-stop-row'),color=el.getBoundingClientRect(),position=row.querySelector('input[type=number]').getBoundingClientRect(),remove=row.querySelector(':scope > button').getBoundingClientRect();return {tabOrder:row.querySelector('input').type==='number',height:row.getBoundingClientRect().height,ordered:position.right<=color.left+1&&color.right<=remove.left+1,aligned:Math.abs(position.top-color.top)<2};});assert.equal(stopGeometry.tabOrder,true);assert.ok(stopGeometry.height<=36);assert.equal(stopGeometry.ordered,true);assert.equal(stopGeometry.aligned,true);
