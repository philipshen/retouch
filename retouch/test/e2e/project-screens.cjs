...
(async()=>{
 ...
 try{
  ...
  const ratio=()=>page.getByRole('button',{name:'Lock Reading view comparison aspect ratio',exact:true});await ratio().click();assert.equal(await ratio().getAttribute('aria-pressed'),'true');const linkedWidth=page.getByLabel('Reading view comparison width',{exact:true});await linkedWidth.fill('2240');await linkedWidth.press('Enter');assert.deepEqual(await comparisonSize(),[2240,1688]);await page.getByRole('button',{name:'Undo Reading view comparison size',exact:true}).click();assert.deepEqual(await comparisonSize(),[1120,844]);await ratio().click();
  ...
  assert.equal(await ratio().getAttribute('aria-pressed'),'true','Undo screen-set load restores ratio lock');
