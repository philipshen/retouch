...
exports.run=async({page,app,phone:live,file,original:source,state})=>{
 ...
 assert.equal(await live.locator('a[aria-label=Docs]').getAttribute('href'),url);
 assert.equal(await anchor.locator('strong').textContent(),'Read');assert.equal(await anchor.getAttribute('target'),'_blank');assert.ok(changed.includes('onclick={()=>count++}'));
 ...
 for(const [text,href] of [[removed,null],[changed,url],[source,'/old']]){await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();assert.equal(read(),text);assert.equal(await anchor.getAttribute('href'),href);assert.equal(await live.locator('a[aria-label=Docs]').getAttribute('href'),href);await state();}
 for(const [text,href] of [[changed,url],[removed,null],[restored,'#restored']]){await page.getByRole('button',{name:'Redo',exact:true}).click();await settled();assert.equal(read(),text);assert.equal(await anchor.getAttribute('href'),href);assert.equal(await live.locator('a[aria-label=Docs]').getAttribute('href'),href);await state();}
 ...
 console.log('SVELTE LINK DESTINATION, REMOVAL, VALIDATION, BINDING GUARD AND EXACT HISTORY PASS');
};
