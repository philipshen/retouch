...
exports.run=async({page,app,phone,file,original,state,server})=>{
 ...
 const assertArtworkPosition=async top=>{const view=await page.evaluate(()=>{const node=document.querySelector('.picture-sources'),panel=document.getElementById('panel'),box=node.getBoundingClientRect(),clip=panel.getBoundingClientRect();return {top:box.top,bottom:box.bottom,clipTop:clip.top,clipBottom:clip.bottom,scroll:panel.scrollTop,max:panel.scrollHeight-panel.clientHeight};});const wanted=Math.max(0,Math.min(view.max,view.scroll+view.top-top));assert.ok(Math.abs(view.scroll-wanted)<2,'Artwork position changed beyond the scroll limits: '+JSON.stringify({before:top,...view}));assert.ok(view.bottom>view.clipTop&&view.top<view.clipBottom,'Artwork controls remain in view');};
 ...
 const idle=()=>page.waitForFunction(()=>!undoBusy&&!sourceRequests&&!panelTasks&&!viewportRenderPending&&!panelPointer),press=async name=>{await page.getByRole('button',{name,exact:true}).click();await idle();};
 ...
 await press('Add picture source');assert.deepEqual(readAll(),originalFiles);await page.getByRole('alert').filter({hasText:'inline or injected'}).waitFor();await app.locator('#picture-ui-unmapped').evaluate(node=>node.remove());await page.evaluate(()=>renderPanel());assert.equal(await page.getByLabel('Artwork image path',{exact:true}).inputValue(),'/small.svg');assert.equal(await page.getByLabel('Artwork maximum width (px)',{exact:true}).inputValue(),'600');
 await idle();await page.evaluate(()=>[...document.querySelectorAll('.picture-sources button')].find(button=>button.textContent==='Add picture source').scrollIntoView({block:'nearest'}));const artworkTop=await page.evaluate(()=>document.querySelector('.picture-sources').getBoundingClientRect().top);
 await press('Add picture source');await assertArtworkPosition(artworkTop);const changedFiles=readAll();assert.notDeepEqual(changedFiles,originalFiles);
 ...
 for(const [direction,wrapped] of [['Undo',false],['Redo',true],['Undo',false]]){const top=await page.evaluate(()=>document.querySelector('.picture-sources').getBoundingClientRect().top);await press(direction);await settled(wrapped);assert.deepEqual(readAll(),wrapped?changedFiles:originalFiles);await assertArtworkPosition(top);}
