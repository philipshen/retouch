...
exports.run=async({page,app,read,wait,settled,kind})=>{
 ...
 const scopedHidden=read();await page.getByLabel('Style screen scope').selectOption('');await settled();assert.equal(await alpha.inputValue(),'37.5');await page.getByRole('button',{name:'Hide background color',exact:true}).waitFor();assert.equal((await rendered()).alpha,0);await color.focus();await color.press('Tab');await settled();assert.equal(read(),scopedHidden);await page.getByLabel('Style screen scope').selectOption(kind==='html'?'min-[768px]:':'md:');await settled();
 await size('390x844');assert.equal((await rendered()).alpha,.375);
 const eye=page.locator('.background-visibility');await wait(async()=>await eye.isEnabled()&&await eye.getAttribute('aria-label')==='Show background color');const outside=read();await eye.click();await wait(()=>read()!==outside);await settled();assert.equal((await rendered()).alpha,.375,'inactive tablet visibility leaves the phone unchanged');await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===outside);await settled();
