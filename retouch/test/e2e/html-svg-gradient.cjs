...
(async()=>{
 ...
 const original='<html><head></head><body><svg width="400" height="240" viewBox="0 0 400 240"><defs><linearGradient id="paint"><stop offset="0" stop-color="#ff0000"/><stop offset="100%" stop-color="#0000ff"/></linearGradient><radialGradient id="radial"><stop offset="0" stop-color="white"/><stop offset="1" stop-color="black"/></radialGradient></defs><rect aria-label="Gradient box" x="10" y="10" width="180" height="100" fill="url(#paint)"/><circle aria-label="Shared gradient" cx="260" cy="60" r="50" fill="url(#paint)"/><rect aria-label="Radial box" x="10" y="140" width="180" height="80" fill="url(#radial)"/></svg></body></html>';
 ...
 try{
  ...
  await require('./svg-gradient-detach.cjs').run({page,app,file,wait,settled});
