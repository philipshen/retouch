...
module.exports=async({page,app,read,settled,wait})=>{
 ...
 for(const retained of [1,2]){
  ...
  if(retained===2){await require('./svg-retained-stroke-shared-properties.cjs')({page,app,read,settled,wait});await require('./svg-retained-stroke-shared-paints.cjs')({page,app,read,settled,wait});}
