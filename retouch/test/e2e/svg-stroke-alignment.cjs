...
(async()=>{const browser=await require(path.join(fixture,'node_modules/playwright'))[engine].launch();let checks=0;try{
 ...
 for(const [name,data,fillRule]of shapes)for(const position of ['inside','center','outside'])for(const matrix of [[1,0,0,1,0,0],[.75,0,0,1.1,10,-5],[.8,.2,-.2,.8,20,0]])for(const scale of [2,3]){
  const model={document:G.parseCompound(data),width:8,position,fillRule,matrix,fill:'#ff0000',stroke:'#0000ff'},before=JSON.stringify(model),markup=S.render(model,'rt-stroke-0123456789abcdef');assert.equal(JSON.stringify(model),before);
 ...
 assert.deepEqual(errors,[]);console.log(engine+': PASS '+checks+' stroke alignment pixel checks across rectangles, arcs, rounded curves, evenodd/nonzero holes, affine transforms and viewBox scales');
