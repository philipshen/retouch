...
(async()=>{
 ...
 const close=(a,b)=>assert.ok(Math.abs(a-b)<.3,`${a} differs from ${b}`),box=()=>app.locator('.box').evaluate(el=>{const r=el.getBoundingClientRect(),p=el.parentElement,pr=p.getBoundingClientRect();return{x:r.left-pr.left-p.clientLeft,y:r.top-pr.top-p.clientTop};});
 ...
 try{
  ...
  console.log(engine+': PASS sibling/container snapping at 50/100/200% zoom, visible guides, hidden sibling exclusion, Alt bypass, Shift lock, precise keyboard nudges, source isolation and exact undo');
