...
module.exports=async({page,shape,read,wait,settled,screenshot,scoped=false,kind='html'})=>{
 const screen=page.getByLabel('Screen size',{exact:true});
 if(scoped){await screen.focus();await screen.selectOption('768x1024');await settled();await page.getByLabel('Style screen scope',{exact:true}).selectOption(kind==='html'?'min-[768px]:':'md:');await settled();}
 const original=read();
 for(const paint of ['fill','stroke']){
  const appearance=()=>shape.evaluate((el,p)=>getComputedStyle(el).getPropertyValue(p),paint),initial=await appearance();
  const phoneUnchanged=async()=>{if(!scoped)return;const source=read();await screen.focus();await screen.selectOption('390x844');await settled();await wait(async()=>await appearance()===initial);assert.equal(read(),source);await screen.focus();await screen.selectOption('768x1024');await settled();};
  if(initial!=='none'){await page.getByRole('button',{name:'Remove '+paint,exact:true}).click();await wait(()=>read()!==original);await settled();}
  await phoneUnchanged();await wait(async()=>await appearance()==='none');const empty=read(),section=page.locator('#panelBody > [data-section="'+paint+'"]'),add=page.getByRole('button',{name:'Add '+paint,exact:true});
  ...
  await add.click();await page.getByRole('menu',{name:'Add '+paint,exact:true}).waitFor();await page.evaluate(async()=>{queueViewportPanelRefresh();await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));});assert.equal(await page.getByRole('menu',{name:'Add '+paint,exact:true}).count(),1,'inspector refresh preserves the open paint menu');await page.keyboard.press('Escape');assert.equal(read(),empty);assert.equal(await add.evaluate(el=>el===document.activeElement),true);
  ...
  const painted=read(),paintedAppearance=await appearance();await phoneUnchanged();await wait(async()=>await appearance()===paintedAppearance);assert.equal(read(),painted);await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===empty);await settled();await add.waitFor();
  ...
  }else if(initial!=='none'){await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===original);await settled();}
