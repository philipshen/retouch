...
module.exports=async({page,app,select,read,wait,settled,original,kind})=>{
 ...
 await select('rect','Box');await app.locator('body').evaluate(el=>{el.tabIndex=-1;el.focus();});await page.keyboard.down('ArrowLeft');await surface.waitFor();await page.keyboard.press('Escape');await page.keyboard.up('ArrowLeft');assert.equal(read(),original);assert.equal(await surface.isVisible(),false);assert.equal(Number(await page.getByRole('textbox',{name:'Vector width',exact:true}).inputValue()),60);
 for(const zoom of ['50','200']){
  const zoomInput=page.getByLabel('Canvas zoom (%)',{exact:true});await zoomInput.fill(zoom);await zoomInput.press('Enter');await select('rect','Box');const target=app.locator('[aria-label="Box"]'),before=await center(target);await app.locator('body').evaluate(el=>el.focus());await page.keyboard.press('ArrowRight');await settled();await wait(()=>read()!==original);const after=await center(target);assert.ok(Math.abs(after.x-before.x-1)<.1&&Math.abs(after.y-before.y)<.1,'one document pixel at '+zoom+'% zoom');await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===original);
 }
