...
(async()=>{
 ...
 const choose=async name=>{await page.getByRole('treeitem',{name,exact:true}).click();await page.getByLabel('Arrange children',{exact:true}).waitFor({state:'attached'});if(!await page.getByLabel('Arrange children',{exact:true}).isVisible())await page.getByLabel('Layout options',{exact:true}).locator(':scope > summary').click();};
 ...
 try {
  ...
  await action(()=>page.getByRole('combobox',{name:'Align children',exact:true}).selectOption('center'),async()=>await css('alignItems')==='center','alignment');
