...
(async()=>{
 ...
 try{
  ...
  await run('Redo last edit');await wait(async()=>fs.readFileSync(file,'utf8')===source&&await app.locator('[data-rt-frame]').count()===0);await wait(locked);await selectParents();await run('Move into…');await page.getByLabel('Destination container',{exact:true}).selectOption({label:'footer'});await page.getByRole('button',{name:'Move layer',exact:true}).click();await wait(async()=>await app.locator('footer > section,footer > aside').count()===2);await wait(locked);const moved=fs.readFileSync(file,'utf8');assert.ok(!moved.includes('data-rt-move-'));
  await run('Undo last edit');await wait(async()=>fs.readFileSync(file,'utf8')===source&&await app.locator('main > section,main > aside').count()===2);await wait(locked);
  await run('Redo last edit');await wait(async()=>fs.readFileSync(file,'utf8')===moved&&await app.locator('footer > section,footer > aside').count()===2);await wait(locked);
  await run('Undo last edit');await wait(async()=>fs.readFileSync(file,'utf8')===source&&await app.locator('main > section,main > aside').count()===2);await wait(locked);assert.deepEqual(errors,[]);console.log('MULTI-LAYER REPARENT/DESCENDANT AND SIBLING LOCKS/UNDO/REDO PASS',engine);console.log('FRAME/UNFRAME/DESCENDANT AND SIBLING LOCKS/UNDO/REDO PASS',engine);console.log('MULTI-LAYER COPY/DELETE/DESCENDANT AND SIBLING LOCKS/UNDO/REDO PASS',engine);
