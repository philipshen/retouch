...
(async()=>{
 ...
 try{
  ...
  await page.getByLabel('Find a layer',{exact:true}).fill('A');await action('Select sibling layers','h1 · A');assert.equal(await page.getByLabel('Find a layer',{exact:true}).inputValue(),'');assert.deepEqual(await page.getByRole('treeitem',{selected:true}).allTextContents(),['h1 · A','p · C']);assert.equal(await page.getByRole('treeitem',{name:'span · Nested',exact:true}).getAttribute('aria-selected'),'false');
