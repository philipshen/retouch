...
module.exports=async({page,shape,read,wait,settled,screenshot})=>{
 const initiallyPath=await shape.evaluate(el=>el.localName)==='path',converted=!!process.env.RT_E2E_ARROW_PATH&&!initiallyPath,usesPath=initiallyPath||converted,dashStates=[];let beforeConversion=read();
 ...
 const original=read(),points=async()=>usesPath?require('../../shell/svg-parametric.js').pointsFromPath(await shape.getAttribute('d')):shape.getAttribute('points'),endpoints=value=>value.split(' ').slice(0,2),before=endpoints(await points()),states=[original];
 ...
 const raw=page.getByLabel(usesPath?'Shape Path data':'Shape Points',{exact:true});
