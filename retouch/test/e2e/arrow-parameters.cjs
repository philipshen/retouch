...
module.exports=async({page,shape,read,wait,settled,screenshot})=>{
 const converted=!!process.env.RT_E2E_ARROW_PATH,dashStates=[];let beforeConversion=read();
 ...
 if(converted){const id=await shape.getAttribute('data-rt'),frame=await(await shape.elementHandle()).ownerFrame();shape=frame.locator('[data-rt="'+id+'"]');await page.getByRole('button',{name:'Convert to vector path',exact:true}).click();await wait(()=>read()!==beforeConversion);await settled();await wait(async()=>await shape.evaluate(el=>el.localName)==='path');}
 const original=read(),points=async()=>converted?require('../../shell/svg-parametric.js').pointsFromPath(await shape.getAttribute('d')):shape.getAttribute('points'),endpoints=value=>value.split(' ').slice(0,2),before=endpoints(await points()),states=[original];
 ...
 const raw=page.getByLabel(converted?'Shape Path data':'Shape Points',{exact:true});
 ...
 await raw.fill(converted?'M0 0 L100 0 M80 5 L100 0 L80 -9':'0,0 100,0 80,5 100,0 80,-9');await raw.press('Tab');await wait(()=>read()!==original);await settled();
