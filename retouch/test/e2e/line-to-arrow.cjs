...
module.exports=async({page,shape,read,wait,settled})=>{
 ...
 for(const property of ['stroke','fill']){
  await shape.evaluate((el,property)=>{const style=el.ownerDocument.createElement('style');style.id='arrow-conversion-override';style.textContent='polyline[data-rt="'+el.getAttribute('data-rt')+'"]{'+property+':red!important}';el.ownerDocument.head.append(style);},property);
 ...
 const arrow=page.frameLocator('#app').locator('[data-rt="'+id+'"]');await wait(async()=>await arrow.evaluate(el=>el.tagName.toLowerCase())==='polyline');
 const actual=await arrow.evaluate(el=>({endpoints:el.getAttribute('points').split(' ').slice(0,2).flatMap(p=>p.split(',').map(Number)),stroke:getComputedStyle(el).stroke,width:getComputedStyle(el).strokeWidth,fill:getComputedStyle(el).fill}));
 assert.deepEqual(actual.endpoints,snapshot.endpoints);assert.equal(actual.stroke,snapshot.stroke);assert.equal(actual.width,snapshot.width);assert.equal(actual.fill,'none');
 await page.getByLabel('Arrowhead length',{exact:true}).waitFor();const converted=read();
 ...
 await page.getByRole('button',{name:'Redo',exact:true}).click();await wait(()=>read()===converted);await settled();await page.getByLabel('Arrowhead length',{exact:true}).waitFor();
