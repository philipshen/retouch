...
(async()=>{
 const root=fs.mkdtempSync(path.join(os.tmpdir(),'retouch-layout-limits-')),file=path.join(root,'app/page.jsx');fs.mkdirSync(path.dirname(file));fs.symlinkSync(path.join(fixture,'node_modules'),path.join(root,'node_modules'),'dir');
 ...
 const original='"use client";export default function Page(){return <main><section id="layout" className="!min-w-[80px] !max-w-[320px] !min-h-[80px] !max-h-[320px] sm:![min-width:100px] sm:![max-width:300px] sm:![min-height:100px] sm:![max-height:300px] md:[max-width:280px] w-[200px] h-[200px]"><div>Content</div></section></main>}';fs.writeFileSync(file,original);const read=()=>fs.readFileSync(file,'utf8');
 ...
 try{
  ...
  const keys=['min-width','max-width','min-height','max-height'];
  const snapshot=()=>parent.evaluate(el=>{const css=getComputedStyle(el);return ['min-width','max-width','min-height','max-height'].map(key=>css.getPropertyValue(key));});
  const initial=['100px','300px','100px','300px'];assert.deepEqual(await snapshot(),initial);
  for(const [index,key] of keys.entries()){
   const before=read(),label=key.replace('min-','Minimum ').replace('max-','Maximum '),input=page.getByLabel(label,{exact:true}),reset=page.getByRole('button',{name:'Reset '+label.toLowerCase(),exact:true});
   assert.equal(await reset.isDisabled(),key!=='max-width');
   await input.fill('-1px');await input.press('Tab');await settled();assert.equal(read(),before);assert.equal(await input.evaluate(el=>el.checkValidity()),false);
   const value=key.startsWith('min-')?'250px':'150px';await input.fill(value);await input.press('Tab');await settled();const expected=initial.map((v,i)=>i===index?value:v);
   await wait(async()=>JSON.stringify(await snapshot())===JSON.stringify(expected));const edited=read();assert.notEqual(edited,before);
   assert.equal(await input.inputValue(),value);
   const axis=key.endsWith('width')?'width':'height';assert.equal(await parent.evaluate((el,axis)=>getComputedStyle(el)[axis],axis),value);
   assert.equal(await parent.evaluate((el,axis)=>getComputedStyle(el)[axis],axis==='width'?'height':'width'),'200px');
   ...
   await page.getByLabel('Screen size',{exact:true}).focus();await page.getByLabel('Screen size',{exact:true}).selectOption('390x844');await wait(async()=>JSON.stringify(await snapshot())==='["80px","320px","80px","320px"]');
  ...
  assert.equal(read(),original);assert.deepEqual(errors,[]);console.log(engine+': PASS four size limits, inherited important constraints, actual dimensions, reset, validation, Phone isolation and exact history');
