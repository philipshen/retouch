...
exports.run=async({page,app,read,wait,settled,kind})=>{
 ...
 if(scaling&&['html','liquid','react'].includes(kind)&&process.env.RT_E2E_SCALED_UNGROUP){
  ...
  if(kind==='react'&&process.env.RT_E2E_RELEASED_NESTED){
   const heading=page.getByRole('treeitem',{name:'h1 · Headline',exact:true});await heading.click();if(await heading.getAttribute('aria-expanded')==='false')await heading.press('ArrowRight');await page.getByRole('treeitem',{name:'span · Nested A',exact:true}).click();await settled();await page.getByRole('treeitem',{selected:true}).press('Meta+d');await wait(()=>read()!==released);await settled();const copied=read();
   const checkNested=async order=>{for(const frame of frames)await wait(async()=>await frame.locator('h1 span').evaluateAll((nodes,order)=>nodes.map(e=>e.getAttribute('data-rt-layer-name')).join(',')===order&&new Set(nodes.map(e=>e.getAttribute('data-rt-scale-member'))).size===nodes.length&&nodes.every(e=>e.ownerDocument[Symbol.for('retouch.group-scale.runtime')]?.manages(e.closest('h1'))),order));};
   await checkNested('Nested A,Nested A,Nested B');await page.getByRole('treeitem',{selected:true}).press('Meta+]');await wait(()=>read()!==copied);await settled();const reordered=read();await checkNested('Nested A,Nested B,Nested A');await page.getByRole('treeitem',{selected:true}).press('Delete');await wait(()=>read()!==reordered);await settled();const deleted=read();await checkNested('Nested A,Nested B');await check(false);
   for(const [source,order]of [[reordered,'Nested A,Nested B,Nested A'],[copied,'Nested A,Nested A,Nested B'],[released,'Nested A,Nested B']]){await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===source);await settled();await checkNested(order);}await check(false);
   for(const [source,order]of [[copied,'Nested A,Nested A,Nested B'],[reordered,'Nested A,Nested B,Nested A'],[deleted,'Nested A,Nested B']]){await page.getByRole('button',{name:'Redo',exact:true}).click();await wait(()=>read()===source);await settled();await checkNested(order);}await check(false);console.log('REACT RELEASED NESTED COPY ORDER DELETE HISTORY PASS',{scoped});
  }
  if(kind==='react'&&process.env.RT_E2E_RELEASED_REORDER){
   await page.getByRole('treeitem',{name:'h1 · Headline',exact:true}).click();await settled();await page.getByRole('treeitem',{selected:true}).press('Meta+]');await wait(()=>read()!==released);await settled();const reordered=read();
   const checkOrder=async order=>{for(let f=0;f<frames.length;f++){const factor=!scoped||f===0||f===3?1.5:1;await wait(async()=>await frames[f].locator('[data-rt-scale-member]').evaluateAll((nodes,{order,factor})=>nodes.map(e=>e.tagName).join(',')===order&&nodes.every(e=>{const scale=getComputedStyle(e).scale;return e.ownerDocument[Symbol.for('retouch.group-scale.runtime')]?.manages(e)&&Math.abs((scale==='none'?1:parseFloat(scale))-factor)<.001;}),{order,factor}));}await wait(async()=>await page.getByRole('treeitem',{name:'h1 · Headline',exact:true}).getAttribute('aria-selected')==='true');};
   await checkOrder('P,H1');await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===released);await settled();await check(false);await checkOrder('H1,P');await page.getByRole('button',{name:'Redo',exact:true}).click();await wait(()=>read()===reordered);await settled();await checkOrder('P,H1');await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===released);await settled();await check(false);console.log('REACT RELEASED ORDER AND HISTORY PASS',{scoped});
  }
