...
module.exports=async({page,app,kind,read,wait,settled})=>{
 ...
 for(const [property,label,choices]of [['font-variant-caps','Capital forms',['small-caps','all-small-caps','normal']],['font-variant-position','Number position',['super','sub','normal']]]){
 ...
 await screen.selectOption('768x1024');await settled();await page.getByLabel('Style screen scope').selectOption(kind==='html'?'min-[768px]:':'md:');await settled();await group();const input=page.getByLabel('Shared '+label,{exact:true});
 for(const value of choices){
  ...
  const reset=page.getByRole('button',{name:'Reset shared '+label.toLowerCase(),exact:true,includeHidden:true});await show(reset);await reset.click();await wait(()=>read()!==changed);await settled();assert.deepEqual(await measure(),initial);await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===changed);await settled();await page.getByRole('button',{name:'Undo',exact:true}).click();await wait(()=>read()===original);await settled();
 }
