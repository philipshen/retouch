...
module.exports=async({page,read,wait,settled})=>{
 ...
 await page.setViewportSize({width:1000,height:280});await wait(()=>page.locator('#main').evaluate(el=>el.classList.contains('compact-workspace')));await settled();const toggle=page.getByRole('button',{name:'Toggle Inspector panel',exact:true});if(await toggle.getAttribute('aria-expanded')==='false')await toggle.click();await opener.click();await dialog.waitFor();await wait(bounded);
