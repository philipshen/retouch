...
(async()=>{
 ...
 try{
  ...
  for(const length of [100,113,150])for(const dash of ['8 6','3 5 9 4','5.5 2.5'])for(const offset of [0,3,7])for(const startArrow of [false,true])for(const endArrow of [false,true]){
   ...
   cases.push({length,dash,offset,startArrow,endArrow,points:model.generate(spec)});
  }
  const results=await page.evaluate(async cases=>{
   ...
   for(const row of cases){
    ...
    let changedPixels=0,separatedChangedPixels=0;
    for(let x=40;x<row.length;x++){if(Math.abs(expected[x*4+3]-actual[x*4+3])>1)changedPixels++;if(Math.abs(expected[x*4+3]-separated[x*4+3])>1)separatedChangedPixels++;}
    results.push({...row,points:undefined,changedPixels,separatedChangedPixels});
   }
   return results;
  },cases);
  const failures=results.filter(row=>row.changedPixels),separatedFailures=results.filter(row=>row.separatedChangedPixels);
  console.log(JSON.stringify({engine,version:browser.version(),total:results.length,currentPassed:results.length-failures.length,separatedPassed:results.length-separatedFailures.length,failures},null,2));
  assert.equal(separatedFailures.length,0,'Independent head geometry must preserve shaft dashes');
  assert.equal(failures.length,0,'Changing arrowheads must preserve shaft dashes');
