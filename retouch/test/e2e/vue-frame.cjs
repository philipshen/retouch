...
exports.run = async ({ page, app, live, file, source }) => {
  ...
  await page.getByRole('button', { name: 'Unlock div · Third', exact: true }).click(); await row('h1 · Hello Vite').click(); await settled();
  console.log('VUE FRAME/GROUP, GROUP LAYOUT, STYLE CLEANUP, LOCKS AND EXACT HISTORY PASS');
};
