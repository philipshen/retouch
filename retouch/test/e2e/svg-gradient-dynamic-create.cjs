...
exports.run=async({page,app,file,wait,settled})=>{
 ...
 assert.ok(original.includes('opacity:active?0.6:0.8'));
 ...
 const interact=async source=>{
  await page.getByRole('button',{name:'Edit mode',exact:true}).click();await solid.click();await wait(async()=>await solid.getAttribute('width')==='60');await wait(async()=>await solid.evaluate(el=>getComputedStyle(el).opacity)==='0.6');await wait(async()=>await dynamic.getAttribute('fill')==='#00ff00');assert.equal(read(),source);
  await solid.click();await wait(async()=>await solid.getAttribute('width')==='50');await wait(async()=>await solid.evaluate(el=>getComputedStyle(el).opacity)==='0.8');await wait(async()=>await dynamic.getAttribute('fill')==='#0000ff');assert.equal(read(),source);await page.getByRole('button',{name:'Interact mode',exact:true}).click();
 };
 ...
 console.log('PASS React SVG gradient creation preserves event handlers, stateful opacity and geometry, independently controlled paint, source identity and exact undo/redo');
};
