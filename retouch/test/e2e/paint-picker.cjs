...
(async()=>{
 ...
 try{
  ...
  const cleanup=await page.evaluate(()=>{
   ...
   el.removeAttribute('style');document.querySelector('input').retouchPaintPreview=start;window.previewElement=el;return {absent,concurrent,external,layers:layers.length,neighbor:layers[1].angle};
  });assert.deepEqual(cleanup,{absent:true,concurrent:true,external:true,layers:2,neighbor:45});
