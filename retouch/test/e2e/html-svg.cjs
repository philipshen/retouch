...
(async()=>{
 ...
 try{
  ...
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===overlapPainted);
  await page.getByRole('button',{name:'Bring to front',exact:true}).click();await settled();await wait(async()=>await topShape()==='rect');const frontSource=read();assert.ok(frontSource.indexOf('<line')<frontSource.indexOf('<rect'));assert.equal(await page.getByRole('treeitem',{name:'rect · Box',exact:true}).getAttribute('aria-selected'),'true');assert.equal(await page.getByRole('button',{name:'Bring to front',exact:true}).isDisabled(),true);
  await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===overlapPainted);await wait(async()=>await topShape()==='circle');await page.getByRole('button',{name:'Redo',exact:true}).click();await settled();await wait(()=>read()===frontSource);await wait(async()=>await topShape()==='rect');
  await page.getByRole('button',{name:'Send to back',exact:true}).click();await settled();await wait(()=>read()===overlapPainted);await wait(async()=>await topShape()==='circle');assert.equal(await page.getByRole('button',{name:'Send to back',exact:true}).isDisabled(),true);await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===frontSource);await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===overlapPainted);
