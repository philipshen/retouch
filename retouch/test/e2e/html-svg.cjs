...
(async()=>{
 ...
 try{
  ...
  const paint=async(label,value)=>{const settings=page.locator('summary[aria-label="Advanced stroke settings"]');if(!['fill','stroke','stroke width'].includes(label)&&await settings.count()&&!await settings.evaluate(el=>el.parentElement.open))await settings.click();if(label==='dash pattern')await page.getByLabel('Stroke style',{exact:true}).selectOption('custom');const field=page.getByLabel('SVG '+label,{exact:true});if(await field.evaluate(el=>el.tagName)==='SELECT')await field.selectOption(value);else{await field.fill(value);await field.press('Tab');}await settled();};
