...
(async()=>{
 ...
 try{
  ...
  if(process.env.RT_E2E_EMPTY_PAINT){await require('./empty-svg-paint.cjs')({page,shape:app.locator('rect'),read,wait,settled,screenshot:process.env.RT_E2E_EMPTY_PAINT_SCREENSHOT,scoped:!!process.env.RT_E2E_EMPTY_PAINT_SCOPE});assert.deepEqual(errors,[]);console.log(engine+': PASS compact empty SVG paint, add/remove, preview, cancellation, '+(process.env.RT_E2E_EMPTY_PAINT_SCOPE?'tablet scope and unchanged phone paint':'gradient creation')+' and exact history');return;}
  assert.ok(await page.getByLabel('Shape Width',{exact:true}).isVisible());assert.equal(await page.getByLabel('Shape Width',{exact:true}).evaluate(el=>el.closest('.inspector-section').dataset.section),'geometry');const geometry=page.locator('[data-section="geometry"]');assert.equal(await geometry.locator('.property-pair').count(),3);assert.ok(await geometry.getByText('SVG coordinates · Shared across screen sizes',{exact:true}).isVisible());assert.ok(await page.getByLabel('Shape Width',{exact:true}).evaluate(el=>{const w=el.getBoundingClientRect(),h=document.querySelector('[aria-label="Shape Height"]').getBoundingClientRect();return Math.abs(w.y-h.y)<1&&w.right<=h.left;}));if(process.env.RT_E2E_GEOMETRY_SCREENSHOT)await page.screenshot({path:process.env.RT_E2E_GEOMETRY_SCREENSHOT});
  ...
  const paint=async(label,value)=>{
   if(['fill','stroke'].includes(label)&&await page.getByRole('button',{name:'Add '+label,exact:true}).count()){
    await page.getByRole('button',{name:'Add '+label,exact:true}).click();await page.getByRole('menuitem',{name:'Solid',exact:true}).click();
    const picker=page.getByRole('dialog',{name:'Edit SVG '+label,exact:true});await picker.getByLabel('Color value',{exact:true}).fill(value);await picker.getByRole('button',{name:'Apply color',exact:true}).click();await settled();return;
   }
   const settings=page.locator('summary[aria-label="Advanced stroke settings"]');if(!['fill','stroke','stroke width'].includes(label)&&await settings.count()&&!await settings.evaluate(el=>el.parentElement.open))await settings.click();if(label==='dash pattern')await page.getByLabel('Stroke style',{exact:true}).selectOption('custom');if(['line ends','line joins'].includes(label)){await page.getByRole('button',{name:label==='line ends'?({butt:'No caps',square:'Square caps',round:'Round caps'})[value]:({miter:'Miter join',bevel:'Bevel join',round:'Round join'})[value],exact:true}).click();await settled();return;}const field=page.getByLabel('SVG '+label,{exact:true});if(await field.evaluate(el=>el.tagName)==='SELECT')await field.selectOption(value);else{await field.fill(value);await field.press('Tab');}await settled();};
  ...
  await page.getByRole('button',{name:'Add fill',exact:true}).click();await page.getByRole('menuitem',{name:'Reset override',exact:true}).click();await settled();await wait(async()=>await computed('fill')==='rgb(0, 255, 0)');
  ...
  await page.getByRole('button',{name:'Lock rect · Box',exact:true}).click();await page.getByRole('button',{name:'Lock circle',exact:true}).click();
  ...
  for(const [preset,tag]of [['rectangle','rect'],['circle','circle'],['ellipse','ellipse'],['line','line']]){
   ...
   await page.getByRole('button',{name:'Undo',exact:true}).click();await settled();await wait(()=>read()===original);await wait(insertionLocks);assert.equal(await page.getByRole('treeitem',{name:'body',exact:true}).getAttribute('aria-selected'),'true');
   await page.getByRole('button',{name:'Redo',exact:true}).click();await settled();await wait(()=>read()===added);await wait(insertionLocks);await wait(async()=>await app.locator('[data-rt="'+id+'"]').count()===1);assert.equal(await page.getByRole('treeitem',{name:tag,exact:true}).last().getAttribute('aria-selected'),'true');
