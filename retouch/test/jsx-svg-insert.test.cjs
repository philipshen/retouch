...
test('JSX SVG insertion supports preset and drawn geometry without changing existing IDs',()=>{
 ...
 for(const tag of ['svg','g'])for(const preset of ['rectangle','circle','ellipse','line','arrow'])for(const points of [undefined,[10,20,60,70]]){
  ...
  if(['line','arrow'].includes(preset)){assert.ok(next.source.includes('strokeWidth='));assert.ok(!next.source.includes('stroke-width='));}
...
test('JSX shapes create viewports and expand self-closing containers without changing expressions or IDs',()=>{
 for(const markup of ['<main/>','<main>{title}<Widget /></main>','<svg/>','<svg><g /></svg>'])for(const preset of ['rectangle','circle','ellipse','line','arrow']){
