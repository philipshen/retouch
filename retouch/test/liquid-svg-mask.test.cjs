'use strict';
const {test}=require('node:test'),assert=require('node:assert/strict'),html=require('../src/adapters/liquid.cjs'),mask=require('../src/liquid-svg-mask.cjs');
...
function resolve(text=source,id){const elements=html.collect(text,'index.liquid').elements;return {source:text,elements,element:id?elements.find(e=>e.id===id):elements.find(e=>e.tag==='circle'),relPath:'index.liquid',file:'/tmp/index.liquid',hash:html.contentHash(text)};}
...
test('mask shape stays source-addressable and retains edits when released',()=>{
 const initial=create(resolve()),r=resolve(initial.edits[0].after),circle=r.elements.find(e=>e.tag==='circle'),changed=require('../src/liquid-svg-geometry.cjs').plan({...r,element:circle},{fileHash:r.hash,property:'r',value:'20'});assert.ok(changed.ok,changed.reason);const edited=resolve(changed.edits[0].after,initial.selectionIds[0]),released=mask.plan(edited,{type:'releaseSVGMask',fileHash:edited.hash});assert.ok(released.ok,released.reason);assert.equal(released.edits[0].after,source.replace('r="35"','r="20"'));
});
test('Liquid mask creation and release refuse stale, reordered, nonconsecutive and altered wrapper structures',()=>{
...
test('create, edit and release masks restore exact source through transaction history',t=>{
 const fs=require('node:fs'),os=require('node:os'),path=require('node:path'),{SourceHistory}=require('../src/history.cjs'),root=fs.mkdtempSync(path.join(os.tmpdir(),'rt-mask-history-')),file=path.join(root,'index.liquid'),history=new SourceHistory(),states=[source],entries=[];t.after(()=>fs.rmSync(root,{recursive:true,force:true}));fs.writeFileSync(file,source);
...
test('Liquid masks preserve expressions outside the SVG but refuse dynamic and repeated artwork',()=>{
 const outside='{% assign label = "keep" %}<p>{{ label }}</p>'+source+'{% render "icon" %}{% schema %}{"name":"Mask example"}{% endschema %}';
 const made=create(resolve(outside));assert.ok(made.ok,made.reason);const fresh=resolve(made.edits[0].after,made.selectionIds[0]);assert.equal(mask.plan(fresh,{type:'releaseSVGMask',fileHash:fresh.hash}).edits[0].after,outside);
 for(const text of ['{% for item in items %}'+source+'{% endfor %}',source.replace('r="35"','r="{{ radius }}"'),source.replace('<circle','{% if shown %}<circle').replace('/><rect','/>{% endif %}<rect')])assert.equal(create(resolve(text)).refused,true);
});

test('Liquid layer names remain editable through mask grouping and release',()=>{
 const r=resolve(),named=html.planOp(r,{type:'renameElement',fileHash:r.hash,name:'Avatar outline'});assert.ok(named.ok,named.reason);const original=named.edits[0].after,made=create(resolve(original));assert.ok(made.ok,made.reason);
 const group=resolve(made.edits[0].after,made.selectionIds[0]),renamed=html.planOp(group,{type:'renameElement',fileHash:group.hash,name:'Avatar mask'});assert.ok(renamed.ok,renamed.reason);
 const fresh=resolve(renamed.edits[0].after,group.element.id);assert.equal(html.describe(fresh).layerName,'Avatar mask');assert.equal(html.describe(fresh).svgMask.canRelease,true);
 const released=html.planOp(fresh,{type:'releaseSVGMask',fileHash:fresh.hash});assert.ok(released.ok,released.reason);assert.equal(released.edits[0].after,original);
});
