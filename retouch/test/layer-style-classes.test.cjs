...
test('resolved class paste composes all clipboard groups while retaining other screens and custom classes',()=>{
 const changes={opacity:'.6','background-color':'rgb(20, 30, 40)','background-image':'none',...require('../shell/paint-order.js').defaults,'border-color':'red','border-width':'1px 2px','border-style':'solid','border-radius':'10px 20px / 30px','box-shadow':'none',filter:'blur(2px)','backdrop-filter':'none','mix-blend-mode':'multiply',isolation:'isolate',color:'blue','font-family':'Arial, sans-serif','font-size':'24px','font-weight':'700','font-style':'italic','line-height':'32px','letter-spacing':'normal','text-align':'center','text-decoration-line':'underline','text-transform':'uppercase',display:'flex','flex-direction':'row-reverse','flex-wrap':'wrap','justify-content':'space-between','align-items':'center',gap:'10px 20px',padding:'12px 18px',width:'300px',height:'auto'};
...
test('class paste refuses ambiguous important ownership and unrepresentable source values',()=>{
 for(const [classes,changes]of [['!p-4',{padding:'20px'}],['![flex-flow:row_wrap]',{'flex-direction':'column'}],['!size-20',{width:'300px'}],['![all:unset]',{color:'red'}],['!ring-2',{'box-shadow':'none'}],['![font:italic_12px_serif]',{'font-size':'24px'}],['',{position:'fixed'}],['',{opacity:'.5;display:none'}],['',{'background-image':'url(javascript:alert(1))'}]])assert.throws(()=>compose(classes,changes));
...
test('pasted image stacks preserve URLs and replace only image utilities in the active scope',()=>{
 const image='linear-gradient(90deg, red 0%, transparent 100%), url("https://example.com/paint_asset.svg?theme=a_b")',next=compose('!bg-none bg-cover bg-red-500 md:!bg-none md:bg-contain hover:bg-none',{'background-image':image,'background-size':'auto, 22px 16px','background-position':'0% 0%, 7px 11px'},'md:');
 assert.ok(next.includes('url(https://example.com/paint_asset.svg?theme=a_b)'));assert.ok(next.includes('!bg-none bg-cover bg-red-500'));assert.ok(!next.split(' ').includes('md:!bg-none'));assert.ok(!next.includes('md:bg-contain'));assert.ok(next.includes('hover:bg-none'));assert.ok(next.includes('md:![--rt-hidden-paints:none]'));assert.equal(compose(next,{'background-image':image,'background-size':'auto, 22px 16px','background-position':'0% 0%, 7px 11px'},'md:'),next);
 assert.throws(()=>compose('![background:red]',{'background-image':image}));assert.throws(()=>compose('![background:red]',{'background-size':'cover'}));assert.throws(()=>compose('![background-position-x:20px]',{'background-position':'center'}));
});
