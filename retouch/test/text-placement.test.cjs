...
for(const language of ['html','react','liquid']){
 ...
 test(language+' canvas text insertion validates coordinates and preserves existing layers',()=>{
  ...
  const boxed=adapter.planOp(resolved,{type:'insertElement',preset:'text',position:{x:12,y:24,width:140,height:80},fileHash:resolved.hash});assert.equal(boxed.ok,true,boxed.reason);assert.match(boxed.edits[0].after,/140px/);assert.match(boxed.edits[0].after,/80px/);assert.match(boxed.edits[0].after,/pre-wrap/);
  for(const position of [null,[],{x:0},{x:0,y:0,width:20},{x:0,y:0,width:0,height:20},{x:0,y:0,width:10,height:100001},{x:'1',y:2},{x:Infinity,y:0},{x:100001,y:0},{x:0,y:0,css:'color:red'}]){const refusal=adapter.planOp(resolved,{type:'insertElement',preset:'text',position,fileHash:resolved.hash});assert.equal(refusal.refused,true);assert.equal(refusal.edits,undefined);}
