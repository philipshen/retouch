...
test('Set variable accepts only typed literals and rejects unrelated action fields',()=>{
 const item={trigger:'click',action:'set-variable',assignment:{id:'00000000-0000-4000-8000-000000000003',type:'number',value:0}};
 assert.deepEqual(V.validate([item]),[item]);
 for(const [type,value]of [['boolean',false],['string',''],['color','#123456ff']])assert.equal(V.validate([{...item,assignment:{...item.assignment,type,value}}])[0].assignment.value,value);
 for(const assignment of [undefined,null,{...item.assignment,id:'bad'},{...item.assignment,value:'0'},{...item.assignment,value:Infinity},{...item.assignment,value:1000001},{...item.assignment,type:'boolean',value:0},{...item.assignment,type:'string',value:'a'.repeat(4097)},{...item.assignment,type:'color',value:'url(/bad)'},{...item.assignment,extra:1}])assert.throws(()=>V.validate([{...item,assignment}]));
 for(const extra of [{destination:'/'},{transition:{type:'instant'}},{preserveScroll:true},{overlay:V.overlay()},{scrollOffset:{x:0,y:0}}])assert.throws(()=>V.validate([{...item,...extra}]));
 assert.throws(()=>V.validate([{...item,action:'back'}]));
});

test('variable-copy assignments are explicit, typed and mutually exclusive with literals',()=>{
 const copy={id:'00000000-0000-4000-8000-000000000003',type:'color',variableId:'00000000-0000-4000-8000-000000000004'};
 for(const type of ['color','number','boolean','string'])assert.deepEqual(V.assignment({...copy,type}),{...copy,type});
 assert.deepEqual(V.assignment({...copy,variableId:copy.id}),{...copy,variableId:copy.id});
 for(const bad of [{...copy,value:'#fff'},{...copy,value:undefined},{...copy,variableId:undefined},{...copy,variableId:'bad'},{...copy,type:'object'},{...copy,modes:{}},{...copy,extra:true}])assert.throws(()=>V.assignment(bad));
});

test('expression assignments require one value source and the target result type',()=>{
 const base={id:'00000000-0000-4000-8000-000000000003',type:'number',expression:{kind:'literal',type:'number',value:2}};
 assert.deepEqual(V.assignment(base),base);for(const bad of [{...base,value:2},{...base,variableId:base.id},{...base,type:'color'},{...base,expression:null}])assert.throws(()=>V.assignment(bad));
});

 test('expression interactions refuse serialized source that cannot be parsed back',()=>{
 const leaf={kind:'literal',type:'string',value:'x'.repeat(4096)};
 const tree=depth=>depth?{kind:'operation',op:'concat',args:[tree(depth-1),tree(depth-1)]}:leaf;
 const item=depth=>({trigger:'click',action:'set-variable',assignment:{id:'00000000-0000-4000-8000-000000000003',type:'string',expression:tree(depth)}});
 const accepted=V.validate([item(4)]);assert.deepEqual(V.parse(JSON.stringify(accepted)),accepted);
 assert.throws(()=>V.validate([item(5)]),/source size limit/);
 });

test('mode interactions require exact collection/mode identities and exclude unrelated fields',()=>{
 const modeChange={collectionId:'00000000-0000-4000-8000-000000000001',modeId:'00000000-0000-4000-8000-000000000002'},item={trigger:'click',action:'set-variable-mode',modeChange};assert.deepEqual(V.validate([item]),[item]);
 for(const change of [undefined,null,[],{...modeChange,extra:1},{...modeChange,modeId:'bad'},{collectionId:modeChange.collectionId}])assert.throws(()=>V.validate([{...item,modeChange:change}]));
 for(const extra of [{destination:'/'},{assignment:{id:modeChange.collectionId,type:'number',value:1}},{transition:{type:'instant'}},{overlay:V.overlay()},{preserveScroll:true}])assert.throws(()=>V.validate([{...item,...extra}]));
 assert.throws(()=>V.validate([{trigger:'click',action:'back',modeChange}]));
});

test('assignment mode fields accept only identities and copied sources',()=>{
 const base={id:'00000000-0000-4000-8000-000000000003',type:'number',value:1},modeId='00000000-0000-4000-8000-000000000002';assert.deepEqual(V.assignment({...base,modeId}),{...base,modeId});
 for(const invalid of [{...base,modeId:''},{...base,modeId:null},{...base,modeId:undefined},{...base,sourceModeId:modeId},{id:base.id,type:base.type,expression:{kind:'literal',type:'number',value:1},sourceModeId:modeId},{id:base.id,type:base.type,variableId:base.id,sourceModeId:'bad'}])assert.throws(()=>V.assignment(invalid));
});

test('action-list interactions retain trigger rules and reject ambiguous single/list data',()=>{
 const item={trigger:'click',actions:[{action:'back'}]};assert.deepEqual(V.validate([item]),[item]);
 for(const bad of [{...item,action:'back'},{...item,destination:'/'},{...item,actions:[]},{...item,actions:[{actions:[{action:'back'}]}]},{...item,actions:[{action:'back',trigger:'click'}]},{...item,delay:1}])assert.throws(()=>V.validate([bad]));
 assert.throws(()=>V.validate([item,item]));
});
