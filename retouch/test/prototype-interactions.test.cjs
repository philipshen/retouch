...
test('delayed triggers need bounded integer times and each trigger is unique',()=>{
 ...
 const all=V.triggers.map(trigger=>({trigger,action:'back',...(trigger==='after-delay'?{delay:1}:{})}));assert.deepEqual(V.validate(all),all);
});
