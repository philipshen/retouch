...
for(const [name,file,original]of [['html','index.html','<main><button>Continue</button></main>'],['react','App.jsx','export default function App(){return <main><button>Continue</button></main>}'],['liquid','page.liquid','<main><button>Continue</button></main>'],['vue','App.vue','<template><main><button>Continue</button></main></template>']])test(name+' prototype source round trip, identity preservation, stale writes and exact undo',t=>{
 ...
 const before=resolve(original),history=new SourceHistory(),plan=P.plan(before,{type:'setPrototypeInteractions',fileHash:before.hash,interactions:[interaction]},adapter);assert.equal(plan.ok,true,plan.reason);const result=history.commit(root,plan);assert.equal(result.ok,true,result.reason);
 const after=fs.readFileSync(absolute,'utf8');assert.deepEqual(P.describe(resolve(after),adapter).prototypeInteractions,[interaction]);assert.equal(resolve(after).element.id,before.element.id);assert.equal(P.plan(resolve(after),{fileHash:before.hash,interactions:[]},adapter).ok,false);
 ...
 const reordered=after.replace('<main>','<main><p>New sibling</p>');assert.deepEqual(P.describe(resolve(reordered),adapter).prototypeInteractions,[interaction],'interaction stays on the button after sibling insertion');
});
