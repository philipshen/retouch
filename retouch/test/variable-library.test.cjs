...
test('authenticated variable API persists across restart and restores history',async t=>{
 ...
 try{
  ...
  const expression={kind:'operation',op:'if',args:[{kind:'operation',op:'==',args:[{kind:'variable',id:id(3),type:'color'},{kind:'literal',type:'color',value:'#123456'}]},{kind:'literal',type:'string',value:'Assigned'},{kind:'literal',type:'string',value:'Default'}]};
  const calculated=await post('/rt/__api/variables/resolve',{revision:saved.body.revision,expression,overrides:runtime.overrides});assert.equal(calculated.status,200);assert.deepEqual(calculated.body.result,{type:'string',value:'Assigned'});assert.equal(calculated.body.undoId,undefined);
  const modeOverrides={[id(3)]:{[id(2)]:'#123456'}};
  for(const selection of [{expression},{variableId:id(3)},{bindings:runtime.bindings}]){const response=await post('/rt/__api/variables/resolve',{revision:saved.body.revision,modeOverrides,...selection});assert.equal(response.status,200);assert.equal(response.body.result?.value||response.body.values?.[0].value||response.body.bindings?.[0].value,selection.expression?'Assigned':'#123456ff');assert.equal(response.body.undoId,undefined);}
  for(const modeOverrides of [null,{[id(3)]:{[id(99)]:'#fff'}},{[id(3)]:{[id(2)]:false}}]){const response=await post('/rt/__api/variables/resolve',{revision:saved.body.revision,modeOverrides,expression});assert.equal(response.status,422);assert.equal(response.body.result,undefined);}
  const initial=await post('/rt/__api/variables/resolve',{revision:saved.body.revision,expression});assert.deepEqual(initial.body.result,{type:'string',value:'Default'});
  for(const invalid of [{revision:null,expression},{revision:saved.body.revision,expression,bindings:[]},{revision:saved.body.revision,expression:null},{revision:saved.body.revision,expression:{kind:'operation',op:'/',args:[{kind:'literal',type:'number',value:1},{kind:'literal',type:'number',value:0}]}},{revision:saved.body.revision,expression:{kind:'variable',type:'number',id:id(3)}}]){const response=await post('/rt/__api/variables/resolve',invalid);assert.equal(response.status,invalid.revision===null?409:422);assert.equal(response.body.result,undefined);}
...
test('focused binding previews resolve only the requested alias chain without hiding whole-library cycles',t=>{
 ...
 const batch=library.resolve(root,{...request,variableIds:[id(3),id(6),id(99)]});assert.deepEqual(batch.values,[focused.values[0]]);assert.deepEqual(batch.errors.map(error=>error.id),[id(3),id(99)]);assert.match(batch.errors[0].reason,/cycle/);assert.match(batch.errors[1].reason,/Unknown variable/);
 for(const variableIds of [[],[id(3),id(3)],Array.from({length:101},(_,i)=>id(i+1)),null,'bad'])assert.throws(()=>library.resolve(root,{...request,variableIds}),/100 distinct/);assert.throws(()=>library.resolve(root,{...request,variableIds:[id(6)],variableId:id(6)}),/100 distinct/);assert.throws(()=>library.resolve(root,{...request,variableIds:[id(6)],revision:null}),/changed/);
...
test('collection import preserves existing definitions and portable identities with exact history',t=>{
 const root=setup(t),history=new SourceHistory();library.commitPlan(root,library.planChange(root,{type:'replace',revision:null,library:data()}),(root,plan)=>history.commit(root,plan));
 const before=library.read(root),incoming={version:1,collections:[{id:id(10),name:'Shared',defaultMode:id(11),modes:[{id:id(11),name:'Day'},{id:id(12),name:'Night'}]}],variables:[{id:id(13),collectionId:id(10),name:'Color',type:'color',values:{[id(11)]:'#123456',[id(12)]:'#abcdef'}},{id:id(14),collectionId:id(10),name:'Alias',type:'color',values:{[id(11)]:{alias:id(13)},[id(12)]:{alias:id(13)}}}]};
 const imported=library.commitPlan(root,library.planChange(root,{type:'import',revision:before.revision,library:incoming}),(root,plan)=>history.commit(root,plan)),after=library.read(root);
 assert.deepEqual(after.collections[0],before.collections[0]);assert.deepEqual(after.variables[0],before.variables[0]);assert.equal(after.variables[2].id,id(14));assert.equal(after.variables[2].values[id(12)].alias,id(13));assert.equal(library.resolve(root,{revision:after.revision,modes:{[id(10)]:id(12)},variableId:id(14)}).values[0].value,'#abcdefff');
 assert.equal(library.planChange(root,{type:'import',revision:after.revision,library:incoming}).edits.length,0);
 assert.throws(()=>library.planChange(root,{type:'import',revision:before.revision,library:incoming}),/changed/);
 const conflict=structuredClone(incoming);conflict.variables[0].values[id(11)]='#000000';assert.throws(()=>library.planChange(root,{type:'import',revision:after.revision,library:conflict}),/conflicts with an existing identity/);assert.deepEqual(library.read(root),after);
