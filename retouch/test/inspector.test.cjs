...
test('local position retains fractional insets, margins and content-box dimensions',()=>{
 ...
 el.ownerDocument.defaultView.DOMMatrix=class {constructor(){this.is2D=false;}};css.transform='matrix3d(...)';assert.throws(()=>localPositionGeometry(el),/transform matrix/);css.transform='none';
...
test('local matrix translation and skew compose before individual scale and rotation',()=>{
 const {localPositionCorners}=require('../shell/inspector.js');
 const points=localPositionCorners({x:10,y:20,width:100,height:40,rotation:90,scaleX:-1,scaleY:2,transformOrigin:'20px 10px',transformMatrix:[1,.5,.25,1,5,-3]});
 const expected=[[76,47.5],[-24,-52.5],[-104,-62.5],[-4,37.5]];
 points.forEach((p,i)=>{assert.ok(Math.abs(p.x-expected[i][0])<1e-9);assert.ok(Math.abs(p.y-expected[i][1])<1e-9);});
