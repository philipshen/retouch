'use strict';
const {test}=require('node:test'),assert=require('node:assert/strict'),html=require('../src/adapters/react.cjs'),mask=require('../src/jsx-svg-mask.cjs');
const source='export default function Page(){return <main><svg viewBox="0 0 200 100"><circle cx="50" cy="50" r="35" fill="red"/><rect x="0" y="0" width="100" height="100" fill="blue"/><path d="M150 0h20v20h-20Z"/></svg><p>Keep</p></main>}';
function resolve(text=source,id){const elements=html.collect(text,'page.jsx').elements.map(e=>({...e,tag:require('../src/id.cjs').jsxElementName(e.node)}));return {source:text,elements,element:id?elements.find(e=>e.id===id):elements.find(e=>e.tag==='circle'),relPath:'page.jsx',file:'/tmp/page.jsx',hash:html.contentHash(text)};}
...
for(const mode of ['alpha','luminance'])test(mode+' mask retains original nodes and releases to exact original source',()=>{
 const r=resolve(),result=create(r,mode);assert.ok(result.ok,result.reason);const text=result.edits[0].after,fresh=resolve(text,result.selectionIds[0]);assert.match(text,new RegExp('maskType="'+mode+'"'));assert.ok(mask.describe(fresh).canRelease);
...
test('existing mask type changes preserve nodes and source identities and reject invalid or stale writes',()=>{
 const created=create(resolve()),r=resolve(created.edits[0].after,created.selectionIds[0]),op={type:'setSVGMaskType',fileHash:r.hash,mode:'luminance'},result=html.planOp(r,op);assert.ok(result.ok,result.reason);assert.equal(result.edits[0].after,r.source.replace('maskType="alpha"','maskType="luminance"'));assert.deepEqual(result.sourceIdMap,[]);assert.deepEqual(result.removedSourceIds,[]);assert.deepEqual(result.selectionIds,[r.element.id]);assert.equal(html.describe(resolve(result.edits[0].after,r.element.id)).svgMask.mode,'luminance');assert.equal(html.planOp(r,{...op,mode:'alpha'}).unchanged,true);
...
test('React masks refuse expression gaps and dynamic wrappers while retaining literal JSX expressions',()=>{
 const gap=source.replace('/><rect','/>{shown && <path d="M0 0h10v10Z"/>}<rect');assert.equal(create(resolve(gap)).refused,true);
 const literal=source.replace('r="35"','r={35}').replace('fill="red"','fill={color}');const made=create(resolve(literal));assert.ok(made.ok,made.reason);const r=resolve(made.edits[0].after,made.selectionIds[0]);assert.equal(mask.plan(r,{type:'releaseSVGMask',fileHash:r.hash}).edits[0].after,literal);
 for(const text of [r.source.replace('maskType="alpha"','maskType={mode}'),r.source.replace('data-rt-mask-group=""','data-rt-mask-group="" {...props}')]){const changed=resolve(text,r.element.id);assert.equal(mask.plan(changed,{type:'releaseSVGMask',fileHash:changed.hash}).refused,true);}
});

test('React masks preserve shape names and comments and refuse dropping wrapper comments',()=>{
