...
test('variable copies snapshot typed current values, defaults and aliases in trigger order',async()=>{
 ...
 for(const [target,source,type,expected]of [[4,3,'number',12],[6,5,'boolean',false],[8,7,'string',''],[9,10,'color','#ff0000ff']]){
  await runtime.assign({id:id(target),type,variableId:id(source)});assert.equal(requests.at(-1).modeOverrides[id(target)][id(2)],expected);
 }
 await Promise.all([runtime.assign({id:id(3),type:'number',value:0}),runtime.assign({id:id(4),type:'number',variableId:id(3)})]);assert.equal(requests.at(-1).modeOverrides[id(4)][id(2)],0);
 await runtime.assign({id:id(3),type:'number',value:99});assert.equal(requests.at(-1).modeOverrides[id(4)][id(2)],0,'copy is not a lasting alias');
 await runtime.assign({id:id(4),type:'number',variableId:id(4)});assert.equal(requests.at(-1).modeOverrides[id(4)][id(2)],0,'self-copy keeps the current value');
 runtime.reset();await runtime.assign({id:id(4),type:'number',variableId:id(3)});assert.deepEqual(requests.at(-1).modeOverrides,{[id(4)]:{[id(2)]:12}});
...
test('missing or incompatible copy sources fail without changing presentation values',async()=>{
 ...
 await runtime.assign({id:id(3),type:'number',variableId:id(4)});assert.equal(requests.at(-1).modeOverrides[id(3)][id(2)],7);assert.equal(requests.at(-1).modeOverrides[id(4)][id(2)],7);
});
test('reset cancels a copy whose source value is still being resolved',async()=>{
 ...
 await runtime.assign({id:id(4),type:'number',value:2});assert.deepEqual(requests.at(-1).modeOverrides,{[id(4)]:{[id(2)]:2}});
});
...
test('expression assignments read current state in order and errors preserve the last value',async()=>{
 ...
 await Promise.all([runtime.assign(assignment),runtime.assign(assignment)]);assert.equal(requests.at(-1).modeOverrides[id(3)][id(2)],14);
 ...
 await runtime.assign({id:id(4),type:'number',variableId:id(3)});assert.equal(requests.at(-1).modeOverrides[id(4)][id(2)],14);
});
...
test('reset cancels an expression preview before it can assign its result',async()=>{
 ...
 await runtime.assign({id:id(4),type:'number',variableId:id(3)});assert.equal(requests.at(-1).modeOverrides[id(4)][id(2)],12);
});
...
test('mode actions keep assignments independent and expressions read the selected mode',async()=>{
 ...
 await runtime.assign({id:id(4),type:'number',variableId:id(3)});assert.equal(requests.at(-1).modeOverrides[id(4)][id(20)],50);
 await runtime.setMode({collectionId:id(1),modeId:id(2)});
 await runtime.assign({id:id(3),type:'number',expression:{kind:'variable',id:id(4),type:'number'}});assert.equal(requests.at(-1).modeOverrides[id(3)][id(2)],24);
 await runtime.setMode({collectionId:id(1),modeId:id(99)});assert.match(errors[0],/mode is missing/);
 runtime.reset();await runtime.assign({id:id(4),type:'number',variableId:id(3)});assert.equal(requests.at(-1).modeOverrides[id(4)][id(2)],12);assert.deepEqual(requests.at(-1).modes,{});assert.equal(JSON.stringify(library),before);
});
