...
test('capture HTML is sanitized independently of the remote page serializer',()=>{
 const result=sanitize('<html><head><meta http-equiv="refresh" content="0;url=https://evil.test"><script>alert(1)</script></head><body onload="bad()"><iframe srcdoc="bad"></iframe><svg><foreignObject><script>bad()</script></foreignObject><use href="https://evil.test/a.svg#x"/><rect onclick="bad()"/></svg><a href="java&#10;script:bad()" ping="https://evil.test">Link</a><form action="https://evil.test"><button formaction="https://evil.test">Submit</button></form><img src="data:image/png;base64,AA" onerror="bad()"><div data-rt="forged">Safe</div></body></html>');
 assert.doesNotMatch(result,/script|onload|onerror|onclick|iframe|foreignObject|formaction|http-equiv|data-rt|evil\.test/);assert.match(result,/<form method="dialog">/);assert.match(result,/data:image\/png;base64,AA/);assert.match(result,/>Safe</);assert.equal(sanitize(result),result);
});
