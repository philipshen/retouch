...
for(const language of ['react','liquid','html']){
 ...
 test(language+' nested ordering moves the outer layer once and preserves authored identities',()=>{
  const r=setup('<main><a>A</a><section id="frame"><b id="text">B</b></section><i>I</i></main>',['section','b']),result=plan(r,{type:'moveSelection',direction:'last',ids:r.ids,fileHash:r.hash});assert.equal(result.ok,true,result.reason);assert.equal(result.rootCount,1);assert.equal(result.edits[0].after,wrap('<main><a>A</a><i>I</i><section id="frame"><b id="text">B</b></section></main>'));const final=adapter.collect(result.edits[0].after,r.relPath).elements;assert.equal(result.selectionIds.length,1);assert.equal(tag(final.find(el=>el.id===result.selectionIds[0])),'section');const mapping=new Map(result.sourceIdMap);for(const el of r.elements)assert.equal(tag(final.find(item=>item.id===(mapping.get(el.id)||el.id))),tag(el));
 });
