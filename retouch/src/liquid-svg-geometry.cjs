...
function describe(resolved){
 ...
 const metadata=(node.attributes||[]).filter(a=>a.name==='data-rt-shape'),coordinates=(node.attributes||[]).filter(a=>a.name===(node.tag==='path'?'d':'points'));
 return {parametric:['polygon','polyline','path'].includes(node.tag)&&!node.attributeExpressions&&metadata.length===1&&coordinates.length===1?require('../shell/svg-parametric.js').describe(node.tag==='path'?require('../shell/svg-parametric.js').pointsFromPath(coordinates[0].value):coordinates[0].value,metadata[0].value):null,fields:shape.fields.map(field=>{const attrs=(node.attributes||[]).filter(attr=>attr.name===field.name),attr=attrs[0],dynamic=attr&&/\{[%{]/.test(attr.value||''),editable=!node.attributeExpressions&&attrs.length<2&&!dynamic;return {...field,value:attr?.value??null,editable,reason:editable?null:node.attributeExpressions?'Liquid attribute expressions may control this value.':dynamic?'This coordinate is controlled by a Liquid expression.':'This coordinate has duplicate attributes.'};})};
}
