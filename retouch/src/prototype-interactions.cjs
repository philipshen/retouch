...
function metadata(resolved,adapter){
 const e=resolved.element,name=V.attribute;let attrs,insert;
 ...
 return {old:attrs[0],insert,interactions:V.parse(attrs[0]?.value??null)};
}
function describe(resolved,adapter){try{return {prototypeEditable:true,prototypeInteractions:metadata(resolved,adapter).interactions};}catch(error){return {prototypeEditable:false,prototypeReason:error.message};}}
