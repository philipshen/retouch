...
function inspect(def,name){
 ...
 const contract=(def.contracts||require('./svelte-component-choices.cjs').read(def.text)).get(name);if(contract&&(!contract.supported||contract.type!==value.type||contract.choices&&!contract.choices.includes(value.value)))return null;
