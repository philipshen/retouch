...
function transform(source,{transformSelector=selector}={}){
 ...
 root.walkRules(rule=>{
  ...
  const before=rule.selector,after=transformSelector(before);size+=Buffer.byteLength(after)-Buffer.byteLength(before);
