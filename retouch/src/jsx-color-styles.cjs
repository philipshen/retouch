...
function plan(resolved,op,style){
 try{
  ...
  if(['applyColorStyle','resetColorStyle','refreshColorStyle'].includes(op.type)){
   ...
   if(!override){const composed=classes.composeStyle(info.className||'',op.property,value,scope,op.backgroundPaint),result=react.planOp(resolved,{type:'setClasses',classes:composed,fileHash:resolved.hash});if(!result.ok)return result;source=result.edits[0]?.after||source;}
