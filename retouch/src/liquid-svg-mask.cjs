'use strict';
// Reuse lossless mask edits while preserving the Liquid adapter's own IDs.
...
function plan(r,op){
 ...
 const result=mask.plan(c.resolved,translated);if(!result.ok||result.unchanged)return result;
 ...
 if(next.length!==c.elements.length-removed.length+(op.type==='createSVGMask'?3:0))return refuse('Masking would change surrounding Liquid structure.');
 const translate=id=>{const h=nextHTML.find(e=>e.id===id);return next.find(e=>e.tagStart===h?.location.startOffset&&e.tag===h.tag)?.id;};
 const parentId=translate(result.parentId),selectionIds=result.selectionIds.map(translate);if(!parentId||selectionIds.some(id=>!id))return refuse('Mask selection could not be preserved.');
 return {...result,hash:liquid.contentHash(after),parentId,selectionIds,sourceIdMap:[...mapping].filter(([a,b])=>a!==b),removedSourceIds:removed};
}
