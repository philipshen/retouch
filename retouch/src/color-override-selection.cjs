...
function plan(resolved,op,adapter=react){try{
 ...
 if(op.valuesById!==undefined&&(!op.valuesById||typeof op.valuesById!=='object'||Array.isArray(op.valuesById)||Object.hasOwn(op,'value')||Object.keys(op.valuesById).length!==op.ids.length||op.ids.some(id=>!Object.hasOwn(op.valuesById,id))))throw Error('Provide one color value for every selected layer.');
 ...
 const value=op.valuesById===undefined?op.value:op.valuesById[id];
 changes[id]=visibility?(Object.keys(op.changesById[id]).length?paint.composeBackground(info.className||'',op.changesById[id],op.scope||''):info.className||''):value===null?paint.compose(info.className||'',op.property,null,op.scope||''):paint.composeStyle(info.className||'',op.property,value,op.scope||'',op.backgroundPaints?.[id]);}
