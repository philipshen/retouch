...
function plan(resolved,op,language){
 ...
 try{
  ...
  for(const root of roots){
   ...
   if(moving){const siblings=structure.ranges(state,language,{templateChildren:true,scaleChildren:true}),index=siblings.findIndex(item=>item.selected),offset=op.direction==='before'?-1:1,peer=siblings[index+offset];if(op.direction==='first'&&index===0||op.direction==='last'&&index===siblings.length-1)continue;if(['before','after'].includes(op.direction)&&(!peer||roots.some(root=>{const item=current.find(item=>item.id===mapping.get(root.id));return item&&start(item)===peer.start;})))continue;}
