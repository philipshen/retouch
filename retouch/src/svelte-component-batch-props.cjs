...
function plan(resolved,op,adapter){try{
 ...
 let targets=selection?(Array.isArray(op.ids)?op.ids.map(id=>({id,properties:[{name:op.name,value:op.value,definitionHash:op.definitionHashes?.[id],...(op.reset?{reset:true}:{})}]})):null):op.targets;
 if(resetting){
 ...
 for(const target of targets){
  ...
  for(const property of target.properties){
   if(!property||typeof property.name!=='string'||property.unset!==undefined||!selection&&!resetting&&property.reset!==undefined)throw Error('Choose an existing literal component property.');
