...
function plan(resolved,op){try{
 ...
 if(!['apply','remove','reset','gradient','order','add'].includes(action))return refuse('Choose an image fill action.');
 ...
 if(stack){if(JSON.stringify(stack).length>16384)return refuse('The paint stack is too large.');if(!['apply','gradient','order','add'].includes(action)||!Array.isArray(stack.layers)||stack.layers.length<(action==='add'?0:2)||stack.layers.length>8||!['order','add'].includes(action)&&(!Number.isInteger(stack.index)||stack.index<0||stack.index>=stack.layers.length))return refuse('Choose an image in the paint stack.');
  ...
  else if(!['order','add'].includes(action)&&!V.imageURL(stack.layers[stack.index])&&!/^var\(--rt-image-fill-[a-f0-9]{10}\)$/.test(stack.layers[stack.index]))return refuse('Choose an image paint to replace.');
 }
 if(['gradient','order','add'].includes(action)&&!stack)return refuse('Choose a gradient in the paint stack.');
 ...
 if(action==='add'){const layer=op.src!=null?'var('+variable+')':stack.value,changes=P.prepend(stack.layers,stack.framing||{},layer);local=P.frameClasses(I.replace(local,F.imageToken,'![background-image:'+F.stackValue([layer,...stack.layers])+']'),changes);}
 ...
 const classes=R.replaceScope(before,local,scope),result=liquid.planOp(resolved,{type:'setClasses',classes,fileHash:resolved.hash});if(!result.ok||['gradient','order'].includes(action)||action==='add'&&op.src==null)return result;
