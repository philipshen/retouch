...
function plan(resolved,op){try{
 ...
 if(stack){if(JSON.stringify(stack).length>16384)return refuse('The paint stack is too large.');if(action!=='apply'||!Array.isArray(stack.layers)||stack.layers.length<2||stack.layers.length>8||!Number.isInteger(stack.index)||stack.index<0||stack.index>=stack.layers.length)return refuse('Choose an image in the paint stack.');
  ...
  if(!V.imageURL(stack.layers[stack.index])&&!/^var\(--rt-image-fill-[a-f0-9]{10}\)$/.test(stack.layers[stack.index]))return refuse('Choose an image paint to replace.');
 }
