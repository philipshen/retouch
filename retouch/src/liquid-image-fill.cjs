...
function plan(resolved,op){try{
 ...
 if(!['apply','remove','reset','gradient','order','add','frame','visibility'].includes(action))return refuse('Choose an image fill action.');
 ...
 if(stack){if(JSON.stringify(stack).length>16384)return refuse('The paint stack is too large.');if(!['apply','gradient','order','add','frame','visibility','remove'].includes(action)||!Array.isArray(stack.layers)||stack.layers.length<(action==='add'?0:1)||stack.layers.length>8||!['order','add','remove'].includes(action)&&(!Number.isInteger(stack.index)||stack.index<0||stack.index>=stack.layers.length))return refuse('Choose an image in the paint stack.');
  ...
  if(action==='visibility'){if(V.parseGradients(stack.layers[stack.index])?.length!==1||typeof stack.hidden!=='boolean')return refuse('Choose a solid or gradient paint visibility.');}
  else if(action==='gradient'){if(V.parseGradients(stack.layers[stack.index])?.length!==1||V.parseGradients(stack.value)?.length!==1)return refuse('Choose a supported gradient paint.');}
  else if(!['order','add','frame','remove'].includes(action)&&!V.imageURL(stack.layers[stack.index])&&!/^var\(--rt-image-fill-[a-f0-9]{10}\)$/.test(stack.layers[stack.index]))return refuse('Choose an image paint to replace.');
 }
 if(['gradient','order','add','frame','visibility'].includes(action)&&!stack)return refuse('Choose a gradient in the paint stack.');
 ...
 const stored=stack?.visibility??'none';if(!V.valid(V.paintVisibilityProperty,stored))return refuse('The stored paint visibility is invalid.');
 let local=R.project(before,scope);if(action==='apply'&&op.initialize)local=F.classes(local,F.framing('fill',1,1));
 if(action==='visibility')local=P.frameClasses(local,P.toggleVisibility(stack.layers,stack.framing||{},stored,stack.index,stack.hidden));
 else if(action==='frame')local=P.frameClasses(local,P.edit(stack.layers,stack.framing||{},stack.index,stack.changes));
 else if(action==='add'){const layer=op.src!=null?'var('+variable+')':stack.value,changes=stored==='none'?P.prepend(stack.layers,stack.framing||{},layer):P.prependVisibility(stack.layers,stack.framing||{},stored,layer);local=P.frameClasses(I.replace(local,F.imageToken,'![background-image:'+F.stackValue([layer,...stack.layers])+']'),changes);}
 else if(action==='order'){const changes=stored==='none'?P.reorder(stack.layers,stack.framing||{},stack.order):P.reorderVisibility(stack.layers,stack.framing||{},stored,stack.order),layers=stack.order.map(index=>stack.layers[index]);local=P.frameClasses(I.replace(local,F.imageToken,'![background-image:'+F.stackValue(layers)+']'),changes);}
 else if(action==='reset')local=F.classes(local,F.reset());else if(action==='remove')local=F.classes(local,stack?P.clearVisibility(stack.layers,stack.framing||{},stored):{'background-image':'none',[V.paintVisibilityProperty]:'none'});else if(stack){const layers=stack.layers.map((value,index)=>index===stack.index?(action==='gradient'?stack.value:'var('+variable+')'):value);local=I.replace(local,F.imageToken,'![background-image:'+F.stackValue(layers)+']');}else local=I.replace(local,F.imageToken,'!bg-[image:var('+variable+')]');
 if(action==='gradient'&&stored!=='none')local=P.frameClasses(local,P.editVisibilityPaint(stack.layers,stack.framing||{},stored,stack.index,stack.value));
 const classes=R.replaceScope(before,local,scope),result=liquid.planOp(resolved,{type:'setClasses',classes,fileHash:resolved.hash});if(!result.ok||['gradient','order','frame','visibility'].includes(action)||action==='add'&&op.src==null)return result;
