...
function plan(resolved,op){try{
 const liquid=require('./adapters/liquid.cjs'),node=resolved.element,scope=op.scope||'';
 ...
 if(typeof op.src!=='string'||!/^\/assets\/[a-zA-Z0-9_-][a-zA-Z0-9_.-]*$/.test(op.src))return refuse('Choose an uploaded theme asset.');
 const variable='--rt-image-fill-'+crypto.createHash('sha256').update(scope).digest('hex').slice(0,10),before=node.classAttr?.value||'';
 let local=R.project(before,scope);if(op.initialize)local=F.classes(local,F.framing('fill',1,1));
 local=I.replace(local,token=>/^bg-(?:none|\[(?:image:)?(?:url|var)\(.*\)\])$/.test(token),'!bg-[image:var('+variable+')]');
 ...
 const quote=outer==='"'?"'":'"',declaration=variable+':url({{ '+quote+op.src.slice(8)+quote+' | asset_url }});',ms=new MagicString(source);
 if(attr){let value=source.slice(attr.valueStart,attr.valueEnd);const existing=new RegExp(variable+':url\\(\\{\\{ [\'"][a-zA-Z0-9_.-]+[\'"] \\| asset_url \\}\\}\\);','g'),matches=value.match(existing)||[];
  ...
  value=matches.length?value.replace(existing,declaration):value+(value.trim().endsWith(';')?'':';')+declaration;ms.overwrite(attr.valueStart,attr.valueEnd,value);
 }else ms.appendLeft(current.nameEnd,' style="'+declaration+'"');
