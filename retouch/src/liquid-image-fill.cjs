...
function plan(resolved,op){try{
 ...
 if(stack){if(JSON.stringify(stack).length>16384)return refuse('The paint stack is too large.');if(!['apply','gradient','order','add','frame','visibility'].includes(action)||!Array.isArray(stack.layers)||stack.layers.length<(action==='add'?0:1)||stack.layers.length>8||!['order','add'].includes(action)&&(!Number.isInteger(stack.index)||stack.index<0||stack.index>=stack.layers.length))return refuse('Choose an image in the paint stack.');
