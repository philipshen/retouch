...
function plan(resolved,op){try{
 ...
 if(!['apply','remove','reset','gradient','order'].includes(action))return refuse('Choose an image fill action.');
 ...
 if(stack){if(JSON.stringify(stack).length>16384)return refuse('The paint stack is too large.');if(!['apply','gradient','order'].includes(action)||!Array.isArray(stack.layers)||stack.layers.length<2||stack.layers.length>8||action!=='order'&&(!Number.isInteger(stack.index)||stack.index<0||stack.index>=stack.layers.length))return refuse('Choose an image in the paint stack.');
  ...
  else if(action!=='order'&&!V.imageURL(stack.layers[stack.index])&&!/^var\(--rt-image-fill-[a-f0-9]{10}\)$/.test(stack.layers[stack.index]))return refuse('Choose an image paint to replace.');
 }
 if(['gradient','order'].includes(action)&&!stack)return refuse('Choose a gradient in the paint stack.');
 const variable=stack&&action==='apply'?imageVariable(resolved,scope,stack):'--rt-image-fill-'+crypto.createHash('sha256').update(scope).digest('hex').slice(0,10),before=node.classAttr?.value||'';
 let local=R.project(before,scope);if(action==='apply'&&op.initialize)local=F.classes(local,F.framing('fill',1,1));
 if(action==='order'){const changes=P.reorder(stack.layers,stack.framing||{},stack.order),layers=stack.order.map(index=>stack.layers[index]);local=P.frameClasses(I.replace(local,F.imageToken,'![background-image:'+F.stackValue(layers)+']'),changes);}
 ...
 const classes=R.replaceScope(before,local,scope),result=liquid.planOp(resolved,{type:'setClasses',classes,fileHash:resolved.hash});if(!result.ok||['gradient','order'].includes(action))return result;
