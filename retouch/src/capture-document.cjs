...
module.exports=function captureDocument({shadowKey,responsive=false,sourceDocument,exportState=false}={}){
 const document=sourceDocument||globalThis.document,window=document.defaultView,{Node,CSSRule,CSS,location}=window,getComputedStyle=window.getComputedStyle.bind(window),matchMedia=window.matchMedia.bind(window),scroll=[];
 ...
 function clone(node){
  ...
  const frozen=!responsive||!!scope(node)||!!shadow||tag==='canvas';target.setAttribute('style',frozen?styles(node):node.getAttribute('style')||'');const id=String(count);target.setAttribute('data-capture-node',id);if(exportState&&(node.scrollLeft||node.scrollTop))scroll.push({id,x:node.scrollLeft,y:node.scrollTop});
 ...
 return {html:'<!doctype html>\n'+output.documentElement.outerHTML+'\n',title:document.title,url:location.href,baseURL:document.baseURI,layers:count,shadowRoots:scopeCount,fontFaces,styleSheets,warnings:[...warnings],...(exportState?{scroll,rootScroll:{x:window.scrollX,y:window.scrollY}}:{})};
};
