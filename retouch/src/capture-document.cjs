...
module.exports=function captureDocument({shadowKey,responsive=false,sourceDocument,exportState=false,selectedNodes=[]}={}){
 ...
 function clone(node){
  ...
  const frozen=!responsive||!!scope(node)||!!shadow||tag==='canvas';target.setAttribute('style',frozen?styles(node):node.getAttribute('style')||'');const id=String(count);target.setAttribute('data-capture-node',id);if(selectedNodes.includes(node)){selectionIds.push(id);selectionNames.push((node.getAttribute('data-rt-layer-name')||node.getAttribute('aria-label')||node.id||node.localName).slice(0,200));}if(exportState&&(node.scrollLeft||node.scrollTop))scroll.push({id,x:node.scrollLeft,y:node.scrollTop});
 ...
 return {html:'<!doctype html>\n'+output.documentElement.outerHTML+'\n',title:document.title,url:location.href,baseURL:document.baseURI,layers:count,shadowRoots:scopeCount,fontFaces,styleSheets,warnings:[...warnings],...(exportState?{scroll,selectionIds,selectionNames,rootScroll:{x:window.scrollX,y:window.scrollY}}:{})};
};
