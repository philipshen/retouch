...
function choices(resolved,name,definition){
 try{
  ...
  const contract=resolve(param?.typeAnnotation?.typeAnnotation),members=contract?.type==='TSTypeLiteral'?contract.members:contract?.type==='TSInterfaceBody'?contract.body:null;
  if(!members)return null;
  const fields=members.filter(p=>p.type==='TSPropertySignature'&&!p.computed&&(p.key.name??p.key.value)===name);if(fields.length!==1)return null;
