...
function plan(resolved,op,style,adapter,family='text'){
 try{
  ...
  if(!['html','react','liquid','vue'].includes(adapter.name)||adapter.name==='vue'&&family!=='text')return refuse('These layers do not support this shared style family.');
  ...
  const renderer=liquid?'liquid':adapter.name==='react'?'jsx':'html',linked=adapter.name==='vue'?require('./vue-text-styles.cjs').create(adapter):require('./'+renderer+'-'+(effect?'effect':color?'color':'text')+'-styles.cjs');let source=resolved.source;
  for(const id of ids){
