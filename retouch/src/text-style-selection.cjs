...
function plan(resolved,op,style,adapter,family='text'){
 try{
  ...
  const linked=require(color?(adapter.name==='react'?'./jsx-color-styles.cjs':'./html-color-styles.cjs'):adapter.name==='react'?'./jsx-text-styles.cjs':'./html-text-styles.cjs');let source=resolved.source;
  for(const id of ids){
