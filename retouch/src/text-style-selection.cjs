...
function plan(resolved,op,style,adapter,family='text'){
 try{
  ...
  if(color&&adapter.name!=='html')return refuse('Color style selection is not available for this renderer yet.');
  ...
  const type={applyColorStyleSelection:'applyColorStyle',resetColorStyleSelection:'resetColorStyle',detachColorStyleSelection:'detachColorStyle',applyTextStyleSelection:'applyTextStyle',resetTextStyleSelection:'resetTextStyle',detachTextStyleSelection:'detachTextStyle'}[op.type||'applyTextStyleSelection'];
  if(!type||type.includes('Color')!==color)return refuse('Unsupported selection style operation.');
  ...
  const linked=require(color?'./html-color-styles.cjs':adapter.name==='react'?'./jsx-text-styles.cjs':'./html-text-styles.cjs');let source=resolved.source;
  for(const id of ids){
   ...
   const reason=color?description.colorStyleReason:description.textStyleLinkReason;if(reason)return refuse(reason);
   const link=color?description.colorStyleLinks?.[scope]?.[op.property]:description.textStyleLinks?.[scope];let selectedStyle=style;
