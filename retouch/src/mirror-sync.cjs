...
function relevant(relative) { return !relative.split(path.sep).some(part => SKIP.has(part)) && /\.(liquid|json|css|scss|sass|js|mjs|cjs|ts|tsx|jsx)$/i.test(relative); }
const hash = value => value === null ? null : crypto.createHash('sha256').update(value).digest('hex');
function watchSource(root) {
  root = path.resolve(root);
  const known = new Map(); let revision = 0, styleRevision = 0, watcher = null;
  ...
  function check(relative) {
    if (!relative || relative.split(path.sep).some(part => SKIP.has(part))) return;
    if (!relevant(relative)) {
      ...
      const inspect = dir => {
        try { for (const entry of fs.readdirSync(dir, {withFileTypes:true})) {
          ...
          const file = path.join(dir,entry.name);
          if (entry.isDirectory()) inspect(file); else if (relevant(path.relative(root,file))) check(path.relative(root,file));
        } } catch {}
      };
      inspect(directory);
      for (const file of known.keys()) if (file.startsWith(directory+path.sep)) check(path.relative(root,file));
      return;
    }
    const file = path.resolve(root, relative);
    if (!file.startsWith(root+path.sep)) return;
    const value = hash(read(file));
    if ((known.get(file) ?? null) !== value) { known.set(file,value); if (/\.(css|scss|sass)$/i.test(file)) styleRevision++; else revision++; }
  }
  try { watcher = fs.watch(root, { recursive: true }, (_event, relative) => check(relative && String(relative))); } catch {}
  return {
    state: () => ({ revision, styleRevision, available: !!watcher }),
