...
function links(resolved){
 ...
 if(raw.length>128*1024)throw Error('The layer effect style links are too large.');
 const input=JSON.parse(raw);if(!input||typeof input!=='object'||Array.isArray(input)||Object.keys(input).length>32)throw Error('Invalid layer effect style links.');
 for(const [width,value]of Object.entries(input)){
  if(!/^(0|[1-9]\d*)$/.test(width)||Number(width)>7680||!value||typeof value!=='object'||Array.isArray(value)||Object.keys(value).some(key=>!['id','properties','overrides'].includes(key)))throw Error('Invalid layer effect style link.');
  catalog.validate({version:1,styles:[{id:value.id,properties:value.properties,name:'Linked style'}]});
  if(value.overrides!==undefined&&(!Array.isArray(value.overrides)||value.overrides.length>catalog.properties.length||new Set(value.overrides).size!==value.overrides.length||value.overrides.some(property=>!catalog.properties.includes(property))))throw Error('Invalid effect style overrides.');
 }
 return input;
}
function describe(resolved){try{
 ...
 const overrides=Object.fromEntries(Object.entries(state).map(([width,link])=>[width,[...new Set([...(link.overrides||[]),...Object.keys(link.properties).filter(property=>!owners[property]&&(rules.cssRules[width]?.[property]!==link.properties[property]||visibility[property]&&(rules.cssRules[width]?.[visibility[property]]||'none')!==(link.properties[visibility[property]]||'none')))])].sort()]));
...
function plan(resolved,op,style){
 try{
  ...
  if(op.type==='applyEffectStyle'||op.type==='refreshEffectStyle'||op.type==='resetEffectStyle'){
   const validated=catalog.validate({version:1,styles:[style]}).styles[0];
   let changes=validated.properties,overrides=[];
   if(op.type==='resetEffectStyle'){
    const baseline=current[op.width];if(!baseline||baseline.id!==validated.id)return refuse('The layer is no longer linked to this effect style.');
   ...
   if(op.type==='refreshEffectStyle'){
    ...
    for(const property of new Set([...Object.keys(baseline.properties),...Object.keys(validated.properties)])){
     if(owners[property])continue;
     const key=visibility[property],same=Object.hasOwn(baseline.properties,property)?own[property]===baseline.properties[property]:!Object.hasOwn(own,property),sameVisibility=!key||(own[key]||'none')===(baseline.properties[key]||'none');
     if(retained.has(property)||key&&retained.has(key)||!same||!sameVisibility){retained.add(property);continue;}
     changes[property]=validated.properties[property]??null;
     if(key&&(Object.hasOwn(baseline.properties,key)||Object.hasOwn(validated.properties,key)))changes[key]=validated.properties[key]??(changes[property]===null?null:'none');
    }
    for(const [key,property]of Object.entries(owners))if(retained.delete(key))retained.add(property);
    overrides=[...retained].sort();
   }
   for(const [property,key]of Object.entries(visibility))if(Object.hasOwn(changes,key)&&changes[key]===null&&changes[property]!=null)changes[key]='none';
...
function planFile(file,relPath,before,style){
 try{
  ...
  function checkCoverage(node){
   if(node.attrs?.some(item=>item.name===attribute)&&!indexedStarts.has(node.sourceCodeLocation?.startTag?.startOffset))throw Error('A effect style link belongs to unsupported or ambiguous markup. Resolve that layer before updating.');
  ...
  for(const target of targets){
   ...
   const result=plan({file,relPath,source,hash:html.contentHash(source),element},{type:'refreshEffectStyle',width:target.width},style);
