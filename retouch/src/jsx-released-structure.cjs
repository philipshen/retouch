...
function plan(resolved,op){try{
 ...
 if(op.type==='moveSelection'&&(!Array.isArray(op.ids)||op.ids.some(id=>!roots.some(e=>e.id===id))))throw Error('Reorder released roots from the same group.');
 let source=resolved.source,groupId,createdId,movedId,selectionIds=[];const mapping=new Map(elements.map(e=>[e.id,e.id]));
 const advance=next=>{if(!next.ok)throw Error(next.reason);if(next.edits.length!==1||next.edits[0].file!==resolved.file||next.edits[0].before!==source)throw Error('Editing released layers requires one source transaction.');const remap=new Map(next.sourceIdMap||[]),removed=new Set(next.removedSourceIds||[]);for(const [old,id]of mapping){if(removed.has(id))throw Error('Editing released layers removed an existing source layer.');mapping.set(old,remap.get(id)||id);}if(groupId)groupId=remap.get(groupId)||groupId;if(createdId)createdId=remap.get(createdId)||createdId;if(movedId)movedId=remap.get(movedId)||movedId;selectionIds=selectionIds.map(id=>remap.get(id)||id);source=next.edits[0].after;};
 ...
 const selected=state(mapping.get(resolved.element.id)),operation={...op,fileHash:selected.hash,...(op.type==='pasteElement'?{copiedId:mapping.get(op.copiedId),copiedHash:selected.hash}:{}),...(op.type==='moveSelection'?{ids:op.ids.map(id=>mapping.get(id))}:{})},edited=op.type==='moveSelection'?require('./native-structure-selection.cjs').plan(selected,operation,'react'):require('./structure.cjs').planOp(selected,operation,'react');advance(edited);createdId=edited.createdId;movedId=edited.movedId;selectionIds=edited.selectionIds||[createdId||movedId];
 ...
 return {ok:true,hash:identity.contentHash(source),structural:true,parentId:released.parentId,...(createdId?{createdId}:{}),...(movedId?{movedId}:{}),...(edited.rootCount?{rootCount:edited.rootCount}:{}),selectionIds,sourceIdMap:[...mapping].filter(([before,after])=>before!==after),removedSourceIds:[],edits:[{file:resolved.file,before:resolved.source,after:source}]};
