    ...
    const build = (children) =>
      children
        .map((c) => {
          ...
          seen.add(c.id);
          const hrefEdit=Object.hasOwn(c,'href'),keptElement={node:kept},patchBlock=raw=>{if(Object.hasOwn(c,'tag'))raw=blocks.patchTag(raw,tagOf(kept),c.tag);if(Object.hasOwn(c,'listSpacing'))raw=require('./list-spacing.cjs').patch(raw,c.listSpacing,true);if(Object.hasOwn(c,'start'))raw=require('./list-start.cjs').patch(raw,c.start,true);if(Object.hasOwn(c,'spacing'))raw=require('./text-paragraphs.cjs').patchSpacing(raw,c.spacing,true);if(c.paragraph==='inline')raw=require('./text-paragraphs.cjs').inline(raw,true);return c.marker?require('./list-markers.cjs').patch(raw,c.tag||tagOf(kept),c.marker,true):raw;};
