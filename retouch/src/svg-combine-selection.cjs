...
function context(r,kind){
 const react=kind==='react',liquid=kind==='liquid',node=r.element.node,tag=react?ids.jsxElementName(node):r.element.tag;
 if(!tags.has(tag)||liquid&&node.attributes?.some(a=>['v-if','v-for','x-if','x-for'].includes(a.name)))return null;
 const deletion=liquid?require('./structure.cjs').describe(r,'liquid'):require(react?'./jsx-svg-delete.cjs':'./svg-delete.cjs').describe(r);
 const geometry=react?require('./jsx-svg-geometry.cjs').describe(r):liquid?require('./liquid-svg-geometry.cjs').describe(r):require('./svg-geometry.cjs').describe(r.element);
 if(!deletion?.canDelete||!geometry||geometry.fields.some(f=>f.editable===false))return null;
 // Animated geometry and nested drawable content cannot be flattened silently.
 const children=node.children||node.childNodes||[];
 if(children.some(child=>liquid?!['title','desc'].includes(child.tag):react?!(child.type==='JSXText'&&!child.value.trim()||child.type==='JSXExpressionContainer'&&child.expression.type==='JSXEmptyExpression'||child.type==='JSXElement'&&['title','desc'].includes(ids.jsxElementName(child))):!(child.nodeName==='#comment'||child.nodeName==='#text'&&!child.value.trim()||['title','desc'].includes(child.tagName))))return null;
 return {tag,node,parentId:deletion.parentId,properties:[...geometry.fields.map(f=>f.name),'data-rt-shape'],start:react?node.start:liquid?node.tagStart:r.element.location.startOffset,end:react?node.end:liquid?node.closeEnd:r.element.location.endOffset};
}
function plan(r,op,kind){
 const refuse=reason=>({ok:false,refused:true,reason});
 if(!['html','react','liquid'].includes(kind))return refuse('This source adapter does not yet support combining SVG layers.');
 ...
 const base=selected[0],baseContext=contexts[0],react=kind==='react',liquid=kind==='liquid',name=e=>react?ids.jsxElementName(e.node):e.tag,start=e=>react?e.node.start:liquid?e.tagStart:e.location.startOffset;
 ...
 if(path){
  const node=base.node,location=base.location,attrs=react?node.openingElement.attributes:liquid?node.attributes:node.attrs;
  const openingStart=react?node.openingElement.name.start:liquid?node.tagStart+1:location.startTag.startOffset+1,openingEnd=react?node.openingElement.name.end:liquid?node.nameEnd:openingStart+baseContext.tag.length;
  out.overwrite(openingStart,openingEnd,'path');
  const closing=react?node.closingElement?.name:liquid?(node.selfClosing?null:node.closeStart==null?null:{startOffset:node.closeStart}):location.endTag;
  ...
  for(const attr of attrs){const property=react?attr.name?.name:attr.name;if(!baseContext.properties.includes(property))continue;const loc=react||liquid?attr:location.attrs?.[property];if(loc)out.remove(react?loc.start:liquid?loc.attrStart:loc.startOffset,react?loc.end:liquid?loc.attrEnd:loc.endOffset);}
 ...
 const parents=list=>react?require('./jsx-svg-delete.cjs').parents(list):new Map(list.map(e=>[e.id,liquid?e.parent?.id??null:list.find(p=>p.node===e.node.parentNode)?.id??null]));
