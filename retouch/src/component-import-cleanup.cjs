'use strict';
// Remove only the old binding made unused by this swap. Preserve module evaluation.
module.exports=function cleanupImport(ast,selected,source,replacementName){
 ...
 if(binding.referencePaths.some(ref=>!inside(ref,node)||key&&inside(ref,key)))return null;
 ...
 if(kept.length&&!kept.some(item=>item.importKind!=='type'))after+='\n'+sideEffect;
 if(leading)after=leading+'\n'+after;
 return {start:declaration.start,end:declaration.end,after};
};
