...
function plan(resolved,op,language){
 ...
 try{
  ...
  if(language==='liquid'&&op.type==='removeFrame'&&resolved.element.attributes?.some(a=>a.name==='data-rt-scale')){const adapter=require('./adapters/liquid.cjs'),source=require('./group-scale-runtime.cjs').upgrade(resolved.source),elements=adapter.collect(source,resolved.relPath).elements;resolved={...resolved,source,elements,element:elements.find(e=>e.id===resolved.element.id)};}
  ...
  const shifted=at=>removed?at-(at>=start?start-h.start(removed):0)-(at>=h.end(removed)?h.end(removed)-end-released.length:0):at+(at>=start?opening.length:0)+(at>=end?closing.length:0),mapping=new Map(),used=new Set();
