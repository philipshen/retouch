...
function start({root,port=9400,quiet=false}){
 ...
 const adapter={...html,pages:()=>require('./html-pages.cjs').list(root),assets:{directory:'.',urlPrefix:'/',uploadDirectory:'rt-assets',excludeDirectories:['node_modules'],imageOnly:true},describe:r=>({...html.describe(r),...css.describe(r),classNameDynamic:true,classNameReason:reason}),planOp:(r,op)=>{if(op.type==='setCSSSelection'){const result=require('./html-css-selection.cjs').plan(r,op);if(result.ok)result.selection=result.selection.map(info=>({...info,classNameDynamic:true,classNameReason:reason}));return result;}return op.type==='setCSS'?css.plan(r,op):op.type==='setClasses'?{ok:false,refused:true,reason}:html.planOp(r,op);}};
 let server;
