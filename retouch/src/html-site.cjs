...
function start({root,port=9400,quiet=false}){
 ...
 const adapter={...html,capabilities:{...html.capabilities,ops:[...html.capabilities.ops.filter(op=>op!=='setClasses'),'setCSS','setCSSSelection','frameSelection','groupSelection','removeFrame','reparentSelection','duplicateSelection','deleteSelection','moveSelection']},pages:()=>require('./html-pages.cjs').list(root),assets:{directory:'.',urlPrefix:'/',uploadDirectory:'rt-assets',excludeDirectories:['node_modules'],imageOnly:true},describe:r=>{const base=html.describe(r);return {...base,...css.describe(r),...require('./html-text-styles.cjs').describe(r),...require('./html-color-styles.cjs').describe(r),...require('./html-effect-styles.cjs').describe(r),...require('./html-variable-bindings.cjs').describe(r),structure:{...base.structure,...require('./html-frame-selection.cjs').describe(r)},classNameDynamic:true,classNameReason:reason};},planOp:(r,op)=>{const blocked=require('./svg-boolean-group.cjs').guard(r,op,'html');if(blocked)return blocked;if(op.type==='moveSelection')return require('./native-structure-selection.cjs').plan(r,op,'html');if(['frameSelection','groupSelection','removeFrame'].includes(op.type))return require('./html-frame-selection.cjs').plan(r,op);if(op.type==='reparentSelection')return require('./html-reparent-selection.cjs').plan(r,op);if(['duplicateSelection','deleteSelection'].includes(op.type))return require('./html-structure-selection.cjs').plan(r,op);if(op.type==='setCSSSelection'){const result=require('./html-css-selection.cjs').plan(r,op);if(result.ok)result.selection=result.selection.map(info=>({...info,classNameDynamic:true,classNameReason:reason}));return result;}return op.type==='setCSS'?css.plan(r,op):op.type==='setClasses'?{ok:false,refused:true,reason}:html.planOp(r,op);}};
 let server;
