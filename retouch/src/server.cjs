...
function handle(req, res, ctx) {
  ...
  if (p === '/rt/__api/text-styles' || p === '/rt/__api/color-styles' || p === '/rt/__api/effect-styles') {
    ...
    return readBinary(req,library.LIMIT,bytes=>{
      ...
      try{
        const renderer=ctx.adapter.name==='vue'?'vue':['react','liquid'].includes(ctx.adapter.name)?ctx.adapter.name:'html',linked=ctx.adapter.capabilities?.ops?.includes('setCSS')||['react','liquid'].includes(ctx.adapter.name);
    ...
    return readBody(req, (body) => {
      ...
      try {
        ...
        if(['applyVariable','resetVariable','detachVariable','removeVariable','applyVariableSelection','resetVariableSelection','detachVariableSelection','removeVariableSelection'].includes(op.type)){
          ...
          result=applyPlan(ctx.appRoot,op.type.endsWith('Selection')?require('./text-style-selection.cjs').plan(resolved,op,style,ctx.adapter,'color'):(ctx.adapter.name==='vue'?require('./vue-linked-styles.cjs').create('color',ctx.adapter):require(ctx.adapter.name==='react'?'./jsx-color-styles.cjs':ctx.adapter.name==='liquid'?'./liquid-color-styles.cjs':'./html-color-styles.cjs')).plan(resolved,op,style));
  ...
  if (req.method === 'GET' && (p === '/rt' || p.startsWith('/rt/'))) {
    const html = fs
      ...
      .replace('__RETOUCH_RENDERING__', JSON.stringify({componentInsertion:ctx.adapter.name==='react',componentLibrary:!!ctx.adapter.describeComponent,history:ctx.history.snapshot(),historyPersistenceError:ctx.history.persistenceError,historyRecoveryRequired:ctx.history.recoveryRequired,stateScope:ctx.stateScope,reloadOnServerRestart:ctx.rendering.reloadOnServerRestart===true,selectionStyling:ctx.adapter.capabilities?.collectionSelection===true||ctx.adapter.capabilities?.ops?.some(op=>['setClassesSelection','setCSSSelection'].includes(op))===true,layerReparenting:ctx.adapter.capabilities?.ops?.includes('reparentElement')===true,reloadAfterWrite:ctx.rendering.reloadAfterWrite===true,revalidateStyles:ctx.rendering.revalidateStyles===true}).replace(/</g,'\\u003c'));
