...
function handle(req, res, ctx) {
  ...
  if(p==='/rt/__api/variables'){
    ...
    return readBinary(req,library.LIMIT,bytes=>{
      ...
      try{
        const plan=ctx.adapter.capabilities?.ops?.includes('setCSS')||['react','liquid'].includes(ctx.adapter.name)?require('./variable-update.cjs').plan(ctx.appRoot,operation,['react','liquid','vue','svelte'].includes(ctx.adapter.name)?ctx.adapter.name:'html',ctx.adapter):library.planChange(ctx.appRoot,operation);
  ...
  if (p === '/rt/__api/text-styles' || p === '/rt/__api/color-styles' || p === '/rt/__api/effect-styles') {
    ...
    return readBinary(req,library.LIMIT,bytes=>{
      ...
      try{
        const renderer=['vue','svelte'].includes(ctx.adapter.name)?ctx.adapter.name:['react','liquid'].includes(ctx.adapter.name)?ctx.adapter.name:'html',linked=ctx.adapter.capabilities?.ops?.includes('setCSS')||['react','liquid'].includes(ctx.adapter.name);
    ...
    return readBody(req, async (body) => {
      ...
      try {
        ...
        if(op.type==='connectPrototypeScroll'){const target=ctx.index.resolve(op.targetId);result=applyPlan(ctx.appRoot,require('./prototype-connect.cjs').plan(resolved,target,op,ctx.adapter));}
          ...
          result=applyPlan(ctx.appRoot,op.type.endsWith('Selection')?require('./variable-selection.cjs').plan(resolved,op,model,ctx.adapter):(['vue','svelte'].includes(ctx.adapter.name)?require('./'+ctx.adapter.name+'-linked-styles.cjs').create('variable',ctx.adapter):require(reactVariables?'./jsx-variable-bindings.cjs':liquidVariables?'./liquid-variable-bindings.cjs':'./html-variable-bindings.cjs')).plan(resolved,op,model));
          ...
          if(op.fileHash!==resolved.hash)return json(res,409,{ok:false,reason:'The source changed. Re-select the layer.'});
          if(op.type==='updateEffectStyle')result=applyPlan(ctx.appRoot,require('./text-style-update.cjs').plan(ctx.appRoot,{type:'update',revision:op.libraryRevision,id:op.styleId,name:op.name,properties:op.properties},['vue','svelte'].includes(ctx.adapter.name)?ctx.adapter.name:reactEffects?'react':liquidEffects?'liquid':'html','effect',ctx.adapter));
          else {let style;if(!op.type.startsWith('detachEffectStyle')){const library=require('./effect-styles.cjs').read(ctx.appRoot);if(library.revision!==op.libraryRevision)return json(res,409,{ok:false,reason:'Effect styles changed. Reload the library.'});style=op.type==='resetEffectStyleSelection'?library:library.styles.find(item=>item.id===op.styleId);if(!style)return json(res,409,{ok:false,reason:'That effect style no longer exists.'});}result=applyPlan(ctx.appRoot,op.type.endsWith('Selection')?require('./text-style-selection.cjs').plan(resolved,op,style,ctx.adapter,'effect'):(ctx.adapter.name==='svelte'?require('./svelte-linked-styles.cjs').create('effect',ctx.adapter):ctx.adapter.name==='vue'?require('./vue-linked-styles.cjs').create('effect',ctx.adapter):require(reactEffects?'./jsx-effect-styles.cjs':liquidEffects?'./liquid-effect-styles.cjs':'./html-effect-styles.cjs')).plan(resolved,op,style));}
        }else if(op.type==='pasteClassStylesSelection'){
          result=applyPlan(ctx.appRoot,require('./layer-style-paste.cjs').plan(resolved,op,ctx.adapter));
        }else if(['setColorOverrideSelection','setBackgroundPaintSelection'].includes(op.type)){
          ...
          result=applyPlan(ctx.appRoot,op.type.endsWith('Selection')?require('./text-style-selection.cjs').plan(resolved,op,style,ctx.adapter,'color'):(ctx.adapter.name==='svelte'?require('./svelte-linked-styles.cjs').create('color',ctx.adapter):ctx.adapter.name==='vue'?require('./vue-linked-styles.cjs').create('color',ctx.adapter):require(ctx.adapter.name==='react'?'./jsx-color-styles.cjs':ctx.adapter.name==='liquid'?'./liquid-color-styles.cjs':'./html-color-styles.cjs')).plan(resolved,op,style));
          ...
          result=applyPlan(ctx.appRoot,require('./text-style-update.cjs').plan(ctx.appRoot,{type:'update',revision:op.libraryRevision,id:op.styleId,name:op.name,properties:op.properties},['react','liquid','vue','svelte'].includes(ctx.adapter.name)?ctx.adapter.name:'html','text',ctx.adapter));
