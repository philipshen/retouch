...
function plan(resolved,op){
 ...
 try{
  ...
  out.appendLeft(destination.location.startTag.startOffset+1+destination.tag.length,` ${marker}="parent"`);
  for(const [index,range]of ranges.entries()){
   const chunk=new MagicString(resolved.source.slice(range.start,range.end));chunk.appendLeft(range.element.tag.length+1,` ${marker}="m${index}"`);chunks.push(chunk.toString());out.remove(range.start,range.end);
  }
  out.appendLeft(offset,'\n  '+chunks.join('\n  ')+'\n');
  let after=out.toString();const parsed=html.collect(after,resolved.relPath).elements,marked=value=>parsed.find(e=>e.node.attrs.some(a=>a.name===marker&&a.value===value)),parent=marked('parent');
  if(parsed.length!==elements.length||!parent)return refuse('This move changes the parsed HTML structure.');
  const moved=ranges.map((range,index)=>{const element=marked('m'+index);if(!element||element.tag!==range.element.tag||element.node.parentNode!==parent.node)throw Error('A moved layer changed its parsed parent.');return element;});
