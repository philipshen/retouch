...
function htmlHref(source,node){
 ...
 if(node.tagName!=='a'||hrefs.length!==1||!loc?.attrs?.href)return null;
 ...
 const errors=[];require('parse5').parseFragment(source.slice(loc.startOffset,loc.startTag.endOffset),{onParseError:e=>errors.push(e.code)});
...
function patch(resolved,element,kind,href,openingOnly=false){
 ...
 return resolved.source.slice(start,attr.start)+'href='+require('./rich-text.cjs').hrefMarkup(href,kind==='react')+resolved.source.slice(attr.end,end);
}
