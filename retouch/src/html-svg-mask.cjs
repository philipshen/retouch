...
function releaseContext(r){
 const group=r.element;if(group.tag!=='g'||attr(group,'data-rt-mask-group')===undefined||group.node.attrs.some(a=>a.name!=='data-rt-mask-group'))return null;
...
function plan(r,op){
 ...
 if(op.type==='createSVGMask'){
  ...
  if(!parent||roots.some(e=>e.node.parentNode!==parent.node||e.node.namespaceURI!==namespace||!['g','svg','rect','circle','ellipse','path','polygon','polyline','line','text','image','use'].includes(e.tag)||!require('./svg-delete.cjs').describe({...r,elements,element:e})))return refuse('Choose complete sibling SVG graphics outside rendered templates.');
  ...
  insertions=[{at:start,text:'<g data-rt-mask-group=""><mask id="'+id+'" mask-type="'+mode+'" maskContentUnits="userSpaceOnUse">'},{at:maskEnd,text:'</mask><g data-rt-mask-content="" mask="url(#'+id+')">'},{at:end,text:'</g></g>'}];for(const edit of insertions)out.appendLeft(edit.at,edit.text);wrapperStart=start;
...
module.exports={plan,describe:r=>releaseContext(r)?{canRelease:true}:null};
