...
function sanitize(source,{fragment=false,baseURL}={}){
 ...
 function walk(parent){parent.childNodes=(parent.childNodes||[]).filter(node=>{
  ...
  node.attrs=node.attrs.filter(attr=>{
   ...
   if(['src','href','poster','background'].includes(name)){const value=attr.value.trim();if(name==='href'&&value.startsWith('#'))return true;if(name==='href'&&isSVG&&!['image','feImage'].includes(node.tagName))return false;if(/^(https?:\/\/|blob:)/i.test(value))return true;if(baseURL&&!/^[a-z][a-z0-9+.-]*:/i.test(value)){try{attr.value=new URL(value,baseURL).href;return /^https?:/.test(attr.value);}catch{}}return (name==='src'||isSVG&&['image','feImage'].includes(node.tagName))&&/^data:image\/(?:png|jpeg|gif|webp|avif|svg\+xml)[;,]/i.test(value);}
   return name!=='srcset';
  });
