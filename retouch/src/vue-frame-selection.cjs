...
const structure = require('./vue-structure.cjs'), insertion = require('./vue-insert.cjs');
...
function releaseRange(element) {
  if (!isFrame(element) || element.node.isSelfClosing || element.node.props.some(prop => prop.type === NodeTypes.DIRECTIVE)) throw Error('Choose a complete Retouch frame without Vue directives.');
  const opening = element.node.loc.source.match(/^<(?:[^"'<>]|"[^"]*"|'[^']*')*>/)?.[0];
  if (!opening || /\sv-pre(?:[\s=>]|$)/.test(opening.replace(/"[^"]*"|'[^']*'/g, ''))) throw Error('This frame changes Vue template interpretation.');
  const close = element.node.loc.source.lastIndexOf('</' + element.tag);
  if (close < opening.length) throw Error('The frame has no complete closing tag.');
...
function describe(resolved, adapter) {
  try {
    ...
    let canRemoveFrame = false; try { releaseRange(resolved.element); canRemoveFrame = true; } catch {}
...
function plan(resolved, op, adapter) {
  try {
    ...
    function visit(node) { for (const child of node.children || []) { parents.set(child, node); visit(child); } } visit(parsed.ast);
    ...
    if (op.type === 'removeFrame') {
      ...
      ({ start, end } = releaseRange(removed));
      const data = structure.siblings({ ...resolved, element: removed }, adapter);
      parent = elements.find(element => element.id === data.parentId);
      roots = elements.filter(element => parents.get(element.node) === removed.node);
    } else {
      if (!Array.isArray(op.ids) || !op.ids.length || op.ids.length > 100 || new Set(op.ids).size !== op.ids.length || !op.ids.includes(resolved.element.id)) throw Error('Choose 1 to 100 distinct layers in one Vue component.');
    ...
    if (next.length !== elements.length + (removed ? -1 : 1)) throw Error('Framing changed the parsed Vue structure.');
    ...
    if (!removed && (!frame || !map.get(parent).node.children.includes(frame.node))) throw Error('The frame changed its parsed parent.');
    for (const element of elements) {
      if (element === removed) continue;
      const priorParent = elements.find(item => item.node === parents.get(element.node));
      const expected = roots.includes(element) ? (removed ? map.get(parent) : frame) : map.get(priorParent);
      if (expected && !expected.node.children.includes(map.get(element).node)) throw Error('Framing moved an unrelated layer.');
    }
    if (removed) {
      const styles = require('./vue-css.cjs');
      const state = styles.structuralStyles(resolved, adapter, { start: removed.start, end: start }, false);
