...
function planSelection(r,op,kind){
 ...
 const adapter=require('./adapters/'+kind+'.cjs'),strokes=require('./svg-stroke-source.cjs'),collect=source=>kind==='react'?ids.collectElements(source,r.relPath).elements:adapter.collect(source,r.relPath).elements;let source=r.source;
 const strokeBlocked=strokes.guard(r,op,kind);if(strokeBlocked)return strokeBlocked;
 for(const id of selected){const elements=collect(source),element=elements.find(el=>el.id===id);if(!element)return refuse('Every selected vector must resolve in the same source file.');const hash=adapter.contentHash(source),member={...r,source,elements,element,hash},blocked=require('./svg-boolean-group.cjs').guard(member,op,kind);if(blocked)return blocked;const shape=describe(member,kind);if(shape?.editable&&shape.matrix.every((value,index)=>value===matrices[id][index]))continue;const retained=strokes.context(member,kind),result=retained?strokes.plan(member,{type:'setSVGStrokeSourceTransform',matrix:matrices[id],fileHash:hash},kind):plan(member,{matrix:matrices[id],fileHash:hash},kind);if(!result.ok)return result;source=result.edits[0]?.after||source;}
 const before=r.elements||collect(r.source),after=collect(source);if(before.length!==after.length||before.some((el,i)=>el.id!==after[i].id))return refuse('The transforms change existing source layer identities.');
 const hash=adapter.contentHash(source),elements=after,selection=selected.map(id=>adapter.describe({...r,source,hash,elements,element:elements.find(el=>el.id===id)}));
