...
function inline(raw,jsx=false){
 ...
 if(jsx){
  ...
  if(node.openingElement?.name.name!=='span'||attrs?.length!==1)throw Error('Only explicit text paragraphs can be joined.');start=attrs[0].start;end=attrs[0].end;
 }else{
  ...
  const attr=loc?.attrs?.['data-retouch-paragraph'];if(node?.tagName!=='span'||!attr||errors.some(e=>e.code==='duplicate-attribute'&&e.startOffset<loc.startTag.endOffset))throw Error('Only explicit text paragraphs can be joined.');start=attr.startOffset;end=attr.endOffset;
 }
 return require('./inline-source-property.cjs').patch(raw.slice(0,start)+raw.slice(end),'span','inline',jsx,'display');
}
