...
const shapes={rect:['x','y','width','height','rx','ry'],circle:['cx','cy','r'],ellipse:['cx','cy','rx','ry'],line:['x1','y1','x2','y2']};
const labels={x:'X',y:'Y',width:'Width',height:'Height',rx:'Horizontal radius',ry:'Vertical radius',cx:'Center X',cy:'Center Y',r:'Radius',x1:'Start X',y1:'Start Y',x2:'End X',y2:'End Y'};
...
function valid(name,value){if(value===null)return true;if(typeof value!=='string'||value.length>40||!/^[-+]?(?:\d+\.?\d*|\.\d+)(?:px|%)?$/.test(value))return false;const number=parseFloat(value);return Math.abs(number)<=100000&&(!['width','height','r','rx','ry'].includes(name)||number>=0);}
