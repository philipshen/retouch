...
function describe(element){if(element.node.namespaceURI!=='http://www.w3.org/2000/svg'||!shapes[element.tag])return null;return {parametric:['polygon','polyline','path'].includes(element.tag)?require('../shell/svg-parametric.js').describe(element.tag==='path'?require('../shell/svg-parametric.js').pointsFromPath(element.node.attrs.find(a=>a.name==='d')?.value):element.node.attrs.find(a=>a.name==='points')?.value,element.node.attrs.find(a=>a.name==='data-rt-shape')?.value):null,fields:shapes[element.tag].map(name=>({name,label:labels[name],value:element.node.attrs.find(a=>a.name===name)?.value??null}))};}
