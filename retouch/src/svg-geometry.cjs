...
function valid(name,value){if(value===null)return true;if(name==='d')return !!require('../shell/svg-path.js').parse(value);if(name==='points')return !!points.parse(value);if(typeof value!=='string'||value.length>40||!/^[-+]?(?:\d+\.?\d*|\.\d+)(?:px|%)?$/.test(value))return false;const number=parseFloat(value);return Math.abs(number)<=100000&&(!['width','height','r','rx','ry'].includes(name)||number>=0);}
