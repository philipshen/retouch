...
function css(value,marker,property='list-style-type'){
 ...
 const relevant=part=>(property==='display'?/^\s*(?:display|all)\s*:/i:/^\s*(?:list-style(?:-type)?|all)\s*:/i).test(clean(part));
 ...
 if(last>=0&&(property==='display'?/^ ?display: (?:inline|block)(?: !important)?;$/:/^ ?list-style-type: (?:disc|decimal|lower-alpha|lower-roman|none|inherit)(?: !important)?;$/).test(parts[last]))value=parts.slice(0,last).join('');
...
function patch(raw,tag,marker,jsx=false,property='list-style-type'){
 const camel=property==='display'?'display':'listStyleType';
