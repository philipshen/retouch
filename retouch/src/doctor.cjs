...
function doctor(root = process.cwd()) {
  ...
  try {
    ...
    if (!/^8\.\d+\.\d+(?:[-+].*)?$/.test(vite.version)) {
      ...
      if(vue)console.log(vuePlugin ? `Vue Vite plugin: ${vuePlugin.version}; SFC text, image, tag, layer-name, responsive CSS and native sibling ordering, duplicate, copy/paste, delete, text/frame insertion and moving between compatible containers, and batch duplicate/delete are available. Computed inline styles, other structural operations and linked style libraries remain incomplete.` : 'Vue setup: install @vitejs/plugin-vue and include vue() in the Vite configuration.');
