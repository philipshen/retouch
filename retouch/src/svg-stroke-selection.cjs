...
module.exports=function planSelection(r,op,kind){
 ...
 const perMember=op.values!==undefined;
 if(perMember&&(!['fill','stroke'].includes(op.property)||op.value!==undefined||!op.values||typeof op.values!=='object'||Array.isArray(op.values)||Object.keys(op.values).length!==selected.length||selected.some(id=>!Object.hasOwn(op.values,id)||typeof op.values[id]!=='string')))return refuse('Provide one literal paint for every selected vector.');
 const extra=op.property==='position'?{position:op.value}:op.property==='width'?{width:op.value}:{property:op.property,value:op.value};
 try{
  ...
  for(const originalId of selected){
   ...
   const change=S.plan(member,{type,fileHash:member.hash,...extra,...(perMember?{value:op.values[originalId]}:{})},kind);if(!change.ok)return change;
