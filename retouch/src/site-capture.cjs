...
async function capture({url,directory,width=1440,height=900,wait=1000,browserType,signal}){
 ...
 try{
  ...
  const snapshot=await bounded(page.evaluate(require('./capture-document.cjs'),{shadowKey}));if(typeof snapshot.html!=='string')throw Error('The page did not return a captured document.');if(Buffer.byteLength(snapshot.html)>20*1024*1024)throw Error('The captured document exceeds 20 MiB.');
