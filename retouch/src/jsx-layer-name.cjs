...
function metadata(resolved){
 const opening=resolved.element.node.openingElement,start=(opening.typeParameters||opening.typeArguments||opening.name).end,end=opening.end;
 const ast=resolved.ast||collectElements(resolved.source,resolved.relPath).ast;
 const comments=(ast.comments||[]).filter(c=>c.type==='CommentBlock'&&c.start>=start&&c.end<=end&&!opening.attributes.some(attr=>c.start>=attr.start&&c.end<=attr.end)&&c.value.trim().startsWith(marker));
...
function describe(resolved){return ['host','instance'].includes(resolved.element.kind)?{canRename:true,layerName:metadata(resolved).name}:{};}
function plan(resolved,op){
 const refuse=reason=>({ok:false,refused:true,reason});
 if(op.fileHash!==resolved.hash)return refuse('The source changed. Re-select the layer before naming it.');
 if(!['host','instance'].includes(resolved.element.kind))return refuse('Select a source layer to name.');
