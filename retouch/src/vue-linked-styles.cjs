...
// Share CSS binding semantics with HTML without parsing Vue source as HTML.
...
function create(family,adapter=require('./adapters/vue.cjs')){
 if(!['color','effect','variable'].includes(family))throw Error('Unsupported Vue style family.');
 const attribute=family==='variable'?'data-rt-variables':'data-rt-'+family+'-styles',title=family==='color'?'Color':family==='effect'?'Effect':'Variable',refuse=reason=>({ok:false,refused:true,reason});
 ...
 const renderer={...adapter,collect:(source,relative)=>{const parsed=adapter.collect(source,relative);return {...parsed,elements:parsed.elements.map(project)};}},css={describe:r=>require('./vue-css.cjs').describe(r,adapter),plan:(r,op)=>require('./vue-css.cjs').plan(r,op,adapter)},core=require(family==='variable'?'./html-variable-bindings.cjs':'./html-'+family+'-styles.cjs').create(renderer,css);
 ...
 function describe(r){try{links(r);return core.describe(resolve(r));}catch(error){return family==='variable'?{variables:false,variableReason:error.message}:family==='color'?{colorStyles:false,colorStyleReason:error.message}:{effectStyleLinkReason:error.message};}}
 ...
 function planFile(file,relPath,before,style){try{
  if(family==='variable')require('./variable-collections.cjs').validate(style);else require('./'+family+'-styles.cjs').validate({version:1,styles:[style]});const parsed=adapter.collect(before,relPath),indexed=new Set(parsed.elements.map(e=>e.node.loc.start.offset)),targets=[];
