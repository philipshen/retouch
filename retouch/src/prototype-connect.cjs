...
function plan(source,target,op,adapter){try{
 ...
 if(Object.keys(op).some(key=>!['type','id','fileHash','context','index','targetId','targetHash','targetContext','creation'].includes(key))||!Number.isInteger(op.index))throw Error('Invalid prototype connection.');
 const items=P.metadata(source,adapter).interactions,creating=op.creation!==undefined;if(creating){if(op.index!==items.length||!op.creation||typeof op.creation!=='object'||Array.isArray(op.creation)||Object.keys(op.creation).some(key=>!['trigger','delay','shortcut'].includes(key)))throw Error('Choose a new interaction trigger.');V.validate([...items,{...op.creation,action:'scroll',destination:'pending'}]);}else if(items[op.index]?.action!=='scroll')throw Error('Choose a Scroll to interaction.');
 ...
 const next=creating?[...items,{...op.creation,action:'scroll',destination:anchor}]:items.map((item,index)=>index===op.index?{...item,destination:anchor}:item),update=P.plan(source,{type:'setPrototypeInteractions',id:source.element.id,fileHash:source.hash,context:source.context,interactions:next},adapter);
