'use strict';
const crypto=require('node:crypto'),MagicString=require('magic-string'),P=require('./prototype-interactions.cjs');
...
function plan(source,target,op,adapter){try{
 ...
 if(Object.keys(op).some(key=>!['type','id','fileHash','context','index','targetId','targetHash','targetContext'].includes(key))||!Number.isInteger(op.index))throw Error('Invalid prototype connection.');
 const items=P.metadata(source,adapter).interactions;if(items[op.index]?.action!=='scroll')throw Error('Choose a Scroll to interaction.');
 ...
 const next=items.map((item,index)=>index===op.index?{...item,destination:anchor}:item),update=P.plan(source,{type:'setPrototypeInteractions',id:source.element.id,fileHash:source.hash,context:source.context,interactions:next},adapter);
