...
function rewrite(value,sourceId,children,options) {
  ...
  function build(items) {
    return items.map(item=>{
      ...
      return (Object.hasOwn(item,'tag')||item.marker)?patch(original.open+build(item.children)+original.close):patch(original.open)+build(item.children)+original.close;
    }).join('');
  }
