...
function rewrite(value,sourceId,children,options) {
  ...
  function build(items) {
    return items.map(item=>{
      ...
      if(item.t==='copy'){const original=kept.get(item.id);if(!original||original.opaque)throw Error('Unknown split text source.');const markup=require('./rich-text-copy.cjs').markup(original.raw,build(item.children),false,item);return options?.copyMarkup?options.copyMarkup(markup,original,item):markup;}
