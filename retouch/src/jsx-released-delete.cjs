...
function plan(resolved,op){try{
 ...
 const {elements,record,parent,roots}=released.claim(resolved),ids=op.type==='deleteElement'?[resolved.element.id]:op.ids;
 if(!Array.isArray(ids)||!ids.length||ids.length>100||new Set(ids).size!==ids.length||!ids.includes(resolved.element.id)||ids.some(id=>!roots.some(e=>e.id===id)))throw Error('Select released roots from the same group.');
 const removedRoots=roots.filter(e=>ids.includes(e.id)),survivors=roots.filter(e=>!ids.includes(e.id)),removed=elements.filter(e=>removedRoots.some(root=>e.node.start>=root.node.start&&e.node.end<=root.node.end)),dead=new Set(removed.map(released.member).filter(Boolean));
 ...
 const parentId=mapping.get(parent.id);return {ok:true,hash:identity.contentHash(after),structural:true,parentId,rootCount:removedRoots.length,selectionIds:[parentId],sourceIdMap:[...mapping].filter(([before,after])=>before!==after),removedSourceIds:[...removedIds],edits:[{file:resolved.file,before:resolved.source,after}]};
