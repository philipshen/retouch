...
function describeComponent(r){try{
 ...
 const contracts=require('./svelte-component-choices.cjs').read(def.text),accepts=require('./svelte-component-choices.cjs').accepts;
 for(const property of properties){const contract=contracts.get(property.name);if(!contract)continue;const editor=property.editor,valid=contract.supported&&editor.type===contract.type;Object.assign(editor,{definitionHash,...(contract.choices?{choices:contract.choices}:{}),editable:editor.editable&&valid,canReset:!!editor.canReset&&accepts(contract,defaults.get(property.name)?.value),...(!valid?{reason:'This declared property type is not supported by the literal editor.'}:{})});}
 const names=properties.filter(prop=>prop.editor.canReset).map(prop=>prop.name),resetProperties=names.length?{names,revision:source.contentHash(JSON.stringify([r.hash,definitionHash,names]))}:null;
 return {ok:true,resetProperties,name:def.name,file:def.rel,hash:source.contentHash(def.text),usageHash:r.hash,source:def.text,explicitComponent:true,definitionId:anchor||roots[0]?.id||null,definitionIds:roots.map(e=>e.id),rootGroups:def.meta.rootGroups,canDetach:false,canDuplicate:false,reason:'Svelte component detach and duplication are not available yet.',props:properties};
...
function create(base){
 ...
 const adapter={...base,
  ...
  planOp(r,op){if(r.element.kind!=='instance')return base.planOp(host(r),op);if(['pasteComponentProps','setComponentPropSelection','resetComponentProps','resetComponentPropsSelection'].includes(op.type))return require('./svelte-component-batch-props.cjs').plan(r,op,adapter);if(op.type!=='setComponentProp')return refused('Choose a supported component property edit.');const component=describeComponent(r);if(!component.ok)return component;const def=definition(r),contract=require('./svelte-component-choices.cjs').read(def.text).get(op.name);
   if(op.definitionHash!==undefined&&op.definitionHash!==source.contentHash(def.text))return refused('The component definition changed. Re-select the instance.');
   if(contract){if(op.definitionHash!==source.contentHash(def.text))return refused('The component property type changed. Re-select the instance.');const value=op.reset?require('./svelte-component-defaults.cjs').read(def.text).get(op.name)?.value:op.value;if(!require('./svelte-component-choices.cjs').accepts(contract,value))return refused('Choose a value allowed by the declared component property type.');}
   const result=op.reset||!props.describe(r).props.some(prop=>prop.name===op.name)?require('./svelte-component-defaults.cjs').plan(r,op,def):props.plan(r,op);
  ...
  capabilities:{...base.capabilities,ops:[...base.capabilities.ops,'setComponentProp','setComponentPropSelection','pasteComponentProps','resetComponentProps','resetComponentPropsSelection']}
 };
