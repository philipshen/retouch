...
function definition(r){
 ...
 r.guardComponentResolution?.(resolution);
 const text=fs.readFileSync(file,'utf8'),rel=relative.split(path.sep).join('/'),meta=markers.metadata(text,rel);
 const definitionHash=resolution.dependencies.length?source.contentHash(JSON.stringify([file,source.contentHash(text),resolution.dependencies,resolution.pathChecks])):source.contentHash(text);
 return {name,file,rel,text,meta,definitionHash};
}
