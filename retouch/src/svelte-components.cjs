...
function create(base){
 ...
 const adapter={...base,
  ...
  planOp(r,op){if(r.element.kind!=='instance')return base.planOp(host(r),op);if(require('./svelte-component-structure.cjs').types.includes(op.type))return require('./svelte-component-structure.cjs').plan(r,op,adapter,definition);if(['pasteComponentProps','setComponentPropSelection','resetComponentProps','resetComponentPropsSelection'].includes(op.type))return require('./svelte-component-batch-props.cjs').plan(r,op,adapter);if(op.type!=='setComponentProp')return refused('Choose a supported component property edit.');const component=describeComponent(r);if(!component.ok)return component;const def=definition(r),contract=require('./svelte-component-choices.cjs').read(def.text).get(op.name);
   ...
   if(contract){if(op.definitionHash!==source.contentHash(def.text))return refused('The component property type changed. Re-select the instance.');const value=op.reset?require('./svelte-component-defaults.cjs').read(def.text).get(op.name)?.value:op.value;if(!op.clear&&!require('./svelte-component-choices.cjs').accepts(contract,value))return refused('Choose a value allowed by the declared component property type.');}
  ...
  capabilities:{...base.capabilities,ops:[...base.capabilities.ops,...require('./svelte-component-structure.cjs').types,'setComponentProp','setComponentPropSelection','pasteComponentProps','resetComponentProps','resetComponentPropsSelection']}
 };
