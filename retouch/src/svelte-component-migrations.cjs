 ...
 for(const edit of plan.edits||[]){
  ...
  try{
   ...
   if(structural){
    ...
    const from=source.contentHash(before.signature),to=source.contentHash(after.signature),scriptProof=plan.insertedComponent?.preserveScriptState?{fromScript:require('./svelte-component-state.cjs').metadata(edit.before,relative).script,toScript:require('./svelte-component-state.cjs').metadata(edit.after,relative).script}:{};
