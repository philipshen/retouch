...
function record(root,plan){
 if(!plan?.ok)return;
 const structural=!!(plan.createdComponent||plan.duplicatedComponent||plan.deletedComponent||plan.copiedComponents||plan.deletedComponentIds||plan.movedComponent||plan.movedComponentIds||plan.insertedComponent||plan.detachedComponent||plan.componentDefault?.preserveScriptState),mapping=new Map(plan.createdComponent?.sourceIdMap||plan.detachedComponent?.sourceIdMap||plan.insertedComponent?.sourceIdMap||plan.movedComponent?.sourceIdMap||plan.duplicatedComponent?.sourceIdMap||plan.sourceIdMap||[]),removed=new Set([...(plan.deletedComponent?.removedSourceIds||plan.removedSourceIds||[]),...((plan.detachedComponent||plan.insertedComponent)?.previousInstanceId?[(plan.detachedComponent||plan.insertedComponent).previousInstanceId]:[])]);
 for(const edit of plan.edits||[]){
  ...
  try{
   ...
   if(structural){
    ...
    const from=source.contentHash(before.signature),to=source.contentHash(after.signature),scriptProof=(plan.createdComponent||plan.componentDefault||plan.detachedComponent||plan.insertedComponent)?.preserveScriptState?{fromScript:require('./svelte-component-state.cjs').metadata(edit.before,relative).script,toScript:require('./svelte-component-state.cjs').metadata(edit.after,relative).script}:{};
    for(const [revision,edge]of [[afterRevision,{beforeRevision,from,to,pairs,...scriptProof,ids:before.components.map(e=>e.id)}],[beforeRevision,{beforeRevision:afterRevision,from:to,to:from,...(scriptProof.fromScript?{fromScript:scriptProof.toScript,toScript:scriptProof.fromScript}:{}),pairs:pairs.map(([a,b])=>[b,a]),ids:after.components.map(e=>e.id)}]]){const key=file+'|'+revision,prior=records.get(key)||[];records.delete(key);records.set(key,[...prior.filter(item=>item.beforeRevision!==edge.beforeRevision),edge].slice(-20));}
