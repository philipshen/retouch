...
function plan(resolved,op,library,adapter=html){try{
 const isReact=adapter.name==='react';if(!isReact&&adapter.name!=='html')return refuse('Variable selections need HTML or React layers.');
 const linked=require(isReact?'./jsx-variable-bindings.cjs':'./html-variable-bindings.cjs');
 const scope=isReact?(op.scope??''):op.width;if(isReact)require('../shell/responsive.js').replaceScope('','',scope);
 ...
 if((!isReact&&(!Number.isInteger(op.width)||op.width<0||op.width>7680))||!bindings.properties.includes(op.property))return refuse('Choose a supported property and screen scope.');
 ...
 for(const id of ids){const element=initial.find(element=>element.id===id);if(isReact){if(element?.kind!=='host')return refuse('Every selected layer must be a host layer in the same React source file.');continue;}let node=element?.node;while(node&&node.tagName!=='body')node=node.parentNode;if(!node)return refuse('Every selected layer must belong to the same HTML body.');}
