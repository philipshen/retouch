...
function describe(resolved){
 ...
 return {svgStrokeCreation:require('../svg-stroke-source.cjs').creation(resolved,'html'),svgStrokeSource:require('../svg-stroke-source.cjs').describe(resolved,'html'),svgStrokeOwner:require('../svg-stroke-source.cjs').owner(resolved,'html'),svgBooleanGroup:require('../svg-boolean-group.cjs').describe(resolved,'html'),svgBooleanOwner:require('../svg-boolean-group.cjs').owner(resolved,'html'),svgMask:require('../html-svg-mask.cjs').describe(resolved),svgBooleanReplacement:require('../svg-combine-selection.cjs').describe(resolved,'html'),svgGradientCreation:require('../svg-gradient-create.cjs').describe(resolved,'html'),svgGradients:require('../html-svg-gradient.cjs').describe(resolved),svgTransform:require('../svg-transform.cjs').describe(resolved,'html'),svgConversion:require('../svg-convert.cjs').describe(resolved),svgDuplication,svgMovement:require('../svg-move.cjs').describe(resolved),svgDeletion:require('../svg-delete.cjs').describe(resolved),svgInsertion:require('../svg-insert.cjs').describe(resolved),svgGeometry:require('../svg-geometry.cjs').describe(el),structure:{...structure.describe(resolved,'html'),...insertion.describe(resolved),...require('../svg-delete.cjs').describe(resolved),...require('../svg-move.cjs').describe(resolved),...svgDuplication},id:el.id,kind:'host',tag:el.tag,file:resolved.relPath,hash:resolved.hash,className:attr(el,'class')||'',classNameDynamic:false,
...
function planUnchecked(resolved,op){
 const strokes=require('../svg-stroke-source.cjs'),strokeBlocked=strokes.guard(resolved,op,'html');if(strokeBlocked)return strokeBlocked;
 const groups=require('../svg-boolean-group.cjs'),blocked=groups.guard(resolved,op,'html');if(blocked)return blocked;if(strokes.types.has(op.type))return strokes.plan(resolved,op,'html');if(groups.types.has(op.type))return groups.plan(resolved,op,'html');
 ...
 if(op.type==='setPictureSources')return require('../html-picture-sources.cjs').plan(resolved,op);
 if(op.type==='setResponsiveImageCandidates')return require('../html-responsive-image.cjs').planCandidates(resolved,op);
...
function planOp(resolved,op){return require('../svg-stroke-source.cjs').validatePlan(resolved,op,'html',planUnchecked(resolved,op));}
