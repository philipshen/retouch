...
const adapter={name:'svelte',matches:file=>/\.svelte$/i.test(file),collect:source.collect,stamp:source.stamp,contentHash:source.contentHash,describe,planOp,assets:{directory:'public',urlPrefix:'/',uploadDirectory:'rt-assets',imageOnly:true},capabilities:{classAttr:'class',ops:[...require('../svelte-structure-selection.cjs').types,...require('../svelte-frame-selection.cjs').types,...require('../svelte-structure.cjs').types,'reparentElement','insertElement','setChildren','setText','renameElement','setHref','setSrc','setCSS','setCSSSelection',...['Text','Color','Effect'].flatMap(family=>['apply','reset','detach'].flatMap(action=>[action+family+'Style',action+family+'StyleSelection'])), 'updateTextStyle','updateEffectStyle',...['apply','reset','detach','remove'].flatMap(action=>[action+'Variable',action+'VariableSelection'])]},applyOp:(r,op)=>require('../transactions.cjs').applyPlan(r.appRoot||path.dirname(r.file),planOp(r,op))};
