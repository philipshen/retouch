...
function describe(r){
 ...
 return {id:e.id,kind:'host',tag:e.tag,file:r.relPath,hash:r.hash,renderRevisionAttribute:'data-rt-revision',preserveTextNodes:true,styleAuthoring:false,linkedStyleAuthoring:false,textStyleAuthoring:false,className:attr(e,'class')?.value||'',classNameDynamic:true,classNameReason:'Responsive CSS editing for Svelte is not implemented yet.',text:range?.text??null,renderedText:range?.text??null,textDynamic:!range,textReason:'This Svelte region contains expressions, bindings, or nested markup. Its template logic is preserved.',canSetTag:false,canRename:!bound(e,'data-rt-name'),layerName:attr(e,'data-rt-name')?.value||'',src:attr(e,'src')?.value??null,srcDynamic:bound(e,'src'),canSetSrc,srcReason:canSetSrc?null:'Choose an image without source bindings or a responsive picture.',href:e.tag==='a'?attr(e,'href')?.value??null:undefined,canSetHref:e.tag==='a'&&!bound(e,'href'),hrefReason:'The link destination is controlled by a Svelte binding.',context:r.context||null};
}
...
const adapter={name:'svelte',matches:file=>/\.svelte$/i.test(file),collect:source.collect,stamp:source.stamp,contentHash:source.contentHash,describe,planOp,assets:{directory:'public',urlPrefix:'/',uploadDirectory:'rt-assets',imageOnly:true},capabilities:{classAttr:'class',ops:['setText','renameElement','setHref','setSrc']},applyOp:(r,op)=>require('../transactions.cjs').applyPlan(r.appRoot||path.dirname(r.file),planOp(r,op))};
