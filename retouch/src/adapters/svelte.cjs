...
function describe(r){
 const e=r.element,range=textRange(e,r.source),rich=require('../svelte-rich-text.cjs').describe(r),canSetSrc=e.tag==='img'&&!e.scope.picture&&!bound(e,'src')&&!bound(e,'srcset')&&!attr(e,'srcset');
 return {id:e.id,kind:'host',tag:e.tag,file:r.relPath,hash:r.hash,renderRevisionAttribute:'data-rt-revision',preserveTextNodes:true,structure:{...require('../svelte-structure.cjs').describe(r,adapter),...require('../svelte-insert.cjs').describe(r),...require('../svelte-reparent.cjs').describe(r,adapter),...require('../svelte-frame-selection.cjs').describe(r,adapter)},responsiveImage:require('../svelte-responsive-image.cjs').describe(r),pictureSourceAuthoring:!!require('../svelte-responsive-image.cjs').describe(r)?.picture,selectionStructureOps:[...require('../svelte-frame-selection.cjs').types,...require('../svelte-structure-selection.cjs').types],linkedStyleAuthoring:true,textStyleAuthoring:true,className:attr(e,'class')?.value||'',classNameDynamic:true,classNameReason:'Use the responsive CSS properties for Svelte styles.',text:range?.text??null,renderedText:range?.text??null,textDynamic:!range&&!rich.canSetChildren,mixedText:rich.canSetChildren&&!range,...rich,textReason:'This Svelte region contains expressions, bindings, or nested markup. Its template logic is preserved.',canSetTag:false,canRename:!bound(e,'data-rt-name'),layerName:attr(e,'data-rt-name')?.value||'',src:attr(e,'src')?.value??null,srcDynamic:bound(e,'src'),canSetSrc,srcReason:canSetSrc?null:'Choose an image without source bindings or a responsive picture.',href:e.tag==='a'?attr(e,'href')?.value??null:undefined,canSetHref:e.tag==='a'&&!bound(e,'href'),hrefReason:'The link destination is controlled by a Svelte binding.',context:r.context||null,...require('../svelte-css.cjs').describe(r),...Object.assign({},...['text','color','effect','variable'].map(family=>require('../svelte-linked-styles.cjs').create(family,adapter).describe(r)))};
}
function planOp(r,op){try{
 if(op.fileHash!==r.hash)return refuse('The file changed. Re-select the layer.');
 if(require('../svelte-structure.cjs').types.includes(op.type))return require('../svelte-structure.cjs').plan(r,op,adapter);
 if(require('../svelte-structure-selection.cjs').types.includes(op.type))return require('../svelte-structure-selection.cjs').plan(r,op,adapter);
 if(require('../svelte-frame-selection.cjs').types.includes(op.type))return require('../svelte-frame-selection.cjs').plan(r,op,adapter);
 if(['setResponsiveImage','setResponsiveImageSource','setResponsiveImageCandidates'].includes(op.type))return require('../svelte-responsive-image.cjs')[{setResponsiveImage:'plan',setResponsiveImageSource:'planSource',setResponsiveImageCandidates:'planCandidates'}[op.type]](r,op);
 if(op.type==='setPictureSources')return require('../svelte-picture-sources.cjs').plan(r,op);
 if(op.type==='reparentElement')return require('../svelte-reparent.cjs').plan(r,op,adapter);
 if(op.type==='insertElement')return require('../svelte-insert.cjs').plan(r,op,adapter);
 if(op.type==='setChildren')return require('../svelte-rich-text.cjs').plan(r,op);
...
const adapter={name:'svelte',matches:file=>/\.svelte$/i.test(file),collect:source.collect,stamp:source.stamp,contentHash:source.contentHash,describe,planOp,assets:{directory:'public',urlPrefix:'/',uploadDirectory:'rt-assets',imageOnly:true},capabilities:{classAttr:'class',ops:[...require('../svelte-structure-selection.cjs').types,...require('../svelte-frame-selection.cjs').types,...require('../svelte-structure.cjs').types,'setPictureSources','setResponsiveImage','setResponsiveImageSource','setResponsiveImageCandidates','reparentElement','insertElement','setChildren','setText','renameElement','setHref','setSrc','setCSS','setCSSSelection',...['Text','Color','Effect'].flatMap(family=>['apply','reset','detach'].flatMap(action=>[action+family+'Style',action+family+'StyleSelection'])), 'updateTextStyle','updateEffectStyle',...['apply','reset','detach','remove'].flatMap(action=>[action+'Variable',action+'VariableSelection'])]},applyOp:(r,op)=>require('../transactions.cjs').applyPlan(r.appRoot||path.dirname(r.file),planOp(r,op))};
