...
const adapter={name:'svelte',matches:file=>/\.svelte$/i.test(file),collect:source.collect,stamp:source.stamp,contentHash:source.contentHash,describe,planOp,assets:{directory:'public',urlPrefix:'/',uploadDirectory:'rt-assets',imageOnly:true},capabilities:{classAttr:'class',ops:['setText','renameElement','setHref','setSrc','setCSS','setCSSSelection']},applyOp:(r,op)=>require('../transactions.cjs').applyPlan(r.appRoot||path.dirname(r.file),planOp(r,op))};
