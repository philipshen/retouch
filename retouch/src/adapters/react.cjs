...
const { planOp, describeElement } = require('../writer.cjs');
...
module.exports = {
  ...
  describe: resolved => {
    ...
    if(require('../jsx-group-regroup.cjs').applies(resolved)){Object.assign(base,require('../jsx-group-regroup.cjs').describe(resolved));base.canDelete=require('../jsx-released-delete.cjs').plan(resolved,{type:'deleteElement',fileHash:resolved.hash}).ok;base.canCopy=base.canDuplicate=require('../jsx-released-structure.cjs').plan(resolved,{type:'duplicateElement',fileHash:resolved.hash}).ok;for(const [flag,direction]of Object.entries({canMoveBefore:'before',canMoveAfter:'after',canMoveFirst:'first',canMoveLast:'last'}))base[flag]=require('../jsx-released-structure.cjs').plan(resolved,{type:'moveElement',fileHash:resolved.hash,direction}).ok;}
    return {...require('../jsx-group-scale.cjs').describe(resolved),svgStrokeCreation:require('../svg-stroke-source.cjs').creation(resolved,'react'),svgStrokeSource:require('../svg-stroke-source.cjs').describe(resolved,'react'),svgStrokeOwner:require('../svg-stroke-source.cjs').owner(resolved,'react'),svgBooleanGroup:require('../svg-boolean-group.cjs').describe(resolved,'react'),svgBooleanOwner:require('../svg-boolean-group.cjs').owner(resolved,'react'),svgMask:require('../jsx-svg-mask.cjs').describe(resolved),svgBooleanReplacement:require('../svg-combine-selection.cjs').describe(resolved,'react'),componentMovement:resolved.element.kind==='instance'?require('../move-component.cjs').describe(resolved):null,...describeElement(resolved),...require('../jsx-layer-name.cjs').describe(resolved),...require('../jsx-text-styles.cjs').describe(resolved),...require('../jsx-color-styles.cjs').describe(resolved),...require('../jsx-effect-styles.cjs').describe(resolved),...require('../jsx-variable-bindings.cjs').describe(resolved),classSelection:resolved.element.kind==='host',canCreateComponent:resolved.element.kind==='host',canInsertComponent:require('../insert-component.cjs').canContain(resolved),svgDeletion,svgMovement,svgDuplication,context:resolved.context||null,
  ...
  applyOp: (resolved,op) => require('../transactions.cjs').applyPlan(resolved.appRoot||path.dirname(resolved.file),module.exports.planOp(resolved,op)),
  planOp: (resolved,op) => {
    const scaled=require('../jsx-group-scale.cjs').guard(resolved,op);if(scaled)return scaled;
    if(['duplicateElement','pasteElement','moveElement','moveSelection'].includes(op.type)&&require('../jsx-group-regroup.cjs').applies(resolved))return require('../jsx-released-structure.cjs').plan(resolved,op);
    if(['deleteElement','deleteSelection'].includes(op.type)&&require('../jsx-group-regroup.cjs').applies(resolved))return require('../jsx-released-delete.cjs').plan(resolved,op);
    ...
    if(op.type==='pasteComponentProps')return require('../component-paste.cjs').plan(resolved,op);
