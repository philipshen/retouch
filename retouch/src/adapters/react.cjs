...
module.exports = {
  ...
  applyOp: (resolved,op) => {
    ...
    if(['duplicateElement','pasteElement','moveElement','moveSelection'].includes(op.type)&&require('../jsx-group-regroup.cjs').applies(resolved))return require('../transactions.cjs').applyPlan(resolved.appRoot||path.dirname(resolved.file),require('../jsx-released-structure.cjs').plan(resolved,op));
    if(['deleteElement','deleteSelection'].includes(op.type)&&require('../jsx-group-regroup.cjs').applies(resolved))return require('../transactions.cjs').applyPlan(resolved.appRoot||path.dirname(resolved.file),require('../jsx-released-delete.cjs').plan(resolved,op));
    ...
    if(op.type==='pasteComponentProps')return require('../transactions.cjs').applyPlan(resolved.appRoot||path.dirname(resolved.file),require('../component-paste.cjs').plan(resolved,op));
