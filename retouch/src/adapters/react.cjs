...
module.exports = {
  ...
  describe: resolved => {
    ...
    if(require('../jsx-group-regroup.cjs').applies(resolved)){Object.assign(base,require('../jsx-group-regroup.cjs').describe(resolved));base.canDelete=require('../jsx-released-delete.cjs').plan(resolved,{type:'deleteElement',fileHash:resolved.hash}).ok;base.canCopy=base.canDuplicate=require('../jsx-released-copy.cjs').plan(resolved,{type:'duplicateElement',fileHash:resolved.hash}).ok;base.canPaste=!!base.canFrame;}
  ...
  applyOp: (resolved,op) => {
    ...
    if(['duplicateElement','pasteElement'].includes(op.type)&&require('../jsx-group-regroup.cjs').applies(resolved))return require('../transactions.cjs').applyPlan(resolved.appRoot||path.dirname(resolved.file),require('../jsx-released-copy.cjs').plan(resolved,op));
  ...
  planOp: (resolved,op) => {
    const scaled=require('../jsx-group-scale.cjs').guard(resolved,op);if(scaled)return scaled;
    if(['duplicateElement','pasteElement'].includes(op.type)&&require('../jsx-group-regroup.cjs').applies(resolved))return require('../jsx-released-copy.cjs').plan(resolved,op);
