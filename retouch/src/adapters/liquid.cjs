...
function planOp(resolved, op) {
 ...
 if(op.type==='setSVGGeometry')return require('../liquid-svg-geometry.cjs').plan(resolved,op);
  ...
  if(['duplicateSelection','deleteSelection'].includes(op.type))return require('../native-structure-selection.cjs').plan(resolved,op,'liquid');
