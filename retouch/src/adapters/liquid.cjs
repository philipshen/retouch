 ...
 if(op.type==='setSVGGeometry')return require('../liquid-svg-geometry.cjs').plan(resolved,op);
  ...
  if (op.type === 'setClasses') {
    ...
    const build=items=>items.map(c=>{
      ...
      seen.add(c.id);
      const hrefEdit=Object.hasOwn(c,'href'),patchBlock=raw=>{if(Object.hasOwn(c,'tag'))raw=blocks.patchTag(raw,kept.tag,c.tag);if(c.paragraph==='inline')raw=require('../text-paragraphs.cjs').inline(raw);return c.marker?require('../list-markers.cjs').patch(raw,c.tag||kept.tag,c.marker):raw;};
