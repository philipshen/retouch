...
function describe(resolved) {
  ...
  return {...describeElement(resolved),svgDuplication,svgBooleanGroup:require('../svg-boolean-group.cjs').describe(resolved,'liquid'),svgBooleanOwner:require('../svg-boolean-group.cjs').owner(resolved,'liquid'),svgMask:require('../liquid-svg-mask.cjs').describe(resolved),svgBooleanReplacement:require('../svg-combine-selection.cjs').describe(resolved,'liquid'),svgGradientCreation:require('../svg-gradient-create.cjs').describe(resolved,'liquid'),svgGradients:require('../source-svg-gradient.cjs').describe(resolved,'liquid'),svgTransform:require('../svg-transform.cjs').describe(resolved,'liquid'),svgInsertion:require('../liquid-svg-insert.cjs').describe(resolved),svgConversion:require('../svg-convert.cjs').describe(resolved),svgGeometry:require('../liquid-svg-geometry.cjs').describe(resolved),...layerNames.describe(resolved),...require('../liquid-text-styles.cjs').describe(resolved),...require('../liquid-color-styles.cjs').describe(resolved),...require('../liquid-effect-styles.cjs').describe(resolved),...require('../liquid-variable-bindings.cjs').describe(resolved),components:theme.ancestry(resolved),structure:{...structure.describe(resolved,'liquid'),...require('../native-frame-selection.cjs').describe(resolved,'liquid'),...require('../native-insert.cjs').describe(resolved,'liquid'),...svgDuplication}};
}
 ...
 if(op.type==='setSVGGeometry')return require('../liquid-svg-geometry.cjs').plan(resolved,op);
  ...
  if (op.type === 'setClasses') {
    ...
    const build=items=>items.map(c=>{
      ...
      if(c.t==='paragraph')return require('../text-paragraphs.cjs').markup(build(c.children),false,c.spacing);
      ...
      seen.add(c.id);
      const hrefEdit=Object.hasOwn(c,'href'),patchBlock=raw=>{if(Object.hasOwn(c,'tag'))raw=blocks.patchTag(raw,kept.tag,c.tag);if(Object.hasOwn(c,'listInset'))raw=require('../list-inset.cjs').patch(raw,c.tag||kept.tag,c.listInset);if(Object.hasOwn(c,'listSpacing'))raw=require('../list-spacing.cjs').patch(raw,c.listSpacing);if(Object.hasOwn(c,'start'))raw=require('../list-start.cjs').patch(raw,c.start);if(Object.hasOwn(c,'spacing'))raw=require('../text-paragraphs.cjs').patchSpacing(raw,c.spacing);if(c.paragraph==='inline')raw=require('../text-paragraphs.cjs').inline(raw);return c.marker?require('../list-markers.cjs').patch(raw,c.tag||kept.tag,c.marker):raw;};
      ...
      return patchBlock((hrefEdit?require('../link-source.cjs').patch(resolved,kept,'liquid',c.href,true):resolved.source.slice(kept.tagStart,kept.openEnd))+build(c.children)+resolved.source.slice(kept.closeStart,kept.closeEnd));
    }).join('');
