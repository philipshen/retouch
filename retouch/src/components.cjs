...
function describe(resolved) {
  try {
    ...
    return {ok:true,movement:require('./move-component.cjs').describe(resolved),canDelete:true,swap:require('./component-swap-props.cjs').read(resolved),usageHash:resolved.hash,resetProperties:require('./component-reset.cjs').describe(resolved,def),canDuplicate:duplication.ok,duplicateReason:duplication.reason||null,explicitComponent,name:def.name,file:rel,hash:contentHash(def.source),source:def.source.slice(def.fn.start,def.fn.end),props:[...props.values()].map(prop=>({...prop,editor:require('./component-props.cjs').describe(resolved,prop.name,def),defaultEditor:require('./component-default.cjs').describe(resolved,prop.name,def)})),definitionId:host?.id||null,definitionIds,rootGroups:require('./component-root-groups.cjs')(def.fn,elements,fragments),detached,canDetach:!detached};
